...
export function isSellerListFilterActive({
  cities = [],
  query = "",
  minRating = 0,
  maxResponse = 0,
  ...
  return (
    ...
    (Number(maxResponse) || 0) > 0 ||
    (Number(minProducts) || 0) > 0
  );
}

export function countSellerListFilters({
  cities = [],
  minRating = 0,
  maxResponse = 0,
  minProducts = 0,
  ...
  return (
    citiesSafe.length +
    ((Number(minRating) || 0) > 0 ? 1 : 0) +
    ((Number(maxResponse) || 0) > 0 ? 1 : 0) +
    ((Number(minProducts) || 0) > 0 ? 1 : 0)
  );
}

export function sellerNameSuggestions(sellers, query, limit = 8) {
  ...
  return (Array.isArray(sellers) ? sellers : [])
    .filter((s) => {
      return (
        ...
        String(s.city || "").includes(q)
      );
    })
    .slice(0, limit);
}

export function citySuggestionsFromList(cities, input, limit = 10) {
  ...
  const q = norm(raw);
  return list
    .filter((c) => c.includes(raw) || norm(c).includes(q))
    .slice(0, limit);
}
