...
export function pathForProduct(productName, shopName) {
  ...
  // نام‌های رزرو مسیر سایت نباید به‌عنوان shopSlug استفاده شوند
  const reserved = new Set(['فروشگاه', 'فروشندگان', 'محصول', 'مجله', 'برندها', 'سبد-خرید', 'حساب', 'خانه', 'پیشنهادها']);
...
export function pushFaUrl(path, state = {}) {
  ...
  if (cur === next) {
    // همان URL — فقط state را تازه کن (برای re-apply)
