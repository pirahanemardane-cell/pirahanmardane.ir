...
export function mapExternalRowToProduct(
  ...
  const matchCatalogColor = (name) => matchCatalogColorLib(name, colors);
  const matchCatalogSize = (name) => matchCatalogSizeLib(name, sizes);
  const matchCatalogBrand = (name) => matchCatalogBrandLib(name, brands);
  const matchCategory = (name) => matchCategoryLib(name, categories);
