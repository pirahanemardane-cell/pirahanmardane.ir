...
export function adminSessionMaxAgeSec() {
  const n = Number(process.env.ADMIN_SESSION_MAX_AGE_SEC || 2 * 60 * 60)
  return Number.isFinite(n) && n > 300 ? n : 2 * 60 * 60
}
...
export async function requireAdmin() {
  ...
  // عمر سشن ادمین (کوکی pm_admin_since)
  try {
    const jar = await cookies()
    const sinceRaw = jar.get('pm_admin_since')?.value || ''
    const since = Number(sinceRaw)
    const maxAge = adminSessionMaxAgeSec() * 1000
    if (!since || !Number.isFinite(since) || Date.now() - since > maxAge) {
      return {
        error: NextResponse.json(
          {
            ok: false,
    ...
    // اگر cookies() در این runtime در دسترس نبود، فقط role+phone کافی است
  }
