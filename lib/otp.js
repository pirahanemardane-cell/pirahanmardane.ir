  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (process.env.OTP_DEBUG === '1') {
        console.info('[otp-debug]', { phone, codeLen: String(code).length, rowId: row.id, match: row.code_hash === code_hash })
      }
...
export async function markPhoneVerified(phoneRaw) {
  ...
  try {
    const sb = createAdminClient()
    // سشن تأیید ۳۰ دقیقه‌ای برای عملیات حساس (رمز / شبا / تغییر موبایل)
    const expires = new Date(Date.now() + 30 * 60 * 1000).toISOString()
    await sb.from('login_otps').insert({
      phone,
      code_hash: 'verified_session',
      role: 'verified_session',
      expires_at: expires,
    })
  } catch (e) {
    console.warn('[markPhoneVerified]', e?.message || e)
  }
  return { ok: true, phone }
}
...
export function isOtpMock() {
  ...
  // production: پیش‌فرض false (امن). dev: پیش‌فرض true برای راحتی
  if (v === undefined || v === '') {
...
export async function sendMelliPatternSms(phone, code, opts = {}) {
  ...
  const bodyId =
    process.env.MELLI_PATTERN_CODE || process.env.MELIPAYAMAK_BODY_ID || ''
