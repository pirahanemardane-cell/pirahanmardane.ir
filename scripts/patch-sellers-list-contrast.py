...
old1 = '''                          <img src={s.image} alt={s.name} className="absolute inset-0 w-full h-full object-cover transition duration-500 group-hover:opacity-95" loading="lazy" decoding="async" />
                          <div className="absolute inset-0 bg-primary-950/55 bg-primary-950/60" />
                          {idx < 3 && (
                          ...
                          <div className="relative z-10 flex flex-col h-full justify-end p-4 sm:p-5 min-h-[160px] sm:min-h-[180px]">
                            <h3 className="text-base font-bold text-white !text-white">{s.name}</h3>
                            <p className="text-xs text-white/90 !text-white mt-0.5">{s.desc}</p>
                            <div className="flex items-center justify-between mt-3">
                              <span className="text-xs text-white/80 !text-white">{toFa(s.productsSafe)} محصول · {toFa(Number(s.rating).toFixed(1))}★</span>
                              <span className="text-xs font-medium text-white !text-white group-hover:underline">مشاهده</span>
                            </div>
                          </div>'''
