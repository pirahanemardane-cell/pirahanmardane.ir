  ...
  return (
    <>
          ...
          <main className="flex-1">
            ...
            {/* PM_POST_HERO_STACK */}
            <div className="relative z-[1] isolate bg-primary-50 bg-primary-950">
            <FeaturesTrust
              items={features}
            ...
            {/* Categories */}
            <section id="categories" className="py-8 sm:py-12 bg-primary-50 bg-primary-950 transition-colors">
                ...
                <div className="grid grid-cols-3 md:grid-cols-6 gap-x-3 gap-y-5 sm:gap-x-6 md:gap-x-8 justify-items-center">
                  {categories.map((cat, i) => (
                    <button key={i} type="button" onClick={() => openPLP({ cat: cat.name })} className="gsap-cat flex flex-col items-center gap-1.5 sm:gap-3 group w-full max-w-[140px] md:max-w-none">
                      ...
                      <span className="text-xs sm:text-xs md:text-xs font-medium text-primary-800 text-white group-hover:text-primary-700 group-hover:text-white text-center line-clamp-2 leading-snug" title={cat.name}>{cat.name}</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            <section className="py-8 sm:py-10 bg-white bg-primary-900 border-y border-primary-200 border-white/30 transition-colors">
              <div className="max-w-7xl mx-auto px-3 sm:px-4">
                <div className="flex items-center justify-between mb-5 sm:mb-6 gap-3">
                  <h2 className="text-xl font-medium leading-7 sm:text-[22px] sm:leading-7">برندهای منتخب</h2>
                  <button
                    type="button"
                    onClick={() => openStaticPage('brands')}
                    className="text-sm font-medium leading-5 tracking-wide text-apple-link hover:underline flex items-center gap-1"
                  >
                    مشاهده همه
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          try {
                            ...
                            else if (typeof setBrandDetailId === 'function') {
                              setBrandDetailId(b.id || b.slug || b.name);
                              if (typeof openStaticPage === 'function') openStaticPage('brands');
                            } else {
                              openPLP({ brand: b.name, brandSlug: b.slug || b.name });
                            }
                          } catch (_) {
                            try { openPLP({ brand: b.name, brandSlug: b.slug || b.name }); } catch (__) {}
                          }
                        }}
                        className="relative z-[2] flex-shrink-0 w-[104px] h-[88px] rounded-xl border border-primary-200 border-white/25 bg-primary-50 bg-primary-800 flex flex-col items-center justify-center text-center p-2 cursor-pointer transition hover:bg-[#023047] hover:border-[#023047] hover:bg-primary-700 hover:border-white/40 group/brand shadow-sm touch-manipulation"
                      >
                        <span className="font-bold text-xs leading-tight text-[#023047] text-white group-hover/brand:text-white group-hover/brand:text-[#023047] text-[#13ABC4] line-clamp-2 pointer-events-none">{b.name}</span>
                      </button>
                    ))}
                  </div>
                </div>
            ...
            <section className="py-8 sm:py-10 bg-primary-50 bg-primary-950 transition-colors">
              <div className="max-w-7xl mx-auto px-3 sm:px-4">
                <div className="flex items-center justify-between mb-5 sm:mb-6 gap-3">
                  <h2 className="text-xl font-medium leading-7 sm:text-[22px] sm:leading-7">برترین فروشندگان</h2>
                  <button
                    type="button"
                ...
                <div className="relative">
                  ...
                  {topSellers.slice(0, 6).map((s, i) => {
                    ...
                    >
                      ...
                      <div className={`absolute inset-0 flex items-center justify-center bg-primary-200 bg-primary-800 ${hasPhoto ? 'hidden' : ''}`} aria-hidden={hasPhoto ? 'true' : undefined}>
                        <span className="text-5xl sm:text-6xl font-bold text-primary-700 text-white select-none" style={{ fontFamily: 'IRANYekanX, var(--font-app), sans-serif' }}>{letter}</span>
                      </div>
            ...
            {/* Bestsellers */}
            <section id="products" className="py-8 sm:py-12 bg-primary-50 bg-primary-950 transition-colors">
              <div className="max-w-7xl mx-auto px-3 sm:px-4">
                <div className="flex items-center justify-between mb-5 sm:mb-8 gap-3">
                  <h2 className="gsap-reveal text-xl font-medium leading-7 sm:text-[22px] sm:leading-7">
                    پرفروش ترین محصولات
                  </h2>
                  <a href="/فروشگاه?sort=popular" onClick={(e) => { e.preventDefault(); openPLP({ sort: 'popular' }); }} className="text-sm font-medium leading-5 tracking-wide text-apple-link hover:underline flex items-center gap-1">
                    مشاهده همه
                    <Icon name="chevronLeft" size={14} />
                  </a>
                </div>
            ...
            <section className="py-8 sm:py-12 bg-white bg-primary-900 transition-colors">
              <div className="max-w-7xl mx-auto px-3 sm:px-4">
                <div className="flex items-center justify-between mb-4 sm:mb-6 gap-3">
                  <div className="flex items-center gap-2">
                    <h2 className="text-xl font-medium leading-7 sm:text-[22px] sm:leading-7">جدیدترین محصولات</h2>
                  </div>
                  <a href="/فروشگاه?sort=newest" onClick={(e) => { e.preventDefault(); openPLP({ sort: 'newest' }); }} className="text-sm font-medium leading-5 tracking-wide text-apple-link hover:underline flex items-center gap-1">
                    مشاهده همه
                    <Icon name="chevronLeft" size={14} />
                  </a>
                </div>
                    ...
                    <button
                      ...
                      className={`flex-shrink-0 px-3 sm:px-4 py-1.5 rounded-full text-xs sm:text-sm font-medium transition whitespace-nowrap ${
                        newestTab === tab
                          ...
                          : 'bg-primary-50 bg-primary-900 text-primary-700 !text-white border border-primary-200 border-white/30 hover:bg-primary-100 hover:bg-primary-800'
                      }`}
                    >
                      {tab}
                    </button>
              ...
              <div className="max-w-7xl mx-auto px-3 sm:px-4">
                <div className="flex items-center justify-between mb-5 sm:mb-6 gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-xl font-medium leading-7 sm:text-[22px] sm:leading-7">پیشنهادات شگفت‌انگیز</h2>
                  </div>
                  <a href="#" onClick={(e) => { e.preventDefault(); openStaticPage('deals'); }} className="text-sm font-medium leading-5 tracking-wide text-apple-link hover:underline flex items-center gap-1">
                    مشاهده همه
                    <Icon name="chevronLeft" size={14} />
                  </a>
                </div>
                <div className="relative">
                  ...
                  <div className="flex-shrink-0 w-[78%] min-[400px]:w-[70%] sm:w-[42%] md:w-[calc((100%-2.5rem)/3.3)] lg:w-[calc((100%-3.5rem)/4.3)] rounded-xl sm:rounded-2xl overflow-hidden bg-[#023047] bg-[#13ABC4] text-white flex flex-col items-center justify-center p-3 sm:p-4 text-center shadow-lg snap-start min-h-[200px] sm:min-h-[280px]">
            ...
            <section className="py-8 sm:py-12 bg-primary-50 bg-primary-950 transition-colors">
              <div className="max-w-7xl mx-auto px-3 sm:px-4">
                <div className="flex items-center justify-between mb-5 sm:mb-6">
                  <div>
                    <h2 className="text-xl font-medium leading-7 sm:text-[22px] sm:leading-7">نظرات مشتریان واقعی</h2>
                    <p className="text-sm text-primary-500 text-white">تجربه خریداران پیراهن مردانه از سراسر ایران</p>
                  </div>
                </div>
                <div className="carousel-track flex gap-4 overflow-x-auto no-scrollbar pb-2 scroll-smooth snap-x" style={{ WebkitOverflowScrolling: 'touch' }}>
                  {displayReviews.map((r, i) => (
                    <div key={i} className="flex-shrink-0 w-[78%] min-[400px]:w-[70%] sm:w-[42%] md:w-[calc((100%-2.5rem)/3.3)] lg:w-[calc((100%-3.5rem)/4.3)] bg-white bg-black rounded-xl sm:rounded-2xl p-4 sm:p-5 border border-primary-200 border-white shadow-sm flex flex-col snap-start">
                      <div className="flex items-center gap-3 mb-2">
                        <Avatar name={r.name || r.display_name || "خریدار"} src={r.image || ""} size={48} className="w-12 h-12" />
                        <div className="min-w-0">
                          <p className="font-bold text-sm text-primary-900 text-white">{r.name}</p>
                          <p className="text-xs text-primary-400 text-white">{r.city}</p>
                        </div>
                      </div>
                      {r.seller ? (
                        r.sellerId ? (
                          <button
                            ...
                            className="review-shop-badge inline-flex self-start items-center h-6 px-2 rounded-md bg-primary-100 bg-white text-primary-900 text-[#13ABC4] !text-[#0A0A0A] text-xs sm:text-xs font-medium mb-2 hover:bg-primary-200 hover:bg-white transition cursor-pointer"
                          >
                            {r.seller}
                          </button>
                        ) : (
                          <span className="review-shop-badge inline-flex self-start items-center h-6 px-2 rounded-md bg-primary-100 bg-white text-primary-900 text-[#13ABC4] !text-[#0A0A0A] text-xs sm:text-xs font-medium mb-2">
                            {r.seller}
                          </span>
                        )
                      ) : null}
                      <div className="flex gap-0.5 text-primary-400 text-[#13ABC4] mb-3">
                        {[...Array(5)].map((_, j) => (
                          <Icon key={j} name="star" size={14} className={j < r.rating ? "text-primary-400 fill-primary-400" : "text-primary-200 text-primary-700"} />
                        ))}
                      </div>
                      {(() => {
                        ...
                        return <p className="text-sm text-primary-600 text-white leading-relaxed flex-1">«{t}»</p>
                      })()}
                    </div>
                  ))}
                </div>
      ...
      {(() => {
        ...
        return (
          <section className="max-w-7xl mx-auto px-3 sm:px-4 py-6 sm:py-8" data-section="top-shirts">
            <h2 className="text-xl font-medium leading-7 sm:text-[22px] sm:leading-7">برترین های پیراهن</h2>
                ...
                typeof renderProductCard === 'function'
                  ...
                  : (
                    <button
                      ...
                      className="text-right rounded-2xl border border-primary-100 border-white/10 bg-white bg-primary-900 overflow-hidden shadow-sm hover:shadow-md transition"
                    >
                      <div className="aspect-[3/4] bg-primary-50 bg-primary-950">
                        <img src={p.image || p.images?.[0] || p.thumb || ''} alt={p.name || ''} className="w-full h-full object-cover" loading="lazy" />
                      </div>
                      <div className="p-2.5">
                        <p className="text-xs sm:text-sm font-bold text-primary-900 text-white line-clamp-2">{p.name}</p>
                        {p.priceText && <p className="text-xs text-primary-600 text-white/70 mt-1">{p.priceText} تومان</p>}
                      </div>
                    </button>
                  )
              ))}
            ...
            {/* Recently Viewed */}
            <section className="py-8 sm:py-12 bg-white bg-primary-900 transition-colors" data-section="recent">
              <div className="max-w-7xl mx-auto px-3 sm:px-4">
                <div className="flex items-center justify-between mb-5 sm:mb-6">
                  <h2 className="text-xl font-medium leading-7 sm:text-[22px] sm:leading-7">
                    <Icon name="eye" size={22} className="text-primary-600" />
                    اخیراً دیده‌شده
                  </h2>
                  <button type="button" onClick={openRecentPage} className="text-sm font-medium leading-5 tracking-wide text-apple-link hover:underline flex items-center gap-1">
                    مشاهده همه
                    <Icon name="chevronLeft" size={14} />
                  </button>
                </div>
            ...
            <section className="py-8 sm:py-12 bg-primary-50 bg-primary-950 transition-colors">
              <div className="max-w-7xl mx-auto px-3 sm:px-4">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-6">
                  {stats.map((s, i) => (
                    <div key={i} className="gsap-stat bg-white bg-black rounded-xl p-4 sm:p-6 text-center shadow-sm border border-primary-200 border-white text-primary-800 text-white">
                      <div className="w-11 h-11 sm:w-14 sm:h-14 mx-auto mb-3 sm:mb-4 rounded-full bg-primary-50 bg-black text-primary-700 text-white flex items-center justify-center border border-transparent border-white/20">
                        <Icon name={s.icon} size={20} />
                      </div>
                      <h3 className="font-bold text-primary-900 text-white text-xs sm:text-base mb-0.5 leading-tight">{s.title}</h3>
                      <p className="text-xs sm:text-sm text-primary-500 text-white leading-snug">{s.desc}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <section className="py-8 sm:py-12 bg-primary-50 bg-primary-950 transition-colors">
              <div className="max-w-7xl mx-auto px-3 sm:px-4">
                <div className="flex items-center justify-between mb-5 sm:mb-8 gap-3">
                  <h2 className="text-lg sm:text-xl font-bold leading-tight text-primary-900 text-white">مجله پیراهن مردانه</h2>
                  <button
                    type="button"
                  ...
                  <div ref={blogsTrackRef} className="carousel-track flex gap-3 sm:gap-4 overflow-x-auto no-scrollbar pb-2 scroll-smooth snap-x px-0 sm:px-10" style={{ WebkitOverflowScrolling: 'touch' }}>
                    {homeBlogs.map((b) => (
                      <article
                        ...
                        className="gsap-blog flex-shrink-0 w-[78%] min-[400px]:w-[70%] sm:w-[42%] md:w-[calc((100%-2.5rem)/3.3)] lg:w-[calc((100%-3.5rem)/4.3)] bg-white bg-black rounded-xl sm:rounded-2xl overflow-hidden border border-primary-200 border-white shadow-sm hover:shadow-md transition group snap-start cursor-pointer text-right"
                      >
                        <div className="aspect-video overflow-hidden bg-primary-100 bg-primary-800">
                          <img
                            src={b.image}
                            alt={b.title}
                            loading="lazy"
                        ...
                        <div className="p-3.5 sm:p-5">
                          {b.category ? (
                            <button
                              ...
                              className="inline-flex self-start items-center h-6 px-2 mb-1.5 rounded-md bg-primary-100 bg-primary-700 text-primary-900 text-white text-xs font-medium hover:bg-primary-200 hover:bg-primary-600 transition"
                            >{b.category}</button>
                          ) : null}
                          <h3 className="text-base font-bold text-primary-900 text-white mt-1 mb-1.5 line-clamp-2 group-hover:text-primary-600 group-hover:text-white transition" title={b.title}>{b.title}</h3>
                          <p className="text-xs text-primary-500 text-white mb-1">{b.author}</p>
                          {b.date ? <time className="text-xs text-primary-400 text-white/70">{b.date}</time> : null}
                        </div>
                      </article>
                    ))}
                  </div>
            ...
            <section className="py-6 sm:py-8 bg-white bg-primary-900 transition-colors">
              <div className="max-w-7xl mx-auto px-3 sm:px-4">
                <div className="relative overflow-hidden rounded-2xl sm:rounded-3xl min-h-[180px] sm:min-h-[220px]">
                  ...
                  <div className="relative z-10 w-full box-border px-4 py-6 sm:px-10 sm:py-10 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 sm:gap-5 min-h-[180px] sm:min-h-[220px]">
                    ...
                    <button
                      ...
                      className="seller-banner-cta w-full sm:w-auto max-w-full inline-flex items-center justify-center gap-2 bg-transparent hover:bg-[#023047] !text-white hover:!text-white px-5 sm:px-6 py-2.5 sm:py-3 rounded-full font-semibold text-sm border border-white active:opacity-90 transition"
                    >
            ...
            {/* Newsletter */}
            <section className="gsap-newsletter h-full rounded-2xl bg-primary-50 bg-primary-900 text-primary-800 text-primary-100 transition-colors border border-primary-100 border-white/10">
              <div className="flex flex-col items-center text-center h-full p-6 sm:p-8">
                <div className="w-12 h-12 rounded-full bg-primary-100 bg-black text-primary-700 text-white flex items-center justify-center flex-shrink-0">
                  <Icon name="phone" size={22} />
                </div>
                <h2 className="mt-3 text-lg sm:text-xl font-bold text-primary-900 text-white text-center leading-snug">
                  از تخفیف‌های ما باخبر شوید
                </h2>
                <p className="mt-1 text-xs sm:text-sm text-primary-500 text-white/70 text-center leading-relaxed min-h-[2.75rem]">
                ...
                <div className="mt-auto pt-5 flex w-full max-w-md gap-2 flex-row justify-center items-center">
                  <button
                    ...
                    className="h-11 min-w-[7.5rem] px-5 rounded-full bg-apple-blue text-white text-sm font-bold hover:opacity-90 bg-[#13ABC4] transition whitespace-nowrap flex-shrink-0 inline-flex items-center justify-center"
                  >
                    ثبت شماره
                  </button>
                  <input
                    ...
                    className="flex-1 min-w-0 h-11 px-4 rounded-full bg-white bg-primary-950 border border-primary-300 border-white/40 text-primary-900 text-white text-sm text-left focus:outline-none focus:border-apple-blue"
                  />
                </div>
              </div>
            </section>
            ...
            {/* پیگیری سفارش */}
            <section id="home-track-order" className="h-full rounded-2xl bg-primary-50 bg-primary-900 text-primary-800 text-primary-100 transition-colors border border-primary-100 border-white/10">
              <div className="flex flex-col items-center text-center h-full p-6 sm:p-8">
                <div className="w-12 h-12 rounded-full bg-primary-100 bg-black text-primary-700 text-white flex items-center justify-center flex-shrink-0">
