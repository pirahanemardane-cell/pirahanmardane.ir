          ...
          {showComparePage && !pdpProduct && !showProfilePage && (() => {
            ...
            <div className="w-full max-w-7xl mx-auto px-3 sm:px-4 py-6 sm:py-10 flex-1">
              <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
                <div>
                  <h1 className="text-xl sm:text-2xl font-bold text-primary-900 text-white flex items-center gap-2">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {list.length >= 2 && (
                    <label className="flex items-center gap-1.5 text-xs text-primary-700 text-white cursor-pointer select-none">
                  ...
                  <button type="button" onClick={() => { closeComparePage(); openPLP(); }} className="text-xs px-3 py-1.5 rounded-full border plp-filter-chip border-primary-300 border-white/50 !text-primary-900 !text-white bg-white bg-[#2A2C30] font-medium">افزودن کالا</button>
                </div>
              </div>

              {list.length === 0 && (
                <div className="text-center py-20 rounded-2xl border border-primary-200 border-white/20 bg-primary-50/40 bg-primary-900/30">
                  <p className="font-bold text-primary-900 text-white">هنوز کالایی برای مقایسه نیست</p>
              ...
              {list.length === 1 && (
                <div className="text-center py-12 rounded-2xl border border-primary-200 border-white/20 bg-primary-50/40 bg-primary-900/30 mb-6">
                  <p className="font-bold text-primary-900 text-white">حداقل ۲ کالا برای مقایسه لازم است</p>
              ...
              {list.length > 0 && (
                <div className="compare-table-wrap overflow-x-auto no-scrollbar" style={{ WebkitOverflowScrolling: 'touch' }}>
                  <Table className="w-full text-sm min-w-[640px] md:min-w-0">
                    <TableHeader>
                      <TableRow className="border-b border-primary-200 border-white/20">
                        <TableHead className="sticky right-0 z-20 bg-[#F7F7F8] !bg-[#1A1C20] py-3 px-3 text-right text-primary-600 !text-white/80 font-semibold min-w-[110px] text-sm">ویژگی</TableHead>
                        {list.map(p => {
                          ...
                          return (
                            <TableHead key={p.id} className="py-3 px-3 text-right min-w-[170px] align-top">
                              <div className="flex flex-col items-stretch w-full text-right" dir="rtl">
                                ...
                                <button type="button" className="mt-2 font-medium text-primary-900 text-white line-clamp-2 text-xs sm:text-sm min-h-[2.5rem] w-full text-right" onClick={() => { closeComparePage(); openPDP(p); }}>{p.name}</button>
                                ...
                                <div className="flex justify-start items-center gap-1 mt-1.5 flex-nowrap w-full" dir="rtl">
                                  ...
                                  <button type="button" onClick={() => toggleFavorite(p.id)} className="text-xs px-2 py-1 rounded-full border plp-filter-chip border-primary-300 border-white/50 !text-primary-900 !text-white bg-white bg-[#2A2C30] font-medium inline-flex items-center justify-center shrink-0">
                                    <Icon name={isFavorite(p.id) ? 'heartFilled' : 'heart'} size={14} />
                                  </button>
                                  <button type="button" onClick={() => toggleCompare(p)} className="text-xs px-2 py-1 rounded-full border border-primary-200 border-white/30 text-primary-500 text-white shrink-0">حذف</button>
                                </div>
                              </div>
                            </TableHead>
                          );
                    ...
                    <TableBody>
                      {visibleRows.map((r) => (
                        <TableRow key={r.key} className="border-b border-primary-100 border-white/10">
                          <TableCell className="sticky right-0 z-10 bg-[#FAFAFB] !bg-[#16161a] py-3 px-3 text-primary-600 !text-white/80 text-xs font-semibold">{r.label}</TableCell>
                          {list.map(p => {
                            const best = r.best(p);
                            return (
                              <TableCell key={p.id} className={`py-2.5 px-3 text-right text-xs sm:text-sm ${best ? 'text-apple-blue font-bold bg-apple-blue/5' : 'text-primary-800 text-white'}`}>
                                {r.key === 'colors' ? (
                                  <div className="flex justify-start gap-1.5 flex-wrap" dir="rtl">
                                    {(p.colors || []).slice(0, 6).map(c => (
                                      <span key={c.name} title={c.name} className="color-swatch w-7 h-7 rounded-full border-2 border-primary-300 border-white/70" style={{ ["--swatch-color"]: c.hex || '#888', backgroundColor: c.hex || '#888' }} />
                                    ))}
                                  </div>
                                ) : r.get(p)}
                              </TableCell>
                      ...
                      <TableRow>
                        <TableCell className="sticky right-0 z-10 bg-white !bg-[#121214] py-3 px-3"></TableCell>
                        {list.map(p => (
