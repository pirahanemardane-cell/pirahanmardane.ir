  ...
  return (
    <div className="site-modal-root" role="dialog" aria-modal="true" aria-label="ورود">
      ...
      <div className="site-modal-panel bg-white dark:bg-primary-900 p-5 sm:p-6 border border-primary-200 dark:border-white/15 relative">
        <button type="button" onClick={closeAuth} className="absolute top-3 left-3 p-2 rounded-full hover:bg-primary-50 dark:hover:bg-primary-800 text-primary-500" aria-label="بستن">
          <Icon name="x" size={18} />
        </button>
        <div className="text-center py-6 space-y-3">
          ...
          <h3 className="text-lg font-bold text-primary-900 dark:text-white">{isSeller ? 'ورود فروشنده' : 'ورود'}</h3>
          <p className="text-sm text-primary-500 dark:text-white/70">سیستم ورود فعلاً غیرفعال است.</p>
          <button type="button" onClick={closeAuth} className="mt-2 px-6 py-2.5 rounded-full bg-apple-blue dark:bg-[#13ABC4] text-white text-sm font-bold">بستن</button>
        </div>
      </div>
    </div>
  );
