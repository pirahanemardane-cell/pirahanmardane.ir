...
const toggleHtmlMode = () => {
  ...
  return (
    <>
      ...
      {editorDialog && (
        <div className="site-modal-root" role="dialog" aria-modal="true">
          ...
          <div className="site-modal-panel bg-white bg-neutral-900 border border-neutral-200 border-white/15 p-4 sm:p-5 space-y-3">
            <h3 className="text-sm font-bold text-neutral-900 text-white text-right">{editorDialog.title}</h3>
            {editorDialog.message ? (
              <p className="text-sm text-neutral-600 text-white/80 text-right">{editorDialog.message}</p>
            ) : null}
            {editorDialog.mode !== 'alert' && (
              <input
                ...
                dir="ltr"
                className="w-full px-3 py-2.5 rounded-xl border border-neutral-200 border-white/20 bg-transparent text-sm text-left text-neutral-900 text-white"
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    e.preventDefault();
            ...
            <div className="flex gap-2 justify-start">
              ...
              {editorDialog.mode !== 'alert' && (
                <button
                  type="button"
                  className="px-5 py-2 rounded-full border border-neutral-200 border-white/30 text-sm"
                  onClick={() => {
                    const r = editorDialog.resolve;
                    setEditorDialog(null);
                    r?.(null);
    ...
    <div
      className={`simple-editor rounded-xl border border-primary-200 border-white/20 bg-white bg-primary-950 overflow-hidden ${className}`}
      data-appearance={appearance}
      data-mode={mode}
    >
      <div className="flex flex-wrap items-center gap-1 px-2 py-1.5 border-b border-primary-100 border-white/10 bg-primary-50/80 bg-primary-900/50">
        {/* سلسله‌مراتب هدینگ */}
        <label className="inline-flex items-center gap-1 text-xs text-primary-600 text-white/80">
          ...
          <select
            ...
            className="px-2 py-1 rounded-lg text-xs font-medium bg-white bg-primary-800 border border-primary-200 border-white/20 text-primary-800 text-white outline-none cursor-pointer max-w-[7.5rem]"
          >
        ...
        {tools.map((t) => (
          <button
            ...
            className={`px-2 py-1 rounded-lg text-xs font-medium transition ${
              t.id === 'bold'
                ? 'font-bold text-primary-900 text-white hover:bg-primary-100 hover:bg-primary-800'
                : t.id === 'html' && htmlMode
                ? 'bg-[#4CCD99] text-white'
                : 'text-primary-700 text-white/90 hover:bg-primary-100 hover:bg-primary-800'
            }`}
          >
            {t.label}
          </button>
        ...
        {/* پالت رنگ متن */}
        <div className="inline-flex items-center gap-0.5 px-1 py-0.5 rounded-lg border border-primary-200 border-white/15 bg-white/80 bg-primary-800/80" title="رنگ متن">
          {TEXT_COLORS.map((c) => (
            <button
              key={c.id}
              type="button"
              ...
              className="color-swatch se-text-color-swatch w-4 h-4 rounded-full border border-black/15 border-white/50 hover:scale-110 transition shrink-0"
              style={{ ["--swatch-color"]: c.value, backgroundColor: c.value }}
            />
          ))}
        </div>

        <span className="mr-auto text-xs text-primary-400 text-white/40 self-center px-1">
      ...
      {htmlMode ? (
        <textarea
          ...
          dir="ltr"
          spellCheck={false}
          className={`w-full ${minH} ${maxH} overflow-y-auto px-3 py-3 text-xs text-right text-primary-900 text-white bg-primary-50/50 bg-black/30 outline-none resize-y leading-6`}
          placeholder="HTML…"
        />
      ) : (
        <div
          ref={ref}
          className={`simple-editor-surface px-3 py-3 text-sm text-primary-900 text-white outline-none ${minH} ${maxH} overflow-y-auto leading-7`}
          contentEditable
