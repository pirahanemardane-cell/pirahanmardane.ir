...
import { toEnglishDigits, normalizeIranMobile, isValidIranMobile } from '../../lib/phone';
...
export default function LoginCardSection({ mode = 'buyer', onClose, onContact }) {
  ...
  const pathIsAdmin =
    ...
    (() => {
      try {
        ...
        return p === '/ashn-pnl' || p.endsWith('/ashn-pnl') || p === '/amirpnl' || p.endsWith('/amirpnl');
      } catch (_) {
        return false;
      }
    })();
  ...
  function mustGoAdminPanel() {
    try {
      ...
      if (path === '/ashn-pnl' || path.indexOf('/ashn-pnl') >= 0 || path === '/amirpnl' || path.indexOf('/amirpnl') >= 0) return true;
    } catch (_) {}
  ...
  function goAfterAuth(data) {
    ...
    try {
      ...
      window.location.assign(r === 'admin' ? '/ashn-pnl' : r === 'seller' ? '/seller' : '/account');
    }
  }
  ...
  async function handlePasswordLogin() {
    ...
    setMsg('');
    try {
      const phone = normalizeIranMobile(emailOrPhone || '');
      ...
      if (!res.ok || !data?.ok) {
        ...
        setView('signup');
        setMsg(data?.error || 'حسابی نیست — ثبت‌نام را تکمیل کنید');
        return;
      }
      ...
      try {
        ...
        if (_p === '/ashn-pnl' || _p.indexOf('/ashn-pnl') >= 0 || _p === '/amirpnl' || _p.indexOf('/amirpnl') >= 0) {
  ...
  async function handleSignup() {
    if (busy) return;
    const phone = normalizeIranMobile(signupPhone || '');
  ...
  async function handleForgotRequest() {
    if (busy) return;
    const phone = normalizeIranMobile(emailOrPhone || signupPhone || '');
    if (!isValidIranMobile(phone)) {
  ...
  async function handleRequestOtp() {
    if (busy) return;
    const phone = normalizeIranMobile(smsPhone || '');
    if (!isValidIranMobile(phone)) {
      setMsg('شماره موبایل ۱۱ رقمی وارد کنید');
      return;
    }
  ...
  async function handleVerifyOtp(code) {
    ...
    setMsg("");
    try {
      const phone = normalizeIranMobile(smsPhone || emailOrPhone || "");
      let pending = null;
      try {
  ...
  return (
    <section className="fixed inset-0 z-[200] bg-zinc-950 text-zinc-50">
      ...
      <div className="relative z-10 h-full w-full grid place-items-center px-4 pt-16">
        <Card className="card-animate w-full max-w-sm max-h-[min(92vh,720px)] overflow-y-auto border-zinc-800 bg-zinc-900/70 backdrop-blur">
              ...
              <>
                {view === 'signup' && isSeller ? (
                  <div className="grid gap-2">
                    ...
                    <Input id="auth-shop" value={shopName} onChange={(e) => setShopName(e.target.value)} style={{ color: "#fff", WebkitTextFillColor: "#fff", caretColor: "#fff" }} type="text" placeholder="نام فروشگاه" className="bg-zinc-950 border-zinc-800 !text-white placeholder:text-zinc-500" />
                  </div>
                ) : null}
                {view === 'signup' ? (
                  <div className="grid gap-2">
