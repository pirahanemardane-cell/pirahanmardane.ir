...
export function Breadcrumb({
  ...
  const linkClass =
    'truncate max-w-[11rem] sm:max-w-[16rem] text-primary-600 text-white/70 hover:text-apple-blue hover:text-[#4CCD99] transition cursor-pointer';

  return (
    <nav className={`w-full relative z-10 border-t border-b border-primary-200 border-white/20 bg-primary-50 bg-primary-950 ${className}`} aria-label="breadcrumb" dir="rtl">
      <div className={`${fullWidth ? 'max-w-none' : 'max-w-none sm:max-w-7xl'} mx-auto px-2 sm:px-4 py-2 sm:py-3`}>
        <div className="flex flex-wrap justify-start items-center gap-x-2 gap-y-1 text-sm text-primary-500 text-white/55 font-medium text-right">
          {(typeof homeOnClick === 'function' || homeHref) && (
            <>
              <a
                ...
                aria-label="خانه"
                className="inline-flex items-center justify-center text-primary-600 text-white/70 hover:text-apple-blue hover:text-[#4CCD99] transition"
              >
                {showHomeIcon ? (
              ...
              <ChevronIcon className="text-primary-300 text-white/30 w-4 h-4 sm:w-5 sm:h-5" />
            </>
          )}
            ...
            return (
              <React.Fragment key={`${item.label}-${i}`}>
                {showAsLink ? (
                  ...
                  <span
                    className="truncate max-w-[11rem] sm:max-w-[18rem] text-apple-blue text-[#4CCD99] font-semibold"
