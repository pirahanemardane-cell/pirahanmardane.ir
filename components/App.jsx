...
import { DEFAULT_LEGAL_PAGES, DEFAULT_SITE_FAQS as LEGAL_DEFAULT_FAQS } from '@/lib/default-legal-content';
...
import {
  ...
  appendGa4Event,
  pushGa4ToWindow,
  ga4FilterEvents as ga4FilterEventsLib,
...
import { isBlogLiked as isBlogLikedLib, toggleBlogLikeList, buildBlogCommentEntry, appendBlogComment, estimateReadingMinutes, formatReadingTimeFa} from '@/lib/blog-helpers'
import { withPriceHistory } from '@/lib/price-history';
...
const VirtualProductGrid = dynamic(() => import('./VirtualProductGrid'), {
  ssr: false,
  loading: () => <div className="min-h-[120px] flex items-center justify-center" role="status"><span className="text-sm text-primary-500 text-[#13ABC4] animate-pulse">در حال بارگذاری…</span></div>,
});
const SimpleEditor = dynamic(() => import('./SimpleEditor'), {
  ssr: false,
  loading: () => (
    <div className="min-h-[120px] flex items-center justify-center" role="status"><span className="text-sm text-primary-500 text-[#13ABC4] animate-pulse">در حال بارگذاری…</span></div>
  ),
});
          ...
          try {
            ...
            if (cancelled || !json?.ok || !Array.isArray(list) || !list.length) return;
              ...
              return {
                ...
                read: formatReadingTimeFa(
                  [p.title, p.excerpt, p.body].filter(Boolean).join(' '),
                  (n) => {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("popstate", onPop);
      ...
      const DEFAULT_SITE_FAQS = LEGAL_DEFAULT_FAQS;
      ...
      const buildGscSeed = () => buildGscSeedLib();
      const loadGscStore = () => loadGscStoreFromLocal();
      const [gscStore, setGscStore] = useStoreField(shopUiStore, 'gscStore');
      const persistGsc = (next) => {
        setGscStore(next);
        persistGscToLocal(next);
      };
      const gscAggregate = (rangeKey = adminGscRange, dim = adminGscDim) =>
        gscAggregateLib(gscStore?.performance || [], rangeKey, dim);
      const gscInspect = (url) => {
        const result = buildGscInspectResult(url, gscStore);
      ...
      const trackGa4Event = (name, params = {}, ctx = {}) => {
        try {
          const row = buildGa4EventRow(name, params, {
            ...ctx,
            user_id: ctx.user_id || (user?.id || 'anon'),
          });
          setGa4Store(prev => appendGa4Event(prev, row));
          pushGa4ToWindow(name, params);
        } catch (_) {}
      };
      const ga4FilterEvents = (rangeKey = adminAnalyticsRange) =>
        ga4FilterEventsLib(ga4Store?.events || [], rangeKey);
      const ga4Aggregate = (rangeKey = adminAnalyticsRange) =>
        ga4AggregateLib(ga4Store?.events || [], rangeKey);
...
/* Deep link فروشگاه/فیلتر — بدون سرور: &sort=&q= */
      ...
      const [plpAttrs, setPlpAttrs] = useStoreField(commerceUiStore, 'plpAttrs');
      const [plpMinSellerRating, setPlpMinSellerRating] = useStoreField(commerceUiStore, 'plpMinSellerRating');
      ...
      const catalogProducts = useMemo(() => {
        ...
        const normalize = (p) =>
          normalizeProduct(p, {
            now,
            toFa: typeof toFa === 'function' ? toFa : (x) => String(x ?? ''),
            isUsableProductImage: typeof isUsableProductImage === 'function' ? isUsableProductImage : undefined,
            pickProductImage: typeof pickProductImage === 'function' ? pickProductImage : undefined,
          });
        const map = new Map();
      ...
      const updateQty = (id, colorName, delta, size) => {
        ...
        const colorStr = colorName || '';
        const sizeStr = size || '';
        const item = (cart || []).find(
          (i) =>
            String(i.id ?? '') === idStr &&
            (i.selectedColor?.name || '') === colorStr &&
            (i.selectedSize || '') === sizeStr
        );
        ...
        // همیشه اول محلی (optimistic) تا دکمه‌ها در صفحه سبد و ستون کناری فوراً کار کنند
        const applyLocal = () => {
          setCart((prev) => {
            ...
            const prod = (catalogProducts || products || []).find(
              (pr) => String(pr.id ?? '') === String(line?.id ?? idStr)
            );
            let maxStock =
              getVariantStock(
                prod || line || {},
                line?.selectedColor?.name || colorStr,
                line?.selectedSize || sizeStr || line?.size,
                line?.selectedAttrs || {}
              ) ||
              line?.stockLeft ||
              prod?.stock ||
            ...
            if (delta > 0 && line && Number(line.qty) >= maxStock) {
              try {
                pushLiveToast(`حداکثر موجودی این ترکیب ${toFa(maxStock)} عدد است`, { type: 'error' });
              } catch (_) {}
            }
            return applyCartQtyDelta(prev, {
              id: line?.id ?? id,
              colorName: colorStr,
              size: sizeStr,
              delta,
        ...
        // همگام‌سازی سرور فقط اگر واقعاً آیتم سرور + session فعال باشد
        const shouldSyncServer =
          item &&
          (item.fromServer || item.serverItemId) &&
          typeof isServerCartEnabled === 'function' &&
          isServerCartEnabled();
        ...
        (async () => {
          try {
            ...
            let res;
            if (nextQty <= 0) {
              res = await apiRemoveCartItem({
                itemId: item.serverItemId,
                productId: item.product_id || item.id,
              });
            } else {
              res = await apiUpdateCartItem({
                itemId: item.serverItemId,
                productId: item.product_id || item.id,
                qty: nextQty,
              });
      ...
      const printOrderInvoice = (o) => {
        if (!o) return;
        const html = buildOrderInvoiceHtml(o);
      ...
      const openPDP = (p, opts = {}) => {
        ...
        try {
          if (pdpProduct && String(pdpProduct.id) === String(p.id) && !opts.force) {
            setPdpProduct(withPriceHistory(p));
            return;
          }
        } catch (_) {}
      ...
      const toggleCompare = (p) => {
        setCompare(prev => {
          if (isInCompareList(prev, p.id)) {
            const next = removeFromCompareList(prev, p.id);
            showCompareToast({ text: 'از مقایسه حذف شد' });
            return persistCompare(next);
          }
          if (!canAddToCompare(prev, COMPARE_MAX)) {
      ...
      const replaceCompareAt = (oldId, newProduct) => {
        setCompare(prev => {
          const final = replaceInCompareList(prev, oldId, newProduct);
      ...
      const getPageCms = (pageKey) => {
        const fromAdmin = getPageCmsFromMap(adminPageContent, pageKey);
        if (fromAdmin && (fromAdmin.body || fromAdmin.image || fromAdmin.video || (fromAdmin.faqs && fromAdmin.faqs.length))) {
          return fromAdmin;
        }
        const fallback = DEFAULT_LEGAL_PAGES && DEFAULT_LEGAL_PAGES[pageKey];
        if (fallback) return { ...fallback, ...(fromAdmin || {}) };
        return fromAdmin;
      };
      ...
      /** فروشگاه بدون فیلتر — از breadcrumb و منو */
      const goShop = () => {
        try { setMobileMenuOpen(false); } catch (_) {}
        try { setMegaOpen(null); } catch (_) {}
        try {
          if (typeof openPLP === 'function') {
            openPLP({ resetFilters: true, forceSkeleton: true });
            return;
          }
        } catch (_) {}
        try {
          ...
          try { window.location.assign(FA_PATHS.shop || '/فروشگاه'); } catch (__) {}
        }
      };
      ...
      useEffect(() => {
        ...
        const tmr = setTimeout(() => { try { endPageLoad(); } catch (_) {} }, 400);
        // safety: اگر به هر دلیل settle نشد، بعد از ۲ثانیه overlay را اجباری بردار
        const hard = setTimeout(() => {
          try { setPageLoadingText(null); } catch (_) {}
          try {
            if (typeof document !== 'undefined' && document.body) {
              if (document.body.dataset.drawerScrollLock !== '1' && !document.body.dataset.panelLock) {
                document.body.style.overflow = '';
                document.body.style.touchAction = '';
              }
            }
        ...
        return () => { clearTimeout(tmr); clearTimeout(hard); };
      }, [
        pageLoadingText,
        staticPage,
        showPLP,
       ...
       */
      const openPLP = (opts = {}) => {
        // اگر همین الان روی فروشگاه هستیم، loader تمام‌صفحه نیاید (علت اصلی «گیر کردن» با فیلتر)
        const _alreadyPlp = !!showPLP && !opts.forceSkeleton;
        if (!(opts && opts.silent) && !_alreadyPlp) beginPageLoad('shop');
        closeStaticPage();
        ...
        if (opts.cat !== undefined) {
          ...
          try { setPlpPriceMin(''); setPlpPriceMax(''); } catch (_) {}
          try { setPlpSellers([]); setPlpFabrics([]); } catch (_) {}
          try { setPlpDiscountOnly(false); setPlpMinDiscount(0); } catch (_) {}
          try { setPlpInStockOnly(false); setPlpFastShipOnly(false); } catch (_) {}
          try { setPlpCities([]); setPlpCityInput(''); } catch (_) {}
          try { setPlpSort(''); setPlpVisible(8); } catch (_) {}
        }
        ...
        try {
          if (!opts.silent) {
            ...
            } else if (opts.sort !== undefined && opts.cat == null && opts.tag == null && opts.brand == null && (opts.query == null || opts.query === '') && opts.cats === undefined && !opts.resetFilters) {
              try {
                const curPath = (typeof window !== 'undefined' && window.location && window.location.pathname) || (FA_PATHS.shop || '/فروشگاه');
                const base = curPath && curPath !== '/' ? curPath : (FA_PATHS.shop || '/فروشگاه');
                const u = new URL(base, 'https://pirahanmardane.ir');
                if (opts.sort) u.searchParams.set('sort', String(opts.sort));
                else u.searchParams.delete('sort');
                const next = u.pathname + (u.search || '');
                pushFaUrl(next, { plp: true, sort: opts.sort || '' });
              } catch (_) {
                pushFaUrl(pathForShop({ sort: opts.sort }), { plp: true });
              }
            } else if (opts.resetFilters || (opts.cat == null && opts.tag == null && opts.brand == null && (opts.query == null || opts.query === '') && opts.cats === undefined && opts.sort === undefined)) {
...
const verifyOtp = async () => {
      ...
      // mapServerProductToSellerUi → @/lib/catalog-map (با sellerUser)
  ...
  pushLiveToast(`بازگردانی ${toFa(list.length)} محصول انجام شد`, { type: 'info' });
      ...
      const ensureCatalogSnapshot = async () => {
        ...
        return snap;
            ...
            // mapExternalRowToProduct → @/lib/import-map
      const mapExternalRowToProduct = (row, source, warnings, idx) =>
        mapExternalRowToProductLib(row, source, warnings, idx, {
          colors: _catColors(),
          sizes: _catSizes(),
          brands: _catBrands(),
          categories: _catCategories(),
        });
      ...
      /** کلید و پیش‌فرض SEO برای صفحهٔ فعلی (ویرایش ادمین روی فرانت) */
            // getCurrentPageSeoContext → @/lib/page-seo-context
      const getCurrentPageSeoContext = () =>
        buildCurrentPageSeoContext({
          seoCfg,
          pdpProduct,
          showPLP,
          plpCats,
      ...
      const analyzeOnPageSeo = (opts) => analyzeOnPageSeoLib(opts);
      ...
      const pingIndexNow = async (urlList) => {
        ...
        try {
          ...
          return false;
            ...
            // buildLocalBusinessSchema → @/lib/seo-build
      const buildLocalBusinessSchema = () => buildLocalBusinessSchemaLib(seoCfg());
            // buildNewsSitemapXml → @/lib/seo-build
      const buildNewsSitemapXml = () => buildNewsSitemapXmlLib(seoCfg(), blogPosts);
            // buildVideoSitemapXml → @/lib/seo-build
      const buildVideoSitemapXml = () => buildVideoSitemapXmlLib(seoCfg(), (catalogProducts || products || []));
      ...
      const renderContentSeoBox = (opts) => {
        ...
        if (analysisOnly) {
          return (
            <div className="rounded-2xl border border-primary-200 border-white/15 bg-primary-50/40 bg-primary-900/20 p-3 sm:p-4 space-y-3" data-seo-analysis="1">
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <p className="text-sm font-bold text-primary-900 text-white flex items-center gap-1.5">
              ...
              <div className="rounded-xl border border-primary-100 border-white/10 bg-white bg-primary-900 p-3 space-y-1.5 max-h-48 overflow-y-auto">
                <p className="text-xs font-bold text-primary-700 text-white mb-1">تحلیل لحظه‌ای</p>
                {analysis.checks.map(c => (
                  <div key={c.id} className="flex items-start gap-2 text-xs">
                    ...
                    <div className="min-w-0">
                      <span className="font-medium text-primary-800 text-white">{c.label}</span>
                      <span className="text-primary-500 text-white/60"> — {c.detail}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        if (sellerStep2) {
          ...
          return (
            <div className="rounded-2xl border border-primary-200 border-white/15 bg-primary-50/40 bg-primary-900/20 p-3 sm:p-4 space-y-3" data-seo-box="seller-step2">
              {partTitle && <p className="text-sm font-bold text-primary-900 text-white">{partTitle}</p>}
              {show('keywords') && (
              <div>
              ...
              <div>
                ...
                <input value={title} onChange={e => onChange?.({ title: e.target.value })} maxLength={90} placeholder="خالی = نام محصول" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-white bg-primary-900 text-sm text-primary-900 text-white" />
              ...
              <div>
                ...
                <textarea value={description} onChange={e => onChange?.({ description: e.target.value })} rows={3} maxLength={320} placeholder="توضیح کوتاه نتایج گوگل" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-white bg-primary-900 text-sm text-primary-900 text-white resize-y min-h-[80px]" />
                <SeoPixelBars report={descReport} />
              </div>
              )}
              {show('faq') && onFaqChange && (
                <div className="rounded-xl border border-primary-100 border-white/10 bg-white bg-primary-900 p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-bold text-primary-700 text-white">FAQ (Schema)</p>
                    <button type="button" className="text-xs text-apple-blue" onClick={() => onFaqChange([...(faqItems || []), { q: '', a: '' }])}>+ پرسش</button>
                  </div>
                  {(faqItems || []).map((f, i) => (
                    <div key={i} className="space-y-1 p-2 rounded-lg border border-primary-50 border-white/5">
                      <input value={f.q || ''} onChange={e => { const next = [...faqItems]; next[i] = { ...next[i], q: e.target.value }; onFaqChange(next); }} placeholder="سؤال" className="w-full px-2 py-1.5 rounded-lg border border-primary-200 border-white/15 bg-transparent text-xs" />
                      <textarea value={f.a || ''} onChange={e => { const next = [...faqItems]; next[i] = { ...next[i], a: e.target.value }; onFaqChange(next); }} placeholder="پاسخ" rows={2} className="w-full px-2 py-1.5 rounded-lg border border-primary-200 border-white/15 bg-transparent text-xs resize-y" />
              ...
              <div className="rounded-xl border border-primary-100 border-white/10 bg-white bg-primary-900 p-3 space-y-2">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <p className="text-xs font-bold text-primary-700 text-white">پیشنهاد هوشمند (AI محلی)</p>
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  <button type="button" className="text-xs px-2.5 py-1.5 rounded-full border border-primary-200 border-white/20 text-primary-800 text-[#13ABC4] footer-link text-white" onClick={() => {
                    const role = sellerLimited ? 'seller' : 'admin';
                    if (!consumeSeoAiQuota(role)) { showToast({ message: 'سقف روزانه AI پر شده است', variant: 'error', duration: 4000, position: 'top-center' }); return; }
                    const g = aiGenerateSeoMeta({ name: contentTitle, desc: bodyText, focusKeywords, mode });
              ...
              {show('social') && (
              <div className="seo-social-preview rounded-xl border border-primary-200 border-white/25 bg-white bg-primary-800 overflow-hidden">
                <p className="text-xs font-semibold text-primary-600 text-[#13ABC4] !text-white/90 px-3 pt-2.5 pb-1.5">پیش‌نمایش شبکه اجتماعی</p>
                <div className="aspect-[1.91/1] max-h-28 bg-primary-100 bg-[#1A1C20] flex flex-col items-center justify-center gap-1.5 overflow-hidden border-y border-primary-100 border-white/15">
                  {ogImage || hasImage ? (
                    ...
                    <>
                      <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-primary-200 bg-primary-700 border border-primary-300 border-white/30">
                        <Icon name="image" size={18} className="text-primary-600 text-[#13ABC4] !text-white" />
                      </span>
                      <span className="text-xs font-medium text-primary-600 text-[#13ABC4] !text-white/85">بدون تصویر OG</span>
                    </>
                  )}
                </div>
                <div className="p-2.5 space-y-1 bg-white bg-primary-800">
                  <p className="text-xs text-primary-500 text-[#13ABC4] !text-white/70 truncate" dir="ltr">{base.replace(/^https?:\/\//, '')}</p>
                  <p className="text-sm font-bold text-primary-900 text-[#13ABC4] !text-white line-clamp-2">{title || contentTitle || 'عنوان'}</p>
                  <p className="text-xs text-primary-600 text-[#13ABC4] !text-white/80 line-clamp-2">{description || 'توضیح…'}</p>
                </div>
              </div>
              )}
              {(show('title') || show('desc')) && (
              <div className="rounded-xl border border-primary-200 border-white/25 bg-white bg-primary-800 p-3 space-y-3">
                <p className="text-xs font-semibold text-primary-600 text-[#13ABC4] !text-white/90">پیش‌نمایش گوگل (پیکسل دسکتاپ / موبایل)</p>
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-emerald-700 text-emerald-300 truncate" dir="ltr">{serpUrl}</p>
                  <p className="text-base text-blue-700 text-blue-400 font-medium" style={{ maxWidth: SEO_PX_LIMITS.titleDesktop, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', fontFamily: 'arial, sans-serif', fontSize: 20 }}>{title || 'عنوان سئو (خالی = نام محتوا)'}</p>
                  <p className="text-xs text-primary-600 text-white/70" style={{ maxWidth: SEO_PX_LIMITS.descDesktop, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', fontFamily: 'arial, sans-serif', fontSize: 14 }}>{description || 'توضیحات متا اینجا نمایش داده می‌شود…'}</p>
                  {(titleReport.deskOver || descReport.deskOver) && <p className="text-[10px] text-red-300">در دسکتاپ بخشی از متن بریده می‌شود</p>}
                </div>
                <div className="space-y-1 pt-2 border-t border-primary-100 border-white/10">
                  <p className="text-xs text-emerald-700 text-emerald-300 truncate" dir="ltr">{serpUrl}</p>
                  <p className="text-sm text-blue-700 text-blue-400 font-medium" style={{ maxWidth: SEO_PX_LIMITS.titleMobile, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', fontFamily: 'arial, sans-serif', fontSize: 16 }}>{title || 'عنوان سئو (خالی = نام محتوا)'}</p>
                  <p className="text-[11px] text-primary-600 text-white/70" style={{ maxWidth: SEO_PX_LIMITS.descMobile, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', fontFamily: 'arial, sans-serif', fontSize: 12 }}>{description || 'توضیحات متا اینجا نمایش داده می‌شود…'}</p>
        ...
        if (adminSeoLayout) {
          ...
          const stepTitle =
            ...
            null;
          return (
            <div className="rounded-2xl border border-primary-200 border-white/15 bg-primary-50/40 bg-primary-900/20 p-3 sm:p-4 space-y-3" data-seo-box="admin-layout">
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <p className="text-sm font-bold text-primary-900 text-white flex items-center gap-1.5">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input value={title} onChange={e => onChange?.({ title: e.target.value })} maxLength={90} placeholder="عنوان نمایش در گوگل" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-white bg-primary-900 text-sm text-primary-900 text-white" />
                  <SeoPixelBars report={titleReport} />
                </div>
                <div className="grid sm:grid-cols-2 gap-3">
                  <div className="rounded-xl border border-primary-200 border-white/20 bg-white bg-primary-900 p-3 space-y-1">
                    <p className="text-[10px] font-semibold text-primary-500">پیش‌نمایش گوگل · دسکتاپ</p>
                    <p className="text-sm text-[#1a0dab] text-[#8ab4f8] leading-snug" style={{ maxWidth: SEO_PX_LIMITS.titleDesktop, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', fontFamily: 'arial, sans-serif' }}>{title || 'عنوان سئو'}</p>
                    <p className="text-[11px] text-emerald-700 text-emerald-300 font-latin" dir="ltr" style={{ maxWidth: SEO_PX_LIMITS.titleDesktop, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{previewUrl || 'https://example.com/page'}</p>
                    <p className="text-[11px] text-primary-600 text-white/70" style={{ maxWidth: SEO_PX_LIMITS.descDesktop, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', fontFamily: 'arial, sans-serif', fontSize: 13 }}>{description || 'توضیحات متا…'}</p>
                  </div>
                  <div className="rounded-xl border border-primary-200 border-white/20 bg-white bg-primary-900 p-3 space-y-1">
                    <p className="text-[10px] font-semibold text-primary-500">پیش‌نمایش گوگل · موبایل</p>
                    <p className="text-sm text-[#1a0dab] text-[#8ab4f8] leading-snug" style={{ maxWidth: SEO_PX_LIMITS.titleMobile, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', fontFamily: 'arial, sans-serif' }}>{title || 'عنوان سئو'}</p>
                    <p className="text-[11px] text-primary-600 text-white/70" style={{ maxWidth: SEO_PX_LIMITS.descMobile, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', fontFamily: 'arial, sans-serif', fontSize: 12 }}>{description || 'توضیحات متا…'}</p>
                  </div>
                </div>
              </div>
              )}
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <textarea value={description} onChange={e => onChange?.({ description: e.target.value })} rows={3} maxLength={200} placeholder="توضیح کوتاه برای نتایج گوگل" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-white bg-primary-900 text-sm text-primary-900 text-white resize-y" />
                  <SeoPixelBars report={descReport} />
                </div>
                <div className="grid sm:grid-cols-2 gap-3">
                  <div className="rounded-xl border border-primary-200 border-white/20 bg-white bg-primary-900 p-3 space-y-1">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-xs font-bold text-primary-800 text-white">FAQ (Schema)</p>
                  <button type="button" className="text-xs text-apple-blue" onClick={() => onFaqChange([...(faqItems || []), { q: '', a: '' }])}>+ پرسش</button>
                </div>
                {(faqItems || []).map((it, idx) => (
                  <div key={idx} className="p-2 rounded-xl border border-primary-100 border-white/10 space-y-1.5">
                    <input value={it.q || ''} onChange={e => { const next = [...faqItems]; next[idx] = { ...next[idx], q: e.target.value }; onFaqChange(next); }} placeholder="سؤال" className="w-full px-2 py-1.5 rounded-lg border border-primary-200 border-white/20 bg-transparent text-xs text-primary-900 text-white" />
                    <textarea value={it.a || ''} onChange={e => { const next = [...faqItems]; next[idx] = { ...next[idx], a: e.target.value }; onFaqChange(next); }} placeholder="پاسخ" rows={2} className="w-full px-2 py-1.5 rounded-lg border border-primary-200 border-white/20 bg-transparent text-xs text-primary-900 text-white resize-y" />
              ...
              {show('index') && (
              <div className="rounded-xl border border-primary-200 border-white/20 bg-white bg-primary-900 p-3 space-y-3">
                <p className="text-xs font-bold text-primary-800 text-white">ایندکس</p>
                <label className="flex items-center justify-between gap-3 py-1">
                  <span className="text-xs text-primary-800 text-white">ایندکس در موتورهای جستجو</span>
                  <button type="button" role="switch" aria-checked={!noindex} onClick={() => onChange?.({ noindex: !noindex })} dir="ltr" className={`relative inline-flex h-7 w-12 items-center rounded-full p-0.5 transition-colors ${!noindex ? 'bg-emerald-500' : 'bg-primary-300 bg-primary-600'}`}>
              ...
              {show('canonical') && (
              <div className="rounded-xl border border-primary-200 border-white/20 bg-white bg-primary-900 p-3 space-y-2">
                <p className="text-xs font-bold text-primary-800 text-white">کنونیکال</p>
                <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">آدرس کنونیکال (Canonical)</label>
                <input value={canonical || ''} onChange={e => onChange?.({ canonical: e.target.value })} dir="ltr" placeholder="خالی = آدرس پیش‌فرض صفحه" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin text-primary-900 text-white" />
              </div>
              )}
              ...
              <div className="seo-social-preview rounded-xl border border-primary-200 border-white/25 bg-white bg-primary-800 overflow-hidden">
                <p className="text-xs font-semibold text-primary-600 text-[#13ABC4] !text-white/90 px-3 pt-2.5 pb-1.5">پیش‌نمایش شبکه اجتماعی</p>
                <div className="aspect-[1.91/1] max-h-28 bg-primary-100 bg-[#1A1C20] flex items-center justify-center overflow-hidden border-y border-primary-100 border-white/15">
                  {(ogImage || hasImage) ? (
                    <img loading="lazy" decoding="async" src={ogImage || undefined} alt="" className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                  ) : (
                    <span className="text-xs text-primary-400 text-white/50 flex flex-col items-center gap-1"><Icon name="image" size={22} /> بدون تصویر OG</span>
                  )}
                </div>
                <div className="px-3 py-2 space-y-0.5">
                  <p className="text-[10px] text-primary-400 text-[#13ABC4] font-latin" dir="ltr">{(previewUrl || '').replace(/^https?:\/\//, '') || 'example.com'}</p>
                  <p className="text-sm font-bold text-primary-900 text-white line-clamp-2">{title || 'عنوان'}</p>
                  <p className="text-xs text-primary-500 text-white/70 line-clamp-2">{description || 'توضیحات…'}</p>
                </div>
                <div className="px-3 pb-3">
                  <label className="text-[10px] text-primary-400 text-[#13ABC4] block mb-1">تصویر OG / شبکه اجتماعی</label>
                  <input value={ogImage || ''} onChange={e => onChange?.({ ogImage: e.target.value })} dir="ltr" placeholder="https://..." className="w-full px-2 py-1.5 rounded-lg border border-primary-200 border-white/20 bg-transparent text-xs text-left" />
                </div>
              </div>
              )}
              ...
              {show('ai') && (
              <div className="space-y-2">
                <p className="text-xs font-bold text-primary-800 text-white">پیشنهاد هوشمند (AI محلی)</p>
                <div className="flex flex-wrap gap-1.5">
                  <button type="button" className="text-xs px-2.5 py-1.5 rounded-full border border-primary-200 border-white/20 text-primary-800 text-[#13ABC4] footer-link text-white" onClick={() => {
        ...
        return (
          <div className="rounded-2xl border border-primary-200 border-white/15 bg-primary-50/40 bg-primary-900/20 p-3 sm:p-4 space-y-3" data-seo-box="1">
            <div className="flex items-center justify-between gap-2 flex-wrap">
              <p className="text-sm font-bold text-primary-900 text-white flex items-center gap-1.5">
            ...
            {/* SERP preview */}
            <div className="rounded-xl border border-primary-200 border-white/25 bg-white bg-primary-800 p-3 space-y-1">
              <p className="text-xs font-semibold text-primary-600 text-[#13ABC4] !text-white/90 mb-1">پیش‌نمایش گوگل</p>
              <p className="text-xs text-emerald-700 text-emerald-300 truncate" dir="ltr">{serpUrl}</p>
              <p className="text-base text-blue-700 text-blue-400 font-medium line-clamp-2">{title || 'عنوان سئو (خالی = نام محتوا)'}</p>
              <p className="text-xs text-primary-600 text-white/70 line-clamp-2">{description || 'توضیحات متا اینجا نمایش داده می‌شود…'}</p>
            </div>
            {/* Social preview */}
            <div className="seo-social-preview rounded-xl border border-primary-200 border-white/25 bg-white bg-primary-800 overflow-hidden">
              <p className="text-xs font-semibold text-primary-600 text-[#13ABC4] !text-white/90 px-3 pt-2.5 pb-1.5">پیش‌نمایش شبکه اجتماعی</p>
            ...
            <div className="rounded-xl border border-primary-100 border-white/10 bg-white bg-primary-900 p-3 space-y-2">
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <p className="text-xs font-bold text-primary-700 text-white">پیشنهاد هوشمند</p>
                <span className="text-xs text-primary-400">باقیمانده امروز: {toFa(getSeoAiQuota(sellerLimited ? 'seller' : 'admin').left)}</span>
              </div>
              <div className="flex flex-wrap gap-1.5">
                <button type="button" className="text-xs px-2.5 py-1.5 rounded-full bg-apple-blue text-white" onClick={() => {
            ...
            {linkIdeas.length > 0 && (
              <div className="rounded-xl border border-primary-100 border-white/10 bg-white bg-primary-900 p-3 space-y-2">
                <p className="text-xs font-bold text-primary-700 text-white">پیشنهاد لینک داخلی {sellerLimited ? '(فقط محصولات شما)' : ''}</p>
                <ul className="space-y-1">
                  {linkIdeas.map(l => (
                    <li key={l.id} className="text-xs flex items-center justify-between gap-2">
                      <span className="text-primary-800 text-white truncate">{l.name}</span>
                      <span className="text-primary-400 text-[#13ABC4] font-latin text-xs flex-shrink-0" dir="ltr">{l.path}</span>
                    </li>
                  ))}
                </ul>
            ...
            <div className="space-y-2">
              <label className="text-xs text-primary-500 text-[#13ABC4] block">تصویر شاخص (Featured / OG)</label>
              <div className="flex flex-wrap items-start gap-3">
                <div className="w-28 h-28 rounded-xl border border-primary-200 border-white/15 bg-primary-100 bg-primary-900 overflow-hidden flex items-center justify-center flex-shrink-0">
                  {ogImage ? (
                    ...
                    <span className="text-[10px] text-primary-400 text-[#13ABC4] text-center px-1">بدون تصویر</span>
                  )}
                </div>
                <div className="flex-1 min-w-[12rem] space-y-2">
                  <label className="inline-flex items-center gap-1.5 text-xs px-3 py-2 rounded-full border border-primary-200 border-white/20 cursor-pointer hover:bg-primary-50 hover:bg-primary-900 text-primary-800 text-white">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-white bg-primary-900 text-xs text-left font-latin text-primary-900 text-white"
                  />
                  {ogImage ? (
              ...
              <div>
                <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">تگ Alt تصویر شاخص</label>
                <input
                  value={imageAlt}
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-white bg-primary-900 text-sm text-primary-900 text-white resize-y min-h-[80px]"
              />
            </div>
            <div>
              <div className="flex items-center justify-between gap-2 mb-1">
            ...
            {!sellerLimited && (
              <>
                <div>
                  <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">Canonical (اختیاری)</label>
                  <input
                    value={canonical}
                    onChange={e => onChange?.({ canonical: e.target.value })}
                    dir="ltr"
                    placeholder="https://…"
                    className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-white bg-primary-900 text-sm text-left font-latin text-primary-900 text-white"
                  />
                </div>
      ...
      const saveAdminFrontEdit = () => {
        ...
        try {
          ...
          /* MAGAZINE_TAG_NOINDEX */
            ...
            // buildRobotsTxt → @/lib/seo-build
      const buildRobotsTxt = () => buildRobotsTxtLib(seoCfg());

            // buildSitemapXml → @/lib/seo-build
      const buildSitemapXml = () => buildSitemapXmlLib(seoCfg(), blogPosts, (catalogProducts || products || []));


            // buildSitemapIndexXml → @/lib/seo-build
      const buildSitemapIndexXml = () => buildSitemapIndexXmlLib(seoCfg(), (catalogProducts || products || []));
...
const openAdminPanel = (tab = 'dashboard', opts = {}) => {
      ...
      const adminFetchJson = async (url, opts = {}) => {
        ...
        return { ok: r.ok, status: r.status, body };
            ...
            // mapAdminProductRow → @/lib/admin-row-map
      const mapAdminProductRow = (p, sellersList) => mapAdminProductRowLib(p, sellersList);
      ...
      const hydrateAdminProducts = async () => {
        ...
        try {
          ...
          setAdminListLoading((s) => ({ ...s, products: false }));
            ...
            // mapAdminSellerRow → @/lib/admin-row-map
      const mapAdminSellerRow = (s) => mapAdminSellerRowLib(s);
      ...
      const clearPlpFilters = () => {
        ...
        setPlpSellers([]);
        setPlpFabrics([]);
        try { setPlpAttrs({}); } catch (_) {}
        try { setPlpMinSellerRating(0); } catch (_) {}
      ...
      const plpFiltered = catalogList.filter(p => {
        ...
        if (q) {
          const hay = `${p.name || ''} ${p.category || ''} ${p.category_name || ''} ${p.brand || ''} ${p.brand_name || ''} ${p.brandName || ''} ${(Array.isArray(p.categories) ? p.categories : []).join(' ')} ${(Array.isArray(p.tags) ? p.tags : []).join(' ')} ${p.seller?.name || ''} ${p.fabric || ''}`.toLowerCase();
        ...
        if (_plpCats.length > 0) {
          ...
          if (!_plpCats.some(c => prodCats.some(pc => pc === c || pc.includes(String(c)) || String(c).includes(pc)))) return false;
        }
        ...
        // seller filter removed from PLP UI
        ...
        // فیلتر ویژگی‌ها (فاز ۳)
        const _plpAttrs = (plpAttrs && typeof plpAttrs === 'object') ? plpAttrs : {};
        for (const [attrKey, selected] of Object.entries(_plpAttrs)) {
          if (!Array.isArray(selected) || selected.length === 0) continue;
          const raw = p.attributes && (p.attributes[attrKey] ?? p.attributes['attr-' + attrKey]);
          const vals = Array.isArray(raw) ? raw.map(String) : (raw != null && raw !== '' ? [String(raw)] : []);
          // fallback: خواندن از فیلدهای مشتق‌شده
          if (!vals.length && attrKey === 'fabric' && p.fabric) vals.push(String(p.fabric));
          if (!selected.some((s) => vals.includes(String(s)))) return false;
        }
        // حداقل امتیاز فروشنده
        const minRating = Number(plpMinSellerRating) || 0;
        if (minRating > 0) {
          const sid = p.seller?.id || p.sellerId || 'own';
          const sellerObj = (Array.isArray(topSellers) ? topSellers : []).find((s) => s && String(s.id) === String(sid));
          const rating = Number(sellerObj?.rating ?? p.rating ?? 0) || 0;
      ...
      Object.entries((plpAttrs && typeof plpAttrs === 'object') ? plpAttrs : {}).forEach(([ak, arr]) => {
        (Array.isArray(arr) ? arr : []).forEach((v) => {
          plpActiveChips.push({
            key: `attr-${ak}-${v}`,
            label: v,
            clear: () => setPlpAttrs((prev) => {
              const next = { ...(prev || {}) };
              const list = (Array.isArray(next[ak]) ? next[ak] : []).filter((x) => x !== v);
              if (list.length) next[ak] = list; else delete next[ak];
              return next;
            }),
          });
        });
      });
      if (Number(plpMinSellerRating) > 0) {
        plpActiveChips.push({
          key: 'min-seller-rating',
          label: `امتیاز فروشنده ≥ ${plpMinSellerRating}`,
          clear: () => setPlpMinSellerRating(0),
        });
      }
      ...
      const plpSortLabel = plpSortLabelOf(plpSort);
      ...
      // formatPrice / scrollCarousel → @/lib/format-price
      ...
      const CarouselArrows = ({ trackRef }) => (
        <>
          <button
            ...
            className="hidden sm:flex absolute top-1/2 -translate-y-1/2 right-0 z-10 w-9 h-9 items-center justify-center rounded-full bg-white/95 bg-primary-800 shadow-md border border-primary-200 border-white/30 text-primary-800 text-white hover:bg-primary-50 hover:bg-primary-700"
          >
          ...
          <button
            ...
            aria-label="بعدی"
            onClick={() => scrollCarousel(trackRef?.current, -1)}
            className="hidden sm:flex absolute top-1/2 -translate-y-1/2 left-0 z-10 w-9 h-9 items-center justify-center rounded-full bg-white/95 bg-primary-800 shadow-md border border-primary-200 border-white/30 text-primary-800 text-white hover:bg-primary-50 hover:bg-primary-700"
          >
            ...
            {list.map((row, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-2 text-xs">
                  ...
                  <div className="flex-1 h-2 rounded-full bg-primary-100 bg-white/10 overflow-hidden">
                    <div className="h-full rounded-full bg-apple-blue" style={{ width: `${Math.min(100, (val / max) * 100)}%` }} />
                  </div>
                  <span className="w-14 text-left tabular-nums text-primary-600 text-white/70" dir="ltr">{toFa(Math.round(val).toLocaleString('en-US'))}</span>
                </div>
              );
            })}
          </div>
      ...
      const renderProductCard = useCallback((p, keyPrefix = '', opts = {}) => { /* card-height-v2 */
        ...
        return (
          <div
            ...
            className={`gsap-card product-card pm-soft-enter bg-white bg-black rounded-xl sm:rounded-2xl overflow-hidden shadow-sm flex flex-col cursor-pointer select-none border border-primary-100/80 border-white/10 ${gridMode ? 'w-full h-full' : 'flex-shrink-0 w-[78%] min-[400px]:w-[70%] sm:w-[42%] md:w-[calc((100%-2.5rem)/3.3)] lg:w-[calc((100%-3.5rem)/4.3)] snap-start'}`}
          >
            <div className="px-2.5 pt-2 sm:px-2.5 sm:pt-2 sm:pb-0">
              <h3 className="text-[18px] sm:text-2xl font-bold text-primary-900 text-white leading-snug">
                <TipText text={p.name} lines={1} className="font-bold text-primary-900 text-white text-[18px] sm:text-2xl leading-snug block w-full" />
              </h3>
              <div className="mt-1 flex flex-col items-start gap-0.5 min-h-[2.5rem] sm:min-h-[2.25rem]">
                <button
                  ...
                  className="product-seller-badge inline-flex items-center gap-1 h-6 sm:h-7 px-1.5 sm:px-2 rounded-md bg-[#023047] bg-[#13ABC4] text-white !text-white text-xs font-bold whitespace-nowrap max-w-full transition border border-[#023047] border-[#13ABC4] cursor-pointer hover:opacity-90"
                ...
                <button
                  ...
                  onClick={() => {
                    try {
                      const cat = (p && (p.category || p.cat)) || '';
                      if (cat && typeof openCategory === 'function') openCategory(cat);
                      else if (typeof openPLP === 'function') openPLP({ cat });
                    } catch (_) {}
                  }}
                  className="product-category-badge inline-flex items-center h-6 sm:h-7 px-1.5 sm:px-2 rounded-md bg-primary-100 bg-primary-700 text-primary-900 text-[#13ABC4] !text-white text-xs font-semibold whitespace-nowrap transition border border-primary-200/80 border-white/25 cursor-pointer hover:opacity-90"
            ...
            <div className="relative aspect-[1/1] overflow-hidden bg-primary-50 bg-black mx-2 my-1.5 sm:mx-2.5 sm:my-1.5 rounded-md sm:rounded-xl group/img">
              <img src={activeColor.image || p?.image || '/logo.webp'} alt={`${p.name || ''} - ${activeColor.name || ''}`} loading="lazy" decoding="async" referrerPolicy="no-referrer" className={`w-full h-full object-cover transition-opacity duration-300 pointer-events-none ${colors.length > 1 ? 'group-hover/img:opacity-0' : ''}`} onError={(e) => { e.currentTarget.classList.add('img-broken'); e.currentTarget.src = (document.documentElement.classList.contains('dark') ? '/blue_w_bg.webp' : '/Pirrahanmardane-logo.webp'); }} />
              ...
              <div className="absolute bottom-1.5 right-1.5 z-10 flex items-center gap-1.5">
                <button
                  ...
                  className={`w-11 h-11 sm:w-10 sm:h-10 rounded-full flex items-center justify-center shadow-md transition ${inCompare ? 'bg-apple-blue text-white !text-white' : 'bg-white/95 bg-primary-800 text-primary-700 !text-white hover:bg-white hover:bg-primary-700 hover:text-[#023047] hover:text-[#7EFAFF] hover:!text-[#7EFAFF] text-[#13ABC4] border border-primary-100 border-white/20'}`}
                  title="مقایسه"
                  aria-label="مقایسه"
                >
                ...
                <button
                  ...
                  className={`w-11 h-11 sm:w-10 sm:h-10 rounded-full flex items-center justify-center shadow-md transition ${isFav ? 'bg-apple-blue text-white !text-white' : 'bg-white/95 bg-primary-800 text-primary-700 !text-white hover:bg-white hover:bg-primary-700 hover:text-[#023047] hover:text-[#7EFAFF] hover:!text-[#7EFAFF] text-[#13ABC4] border border-primary-100 border-white/20'}`}
            ...
            <div className="px-2.5 pb-2.5 sm:px-3.5 sm:pb-3.5 md:px-4 md:pb-4 mt-auto">
                  ...
                  {[1, 2, 3, 4, 5].map((n) => {
                    ...
                    return (
                      <Icon
                        ...
                        size={14}
                        className={filled ? 'text-primary-400' : 'text-primary-200 text-primary-600'}
                      />
                    );
                  })}
                </div>
                <span className="text-xs sm:text-xs text-primary-500 text-white tabular-nums">
                  {toFa(Number(seller.rating ?? p.rating ?? 0).toFixed(1))}
                  {seller.ratingCount ? (
                    <span className="text-primary-400 text-white"> ({toFa(seller.ratingCount)})</span>
                  ) : null}
                </span>
              </div>
              <div className="flex items-center justify-between gap-2 sm:gap-2.5 mt-0.5">
                <div className="min-w-0 flex items-center gap-1.5 relative">
                  <p className="product-card-price font-black text-[15px] sm:text-base leading-none text-primary-900 text-[#13ABC4] !text-white tabular-nums">
                    {p.priceText} <span className="font-medium text-[11px] sm:text-xs text-primary-500 text-[#13ABC4] !text-white/80">تومان</span>
                  </p>
                  {p.oldPrice ? (
                    <button
                      type="button"
                  ...
                  {p.oldPrice && showOld ? (
                    <span className="absolute bottom-full right-0 mb-1.5 z-30 whitespace-nowrap rounded-lg bg-primary-900 bg-primary-800 text-white text-xs px-2.5 py-1.5 shadow-lg">
                ...
                <button
                  ...
                  className="product-add-btn px-3 sm:px-4 py-2 text-xs sm:text-sm font-bold rounded-xl border border-transparent bg-apple-blue !text-white !text-white hover:bg-blue-700 hover:!text-white hover:!bg-[#0f96ad] hover:!text-white transition flex-shrink-0 shadow-sm shadow-apple-blue/20"
                >
                  افزودن
                </button>
              </div>
      ...
      const renderShareBar = (opts = {}) => {
        ...
        const btn = 'w-10 h-10 rounded-full flex items-center justify-center border border-primary-200 border-white/20 bg-white bg-primary-950 text-primary-700 text-white hover:border-apple-blue hover:text-apple-blue transition shadow-sm';
        return (
          <div className="mt-8 sm:mt-10 pt-6 border-t border-primary-100 border-white/15" data-share-bar="1">
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
              <p className="text-sm font-bold text-primary-900 text-white flex items-center gap-2 shrink-0">
...
const params = new URLSearchParams(window.location.search);
      ...
      return (
        ...
        <a href="#main-content" className="skip-to-main">پرش به محتوای اصلی</a>
        <div id="main-content" className="min-h-screen flex flex-col">
          {(pageLoadingText || (awaitingDeepProduct && !pdpProduct)) ? (
            <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-white/80 bg-black/50 backdrop-blur-[2px]" role="status" aria-live="polite">
              <div className="flex flex-col items-center gap-4 px-6 py-6 rounded-2xl min-w-[180px]">
                <LumaSpin />
                <p className="text-sm font-medium text-[#023047] text-center">
          ...
          <header
            ...
            className={`transition-all duration-500 ease-out fixed top-0 left-0 right-0 z-[100] overflow-visible flex flex-col ${
              ...
              (!activeSellerId && !showSellersList && !showPLP && !showTaxonomyHub && !pdpProduct && !showCartPage && !showCheckout && !showWishlistPage && !showRecentPage && !showComparePage && !showProfilePage && !showSellerPanel && !showAdminPanel && !staticPage)
                ...
                : 'translate-y-0 opacity-100'
            } ${scrolled || headerRevealedAfterHero || cartOpen || wishlistOpen || compareOpen || recentOpen || notifPanelOpen || mobileMenuOpen ? 'bg-white bg-primary-900 shadow-md border-b border-primary-100/50 border-primary-800/50' : 'bg-white bg-primary-900 shadow-sm'}`}
          >
            {/* Top bar — موبایل: پایین (منو/جستجو/ورود) · دسکتاپ: بالا */}
            <div className="order-2 md:order-1 bg-primary-50 bg-primary-900 text-primary-400 text-white text-xs sm:text-sm py-1 sm:py-2 px-2.5 sm:px-4 border-b border-primary-200 border-white/30 transition-colors relative overflow-visible">
              <div className="max-w-7xl mx-auto flex justify-between items-center gap-2">
                ...
                <div className="flex md:contents items-center gap-1.5 sm:gap-2 flex-shrink-0">
                  <button
                    ...
                    className="header-burger-btn md:hidden relative w-10 h-10 flex items-center justify-center rounded-full hover:bg-primary-100 hover:bg-primary-800 text-primary-600 text-white"
                    aria-label="منو"
                  >
                    <Icon name={mobileMenuOpen ? "x" : "menu"} size={20} />
                    {!mobileMenuOpen && unreadNotifCount > 0 && (
                      <span className="absolute top-1 right-1 min-w-[8px] h-2 w-2 rounded-full bg-transparent hover:bg-primary-50 hover:bg-white/10 border border-primary-200 border-white/25" aria-hidden />
                    )}
                  </button>
                  </div>
                ...
                <div className="md:hidden flex-1 min-w-0 mx-1 relative z-[300] overflow-visible">
                  <div className="flex items-stretch bg-white bg-primary-900 rounded-2xl border-2 border-primary-200 border-white/25 shadow-sm overflow-hidden h-[48px] focus-within:border-apple-blue focus-within:border-[#023047]">
                    <button
                      type="button"
                      ...
                      className="flex items-center gap-0.5 flex-shrink-0 px-2.5 text-xs font-bold text-primary-800 text-white border-l border-primary-200 border-white/30 bg-primary-50/80 bg-primary-800/40"
                    >
                      <span className="max-w-[3.2rem] truncate">فیلتر</span>
                      <Icon name="chevronDown" size={14} className={`transition-transform text-white ${catOpen ? 'rotate-180' : ''}`} />
                    </button>
                    <div className="relative flex-1 min-w-0">
                      <input
                        ...
                        className={`w-full bg-transparent text-primary-900 text-white py-3 pr-9 text-sm text-right focus:outline-none caret-primary-900 caret-white placeholder:text-sm placeholder:text-primary-400 placeholder:text-white/55 ${(searchQuery || searchColors.length > 0 || searchSizes.length > 0 || searchCategories.length > 0) ? 'pl-7' : 'pl-1.5'}`}
                      />
                      <div className="absolute right-1.5 top-1/2 -translate-y-1/2 text-primary-400 text-[#13ABC4] pointer-events-none">
                  ...
                  {searchSuggestOpen && !catOpen && (
                    <div className="absolute top-full right-0 left-0 mt-1.5 z-[500] w-full max-h-[min(70vh,28rem)] overflow-y-auto bg-white !bg-black rounded-2xl shadow-2xl border border-primary-200 border-white text-right isolate" role="listbox">
                      {!searchQuery.trim() && (recentSearches || []).length > 0 && (
                        <div className="px-3 pt-3 pb-2">
                          <div className="flex items-center justify-between mb-1.5">
                            <span className="text-xs font-bold text-primary-400">جستجوهای اخیر</span>
                            <button type="button" onClick={clearRecentSearches} className="text-xs text-primary-400 text-[#13ABC4] footer-link hover:text-red-300">پاک کردن</button>
                          </div>
                          <div className="flex flex-wrap gap-1.5">
                            {(recentSearches || []).map(q => (
                              <button key={q} type="button" onClick={() => submitSearch(q)} className="search-trend-chip text-xs px-2.5 py-1 rounded-full bg-primary-50 bg-[#2A2C30] !text-primary-800 text-[#13ABC4] !text-white border border-primary-200 border-white/40 font-medium">{q}</button>
                            ))}
                          </div>
                        </div>
                      )}
                      {!searchQuery.trim() && (
                        <div className="px-3 pt-2 pb-3">
                          <span className="text-xs font-bold text-primary-600 text-[#13ABC4] !text-white/90">پرتکرار</span>
                          <div className="flex flex-wrap gap-1.5 mt-1.5">
                            {TREND_QUERIES.map(q => (
                              <button key={q} type="button" onClick={() => submitSearch(q)} className="search-trend-chip text-xs px-2.5 py-1 rounded-full border border-primary-300 border-white/50 !text-primary-900 text-[#13ABC4] !text-white bg-white bg-[#2A2C30] font-medium hover:border-apple-blue">{q}</button>
                            ))}
                          </div>
                        </div>
                      )}
                      {searchQuery.trim() && searchCategorySuggestions.length > 0 && (
                        <div className="px-3 pt-3 pb-1">
                          ...
                          {searchCategorySuggestions.map(c => (
                            <button key={c} type="button" onClick={() => submitSearch(`پیراهن ${c}`)} className="w-full text-right px-2 py-2 text-sm text-primary-800 text-white hover:bg-primary-50 hover:bg-primary-800 rounded-lg flex items-center gap-2">
                      ...
                      {searchQuery.trim() && searchBrandSuggestions.length > 0 && (
                        <div className="px-3 pt-2 pb-1">
                          ...
                          {searchBrandSuggestions.map(b => (
                            <button key={b} type="button" onClick={() => submitSearch(b)} className="w-full text-right px-2 py-2 text-sm text-primary-800 text-white hover:bg-primary-50 hover:bg-primary-800 rounded-lg">{b}</button>
                          ))}
                        </div>
                      )}
                      {searchQuery.trim() && (
                        <div className="px-1 pt-1 pb-1">
                          ...
                          {filteredProducts.length === 0 ? (
                            <div className="px-3 py-5 text-center text-sm text-primary-500">
                              ...
                              <button type="button" onClick={() => submitSearch(searchQuery)} className="block mx-auto mt-2 text-xs underline text-primary-600 text-white/70">جستجوی همه نتایج</button>
                            </div>
                            ...
                            return (
                              <button
                                ...
                                className="w-full flex items-center gap-3 px-3 py-2.5 bg-white !bg-black hover:bg-primary-50 hover:!bg-[#1A1C20] transition border-b border-primary-50 border-white/15 last:border-0 text-right relative z-10"
                              >
                                <img src={col?.image || p.image} alt="" className="w-12 h-12 rounded-lg object-cover flex-shrink-0" loading="lazy" decoding="async" />
                                <div className="min-w-0 flex-1">
                                  <p className="text-sm font-medium text-primary-900 text-white line-clamp-2">{p.name}</p>
                                  <p className="text-xs text-primary-500 text-[#13ABC4] !text-white/70">{p.category} · {p.seller?.name || 'فروشگاه مرکزی'}</p>
                                  <p className="text-xs font-bold text-primary-700 text-[#13ABC4] mt-0.5">{p.priceText} تومان{p.discount ? ` · ${toFa(p.discount)}٪` : ''}</p>
                                </div>
                              </button>
                            );
                          })}
                          {filteredProducts.length > 0 && (
                            <button type="button" onClick={() => submitSearch(searchQuery)} className="w-full py-2.5 text-xs font-bold text-apple-blue hover:bg-primary-50 hover:bg-primary-800">
                ...
                {/* Desktop mega-nav */}
                <nav className="hidden md:flex items-center gap-0.5 lg:gap-1.5 flex-1 justify-start min-w-0 overflow-x-auto no-scrollbar max-w-full">
                  <a href="/" onClick={(e) => { e.preventDefault(); try { beginPageLoad('home'); } catch(_){} try { pushFaUrl(FA_PATHS.home || '/'); } catch(_){} try { applyPathRef.current(); } catch(_){} }} className="px-2.5 py-1 rounded-md text-[#023047] text-white hover:text-[#012536] hover:text-white hover:bg-[rgba(2,48,71,0.08)] hover:bg-primary-800/60 transition whitespace-nowrap text-sm font-medium leading-5 ">خانه</a>
                  ...
                  >
                    <a href="/فروشگاه" onClick={(e) => { e.preventDefault(); openPLP(); }} className={`flex items-center gap-0.5 px-2 lg:px-2.5 py-1 rounded-md transition whitespace-nowrap text-sm font-medium leading-5 ${megaOpen === 'shop' || showPLP ? 'text-primary-800 text-white bg-primary-100/60 bg-primary-800/60' : 'text-[#023047] text-white hover:text-[#012536] hover:text-white hover:bg-[rgba(2,48,71,0.08)] hover:bg-primary-800/60'}`}>
                      فروشگاه
                  ...
                  <div
                    ...
                    onMouseLeave={() => { megaTimeout.current = setTimeout(() => setMegaOpen(null), 180); }}
                  >
                    <button type="button" onClick={() => setMegaOpen(v => v === 'cats' ? null : 'cats')} className={`flex items-center gap-0.5 px-2 lg:px-2.5 py-1 rounded-md transition whitespace-nowrap text-sm font-medium leading-5 ${megaOpen === 'cats' ? 'text-primary-800 text-white bg-primary-100/60 bg-primary-800/60' : 'text-[#023047] text-white hover:text-[#012536] hover:text-white hover:bg-[rgba(2,48,71,0.08)] hover:bg-primary-800/60'}`}>
                      دسته‌بندی‌ها
                      <Icon name="chevronDown" size={14} className={`transition-transform ${megaOpen === 'cats' ? 'rotate-180' : ''}`} />
                    </button>
                  </div>

                  <button type="button" onClick={openSellersList} className="px-2 lg:px-2.5 py-1 rounded-md text-[#023047] text-white hover:text-[#012536] hover:text-white hover:bg-[rgba(2,48,71,0.08)] hover:bg-primary-800/60 transition whitespace-nowrap text-sm font-medium leading-5 ">فروشندگان</button>
                  <button type="button" onClick={openCartPage} className={`px-2 lg:px-2.5 py-1 rounded-md transition whitespace-nowrap text-sm font-medium leading-5 ${showCartPage ? 'text-primary-800 text-white bg-primary-100/60 bg-primary-800/60' : 'text-[#023047] text-white hover:text-[#012536] hover:text-white hover:bg-[rgba(2,48,71,0.08)] hover:bg-primary-800/60'}`}>سبد خرید{hasMounted && cartCount > 0 ? ` (${toFa(cartCount)})` : ''}</button>
                  <button type="button" onClick={openWishlistPage} className={`hidden xl:inline-flex px-2 lg:px-2.5 py-1 rounded-md transition whitespace-nowrap text-sm font-medium leading-5 ${showWishlistPage ? 'text-primary-800 text-white bg-primary-100/60 bg-primary-800/60' : 'text-[#023047] text-white hover:text-[#012536] hover:text-white hover:bg-[rgba(2,48,71,0.08)] hover:bg-primary-800/60'}`}>علاقه‌مندی‌ها{hasMounted && favorites.length > 0 ? ` (${toFa(favorites.length)})` : ''}</button>
                  <button type="button" onClick={openComparePage} className={`hidden xl:inline-flex px-2 lg:px-2.5 py-1 rounded-md transition whitespace-nowrap text-sm font-medium leading-5 ${showComparePage ? 'text-primary-800 text-white bg-primary-100/60 bg-primary-800/60' : 'text-[#023047] text-white hover:text-[#012536] hover:text-white hover:bg-[rgba(2,48,71,0.08)] hover:bg-primary-800/60'}`}>مقایسه{hasMounted && compare.length > 0 ? ` (${toFa(compare.length)})` : ''}</button>
                  <button type="button" onClick={() => openStaticPage('about')} className="hidden md:inline-flex px-2 lg:px-2.5 py-1 rounded-md text-[#023047] text-white hover:text-[#012536] hover:text-white hover:bg-[rgba(2,48,71,0.08)] hover:bg-primary-800/60 transition whitespace-nowrap text-sm font-medium leading-5 ">درباره ما</button>
                  <button type="button" onClick={() => openStaticPage('contact')} className="px-2 lg:px-2.5 py-1 rounded-md text-[#023047] text-white hover:text-[#012536] hover:text-white hover:bg-[rgba(2,48,71,0.08)] hover:bg-primary-800/60 transition whitespace-nowrap text-sm font-medium leading-5 ">تماس با ما</button>
                  <button type="button" onClick={() => openStaticPage('deals')} className="hidden md:inline-flex px-2 lg:px-2.5 py-1 rounded-md text-[#023047] text-white hover:text-[#012536] hover:text-white hover:bg-[rgba(2,48,71,0.08)] hover:bg-primary-800/60 transition whitespace-nowrap text-sm font-medium leading-5 ">شگفت‌انگیز</button>
                  <button type="button" onClick={() => openStaticPage('brands')} className="hidden md:inline-flex px-2 lg:px-2.5 py-1 rounded-md text-[#023047] text-white hover:text-[#012536] hover:text-white hover:bg-[rgba(2,48,71,0.08)] hover:bg-primary-800/60 transition whitespace-nowrap text-sm font-medium leading-5 ">برندها</button>
                </nav>

                <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0 relative z-10 ps-2 ms-1 border-s border-primary-200/80 border-white/20">
                  {user ? (
                    <button
                      type="button"
                      onClick={() => openProfilePage()}
                      className="hover:text-primary-600 hover:text-white flex items-center gap-1 text-primary-400 text-white"
                    >
                    ...
                    {sellerUser ? (
                      <button
                        ...
                        className="hover:text-primary-600 hover:text-white flex items-center gap-1 text-primary-700 text-white whitespace-nowrap text-sm font-medium leading-5 shrink-0 font-medium"
                        title="پنل فروشنده"
                      >
                        <Icon name="user" size={14} />
                        <span className="text-sm font-medium leading-5 max-w-[9rem] truncate">
                      ...
                      <button
                        ...
                        className="hover:text-primary-600 hover:text-white flex items-center gap-1 text-primary-700 text-white whitespace-nowrap text-sm font-medium leading-5 shrink-0 font-medium px-1"
                        title="ورود / ثبت‌نام"
                      >
                        <Icon name="user" size={15} />
                        <span className="hidden sm:inline text-sm font-medium leading-5 whitespace-nowrap text-sm font-medium leading-5 text-[#023047] text-white">ورود</span>
              ...
              {megaOpen && (
                ...
                >
                  <div className="mega-menu-panel bg-white bg-[#1A1C20] border-b border-primary-200 border-white/25 shadow-2xl">
                    <div className="max-w-7xl mx-auto px-4 py-6 relative">
                      <button
                        type="button"
                        onClick={() => setMegaOpen(null)}
                        className="absolute left-4 top-4 z-10 w-9 h-9 flex items-center justify-center rounded-full bg-primary-100 bg-primary-800 text-primary-700 text-[#13ABC4] !text-white hover:bg-primary-200 hover:bg-primary-700 border border-primary-200 border-white shadow-sm"
                        aria-label="بستن منو"
                        title="بستن"
                      >
                        <Icon name="x" size={18} />
                      ...
                      {megaOpen === 'shop' && (
                        <div className="grid grid-cols-12 gap-6">
                          <div className="col-span-3 border-l border-primary-200 border-white/30 pl-5">
                            <h4 className="text-xs font-bold text-primary-400 text-white mb-3 tracking-wide">دسته‌بندی‌ها</h4>
                            <ul className="space-y-1">
                              {([{ name: 'همه محصولات', icon: 'grid' }, ...((Array.isArray(adminCategories) ? adminCategories : []).filter((c) => c && c.active !== false && String(c.name || '').trim()).map((c) => ({ name: String(c.name).trim(), icon: c.icon || 'grid' })))].filter((c, i, arr) => arr.findIndex((x) => x.name === c.name) === i)).map((cat) => (
                                <li key={cat.name}>
                                  <button type="button" onClick={() => { setMegaOpen(null); openPLP(cat.name === 'همه محصولات' ? { resetFilters: true } : { cat: cat.name }); }} className="w-full flex items-center gap-2.5 px-2 py-2 rounded-lg text-sm text-[#023047] !text-white hover:bg-[rgba(2,48,71,0.08)] hover:bg-primary-900 transition group text-right">
                                    <span className="shop-menu-cat-icon w-8 h-8 rounded-full bg-primary-200 bg-primary-900 flex items-center justify-center text-[#023047] !text-white group-hover:bg-[#023047] group-hover:text-white group-hover:bg-[#023047]/20 group-hover:!text-[#7EFAFF] transition flex-shrink-0">
                                      <Icon name={cat.icon || 'grid'} size={16} className="text-current" />
                                    </span>
                                    <span className="whitespace-nowrap text-[#023047] !text-white group-hover:!text-[#7EFAFF]">{cat.name}</span>
                                  </button>
                                </li>
                              ))}
                            </ul>
                          ...
                          <div className="col-span-6">
                            <h4 className="text-xs font-bold text-primary-400 text-white mb-3 tracking-wide">پرفروش‌ترین‌ها</h4>
                            <div className="grid grid-cols-3 gap-3">
                              {products.slice(0, 3).map((p) => {
                                const col = (p.colors && p.colors[0]) || { name: '', image: p.image || p.cover_image || '/logo.webp' };
                                return (
                                  <button key={p.id} type="button" onClick={() => setMegaOpen(null)} className="group rounded-xl border border-primary-200 border-white/30 overflow-hidden hover:shadow-md transition bg-white bg-primary-900 text-right">
                                    <div className="aspect-[4/5] overflow-hidden bg-primary-50 bg-primary-950">
                                    ...
                                    <div className="p-2.5">
                                      <p className="text-xs font-medium text-primary-900 text-white line-clamp-2" title={p.name}>{p.name}</p>
                                      <p className="text-xs font-bold text-primary-700 text-white mt-1">{p.priceText} <span className="font-normal text-primary-400 text-white">تومان</span></p>
                                    </div>
                                  </button>
                                );
                              })}
                          ...
                          <div className="col-span-3">
                            <button type="button" onClick={() => { setMegaOpen(null); try { openStaticPage('deals'); } catch (_) {} }} className="block w-full h-full min-h-[220px] rounded-2xl overflow-hidden relative group text-right isolate ring-1 ring-black/5 ring-white/10">
                              <img loading="lazy" decoding="async"
                                ...
                                loading="lazy"
                                decoding="async"
                                onError={(e) => { try { e.currentTarget.onerror = null; e.currentTarget.src = (document.documentElement.classList.contains('dark') ? '/blue_w_bg.webp' : '/Pirrahanmardane-logo.webp'); } catch (_) {} }}
                              />
                      ...
                      {megaOpen === 'cats' && (
                        <div className="grid grid-cols-12 gap-6">
                          <div className="col-span-8">
                            <h4 className="text-xs font-bold text-primary-400 text-[#13ABC4] !text-white mb-4 tracking-wide">همه دسته‌بندی‌ها</h4>
                            <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                              {([{ name: 'همه محصولات', icon: 'grid' }, ...((Array.isArray(adminCategories) ? adminCategories : []).filter((c) => c && c.active !== false && String(c.name || '').trim()).map((c) => ({ name: String(c.name).trim(), icon: c.icon || 'grid' })))].filter((c, i, arr) => arr.findIndex((x) => x.name === c.name) === i)).map((cat) => (
                                <button
                                  key={cat.name}
                                  type="button"
                                  onClick={() => { setMegaOpen(null); openPLP(cat.name === 'همه محصولات' ? { resetFilters: true } : { cat: cat.name }); }}
                                  className="flex flex-col items-center gap-2 p-4 rounded-2xl border border-primary-200 border-white/30 bg-primary-50/50 bg-primary-900/50 hover:bg-white hover:bg-primary-900 hover:shadow-md transition group"
                                >
                                  <span className="w-12 h-12 rounded-full border-2 border-transparent border-white/50 flex items-center justify-center text-[#023047] !text-white bg-primary-100 bg-primary-900 group-hover:border-[#023047] group-hover:bg-[#023047] group-hover:text-white group-hover:border-[#023047] group-hover:bg-primary-900 group-hover:!text-[#7EFAFF] transition">
                                    <Icon name={cat.icon} size={22} />
                                  </span>
                                  <span className="text-xs font-medium text-[#023047] !text-white text-center whitespace-nowrap group-hover:text-[#023047] group-hover:!text-[#7EFAFF]">{cat.name}</span>
                                </button>
                              ))}
                            </div>
                          </div>
                          <div className="col-span-4 flex flex-col gap-3">
                            <h4 className="text-xs font-bold text-primary-400 text-[#13ABC4] !text-white tracking-wide">دسترسی سریع</h4>
                            {[
                              ...
                              <button key={item.label} type="button" onClick={() => { setMegaOpen(null); item.go(); }} className="flex items-center justify-between px-4 py-3 rounded-xl bg-primary-50 bg-primary-900 text-sm text-[#023047] text-white hover:bg-[rgba(2,48,71,0.08)] hover:bg-primary-800 hover:text-[#7EFAFF] transition text-right">
                                {item.label}
                                <Icon name="chevronLeft" size={14} />
                              </button>
                            ))}
                            <div className="mt-auto rounded-xl bg-gradient-to-br from-primary-800 to-primary-950 from-primary-800 to-primary-900 text-white p-4">
            ...
            <div className="order-1 md:order-2 max-w-7xl mx-auto px-3 sm:px-4 py-1.5 sm:py-2.5 w-full">
              <div className="flex items-center gap-1.5 sm:gap-2 flex-row w-full">
                ...
                <a href="#" onClick={(e) => { e.preventDefault(); setActiveSellerId(null); setShowSellersList(false); setShowPLP(false); setShowCartPage(false); setShowWishlistPage(false); setShowComparePage(false); setShowProfilePage(false); setShowSellerPanel(false); setShowAdminPanel(false); setMobileMenuOpen(false); scrollPageToTop(); }} className="flex items-center flex-shrink-0 order-1" aria-label="پیراهن مردانه — خانه">
                  <img loading="lazy" decoding="async" src={dark ? "/blue_t_bg.webp" : "/Pirrahanmardane-logo-T.webp"} alt="پیراهن مردانه" className="site-logo-img h-8 sm:h-10 md:h-11 w-auto max-w-[148px] sm:max-w-[180px] md:max-w-[200px] object-contain object-right" onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = '/blue_t_bg.webp'; }} />
                </a>

                {/* Search box with category tab — desktop */}
                <div className="hidden md:block flex-1 min-w-0 relative order-2 z-20">
                  <div className="flex items-stretch bg-primary-50 bg-primary-900 rounded-full overflow-hidden border border-transparent border-white focus-within:ring-2 focus-within:ring-primary-300 focus-within:ring-white/30 transition">
                    {/* Category tab */}
                    <button
                      type="button"
                      data-filter-toggle="true"
                      onClick={() => setCatOpen(v => !v)}
                      className="flex items-center gap-1 flex-shrink-0 px-2.5 sm:px-3.5 text-xs sm:text-xs font-medium text-primary-800 text-white border-l border-primary-200 border-white hover:bg-primary-100 hover:bg-primary-800 transition whitespace-nowrap"
                    >
                      <span className="max-w-[4.5rem] sm:max-w-none truncate text-[#023047] text-white">فیلتر</span>
                      <Icon name="chevronDown" size={14} className={`transition-transform text-white ${catOpen ? 'rotate-180' : ''}`} />
                    </button>
                    {/* Input */}
                    <div className="relative flex-1 min-w-0">
                      <input
                        ...
                        className="w-full bg-transparent text-primary-900 text-white caret-primary-900 caret-white py-2 sm:py-2.5 pr-9 pl-28 text-sm focus:outline-none placeholder:text-sm placeholder:text-primary-400 placeholder:text-white/50"
                      />
                      <div className="absolute right-2.5 top-1/2 -translate-y-1/2 text-primary-400 text-[#13ABC4] pointer-events-none">
                      ...
                      <div className="absolute left-1.5 top-1/2 -translate-y-1/2 flex items-center gap-1 z-10">
                        ...
                        {catOpen && (
                          <button
                            ...
                            className="h-7 px-2.5 rounded-full bg-transparent hover:bg-primary-50 hover:bg-white/10 border border-primary-200 border-white/25 bg-[#13ABC4] text-primary-900 text-white text-[11px] font-medium whitespace-nowrap hover:opacity-90 active:scale-[0.98] transition flex-shrink-0 shadow-sm"
                            title="اعمال فیلتر"
                          >
                            اعمال فیلتر
                          </button>
                  ...
                  {searchSuggestOpen && !catOpen && (
                    <div className="absolute top-full right-0 left-0 mt-1.5 z-[200] bg-white !bg-[#111] rounded-[11px] shadow-[0_12px_26px_rgba(0,0,0,0.10)] border border-[#E5E5E5] border-white/15 overflow-hidden max-h-[70vh] overflow-y-auto text-right isolate z-[500]" role="listbox">
                      {!searchQuery.trim() && (recentSearches || []).length > 0 && (
                        <div className="px-3 pt-3 pb-2">
                          <div className="flex items-center justify-between mb-1.5">
                            <span className="text-xs font-bold text-primary-400">جستجوهای اخیر</span>
                  ...
                  {catOpen && (
                    <div
                      ...
                      className="hidden md:block absolute top-full right-0 left-0 mt-1.5 z-[120] bg-white bg-[#1A1C20] rounded-xl shadow-xl border border-primary-200 border-white/40 py-2 overflow-hidden max-h-[min(31vh,320px)] overflow-y-auto w-full"
                    >
                      <div className="px-3 py-1.5 text-xs font-bold text-primary-400 text-primary-500 tracking-wide">دسته‌بندی</div>
                        ...
                        {['رسمی', 'کروات', 'آستین کوتاه'].map(c => {
                          ...
                          return (
                            <button
                              ...
                              className={`px-2.5 py-1 rounded-full text-xs border transition ${on ? 'search-filter-chip--on px-2.5 py-1.5 rounded-full text-xs font-bold border-2 border-[#023047] bg-[#023047] border-[#13ABC4] bg-[#13ABC4] !text-white shadow-sm' : 'px-2.5 py-1.5 rounded-full text-xs font-medium border plp-filter-chip border-primary-300 border-white/50 !text-primary-900 !text-white bg-white bg-[#2A2C30] font-medium'}`}
                            >
                              {c}
                            </button>
                          );
                      ...
                      <div className="border-t border-primary-200 border-white/30 my-1.5" />
                      <div className="px-3 py-1.5 text-xs font-bold text-primary-400 text-primary-500 tracking-wide">رنگ</div>
                      <div className="flex flex-wrap gap-1.5 px-3 pb-2">
                        {allColors.map(c => {
                      ...
                      <div className="border-t border-primary-200 border-white/30 my-1.5" />
                      <div className="px-3 py-1.5 text-xs font-bold text-primary-400 text-primary-500 tracking-wide">سایز</div>
                      <div className="flex flex-wrap gap-1.5 px-3 pb-2">
                        {allSizes.map(s => {
                          ...
                          return (
                            <button
                              ...
                              dir="ltr" lang="en"
                              className={`latin-label px-2.5 py-1 rounded-full text-xs border transition ${on ? 'search-filter-chip--on px-2.5 py-1.5 rounded-full text-xs font-bold border-2 border-[#023047] bg-[#023047] border-[#13ABC4] bg-[#13ABC4] !text-white shadow-sm' : 'px-2.5 py-1.5 rounded-full text-xs font-medium border plp-filter-chip border-primary-300 border-white/50 !text-primary-900 !text-white bg-white bg-[#2A2C30] font-medium'}`}
                            >
                              {s}
                            </button>
                          );
                  ...
                  {searchQuery.trim() && !catOpen && (
                    <div className="absolute top-full right-0 left-0 mt-1.5 z-50 bg-white bg-black rounded-xl shadow-xl border border-primary-200 border-white/50 overflow-hidden max-h-80 overflow-y-auto">
                      {filteredProducts.length === 0 ? (
                        <div className="px-4 py-6 text-center text-sm text-primary-500 text-primary-400">
                          محصولی یافت نشد
                          <button onClick={() => setSearchQuery('')} className="block mx-auto mt-2 text-primary-700 text-[#13ABC4] underline text-xs">پاک کردن</button>
                        </div>
                        ...
                        return (
                          <a
                            ...
                            className="flex items-center gap-3 px-3 py-2.5 hover:bg-primary-50 hover:bg-primary-800 transition border-b border-primary-50 border-primary-800 last:border-0"
                          >
                            <img src={col.image} alt="" className="w-12 h-12 rounded-lg object-cover flex-shrink-0" loading="lazy" decoding="async" />
                            <div className="min-w-0 flex-1 text-right">
                              <p className="text-sm font-medium text-primary-900 text-white line-clamp-2" title={p.name}>{p.name}</p>
                              <p className="text-xs text-primary-700 text-white">{p.category}</p>
                              <p className="text-xs text-primary-700 text-white">فروشنده: {(seller?.name && seller.name !== "undefined") ? seller.name : (seller?.shop_name || "فروشگاه")}</p>
                              <p className="text-xs font-bold text-primary-700 text-[#13ABC4] mt-0.5">{p.priceText} تومان</p>
                            </div>
                          </a>
                        );
                      })}
                ...
                <div className="flex items-center gap-1 sm:gap-1.5 flex-shrink-0 order-3 ms-auto">
                  <div className="header-secondary-slot relative flex-shrink-0 hidden md:block">
                    <button type="button" onClick={() => { setNotifPanelOpen(v => !v); setCartOpen(false); setWishlistOpen(false); setCompareOpen(false); setRecentOpen(false); }} className="header-icon-btn w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-full relative text-primary-800 hover:bg-primary-50 bg-[#2A2C30] text-white border border-white/25 hover:bg-[#1A1C20] hover:text-white" title="اعلان‌ها">
                      <Icon name="bell" size={18} />
                      {unreadNotifCount > 0 && (
                        <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 px-0.5 rounded-full bg-transparent hover:bg-primary-50 hover:bg-white/10 border border-primary-200 border-white/25 text-primary-900 text-white text-xs font-bold flex items-center justify-center">{toFa(unreadNotifCount > 9 ? '9+' : unreadNotifCount)}</span>
                      )}
                    </button>
                  </div>
                  <button type="button" onClick={toggleDarkMode} className="header-icon-btn hidden md:flex w-8 h-8 sm:w-10 sm:h-10 items-center justify-center rounded-full flex-shrink-0 text-primary-800 text-[#13ABC4] footer-link hover:bg-primary-50 bg-[#2A2C30] text-white border border-white/25 hover:bg-[#1A1C20] hover:text-white" title="حالت تاریک">
                    <Icon name={dark ? "sun" : "moon"} size={18} />
                  </button>
                  <button type="button" onClick={() => { setCartOpen(false); setCompareOpen(false); setRecentOpen(false); setWishlistOpen(true); }} className="header-icon-btn hidden md:flex w-8 h-8 sm:w-10 sm:h-10 items-center justify-center rounded-full relative flex-shrink-0 text-primary-800 hover:bg-primary-50 bg-[#2A2C30] text-white border border-white/25 hover:bg-[#1A1C20] hover:text-white" title="علاقه‌مندی‌ها">
          ...
          {!pdpProduct && (() => {
            ...
            const crumbItems = [
              ...
              ...(showPLP && !activeSeller && !activePlpTag && !plpBrand ? [
                { label: 'فروشگاه', href: '/فروشگاه', onClick: () => { try { goShop(); } catch (_) { try { openPLP({ resetFilters: true }); } catch (__) { try { window.location.assign('/فروشگاه'); } catch (___) {} } } } },
...
{/* ===================== PLP — لیست محصولات ===================== */}
      ...
      {roleGateOpen ? (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4" dir="rtl" role="dialog" aria-modal="true">
          ...
          <div className="role-gate-modal relative z-10 w-full max-w-md rounded-3xl border border-primary-200 border-white/15 bg-white bg-[#1a1c20] shadow-2xl p-6 sm:p-8">
            <div className="relative flex items-start justify-center mb-1">
              <div className="w-9 shrink-0" aria-hidden />
              <div className="flex-1 text-center px-1">
                <h2 className="text-lg font-bold text-[#023047]">ورود / ثبت‌نام</h2>
                <p className="text-xs text-primary-500 text-white/55 mt-1 mb-6">حساب مورد نظر را انتخاب کنید</p>
              </div>
              <button type="button" onClick={() => setRoleGateOpen(false)} className="w-9 h-9 shrink-0 rounded-full text-primary-500 text-[#13ABC4] hover:bg-primary-100 hover:bg-white/10 text-lg leading-none flex items-center justify-center" aria-label="بستن">×</button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <button type="button" onClick={() => { setRoleGateOpen(false); openAuth(); }} className="group flex flex-col items-center gap-2 rounded-2xl border border-primary-200 border-white/15 px-4 py-6 hover:border-apple-blue transition bg-primary-50/70 bg-white/5">
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="role-gate-cart-icon text-primary-800 text-white" aria-hidden="true"><circle cx="8" cy="21" r="1" className="role-gate-cart-wheel" /><circle cx="19" cy="21" r="1" className="role-gate-cart-wheel" /><path d="M2.05 2.05h2l2.66 12.42a2 2 0 0 0 2 1.58h9.78a2 2 0 0 0 1.95-1.57l1.65-7.43H5.12" className="role-gate-cart-body" /></svg>
                <span className="text-sm font-bold text-primary-900 text-white">خریدار</span>
                <span className="text-[11px] text-primary-500 text-white/50 text-center">خرید و پیگیری سفارش</span>
              </button>
              <button type="button" onClick={() => { setRoleGateOpen(false); openSellerAuth(); }} className="group flex flex-col items-center gap-2 rounded-2xl border border-primary-200 border-white/15 px-4 py-6 hover:border-apple-blue transition bg-primary-50/70 bg-white/5">
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="role-gate-store-icon text-[#023047] text-white" aria-hidden="true"><path d="m2 7 4.41-4.41A2 2 0 0 1 7.83 2h8.34a2 2 0 0 1 1.42.59L22 7" className="role-gate-store-roof" /><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8" className="role-gate-store-body" /><path d="M15 22v-4a2 2 0 0 0-2-2h-2a2 2 0 0 0-2 2v4" className="role-gate-store-door" /><path d="M2 7h20" className="role-gate-store-base" /><path d="M22 7v3a2 2 0 0 1-2 2 2.7 2.7 0 0 1-1.59-.63.7.7 0 0 0-.82 0A2.7 2.7 0 0 1 16 12a2.7 2.7 0 0 1-1.59-.63.7.7 0 0 0-.82 0A2.7 2.7 0 0 1 12 12a2.7 2.7 0 0 1-1.59-.63.7.7 0 0 0-.82 0A2.7 2.7 0 0 1 8 12a2.7 2.7 0 0 1-1.59-.63.7.7 0 0 0-.82 0A2.7 2.7 0 0 1 4 12a2 2 0 0 1-2-2V7" className="role-gate-store-awning" /></svg>
                <span className="text-sm font-bold text-[#023047] text-white">فروشنده</span>
                <span className="text-[11px] text-[#023047] text-white/50 text-center">پنل فروشگاه و محصولات</span>
              </button>
            </div>
          </div>
        </div>
      ...
      {publicTrackOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/40" role="dialog" aria-modal="true" onClick={() => setPublicTrackOpen(false)}>
          <div className="w-full max-w-md rounded-2xl bg-white bg-primary-900 border border-primary-200 border-white/15 p-5 shadow-xl" onClick={(e) => e.stopPropagation()} dir="ltr">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base font-bold text-primary-900 text-white">پیگیری سفارش</h2>
              <button type="button" onClick={() => setPublicTrackOpen(false)} className="text-primary-500 text-[#13ABC4] text-sm">بستن</button>
            </div>
            <div className="flex gap-2 flex-row" dir="ltr" style={{ direction: "ltr" }}>
              <input
                ...
                placeholder=""
                className="flex-1 px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white"
                dir="ltr"
              />
              <button
                type="button"
            ...
            {publicTrackResult && (
              <div className="mt-4 p-3 rounded-xl border border-primary-100 border-white/10 space-y-1">
                <p className="text-sm font-bold text-primary-900 text-white">{publicTrackResult.order_number || publicTrackResult.id}</p>
                <p className="text-xs text-primary-600 text-white/80">وضعیت: {publicTrackResult.statusLabel || publicTrackResult.status}</p>
...
{cartOpen && (
            <>
              ...
              <div role="dialog" aria-modal="true" className="cart-panel fixed top-0 bottom-0 start-0 z-[200] w-full max-w-[360px] sm:max-w-[400px] bg-white bg-primary-900 h-full min-h-0 max-h-[100dvh] shadow-2xl flex flex-col rounded-l-2xl overflow-hidden" role="dialog" aria-modal="true" aria-label="سبد خرید">
                {/* Header */}
                <div className="flex items-center justify-between p-4 sm:p-5 border-b border-primary-200 border-white/30 bg-primary-50/50 bg-primary-900/40">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="drawer-title-wrap">
                      <h3 data-drawer-title className="font-bold text-base text-primary-900 text-[#13ABC4] !text-white leading-tight">سبد خرید</h3>
                      <p className="text-xs text-primary-500 text-[#13ABC4] !text-white">{toFa(cartCount)} کالا</p>
                    </div>
                  </div>
                    ...
                    {cart.length > 0 && (
                      clearCartConfirm ? (
                        <div className="flex items-center gap-1 ml-1">
                          <button type="button" onClick={clearCart} className="text-xs px-2 py-1 rounded-full bg-transparent hover:bg-primary-50 hover:bg-white/10 border border-primary-200 border-white/25 text-primary-900 text-[#13ABC4] footer-link text-white font-medium">تأیید حذف</button>
                          <button type="button" onClick={() => setClearCartConfirm(false)} className="text-xs px-2 py-1 rounded-full border border-primary-200 border-white/30 text-primary-600 text-white">لغو</button>
                        </div>
                      ) : (
                        <button type="button" onClick={() => setClearCartConfirm(true)} className="text-xs px-2 py-1 rounded-full text-primary-500 text-white/70 hover:bg-primary-100 hover:bg-primary-800 transition">حذف همه</button>
                      )
                    )}
                    <button onClick={() => setCartOpen(false)} className="p-2 hover:bg-primary-100 hover:bg-primary-800 rounded-full transition text-primary-900 text-white" aria-label="بستن سبد">
                      <Icon name="x" size={20} />
                    </button>
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-5" style={{ WebkitOverflowScrolling: 'touch' }}>
                  {cart.length === 0 ? (
                    <div className="text-center py-14 px-2">
                      <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-primary-50 bg-primary-900 flex items-center justify-center text-primary-400 text-white">
                        <Icon name="shoppingBag" size={28} />
                      </div>
                      <p className="text-primary-800 text-white text-sm font-bold mb-1">سبد خرید شما خالی است</p>
                      <p className="text-primary-500 text-[#13ABC4] !text-white text-xs mb-5 leading-relaxed">پیراهن مورد علاقه‌تان را پیدا کنید و استایل‌تان را کامل کنید</p>
                    ...
                    <>
                      {cart.map((item, idx) => {
                        ...
                        return (
                          <div key={`${item.id}-${item.selectedColor?.name || ''}-${item.selectedSize || ''}-${idx}`} className={`cart-item-row ${loading ? 'opacity-60 pointer-events-none' : ''}`} style={{ animationDelay: `${idx * 0.06}s` }}>
                            <div className="flex gap-3 items-start py-3.5">
                              ...
                              <div className="flex-1 min-w-0">
                                <button type="button" className="text-right w-full p-0 border-0 bg-transparent" onClick={() => { setCartOpen(false); openPDP(product || item); }}>
                                  <h4 className="font-medium text-sm text-primary-900 text-white line-clamp-2" title={item.name}>{item.name}</h4>
                                  {item.productCode && <p className="text-[10px] text-primary-400 text-white/50 font-latin mt-0.5" dir="ltr">{item.productCode}</p>}
                                </button>
                                <div className="mt-1 flex flex-wrap items-center gap-1">
                                  <span className="inline-flex items-center gap-1 text-xs px-1.5 py-0.5 rounded-md font-medium bg-primary-50 bg-primary-700 text-primary-600 text-white border border-primary-100 border-white/30">
                                    فروشنده: {(seller?.name && seller.name !== "undefined") ? seller.name : (seller?.shop_name || "فروشگاه")}
                                  </span>
                                  {item.selectedSize && (
                                    <span className="latin-label text-xs px-1.5 py-0.5 rounded-md bg-primary-50 bg-primary-800 text-primary-600 text-white border border-primary-100 border-white/20">سایز {item.selectedSize}</span>
                                  )}
                                </div>
                                  ...
                                  <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
                                    {availableColors.map((c) => (
                                      <button
                                        ...
                                        title={c.name}
                                        className={`p-0.5 rounded-full border-2 transition ${item.selectedColor?.name === c.name ? 'border-primary-800 border-white' : 'border-primary-200 border-primary-600 hover:border-primary-400'}`}
                                      >
                                        <span className="color-swatch block w-3.5 h-3.5 rounded-full border border-primary-300 border-white/70" style={{ ["--swatch-color"]: c.hex || "#888", backgroundColor: c.hex || "#888" }} />
                                      </button>
                                    ))}
                                    <span className="text-xs text-primary-500 text-white mr-1">{item.selectedColor?.name}</span>
                                  </div>
                                )}
                                {stockLeft != null && stockLeft <= 3 && (
                                  <p className="text-xs text-amber-600 text-amber-400 mt-1 font-medium">فقط {toFa(stockLeft)} عدد باقی مانده</p>
                                )}
                                <div className="flex items-baseline gap-2 mt-1.5 flex-wrap">
                                  {item.discount ? (
                                    <>
                                      <span className="text-primary-700 text-white font-bold text-sm">{item.priceText} تومان</span>
                                      {item.oldPrice && <span className="text-xs text-primary-400 text-[#13ABC4] line-through">{item.oldPrice}</span>}
                                ...
                                <p className="text-xs text-primary-500 text-[#13ABC4] !text-white mt-0.5">جمع این کالا: {formatPrice(lineTotal)} تومان</p>
                                <div className="flex items-center gap-2 mt-2.5">
                                  <button type="button" disabled={item.qty <= 1} onClick={() => updateQty(item.id, item.selectedColor?.name, -1, item.selectedSize)} className="w-7 h-7 rounded-full border border-primary-200 border-white/50 flex items-center justify-center hover:bg-primary-100 hover:bg-primary-800 transition text-primary-900 text-white disabled:opacity-40 disabled:cursor-not-allowed" aria-label="کاهش تعداد">
                                    <Icon name="minus" size={14} />
                                  </button>
                                  <span className="text-sm font-medium w-6 text-center text-primary-900 text-white">{toFa(item.qty)}</span>
                                  <button type="button" onClick={() => updateQty(item.id, item.selectedColor?.name, 1, item.selectedSize)} className="w-7 h-7 rounded-full border border-primary-200 border-white/50 flex items-center justify-center hover:bg-primary-100 hover:bg-primary-800 transition text-primary-900 text-white" aria-label="افزایش تعداد">
                                    <Icon name="plus" size={14} />
                                  </button>
                                  <button type="button" onClick={() => removeFromCart(item.id, item.selectedColor?.name, item.selectedSize)} className="cart-trash-btn p-1.5 rounded-full transition text-primary-500 text-[#13ABC4] hover:bg-primary-50 hover:bg-primary-800" aria-label="حذف از سبد">
                            ...
                            {idx < cart.length - 1 && (
                              <div className="border-b border-primary-200 border-white/30" />
                            )}
                          </div>
                        );
                      })}
                      ...
                      {/* Coupon */}
                      <div className="mt-4">
                        <p className="text-xs font-bold text-primary-700 text-white mb-2">کد تخفیف و هدایا</p>
                        {couponApplied ? (
                          <div className="flex items-center justify-between gap-2 p-2.5 rounded-xl border border-apple-blue/30 bg-apple-blue/5">
                            ...
                            <button type="button" onClick={removeCoupon} className="text-xs text-primary-500 text-[#13ABC4] footer-link hover:text-red-300 transition">حذف</button>
                          </div>
                        ) : (
                          <div className="flex gap-2">
                            <input
                              ...
                              className="coupon-input flex-1 min-w-0 px-3 py-2 rounded-xl border border-primary-200 border-white/40 bg-white bg-[#1A1C20] text-sm !text-primary-900 text-[#13ABC4] !text-white placeholder:!text-primary-500 placeholder:!text-white/70 outline-none focus:border-apple-blue"
                            />
                            <button type="button" onClick={applyCoupon} className="px-3 py-2 rounded-xl bg-[#A8E6CF] hover:bg-[#8FDCBE] bg-[#A8E6CF] hover:bg-[#8FDCBE] text-[#023047] !text-[#023047] text-xs font-bold flex-shrink-0 transition">
                              اعمال
                            </button>
                          </div>
                        )}
                      ...
                      {cartUpsell.length > 0 && (
                        <div className="mt-5">
                          <p className="text-xs font-bold text-primary-800 text-white mb-2.5">معمولاً با این کالا می‌خرند</p>
                          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1" style={{ WebkitOverflowScrolling: 'touch' }}>
                            {cartUpsell.map((p) => (
                              <div key={p.id} className="flex-shrink-0 w-[120px] rounded-xl border border-primary-200 border-white bg-primary-50/50 bg-[#1A1C20] p-2">
                                ...
                                <p className="text-xs font-medium text-primary-900 text-white line-clamp-2 leading-snug mb-1">{p.name}</p>
                                <p className="text-xs font-bold text-primary-700 text-white mb-1.5">{p.priceText}</p>
                      ...
                      <div className="mt-5 grid grid-cols-3 gap-2">
                        {[
                          ...
                          <div key={x.t} className="flex flex-col items-center gap-1.5 p-2 rounded-xl bg-primary-50 bg-primary-900 text-center border border-primary-100 border-white/15">
                            <span className="trust-icon-wrap inline-flex items-center justify-center w-9 h-9 rounded-full bg-white bg-primary-800 border border-primary-200 border-white/30">
                              <Icon name={x.icon} size={16} className="text-apple-blue text-[#13ABC4]" />
                            </span>
                            <span className="text-xs font-medium text-primary-700 text-[#13ABC4] !text-white leading-tight">{x.t}</span>
                          </div>
                        ))}
                      </div>
                    </>
                ...
                {/* Sticky footer */}
                {cart.length > 0 && (
                  <div className="p-4 sm:p-5 border-t border-primary-200 border-white/30 space-y-2.5 bg-primary-50/40 bg-primary-900/30">
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-primary-600 text-white/80">جمع جزء</span>
                      <span className="text-primary-800 text-white font-medium">{formatPrice(cartSubtotal)} تومان</span>
                    </div>
                    {couponDiscount > 0 && (
                      <div className="flex justify-between items-center text-sm">
                    ...
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-primary-600 text-white/80">مالیات (۹٪)</span>
                      <span className="text-primary-800 text-white font-medium">{formatPrice(cartTax)} تومان</span>
                    </div>
                    {(cartProductSavings > 0 || couponDiscount > 0) && (
                      <p className="text-xs text-apple-blue text-[#13ABC4] font-medium">شما {formatPrice(cartProductSavings + couponDiscount)} تومان سود کردید</p>
                    )}
                    <div className="flex justify-between items-center pt-1 border-t border-primary-200/60 border-white/10">
                      <span className="text-primary-700 text-white text-sm font-bold">مبلغ قابل پرداخت</span>
                      <span className="font-bold text-lg text-primary-900 text-white">{formatPrice(cartTotal)} تومان</span>
                    </div>
                    ...
                    <button type="button" onClick={() => { setCartOpen(false); openCartPage(); }} className="w-full py-2.5 rounded-full text-sm font-medium text-primary-800 text-[#13ABC4] !text-white border border-primary-200 border-white hover:bg-primary-50 hover:bg-primary-800 transition">
                      مشاهده کامل سبد خرید
                    </button>
                    <button type="button" onClick={() => setCartOpen(false)} className="w-full py-2 rounded-full text-xs font-medium text-primary-500 text-[#13ABC4] !text-white">
                      ادامه خرید
                    </button>
                  </div>
                )}
          ...
          {productImportReport && (
            <div className="site-modal-root" role="dialog" aria-modal="true">
              ...
              <div className="site-modal-panel bg-white bg-primary-900 border border-primary-200 border-white/15 p-5">
                <div className="flex items-center justify-between gap-2 mb-3">
                  <h3 className="text-base font-bold text-primary-900 text-white">گزارش ورود محصولات</h3>
                  <button type="button" onClick={() => setProductImportReport(null)} className="p-1.5 rounded-full hover:bg-primary-50 hover:bg-primary-800"><Icon name="x" size={16} /></button>
                </div>
                <p className="text-xs text-primary-500 text-[#13ABC4] mb-3">
                ...
                {productImportReport.warnings?.length > 0 && (
                  <div className="space-y-1.5">
                    ...
                    <ul className="text-xs text-primary-600 text-white/70 space-y-1 max-h-60 overflow-y-auto">
                      {productImportReport.warnings.map((w, i) => (
                        <li key={i} className="border-b border-primary-50 border-white/5 pb-1">{w}</li>
                      ))}
                    </ul>
                  </div>
                )}
                <p className="text-xs text-primary-400 text-[#13ABC4] mt-4 leading-relaxed">
          ...
          {adminUser && !showAdminPanel && !showSellerPanel && (
            <div className="fixed bottom-20 left-1/2 -translate-x-1/2 z-[160] flex items-center gap-2 px-3 py-2 rounded-full bg-white/95 bg-[#1a1a1a]/95 text-primary-900 text-white shadow-2xl border border-primary-200 border-white/15 backdrop-blur-xl max-w-[95vw]">
              <span className="text-xs sm:text-xs text-primary-900 text-white/80 hidden sm:inline whitespace-nowrap font-medium">حالت ادمین</span>
              <span className="text-xs font-semibold text-primary-900 text-white truncate max-w-[28vw] sm:max-w-[140px]">{getCurrentPageSeoContext().typeLabel}</span>
              <button type="button" onClick={openAdminFrontEdit} className="text-xs px-3 py-1.5 rounded-full bg-transparent border border-primary-200 border-white/25 hover:bg-transparent border border-primary-200 border-white/25 text-primary-900 text-[#13ABC4] footer-link text-white font-medium whitespace-nowrap shadow-sm">ویرایش SEO صفحه</button>
              <button type="button" onClick={() => openAdminPanel('seo')} className="text-xs px-2.5 py-1.5 rounded-full border border-primary-300 border-white/30 text-primary-900 text-white hover:bg-primary-50 hover:bg-white/10 whitespace-nowrap hidden sm:inline font-medium">پنل سئو</button>
              <button type="button" onClick={() => openAdminPanel('dashboard')} className="text-xs px-2.5 py-1.5 rounded-full border border-primary-300 border-white/30 text-primary-900 text-white hover:bg-primary-50 hover:bg-white/10 whitespace-nowrap font-medium">پنل</button>
            </div>
          )}
                  ...
                  return (
                    <>
                      <div className="flex items-start justify-between gap-2 mb-4">
                        <div>
                          <h3 className="text-base font-bold text-primary-900 text-white">ویرایش این صفحه</h3>
                          <p className="text-xs text-primary-500 text-[#13ABC4] mt-0.5">{ctx.typeLabel} · {ctx.label}</p>
                          <p className="text-xs text-primary-400 text-[#13ABC4] mt-0.5 font-mono" dir="ltr">{ctx.key}</p>
                        </div>
                        <button type="button" onClick={() => setAdminFrontEditOpen(false)} className="p-1.5 rounded-full hover:bg-primary-50 hover:bg-primary-800"><Icon name="x" size={16} /></button>
                      </div>
                      <div className="space-y-3">
                        <div>
                          <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">نامک (Slug)</label>
                          <input value={adminFrontEditForm.slug} onChange={e => setAdminFrontEditForm(f => ({ ...f, slug: e.target.value }))} dir="ltr" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin text-primary-900 text-white focus:outline-none focus:border-apple-blue" />
                        </div>
                        {renderContentSeoBox({
                        ...
                        <p className="text-xs text-primary-500 text-[#13ABC4] leading-relaxed bg-primary-50 bg-primary-900/50 rounded-xl p-3">
                        ...
                        <div className="flex gap-2 pt-1">
                          ...
                          <button type="button" onClick={() => setAdminFrontEditOpen(false)} className="px-5 py-2.5 rounded-full border border-primary-200 border-white/30 text-sm text-primary-700 text-white">لغو</button>
                        </div>
                      </div>
                    </>
                  );
          ...
          {siteDialog && (
            <div className="site-modal-root" role="dialog" aria-modal="true">
              ...
              <div
                className="site-modal-panel bg-white bg-primary-900 border border-primary-200 border-white/15 p-4 sm:p-5 space-y-3"
                tabIndex={-1}
                ref={(el) => { try { if (el && siteDialog.mode === 'confirm') el.focus(); } catch (_) {} }}
              >
                <h3 className="text-sm font-bold text-primary-900 text-white text-right">{siteDialog.title}</h3>
                {siteDialog.message ? (
                  <p className="text-sm text-primary-600 text-white/80 text-right whitespace-pre-wrap leading-relaxed">{siteDialog.message}</p>
                ) : null}
                ...
                {siteDialog.mode === 'prompt' && Array.isArray(siteDialog.fields) && (
                  <div className="space-y-2">
                    {siteDialog.fields.map((f, fi) => (
                      <div key={f.key || fi}>
                        <label className="text-xs text-primary-500 text-[#13ABC4] block mb-1 text-right">{f.label}</label>
                        <input
                ...
                <div className="flex gap-2 justify-start pt-1">
                  ...
                  <button
                    type="button"
                    className="px-5 py-2 rounded-full border border-primary-200 border-white/30 text-sm text-primary-700 text-[#13ABC4] footer-link text-white min-w-[5rem]"
          ...
          {quickAdd && (() => {
            ...
            return (
              <div className="site-modal-root" role="dialog" aria-modal="true" aria-label="افزودن سریع به سبد">
                ...
                <div className="quick-add-popup site-modal-panel relative flex flex-col max-h-[92dvh] overflow-hidden bg-white bg-primary-950 rounded-3xl shadow-2xl border border-primary-100/80 border-white/10">
                  {/* هدر */}
                  <div className="flex items-center justify-between gap-3 px-4 sm:px-5 py-3.5 border-b border-primary-100 border-white/10 shrink-0">
                    <button type="button" onClick={() => setQuickAdd(null)} className="w-10 h-10 rounded-full bg-primary-100 bg-primary-800 flex items-center justify-center hover:bg-primary-200 hover:bg-primary-700 transition" aria-label="بستن">
                      <Icon name="x" size={18} />
                    </button>
                    <h2 className="text-sm sm:text-base font-bold text-primary-900 text-white truncate flex-1 text-center px-2">{qp.name}</h2>
                    <button
                      type="button"
                      onClick={() => { const prod = qp; setQuickAdd(null); try { openPDP(prod); } catch (_) {} }}
                      className="text-[11px] font-medium text-apple-blue text-[#13ABC4] whitespace-nowrap hover:underline"
                    >
                      جزئیات کامل
                    </button>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto min-h-0">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-0 sm:gap-6 p-4 sm:p-6">
                      ...
                      <div className="relative">
                        <div className="aspect-[4/5] sm:aspect-square rounded-2xl overflow-hidden bg-[#f0f0f2] bg-primary-900">
                      ...
                      <div className="flex flex-col gap-4 pt-4 sm:pt-0">
                        <div>
                          <p className="text-2xl font-black text-primary-900 text-white tracking-tight">
                            {qPrice} <span className="text-sm font-medium text-primary-400">تومان</span>
                          </p>
                          {qp.oldPrice && (
                            <p className="text-xs text-primary-400 text-[#13ABC4] line-through mt-0.5">{qp.oldPrice} تومان</p>
                          )}
                        </div>

                        {qColors.length > 0 && (
                          <div>
                            <p className="text-xs font-bold text-primary-500 text-[#13ABC4] mb-2">رنگ · <span className="text-primary-900 text-white">{qColor.name}</span></p>
                            <div className="flex flex-wrap gap-2">
                              {qColors.map((c, i) => (
                                <button
                                  ...
                                  className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-xs border transition ${quickColorIdx === i ? 'border-apple-blue bg-apple-blue/10 text-primary-900 text-white' : 'border-primary-200 border-white/20 text-primary-700 text-white/80'}`}
                                >
                        ...
                        <div>
                          <p className="text-xs font-bold text-primary-500 text-[#13ABC4] mb-2">سایز</p>
                          <div className="flex flex-wrap gap-2">
                            {qSizes.map(sz => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`min-w-[2.75rem] px-3 py-2 rounded-xl text-sm font-semibold border transition ${on ? 'bg-[#023047] bg-[#13ABC4] text-white border-transparent' : 'border-primary-200 border-white/20 text-primary-800 text-white hover:border-primary-400'}`}
                                >{sz}</button>
                              );
                            })}
                          </div>
                        ...
                        <div>
                          <p className="text-xs font-bold text-primary-500 text-[#13ABC4] mb-2">تعداد</p>
                          <div className="inline-flex items-center gap-1 rounded-2xl border border-primary-200 border-white/20 px-1">
                        ...
                        {qp.description && (
                          <div className="rounded-2xl border border-primary-100 border-white/10 overflow-hidden">
                            <button type="button" onClick={() => setQuickDescOpen(v => !v)} className="w-full flex items-center justify-between px-3.5 py-2.5 text-xs font-medium text-primary-700 text-white/80">
                              <span>توضیح کوتاه</span>
                              <Icon name={quickDescOpen ? 'chevronUp' : 'chevronDown'} size={16} />
                            </button>
                            {quickDescOpen && (
                              <p className="px-3.5 pb-3 text-xs text-primary-600 text-white/70 leading-relaxed border-t border-primary-50 border-white/5 pt-2">{qp.description}</p>
                            )}
                          </div>
                        )}
                      </div>
                  ...
                  {/* CTA ثابت پایین */}
                  <div className="shrink-0 border-t border-primary-100 border-white/10 p-4 sm:p-5 bg-white bg-primary-950">
                    <button
                      type="button"
                      onClick={() => addToCart(qp, { colorIdx: quickColorIdx, size: quickSize || '', qty: quickQty, requireSize: true })}
                      className="w-full py-3.5 rounded-2xl bg-[#023047] hover:bg-[#012536] hover:bg-white/10 border border-[#023047] border-white/25 bg-[#13ABC4] text-white text-white text-[15px] font-bold shadow-lg hover:opacity-95 active:scale-[0.99] transition"
                    >
                      افزودن به سبد خرید
                    </button>
                  </div>
