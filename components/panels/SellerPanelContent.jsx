                          ...
                          <div className="site-modal-panel bg-white dark:bg-primary-900 p-4 sm:p-5 border border-primary-200 dark:border-white/15 max-sm:!fixed max-sm:!inset-0 max-sm:!max-w-none max-sm:!w-full max-sm:!h-full max-sm:!rounded-none max-sm:!m-0 max-sm:overflow-y-auto max-sm:overflow-x-hidden">
                            ...
                            <div className="space-y-3">
                              ...
                              <div><label className="text-xs text-primary-500 mb-1 block">نام محصول</label>                              <div className="flex items-stretch gap-2">
                                <div className="flex-1 min-w-0"><input value={sellerProductForm.name} onChange={e => setSellerProductForm(f =></div>
                                <button type="button" title="کپی" onClick={async () => {
                                  const ok = await copyTextToClipboard(sellerProductForm.name);
                                  try { showToast({ message: ok ? 'کد محصول کپی شد' : 'کپی نشد', variant: ok ? 'success' : 'error', duration: 2000, position: 'top-center' }); } catch (_) {}
                                }} className="flex-shrink-0 inline-flex items-center justify-center min-w-[2.75rem] px-2.5 rounded-xl border border-primary-200 dark:border-white/15 text-[11px] font-medium text-primary-700 dark:text-white/80 hover:bg-primary-50 dark:hover:bg-white/5">کپی</button>
                              </div> ({ ...f, name: e.target.value, slug: slugifyTaxonomy(e.target.value) }))} className="w-full px-3 py-2.5 rounded-xl border border-primary-200 dark:border-white/20 bg-transparent text-sm text-primary-900 dark:text-white focus:outline-none focus:border-apple-blue shadow-none" /></div>
                              <div>
                              ...
                              <div>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button type="button" title="کپی" onClick={async () => {
                                      ...
                                      try { showToast({ message: ok ? 'نامک کپی شد' : 'کپی نشد', variant: ok ? 'success' : 'error', duration: 2000, position: 'top-center' }); } catch (_) {}
                                    }} className="flex-shrink-0 inline-flex items-center justify-center min-w-[2.75rem] px-2.5 rounded-xl border border-primary-200 dark:border-white/15 text-[11px] font-medium text-primary-700 dark:text-white/80 hover:bg-primary-50 dark:hover:bg-white/5">کپی</button>
                                  </div>
                                  <button
                                    type="button"
