  ...
  return (
    <>
            <div className="panel-content-wrap w-full max-w-none px-2 sm:px-4 py-4 sm:py-10 pb-24">
              <div className="mb-6 p-4 sm:p-5 rounded-2xl border border-primary-200 border-white/20 bg-white bg-primary-900 space-y-3">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  <h1 className="text-xl sm:text-2xl font-bold text-primary-900 text-white truncate">{sellerUser.shopName}</h1>
                  <p className="text-xs text-primary-500 !text-white truncate">{sellerUser.ownerName || sellerUser.name || ''}</p>
                </div>
              </div>
              ...
              {(() => {
                ...
                return (
                  <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2 min-w-0">
                    <p className="text-[11px] sm:text-xs text-primary-500 text-white/70 whitespace-nowrap shrink-0">لینک اختصاصی فروشگاه شما</p>
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="inline-flex items-center gap-1.5 min-w-0 max-w-full px-2.5 py-1.5 rounded-full border border-primary-200 border-white/20 bg-primary-50/80 bg-primary-900/40 text-[11px] sm:text-xs text-primary-700 text-white/90 hover:border-[#4CCD99]/50 transition shadow-none"
                      title={full || path}
                    >
              ...
              <div className="flex flex-wrap items-center gap-2 pt-1">
                <button type="button" onClick={() => setSellerTab('shop')} className="seller-header-btn text-xs px-3 py-1.5 rounded-full border border-primary-300 border-white/50 !text-primary-900 !text-white bg-white bg-[#2A2C30] font-semibold flex items-center gap-1"><Icon name="pencil" size={14} /> ویرایش فروشگاه</button>
                <button type="button" onClick={logoutSeller} className="seller-logout-btn text-xs px-3 py-1.5 rounded-full border border-red-400 border-red-400/80 !text-red-600 !text-red-300 font-semibold flex items-center gap-1"><Icon name="logOut" size={14} /> خروج</button>
              </div>
            </div>
            <div className="flex flex-col md:flex-row gap-6">
                <aside data-panel-sidebar className="panel-sidebar w-full md:w-48 lg:w-56 flex-shrink-0 sticky top-[var(--site-header-h,6.75rem)] z-40 md:static md:overflow-y-auto">
                  <div className="seller-tabs-strip panel-nav flex md:flex-col gap-0.5 overflow-x-auto px-3 py-3 rounded-xl border border-black/5 border-white/5 bg-white/50 bg-white/[0.03]">
                    {[
                ...
                <div className="panel-main w-full max-w-none flex-1 min-w-0 flex-1 min-w-0 p-3 sm:p-5 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 shadow-sm">
                  {/* Dashboard */}
                  {sellerTab === 'pending-approval' && (
                        <div className="p-8 rounded-2xl border border-amber-200 border-amber-900/40 bg-amber-50 bg-amber-950/30 text-center max-w-none w-full my-12">
                          <p className="text-lg font-bold text-primary-900 text-white mb-2">در انتظار تأیید ادمین</p>
                          <p className="text-sm text-primary-600 text-white/70 leading-relaxed">
                    ...
                    <div className="space-y-6">
                      ...
                      {String(sellerUser?.status||'').toLowerCase()==='archived' && (
                        <div className="mb-3 p-3 rounded-xl bg-red-50 bg-red-900/20 border border-red-200 border-red-800 text-sm text-red-700 text-red-300 leading-relaxed">
                      ...
                      {(String(sellerUser?.status||'').toLowerCase()==='suspended' || String(sellerUser?.status||'').toLowerCase()==='blocked') && (
                        <div className="mb-3 p-3 rounded-xl bg-red-50 bg-red-900/20 border border-red-200 border-red-800 text-sm text-red-700 text-red-300">
                      ...
                      {!sellerCanSell() && !['archived','suspended','blocked','banned','rejected'].includes(String(sellerUser?.status||'').toLowerCase()) && (
                        <div className="p-3 rounded-2xl border border-amber-200 border-amber-800/40 bg-amber-50 bg-amber-950/30 text-xs text-amber-800 text-amber-200 leading-relaxed">
                          پنل فروش شما تا <strong>تأیید جواز کسب توسط ادمین</strong> محدود است. پس از آپلود جواز و تأیید، می‌توانید محصول ثبت و بفروشید.
                        </div>
                      )}
                      <div className="flex items-center justify-between gap-3 p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
                        <div className="text-right">
                          <p className="text-sm font-bold text-primary-900 text-white">{sellerShopOpen ? 'الان هستیم' : 'الان نیستیم'}</p>
                          <p className="text-xs text-primary-500 !text-white mt-0.5">{sellerShopOpen ? 'فروشگاه آنلاین باز است' : 'فروشگاه آنلاین تعطیل است'}</p>
                        </div>
                        <button
                          type="button"
                          role="switch"
                        ...
                        >
                          <span className={`text-xs font-bold min-w-[2.5rem] text-left ${sellerShopOpen ? 'text-emerald-600 text-emerald-400' : 'text-primary-500 text-white/60'}`}>
                            {sellerShopOpen ? 'باز' : 'بسته'}
                          </span>
                          <span
                            dir="ltr"
                            className={`relative inline-flex h-8 w-14 items-center rounded-full p-1 transition-colors duration-300 ease-out ${sellerShopOpen ? 'bg-emerald-500' : 'bg-primary-300 bg-primary-600'}`}
                          >
                            <span
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        {[
                          ...
                          <div key={s.label} className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 text-center">
                            <p className={`text-2xl font-bold ${s.color}`}>{toFa(s.value)}</p>
                            <p className="text-xs text-primary-500 !text-white mt-1">{s.label}</p>
                          </div>
                        ))}
                      </div>
                      {/* خلاصه وضعیت تیکت‌ها */}
                      <div className="p-4 rounded-2xl border border-primary-200 border-white/25 bg-white bg-[#1A1C20] shadow-sm">
                        <div className="flex items-center justify-between mb-3">
                          <h3 className="text-sm font-bold text-primary-900 text-white">وضعیت تیکت‌ها</h3>
                          <button type="button" onClick={() => setSellerTab('support')} className="text-xs text-apple-blue text-[#018383] hover:underline">مشاهده همه</button>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                          {(() => {
                            ...
                            return [
                              { label: 'باز', n: openN, cls: 'bg-blue-50 text-blue-700 bg-[#2A3540] text-[#D6E8F7]' },
                              { label: 'منتظر پاسخ', n: waitN, cls: 'bg-amber-50 text-amber-700 bg-[#3A342A] text-[#F5E6C8]' },
                              { label: 'بسته', n: closedN, cls: 'bg-primary-50 text-primary-600 bg-[#2A2C30] text-white' },
                            ].map(x => (
                              <div key={x.label} className={`rounded-xl p-3 text-center border border-transparent border-white/10 ${x.cls}`}>
                      ...
                      <div className="grid sm:grid-cols-3 gap-3">
                        {(() => {
                          ...
                          <div key={s.label} className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
                            <p className="text-xs text-primary-500 !text-white">{s.label}</p>
                            <p className="text-lg font-bold text-primary-900 text-white mt-1">{s.value} <span className="text-xs font-normal">تومان</span></p>
                          </div>
                          ));
                        })()}
                      </div>
                      <div>
                        <div className="flex items-center justify-between mb-3">
                          <h2 className="text-base font-bold text-primary-900 text-white">آخرین سفارش‌ها</h2>
                        ...
                        <div className="space-y-2">
                          {(sellerOrders || []).slice(0, 4).map(o => (
                            <button key={o.id} type="button" onClick={() => { setSellerTab('orders'); setSellerOrderDetailId(o.id); }} className="w-full flex items-center gap-3 p-3 rounded-xl border border-primary-200 border-white/15 bg-white bg-primary-900 text-right hover:border-apple-blue/40 transition">
                              <div className="flex-1 min-w-0">
                                <p className="text-xs font-medium text-primary-900 text-white">{o.id}</p>
                      ...
                      <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
                        <div className="flex items-center justify-between gap-2">
                          <h2 className="text-base font-bold text-primary-900 text-white">نقطه سفارش / اتمام موجودی</h2>
                        ...
                        {(() => {
                          ...
                          if (!low.length && !out.length) {
                            return <p className="text-xs text-primary-500 !text-white py-2">همه محصولات موجودی کافی دارند.</p>;
                          }
                          return (
                            <div className="space-y-2">
                              {out.slice(0, 6).map(p => (
                                <div key={'o'+p.id} className="flex items-center gap-3 p-2.5 rounded-xl border border-red-200/60 border-red-800/40 bg-red-50/50 bg-red-900/10">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <p className="text-xs font-medium text-primary-900 text-white truncate">{p.name}</p>
                                    <p className="text-xs text-red-600 text-red-400">تمام شده</p>
                                  </div>
                                  <span className="text-xs font-bold text-red-600">۰</span>
                                </div>
                              ))}
                              {low.slice(0, 6).map(p => (
                                <div key={'l'+p.id} className="flex items-center gap-3 p-2.5 rounded-xl border border-amber-200/60 border-amber-800/40 bg-amber-50/40 bg-amber-900/10">
                                  <img src={p.colors?.[0]?.image || p.image || p.images?.[0]} alt="" className="w-10 h-12 object-cover rounded-lg flex-shrink-0"  loading="lazy" decoding="async" />
                                  <div className="flex-1 min-w-0">
                                    <p className="text-xs font-medium text-primary-900 text-white truncate">{p.name}</p>
                                    <p className="text-xs text-amber-700 text-amber-300">نزدیک نقطه سفارش</p>
                                  </div>
                                  <span className="text-xs font-bold text-amber-700 text-amber-300">{toFa(p.stock)}</span>
                                </div>
                              ))}
                            </div>
                          );
                  ...
                  {sellerTab === 'products' && (
                    <div>
                      {!sellerCanSell() && (
                        <div className="mb-4 p-3 rounded-2xl border border-amber-200 border-amber-800/40 bg-amber-50 bg-amber-950/30 text-xs text-amber-800 text-amber-200 leading-relaxed">
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                        ...
                        <div className="flex flex-wrap items-center gap-1.5">
                          <label className="text-xs px-3 py-1.5 rounded-full border border-emerald-300 text-emerald-700 text-emerald-300 font-medium flex items-center gap-1 cursor-pointer">
                          ...
                          <button type="button" onClick={() => backupSellerProducts('csv')} className="text-xs px-3 py-1.5 rounded-full border border-primary-200 border-white/20 text-primary-700 text-white font-medium flex items-center gap-1">
                          ...
                          }} className={`text-xs px-3 py-1.5 rounded-full font-medium flex items-center gap-1 ${sellerCanSell() ? 'bg-apple-blue !text-white !text-white' : 'bg-primary-200 bg-primary-800 text-primary-500 cursor-not-allowed'}`}><Icon name="plus" size={14} className={sellerCanSell() ? '!text-white' : ''} /> افزودن محصول</button>
                        </div>
                      </div>
                      <div className="mb-4 space-y-2">
                        <input value={sellerProductSearch} onChange={e => setSellerProductSearch(e.target.value)} placeholder="جستجوی نام..." className="w-full px-3 py-2 rounded-full border border-primary-200 border-white/20 bg-transparent text-xs text-primary-900 text-white focus:outline-none focus:border-apple-blue" />
                        <div className="flex flex-wrap gap-2">
                          {['all','active','inactive','pending'].map(f => (
                            <button key={f} type="button" onClick={() => setSellerProductFilter(f)} className={`px-3 py-1.5 rounded-full text-xs border font-medium ${sellerProductFilter === f ? 'panel-filter-chip--on' : 'plp-filter-chip border-primary-300 border-white/50 !text-primary-900 !text-white bg-white bg-[#2A2C30] font-medium'}`}>{f === 'all' ? 'همه' : f === 'active' ? 'فعال' : f === 'inactive' ? 'غیرفعال' : 'در انتظار'}</button>
                          ))}
                        </div>
                      </div>
                      {(() => {
                        ...
                        if (list.length === 0) return <div className="text-center py-16 rounded-2xl bg-white bg-primary-900 border border-dashed border-primary-200 border-white/20"><Icon name="package" size={32} className="mx-auto text-primary-300 mb-3" /><p className="text-sm text-primary-500">محصولی یافت نشد</p></div>;
                        return (
                          <div className="space-y-2">
                            {list.map(p => (
                              <div key={p.id} className="flex flex-col sm:flex-row sm:items-center gap-3 p-3 rounded-xl border border-primary-200 border-white/15 bg-white bg-primary-900">
                                <div className="flex items-center gap-3 flex-1 min-w-0">
                                <img
                                  src={p.image || p.colors?.[0]?.image || (Array.isArray(p.images) && p.images[0]) || '/logo.webp'}
                                  alt=""
                                  className="w-16 h-20 sm:w-20 sm:h-24 rounded-xl object-cover flex-shrink-0 bg-primary-50 bg-primary-800"
                                  onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = '/logo.webp'; }}
                                />
                                <div className="flex-1 min-w-0 text-right space-y-0.5">
                                  <p className="text-sm font-bold text-primary-900 text-white line-clamp-2">{p.name || 'بدون نام'}</p>
                                <p className="text-xs text-primary-500">
                                  {p.priceText || (toFa(Number(p.price || 0).toLocaleString()) + ' تومان')}
                                </p>
                                <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[11px] text-primary-500 text-white/60">
                                  ...
                                  {(p.category || (Array.isArray(p.categories) && p.categories[0])) ? (
                                    <>
                                      ...
                                      <button
                                        ...
                                        className="hover:text-apple-blue text-[#13ABC4] underline-offset-2 hover:underline"
                                      >
                                        دسته <span className="font-medium text-primary-800 text-white/90">{p.category || p.categories[0]}</span>
                                      </button>
                                    </>
                                  ) : null}
                                  {(p.brand || p.brandName) ? (
                                    <>
                                      ...
                                      <button
                                        ...
                                        className="hover:text-apple-blue text-[#13ABC4] underline-offset-2 hover:underline"
                                      >
                                        برند <span className="font-medium text-primary-800 text-white/90">{p.brand || p.brandName}</span>
                                      </button>
                                    </>
                                  ) : null}
                                </div>
                                  ...
                                  {p.productCode && (
                                    <p className="text-xs text-primary-400 flex items-center gap-1 flex-wrap justify-end">
                                      <span>کد:</span>
                                      <span className="latin-label font-semibold text-primary-700 text-white" dir="ltr">{p.productCode}</span>
                                  ...
                                  <div className="mt-1 flex flex-wrap items-center gap-1.5 justify-end">
                                      <button
                                        ...
                                        className="text-xs px-2 py-0.5 rounded-full border border-emerald-300 text-emerald-700 text-emerald-300 flex items-center gap-1"
                                      >
                                      ...
                                      <button
                                        ...
                                        className="text-xs px-2 py-0.5 rounded-full border border-primary-200 border-white/20 text-primary-600 text-white/80"
                                      >
                                        باز کردن
                                      </button>
                                    </div>
                                ...
                                {p.status === 'pending' ? (
                                  ...
                                  >
                                    <span className={`text-[11px] font-bold ${p.status === 'active' ? 'text-emerald-600 text-emerald-400' : 'text-primary-500 text-white/60'}`}>
                                      {p.status === 'active' ? 'فعال' : 'غیرفعال'}
                                    </span>
                                    <span
                                      dir="ltr"
                                      className={`relative inline-flex h-7 w-12 items-center rounded-full p-0.5 transition-colors duration-300 ease-out ${p.status === 'active' ? 'bg-emerald-500' : 'bg-primary-300 bg-primary-600'}`}
                                    >
                                      <span
                                ...
                                {sellerProductDeleteId === p.id && String(p.status) === 'archived' && (
                                  <div className="flex flex-col items-end gap-1 mt-1">
                                    ...
                                    <div className="flex items-center gap-1">
                                      ...
                                      }} className="text-xs px-2 py-1 rounded-full bg-transparent hover:bg-primary-50 hover:bg-white/10 border border-primary-200 border-white/25 text-primary-900 text-white">ارسال درخواست</button>
                                ...
                                {sellerProductDeleteId === p.id && String(p.status) !== 'archived' && String(p.status) !== 'purge_requested' && (
                                  <div className="flex items-center gap-1">
                                    ...
                                    }} className="text-xs px-2 py-1 rounded-full bg-transparent hover:bg-primary-50 hover:bg-white/10 border border-primary-200 border-white/25 text-primary-900 text-white">آرشیو</button>
                                    <button type="button" onClick={() => setSellerProductDeleteId(null)} className="text-xs px-2 py-1 rounded-full border border-primary-200">لغو</button>
                                  </div>
                                )}
                                </div>
                      ...
                      {sellerProductFormOpen && (
                        <div className="site-modal-root" role="dialog" aria-modal="true">
                          ...
                          <div className="site-modal-panel bg-white bg-primary-900 p-4 sm:p-5 border border-primary-200 border-white/15 max-sm:!fixed max-sm:!inset-0 max-sm:!max-w-none max-sm:!w-full max-sm:!h-full max-sm:!rounded-none max-sm:!m-0 max-sm:overflow-y-auto max-sm:overflow-x-hidden">
                            <div className="flex items-center justify-between mb-4">
                              <h3 className="text-base font-bold text-primary-900 text-white">{editingSellerProductId ? 'ویرایش محصول' : 'افزودن محصول'}</h3>
                              <button type="button" onClick={() => setSellerProductFormOpen(false)} className="w-9 h-9 rounded-full flex items-center justify-center hover:bg-primary-100 hover:bg-primary-800 text-primary-700 text-white" aria-label="بستن">
                                <Icon name="x" size={18} />
                              </button>
                            </div>
                            <div className="panel-wizard-head mb-4 p-3 sm:p-3.5 rounded-2xl border border-primary-100 border-white/10 bg-primary-50/50 bg-white/[0.03]">
                              <div className="flex items-center justify-between gap-2 mb-2">
                                <p className="text-xs font-bold tracking-tight text-primary-800 text-white">مرحله {toFa(sellerProductStep > 11 && !sellerProductForm.multiVariant ? sellerProductStep - 1 : sellerProductStep)} از {toFa(sellerProductForm.multiVariant ? 18 : 17)}</p>
                                <p className="text-[11px] font-medium px-2 py-0.5 rounded-full bg-white bg-primary-900 border border-primary-100 border-white/10 text-primary-500 text-white/60">{sellerProductStep <= 12 ? 'الزامی برای انتشار' : 'اختیاری · سئو'}</p>
                              </div>
                              <div className="panel-progress h-2 rounded-full bg-primary-100/90 bg-primary-800/80 overflow-visible ring-1 ring-inset ring-primary-200/40 ring-white/5">
                            ...
                            <div className="space-y-3">
                              ...
                              <p className="text-sm font-bold text-primary-900 text-white mb-3">۱. نام محصول و کد محصول و نامک و قیمت (تومان)</p>
                              ...
                              <div>
                                ...
                                <div className="flex items-stretch gap-2">
                                  <input
                                    ...
                                    readOnly
                                    className="flex-1 min-w-0 px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-primary-50 bg-primary-900/40 text-sm text-primary-900 text-white font-latin"
                                    dir="ltr"
                                  />
                                  <button
                                    type="button"
                                    ...
                                    className="flex-shrink-0 inline-flex items-center justify-center min-w-[2.75rem] px-2.5 rounded-xl border border-primary-200 border-white/15 text-[11px] font-medium text-primary-700 text-white/80 hover:bg-primary-50 hover:bg-white/5"
                                  >
                                    کپی
                                  </button>
                                </div>
                                <p className="text-[10px] text-primary-400 mt-1">کد یکتا · غیرقابل ویرایش</p>
                              </div>
...
<div className="rounded-xl border border-primary-200 border-white/15 bg-primary-50/50 bg-primary-950/40 p-3">
                                <label className="flex items-start gap-2.5 cursor-pointer">
                                  ...
                                  />
                                  <span>
                                    <span className="text-sm font-medium text-primary-900 text-white">محصولات من دارای چند رنگ، سایز یا قیمت مختلف است</span>
                                    <span className="block text-xs text-primary-500 text-white/60 mt-0.5">با فعال‌سازی، قیمت واحد در این مرحله مخفی می‌شود و تنظیم قیمت/موجودی در مرحله واریانت اجباری خواهد بود.</span>
                                  </span>
                                </label>
                              </div>
                              ...
                              <div>
                                ...
                                <input value={sellerProductForm.price} onChange={e => setSellerProductForm(f => ({ ...f, price: onlyDigits(e.target.value) }))} dir="ltr" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left text-primary-900 text-white focus:outline-none focus:border-apple-blue shadow-none" />
                              </div>
                              )}
                              {!!sellerProductForm.multiVariant && (
                                <p className="text-xs text-apple-blue text-[#4CCD99]">قیمت واحد غیرفعال است — در مرحله واریانت برای هر ترکیب قیمت بگذارید.</p>
                              )}
                              ...
                              <div className="space-y-4">
                              <p className="text-sm font-bold text-primary-900 text-white mb-3">۲. توضیحات محصول به همراه پیشنهاد لینک داخلی (فقط محصولات شما)</p>
                              ...
                              <div>
                                ...
                                {(() => {
                                  ...
                                  return (
                                    <div className="mt-3 rounded-xl border border-primary-100 border-white/10 bg-primary-50/50 bg-primary-900/40 p-3 space-y-2">
                                      <p className="text-xs font-bold text-primary-800 text-white">پیشنهاد لینک داخلی (فقط محصولات شما)</p>
                                      <p className="text-xs text-primary-500 text-white/70">تنها لینک مجاز در توضیح محصول، لینک به محصولات خودتان است.</p>
                              ...
                              <div className="space-y-4">
                              <p className="text-sm font-bold text-primary-900 text-white mb-3">۳. تصاویر محصول(به همراه تگ آلت برای هر تصویری که داره آپلود میشه)و مشخص کردن این که کدوم تصویر - تصویر شاخص هستش</p>
                                ...
                                <div className="flex flex-wrap gap-3 mb-2">
                                  {(sellerProductForm.images || []).map((im, idx) => (
                                    <div key={idx} className="w-[7.5rem] space-y-1.5">
                                      <div className="relative w-full aspect-[4/5] rounded-lg overflow-visible border border-primary-200 border-white/20">
                                        ...
                                        })} className="absolute bottom-0.5 left-0.5 bg-transparent hover:bg-primary-50 hover:bg-white/10 border border-primary-200 border-white/25 text-primary-900 text-white text-xs px-1 rounded">حذف</button>
                                      </div>
                                      <input
                                        ...
                                        className="w-full px-2 py-1 rounded-lg border border-primary-200 border-white/20 bg-transparent text-xs text-primary-900 text-white"
                                      />
                                    </div>
                                  ))}
                                  {(sellerProductForm.images || []).length < 8 && (
                                    <label className="w-[7.5rem] aspect-[4/5] rounded-lg border border-dashed border-primary-300 border-white/30 flex items-center justify-center text-xs text-primary-500 cursor-pointer">
                                      + افزودن
                              ...
                              {sellerProductStep === 4 && (
                              <div className="space-y-4">
                              <p className="text-sm font-bold text-primary-900 text-white mb-3">۴. دسته‌بندی</p>
                              ...
                              {sellerProductStep === 5 && (
                              <div className="space-y-4">
                              <p className="text-sm font-bold text-primary-900 text-white mb-3">۵. انتخاب برند</p>
                              <p className="text-xs text-primary-500 text-white/70 -mt-2">برند اجباری است و فقط از فهرست تعریف‌شده در پنل ادمین انتخاب می‌شود.</p>
                              <div>
                                <div className="flex items-center justify-between gap-2 mb-1">
                                ...
                                <div className="flex flex-wrap gap-1.5 min-h-[2.5rem]">
                                  {(adminCatalogBrands || []).filter(b => b.active !== false).length === 0 ? (
                                    <span className="text-xs text-amber-600 text-amber-400">هنوز برندی در پنل ادمین تعریف نشده — از ادمین بخواهید برند اضافه کند.</span>
                                  ) : (
                                      ...
                                      <button
                                        ...
                                        className={`px-3 py-1.5 rounded-full text-xs border transition ${
                                          sellerProductForm.brandId === b.id
                                            ...
                                            : 'border-primary-200 border-white/25 text-primary-800 text-white hover:border-apple-blue'
                                        }`}
                                      >
                                        {b.name}
                                      </button>
                              ...
                              {sellerProductStep === 6 && (
                              <div className="space-y-4">
                              <p className="text-sm font-bold text-primary-900 text-white mb-3">۵. برچسب</p>
                              ...
                              <div className="space-y-4">
                              <p className="text-sm font-bold text-primary-900 text-white mb-3">۶. موجودی پایه</p>

                              <div><label className="text-xs text-primary-500 mb-1 block">موجودی پایه</label><input value={sellerProductForm.stock} onChange={e => setSellerProductForm(f => ({ ...f, stock: onlyDigits(e.target.value) }))} dir="ltr" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left text-primary-900 text-white focus:outline-none focus:border-apple-blue shadow-none" /></div>

                              </div>
                              )}
                              {sellerProductStep === 8 && !sellerProductForm.multiVariant && (
                              <div className="space-y-4">
                              <p className="text-sm font-bold text-primary-900 text-white mb-3">۷. رنگ‌ها</p>
                              ...
                              <div className="space-y-4">
                              <p className="text-sm font-bold text-primary-900 text-white mb-3">۸. سایزها</p>
                              ...
                              <div className="space-y-4">
                              <p className="text-sm font-bold text-primary-900 text-white mb-3">۱۰. ویژگی‌ها</p>
                              <p className="text-xs text-primary-500 text-white/70 -mt-2 mb-1 whitespace-nowrap shrink-0">تنظیم واریانت فقط در پنل فروشنده است. ادمین فقط رنگ، سایز و ویژگی‌های پایه را تعریف می‌کند.</p>
                              <div className="space-y-3 pt-1">
                                <p className="text-xs font-bold text-primary-600 text-white/70">ویژگی‌ها <span className="font-normal text-primary-400">(برش، یقه، آستین، طرح و … · از پنل ادمین)</span></p>
                              ...
                              <div className="space-y-4">
                              <p className="text-sm font-bold text-primary-900 text-white mb-3">۱۱. واریانت‌ها (رنگ × سایز × ویژگی)</p>
                              <div className="rounded-xl border border-dashed border-primary-200 border-white/20 p-3 space-y-2">
                                <p className="text-xs font-bold text-primary-800 text-white">انتخاب سریع رنگ و سایز (برای ساخت واریانت)</p>
                                <div className="flex flex-wrap gap-1.5">
                                  {(adminCatalogColors || []).filter(c => c.active !== false).map(c => (
                                    <button key={c.id} type="button"
                                      ...
                                      className={`px-2.5 py-1 rounded-full text-xs border ${ (sellerProductForm.colorIds||[]).includes(c.id) ? 'panel-filter-chip--on' : 'border-primary-200 border-white/25' }`}
                                    >{c.name}</button>
                                  ))}
                                </div>
                                <div className="flex flex-wrap gap-1.5">
                                  {(adminCatalogSizes || []).filter(s => s.active !== false).map(s => (
                                    <button key={s.id || s.name} type="button" dir="ltr"
                                      ...
                                      className={`px-2.5 py-1 rounded-full text-xs border font-semibold ${ (sellerProductForm.sizes||[]).includes(s.name) ? 'panel-filter-chip--on' : 'border-primary-200 border-white/25' }`}
                                    >{s.name}</button>
                                  ))}
                                </div>
                              </div>

                              <p className="text-xs text-primary-500 text-white/70 -mt-2">
                                {sellerProductForm.multiVariant
                                  ? 'چون محصول چندقیمتی است، ساخت جدول واریانت و تعیین قیمت هر ردیف الزامی است.'
                                  : 'واریانت اختیاری است. در صورت نیاز جدول را بسازید.'}
                              </p>
                              <div className="rounded-xl border border-primary-200 border-white/15 bg-primary-50/40 bg-primary-950/40 p-3 space-y-2">
                                <div className="flex flex-wrap items-center justify-between gap-2">
                                  <div>
                                    <p className="text-xs font-bold text-primary-900 text-white">واریانت‌ها (رنگ × سایز × ویژگی‌ها)</p>
                                  ...
                                  <button
                                    ...
                                    className="text-xs px-3 py-1.5 rounded-full border border-primary-200 border-white/25 text-primary-800 text-white hover:bg-white/50 hover:bg-primary-800"
                                ...
                                {(() => {
                                  ...
                                  return (
                                    <div className="space-y-2 max-h-80 overflow-y-auto pr-0.5">
                                      {rows.map((row, ri) => (
                                        <div key={row.id || ri} className="rounded-xl border border-primary-200 border-white/15 bg-white bg-primary-900 p-3 space-y-2">
                                          <div className="flex flex-wrap items-center gap-2 text-xs font-medium text-primary-900 text-white">
                                            <span className="px-2 py-0.5 rounded-full bg-primary-100 bg-primary-800">رنگ: {row.color}</span>
                                            <span className="px-2 py-0.5 rounded-full bg-primary-100 bg-primary-800 font-latin" dir="ltr">سایز: {row.size}</span>
                                            {getAttrDimensions(sellerProductForm.attributes || {}, adminCatalogAttributes || []).map(d => (
                                              <span key={d.id} className="px-2 py-0.5 rounded-full border border-primary-200 border-white/20 text-primary-700 text-white/90">
                                          ...
                                          <div className="grid grid-cols-2 gap-2">
                                            <div>
                                              ...
                                              <input
                                                ...
                                                className="w-full px-2 py-1.5 rounded-lg border border-primary-200 border-white/20 bg-transparent text-sm text-left text-primary-900 text-white"
                                              />
                                            </div>
                                            <div>
                                          ...
                                          <div>
                                            ...
                                            <input
                                              ...
                                              className="w-full px-2 py-1.5 rounded-lg border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white"
                                            />
                                          </div>
                                          <div className="flex items-center gap-2">
                                            {row.image ? (
                                              <div className="relative w-12 h-14 rounded-lg overflow-visible border border-primary-200 border-white/20 flex-shrink-0">
                                            ...
                                            <label className="text-[11px] px-2.5 py-1.5 rounded-full border border-dashed border-primary-300 border-white/30 cursor-pointer text-primary-600 text-white/80">
                              ...
                              <div className="space-y-4">
                              <p className="text-sm font-bold text-primary-900 text-white mb-3">۱۰. ویدیو آپارات</p>


                              <div>
                                <label className="text-xs text-primary-500 mb-1 block">ویدیو آپارات (تنها لینک مجاز فروشنده · فقط embed)</label>
                                <input value={sellerProductForm.aparatEmbed || ''} onChange={e => setSellerProductForm(f => ({ ...f, aparatEmbed: e.target.value }))} dir="ltr" placeholder="https://www.aparat.com/video/video/embed/videohash/..." className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-xs text-left font-latin text-primary-900 text-white focus:outline-none focus:border-apple-blue" />
                              </div>

                              <p className="text-xs text-primary-500 text-white/70 rounded-xl border border-primary-100 border-white/10 bg-primary-50/50 bg-primary-950/40 p-3">تا این مرحله می‌توانید محصول را منتشر یا به‌روزرسانی کنید. مراحل بعدی اختیاری است اما برای سئو بهتر است تکمیل شوند.</p>
                              </div>
                              )}
                            ...
                            <div className="flex w-full flex-wrap justify-start gap-2 mt-5" dir="rtl">
                              <button type="button" onClick={() => setSellerProductFormOpen(false)} className="px-5 py-2 rounded-full border border-primary-200 border-white/30 text-sm text-primary-700 text-white min-w-[5.5rem]">لغو</button>
                              {sellerProductStep > 1 && (
                                <button type="button" onClick={() => setSellerProductStep(s => {
                                  let n = Math.max(1, s - 1);
                                  if (n === 11 && !sellerProductForm.multiVariant) n = 10;
                                  return n;
                                })} className="px-5 py-2 rounded-full border border-primary-200 border-white/30 text-sm text-primary-700 text-white min-w-[5.5rem]">مرحله قبل</button>
                              )}
                              {sellerProductStep < 18 && (
                                <button type="button" onClick={() => {
                                ...
                                }} className="seller-form-primary-btn px-5 py-2 rounded-full bg-primary-800 bg-white text-sm font-medium min-w-[5.5rem]">مرحله بعد</button>
                              )}
                            ...
                            >
                              ...
                              <button
                                ...
                                className="px-4 py-2 rounded-full border border-primary-200 border-white/30 text-sm text-primary-700 text-white min-w-[5.5rem]"
                              >
                                ذخیره پیش‌نویس
                              </button>
                              </>
      ...
      {sellerScheduleOpen && (
        <div className="site-modal-root" role="dialog" aria-modal="true" style={{ zIndex: 80 }}>
          ...
          <div className="site-modal-panel bg-white bg-primary-900 border border-primary-200 border-white/15 p-4 max-w-sm mx-auto rounded-2xl">
            <h3 className="text-base font-bold text-primary-900 text-white mb-1">انتشار زمان‌بندی‌شده</h3>
            <p className="text-xs text-primary-500 text-white/60 mb-4">تاریخ و ساعت به‌وقت ایران (تقویم شمسی)</p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-3">
              <div>
                <label className="text-[10px] text-primary-500 block mb-1">روز</label>
                <select
                  className="w-full px-2 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm"
            ...
            <div className="flex gap-2 justify-end">
              <button type="button" onClick={() => setSellerScheduleOpen(false)} className="px-4 py-2 rounded-full border border-primary-200 border-white/30 text-sm">انصراف</button>
              <button
                type="button"
      ...
      {sellerTaxonomyPicker && (
                        <div className="site-modal-root" role="dialog" aria-modal="true">
                          ...
                          <div className="site-modal-panel bg-white bg-primary-900 border border-primary-200 border-white/15 flex flex-col overflow-visible">
                            <div className="flex items-center justify-between gap-2 px-4 py-3 border-b border-primary-100 border-white/10">
                              <h3 className="text-sm font-bold text-primary-900 text-white">
                              ...
                              <button type="button" onClick={() => setSellerTaxonomyPicker(null)} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-primary-100 hover:bg-primary-800 text-primary-700 text-white" aria-label="بستن">
                                <Icon name="x" size={16} />
                              </button>
                            </div>
                            <div className="px-4 py-2 border-b border-primary-100 border-white/10">
                              <input
                                type="search"
                                value={sellerTaxonomySearch}
                                onChange={(e) => setSellerTaxonomySearch(e.target.value)}
                                placeholder="جستجو…"
                                className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white focus:outline-none focus:border-apple-blue"
                              />
                                ...
                                return items.map((item) => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-right transition ${selected ? 'bg-apple-blue/10 bg-[#4CCD99]/15' : 'hover:bg-primary-50 hover:bg-primary-900/50'}`}
                                    >
                                      <span className={`w-5 h-5 rounded-md border flex items-center justify-center flex-shrink-0 ${selected ? 'bg-apple-blue border-apple-blue text-white bg-[#4CCD99] border-[#4CCD99]' : 'border-primary-300 border-white/30'}`}>
                                        {selected && <Icon name="check" size={12} />}
                                      </span>
                                      {mode === 'colors' && item.hex && (
                                        <span className="color-swatch w-4 h-4 rounded-full border border-black/20 border-white/60 flex-shrink-0" style={{ ["--swatch-color"]: item.hex || '#888', backgroundColor: item.hex || '#888' }} />
                                      )}
                                      <span className={`text-sm font-medium text-primary-900 text-white flex-1 ${mode === 'sizes' ? 'font-latin' : ''}`} dir={mode === 'sizes' ? 'ltr' : undefined}>{item.name}</span>
                                    </button>
                                  );
                                });
                              })()}
                            </div>
                            <div className="px-4 py-3 border-t border-primary-100 border-white/10">
                      ...
                      {sellerDescEditorOpen && (
                        <div className="site-modal-root" role="dialog" aria-modal="true">
                          ...
                          <div className="site-modal-panel bg-white bg-primary-900 border border-primary-200 border-white/15 flex flex-col overflow-visible">
                            <div className="flex items-center justify-between gap-2 px-4 py-3 border-b border-primary-100 border-white/10">
                              <div>
                                <h3 className="text-sm font-bold text-primary-900 text-white">توضیحات محصول</h3>
                                <p className="text-xs text-primary-400 mt-0.5">بدون لینک، تصویر و ویدیو</p>
                              </div>
                              <button type="button" onClick={() => setSellerDescEditorOpen(false)} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-primary-100 hover:bg-primary-800 text-primary-700 text-white" aria-label="بستن">
                                <Icon name="x" size={16} />
                              </button>
                            </div>
                            ...
                            <div className="flex w-full justify-end gap-2 px-4 py-3 border-t border-primary-100 border-white/10" dir="ltr">
                              <button type="button" onClick={() => setSellerDescEditorOpen(false)} className="px-5 py-2 rounded-full border border-primary-200 border-white/30 text-sm text-primary-700 text-white min-w-[5.5rem]">لغو</button>
                              <button
                                type="button"
                                onClick={() => {
                  ...
                  {sellerTab === 'inventory' && (
                    <div className="space-y-5">
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                          <h2 className="text-base font-bold text-primary-900 text-white">موجودی انبار</h2>
                      ...
                      {(() => {
                        ...
                        return (
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            {[
                              { l: 'کل کالا', v: totalSku, cls: 'text-primary-900 text-white' },
                              ...
                              <div key={x.l} className="p-3 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 text-center">
                      ...
                      <div className="space-y-2">
                        {(sellerProducts || []).length === 0 ? (
                          <div className="text-center py-14 rounded-2xl bg-white bg-primary-900 border border-dashed border-primary-200 border-white/20">
                          ...
                          (sellerProducts || []).map(p => {
                            ...
                            return (
                              <div key={p.id} className={`p-3 sm:p-4 rounded-2xl border bg-white bg-primary-900 ${isOut ? 'border-red-200 border-red-800/40' : isLow ? 'border-amber-200 border-amber-800/40' : 'border-primary-200 border-white/15'}`}>
                                <div className="flex flex-wrap items-start gap-3">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className="flex flex-wrap items-center gap-2">
                                      <p className="text-sm font-bold text-primary-900 text-white truncate">{p.name}</p>
                                      {isOut && <span className="text-xs px-1.5 py-0.5 rounded-full bg-red-100 text-red-700 bg-red-900/40 text-red-300">ناموجود</span>}
                                      {isLow && <span className="text-xs px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-800 bg-amber-900/40 text-amber-300">نقطه سفارش</span>}
                                      {!isOut && !isLow && <span className="text-xs px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700 bg-emerald-900/40 text-emerald-300">موجود</span>}
                                    </div>
                                    ...
                                    <div className="mt-3 flex flex-wrap items-end gap-3">
                                      <div>
                                        ...
                                        <div className="flex items-center gap-1.5">
                                          <input
                                            ...
                                            className="w-24 px-2.5 py-1.5 rounded-xl bg-white bg-primary-900 border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white focus:outline-none focus:border-apple-blue font-[IRANYekanX,Tahoma,sans-serif]"
                                            dir="rtl"
                                          />
                                          <button
                                            type="button"
                                      ...
                                      <div className="flex flex-wrap gap-1.5 pb-0.5">
                                        {!isOut ? (
                                          <button type="button" onClick={() => { siteConfirm(`«${p.name}» ناموجود شود؟`).then(ok=>{ if(ok) setSellerProductOutOfStock(p.id); }); }} className="text-xs px-3 py-1.5 rounded-full border border-red-200 text-red-600 hover:bg-red-50 hover:bg-red-900/20 font-medium">ناموجود فوری</button>
                                        ) : (
                                        ...
                                        <button type="button" onClick={() => updateSellerProductStock(p.id, stock + 5)} className="text-xs px-3 py-1.5 rounded-full border border-primary-200 border-white/20 text-primary-700 text-white">+۵</button>
                                        <button type="button" onClick={() => updateSellerProductStock(p.id, stock + 10)} className="text-xs px-3 py-1.5 rounded-full border border-primary-200 border-white/20 text-primary-700 text-white">+۱۰</button>
                                      </div>
                                    </div>
                  ...
                  {sellerTab === 'orders' && !sellerOrderDetailId && (
                    <div>
                      <h2 className="text-base font-bold text-primary-900 text-white mb-4">سفارش‌های فروشنده</h2>
                <div className="flex flex-wrap gap-2 mb-4">
                  <button type="button" onClick={() => downloadSellerOrdersCsv("all")} className="text-xs sm:text-sm px-3 py-2 rounded-full border border-primary-200 border-white/20 bg-white bg-primary-900 text-primary-800 text-white hover:border-apple-blue transition">خروجی حسابداری (CSV)</button>
                  <button type="button" onClick={() => downloadSellerOrdersCsv("paid")} className="text-xs sm:text-sm px-3 py-2 rounded-full border border-emerald-200 border-emerald-500/30 text-emerald-700 text-emerald-400 transition">فقط پرداخت‌شده</button>
                  <button type="button" onClick={() => downloadSellerOrdersCsv("pending")} className="text-xs sm:text-sm px-3 py-2 rounded-full border border-amber-200 border-amber-500/30 text-amber-700 text-amber-400 transition">فقط در انتظار</button>
                </div>
                      <div className="flex gap-1.5 overflow-x-auto mb-4">
                        {[{ id: 'all', label: 'همه' },{ id: 'new', label: 'جدید' },{ id: 'preparing', label: 'آماده‌سازی' },{ id: 'shipped', label: 'ارسال‌شده' },{ id: 'delivered', label: 'تحویل‌شده' }].map(f => (
                          <button key={f.id} type="button" onClick={() => setSellerOrdersFilter(f.id)} className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium border ${sellerOrdersFilter === f.id ? 'panel-filter-chip--on' : 'plp-filter-chip border-primary-300 border-white/50 !text-primary-900 !text-white bg-white bg-[#2A2C30] font-medium'}`}>{f.label}</button>
                        ))}
                      </div>
                      {(() => {
                        const list = (sellerOrders || []).filter(o => sellerOrdersFilter === 'all' || o.status === sellerOrdersFilter);
                        if (!list.length) return (
                          <div className="text-center py-16 px-4 rounded-2xl bg-white bg-primary-900 border border-dashed border-primary-200 border-white/20">
                            <p className="text-sm font-medium text-primary-700 text-white mb-1">سفارشی در این فیلتر نیست</p>
                            <p className="text-xs text-primary-500 text-white/60">وقتی خریدار سفارش بدهد اینجا نمایش داده می‌شود</p>
                          </div>
                        );
                        return <div className="space-y-3">{list.map(o => (
                          <button key={o.id} type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setSellerOrderDetailId(o.id); setSellerTab('orders'); }} className="w-full p-4 rounded-xl border border-primary-200 border-white/15 bg-white bg-primary-900 text-right hover:border-apple-blue/40 transition cursor-pointer">
                            <div className="flex justify-between items-start mb-2"><div><p className="text-sm font-bold text-primary-900 text-white">{o.id}</p><p className="text-xs text-primary-500">{o.date} · {o.buyerName}</p></div><span className={`text-xs px-2.5 py-1 rounded-full font-medium ${sellerOrderStatusColor(o.status)}`}>{o.statusLabel}</span></div>
                            <div className="flex items-center justify-between gap-2">
                              <p className="text-xs font-bold text-primary-800 text-white">{toFa(Number(o.total||0).toLocaleString())} تومان · {toFa((o.items||[]).reduce((s,i)=>s+(i.qty||0),0))} کالا</p>
                              {o.status === 'delivered' && (
                                <span className="text-xs text-amber-600 text-amber-400 font-medium flex items-center gap-0.5">
                  ...
                  {sellerTab === 'orders' && sellerOrderDetailId && (() => {
                    ...
                    return (
                      <div>
                        ...
                        <div className="flex justify-between items-start mb-4"><div><h2 className="text-base font-bold text-primary-900 text-white">{o.id}</h2><p className="text-xs text-primary-500">{o.date} · {o.buyerName || 'خریدار'}</p></div><span className={`text-xs px-3 py-1 rounded-full font-medium ${sellerOrderStatusColor(o.status)}`}>{o.statusLabel || o.status}</span></div>
                        <div className="mb-4 p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
                          {items.length === 0 && <EmptyStateBox title="آیتمی ثبت نشده" className="py-4 border-0 bg-transparent" />}
                          {items.map((it,i) => (
                            <div key={i} className="flex gap-3"><img src={it.image || ''} alt="" className="w-14 h-14 rounded-lg object-cover bg-primary-100"  loading="lazy" decoding="async" /><div className="flex-1"><p className="text-xs font-medium text-primary-900 text-white">{it.name}</p><p className="text-xs text-primary-500">{it.color || '—'} · {it.size || '—'} · {toFa(it.qty || 1)} عدد</p><p className="text-xs font-bold mt-0.5">{toFa(Number(it.price || 0).toLocaleString())} ت</p></div></div>
                          ))}
                        </div>
                        <div className="mb-4 p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
                          <h3 className="text-sm font-bold mb-2 text-primary-900 text-white">آدرس ارسال</h3>
                          <p className="text-xs text-primary-600 text-white/70">{ship.address || o.address || '—'}</p>
                        ...
                        <div className="flex flex-col gap-3 mb-4">
                          <div className="flex flex-wrap gap-2">
                            {(o.status === 'new' || o.status === 'pending') && (
                              <>
                                ...
                                <button type="button" onClick={() => setSellerCancelForm({ open: true, orderId: o.id, reason: '' })} className="px-4 py-2 rounded-full border border-red-300 text-red-600 text-xs font-medium hover:bg-red-50 hover:bg-red-900/20">لغو سفارش</button>
                              </>
                            )}
                            ...
                            {(o.status === 'preparing' || o.status === 'new' || o.status === 'pending' || o.status === 'shipped') && o.status !== 'cancelled' && (
                              <>
                                <button type="button" onClick={() => sellerOpenOrderTicket(o)} className="px-4 py-2 rounded-full border border-primary-200 border-white/30 text-primary-800 text-white text-xs font-medium">تیکت</button>
                                <a
                                  ...
                                  className="px-4 py-2 rounded-full border border-primary-200 border-white/30 text-primary-800 text-white text-xs font-medium inline-flex items-center gap-1"
                                >
                          ...
                          {o.status === 'preparing' && o.packingDone && (
                            <div className="w-full space-y-2 p-3 rounded-xl border border-primary-200 border-white/15 bg-white bg-primary-900">
                              <p className="text-xs font-medium text-primary-900 text-white">ثبت ارسال</p>
                              <input value={sellerTrackForm.code} onChange={e => setSellerTrackForm(f => ({...f, code: toEnDigits(e.target.value)}))} placeholder="کد رهگیری" dir="ltr" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left text-primary-900 text-white focus:outline-none focus:border-apple-blue" />
                              <div className="flex gap-2 flex-wrap">{['پست پیشتاز','تیپاکس','اسنپ‌باکس'].map(c => (<button key={c} type="button" onClick={() => setSellerTrackForm(f => ({...f, carrier: c}))} className={`px-3 py-1 rounded-full text-xs border ${sellerTrackForm.carrier===c?'bg-apple-blue text-white border-apple-blue':'plp-filter-chip border-primary-300 border-white/50 !text-primary-900 !text-white bg-white bg-[#2A2C30] font-medium'}`}>{c}</button>))}</div>
                          ...
                          {sellerCancelForm.open && sellerCancelForm.orderId === o.id && (
                            <div className="w-full space-y-2.5 p-3.5 rounded-xl border border-red-200 border-red-500/40 bg-red-50/50 bg-red-950/30">
                              <p className="text-sm font-bold text-red-700 text-red-200">دلیل لغو سفارش</p>
                              <textarea value={sellerCancelForm.reason} onChange={e => setSellerCancelForm(f => ({...f, reason: e.target.value}))} rows={3} placeholder="دلیل لغو را بنویسید (برای خریدار ارسال می‌شود)…" className="w-full px-3 py-2.5 rounded-xl border border-red-200 border-red-500/40 bg-white bg-primary-900 text-sm sm:text-base text-primary-900 !text-white placeholder:text-primary-400 placeholder:text-white/50" />
                              <p className="text-sm text-primary-700 !text-white/85 leading-relaxed">شرایط بازگشت وجه برای خریدار در بخش سفارش‌ها ثبت می‌شود.</p>
                              <div className="flex flex-row-reverse justify-start gap-2">
                                ...
                                }} className="px-4 py-2 rounded-full bg-transparent hover:bg-primary-50 hover:bg-white/10 border border-primary-200 border-white/25 text-primary-900 text-white text-xs font-medium">تأیید لغو</button>
                                <button type="button" onClick={() => setSellerCancelForm({ open: false, orderId: null, reason: '' })} className="px-4 py-2 rounded-full border border-primary-200 border-white/30 text-xs">انصراف</button>
                              </div>
                            </div>
                          )}
                          {o.tracking && <p className="text-xs text-primary-500">رهگیری: <span dir="ltr" className="font-bold text-primary-900 text-white">{o.tracking.code}</span> ({o.tracking.carrier})</p>}
                        ...
                        <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
                          <h3 className="text-sm font-bold mb-2 text-primary-900 text-white">تاریخچه</h3>
                          {(o.history || []).map((h,i) => (<p key={i} className="text-xs text-primary-600 text-white/70 py-1 border-b border-primary-50 border-white/5 last:border-0">{h.label} — {h.date}</p>))}
                        </div>
                      </div>
                    );
                  })()}
                  ...
                  {sellerTab === 'support' && !sellerTicketDetailId && (
                    <div>
                      <div className="flex justify-between items-center mb-4">
                        <h2 className="text-base font-bold text-primary-900 text-white">پشتیبانی</h2>
                      ...
                      {(() => {
                        ...
                        const statusBadge = (status) => {
                          if (status === 'open') return 'bg-blue-100 text-blue-700 bg-blue-900/40 text-blue-300';
                          if (status === 'waiting') return 'bg-amber-100 text-amber-700 bg-amber-900/40 text-amber-300';
                          return 'bg-primary-100 text-primary-600 bg-primary-800 text-white/70';
                        };
                        ...
                        return (
                          <div className="grid sm:grid-cols-2 gap-4">
                            <div>
                              <h3 className="text-xs font-bold text-primary-700 text-white/80 mb-2 flex items-center gap-1.5">
                      ...
                      {sellerNewTicketOpen && (
                        <div className="site-modal-root" role="dialog" aria-modal="true">
                          ...
                          <div className="site-modal-panel bg-white bg-primary-900 p-5 border border-primary-200 border-white/15">
                            <h3 className="text-base font-bold mb-4 text-primary-900 text-white">تیکت جدید</h3>
                            ...
                            <div className="flex gap-2 mb-3">
                              {[
                                ...
                                <button key={opt.id} type="button" onClick={() => setSellerNewTicket(t => ({ ...t, target: opt.id }))} className={`flex-1 py-2 rounded-full text-xs font-medium border ${(sellerNewTicket.target || 'admin') === opt.id ? 'bg-apple-blue text-white border-apple-blue' : 'plp-filter-chip border-primary-300 border-white/50 !text-primary-900 !text-white bg-white bg-[#2A2C30] font-medium'}`}>{opt.label}</button>
                              ))}
                            </div>
                            <input value={sellerNewTicket.subject} onChange={e => setSellerNewTicket(t => ({...t, subject: e.target.value}))} placeholder="موضوع" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm mb-3 text-primary-900 text-white focus:outline-none focus:border-apple-blue" />
                            ...
                            <div className="flex gap-2">
                              ...
                              <button type="button" onClick={() => setSellerNewTicketOpen(false)} className="px-5 py-2.5 rounded-full border border-primary-200 border-white/30 text-sm">لغو</button>
                            </div>
                          </div>
                        </div>
                      )}
                  ...
                  {sellerTab === 'support' && sellerTicketDetailId && (() => {
                    ...
                    return (
                      <div>
                        ...
                        <h2 className="text-base font-bold text-primary-900 text-white mb-1">{t.subject}</h2>
                        <p className="text-xs text-primary-500 mb-4 font-latin" dir="ltr">{t.code || t.id} · {t.date}</p>
                        <div className="space-y-3 mb-4">
                          {t.messages.map((m,i) => (
                            <div key={i} className={`p-3 rounded-xl text-xs ${m.from === 'seller' ? 'bg-apple-blue/10 text-primary-900 text-white mr-6' : 'bg-primary-50 bg-primary-900 text-primary-700 text-white/80 ml-6'}`}>
                  ...
                  {sellerTab === 'gifts' && (
                    <div className="space-y-4">
                      <div>
                        <h2 className="text-base font-bold text-primary-900 text-white">تخفیف‌ها و هدایا</h2>
                        <div className="flex flex-wrap gap-2 mt-3">
                          ...
                          <button
                            ...
                            className="seller-gift-secondary-btn inline-flex items-center gap-1.5 text-xs px-4 py-2 rounded-full bg-white bg-primary-800 !text-primary-900 !text-white font-semibold border border-primary-300 border-white/40 hover:border-[#4CCD99] transition"
                          >
                      ...
                      {(sellerGifts || []).length === 0 ? (
                        <div className="text-center py-14 rounded-2xl border border-dashed border-primary-200 border-white/20 bg-white bg-primary-900">
                        ...
                        <div className="space-y-2">
                          {(sellerGifts || []).map(g => (
                            <div key={g.id} className="flex items-center gap-3 p-3 rounded-xl border border-primary-200 border-white/15 bg-white bg-primary-900">
                              <div className="w-10 h-10 rounded-full bg-emerald-50 bg-emerald-900/30 text-emerald-600 flex items-center justify-center"><Icon name="gift" size={18} /></div>
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-bold text-primary-900 text-white font-latin" dir="ltr">{g.code}</p>
                      ...
                      {sellerPromoModal === 'discount' && (
                        <div className="site-modal-root" role="dialog" aria-modal="true">
                          ...
                          <div className="site-modal-panel bg-white bg-primary-900 border border-primary-200 border-white/15 p-4 sm:p-5">
                            <div className="flex items-center justify-between gap-2 mb-3">
                              <h3 className="text-sm font-bold text-primary-900 text-white">درخواست تخفیف کالا</h3>
                              <button type="button" onClick={() => setSellerPromoModal(null)} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-primary-100 hover:bg-primary-800" aria-label="بستن"><Icon name="x" size={16} /></button>
                            </div>
                            ...
                            <div className="flex gap-2 mb-3">
                              <button type="button" onClick={() => setDiscountMode('percent')} className={`flex-1 py-2 rounded-full text-xs font-medium border ${discountMode === 'percent' ? 'bg-[#4CCD99] text-white border-[#4CCD99]' : 'border-primary-200 border-white/25 text-primary-700 text-white'}`}>درصد یکسان</button>
                              <button type="button" onClick={() => setDiscountMode('price')} className={`flex-1 py-2 rounded-full text-xs font-medium border ${discountMode === 'price' ? 'bg-[#4CCD99] text-white border-[#4CCD99]' : 'border-primary-200 border-white/25 text-primary-700 text-white'}`}>قیمت جدید هر کالا</button>
                            </div>
                            {discountMode === 'percent' && (
                              <div className="mb-3">
                                <label className="text-xs text-primary-500 mb-1 block">درصد تخفیف برای انتخاب‌شده‌ها</label>
                                <input type="number" min={1} max={90} value={discountPercent} onChange={e => setDiscountPercent(Number(e.target.value) || 0)} className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white" dir="ltr" />
                              </div>
                            )}
                            <div className="space-y-2 max-h-64 overflow-y-auto mb-3">
                              {(sellerProducts || []).filter(p => p.status === 'active' || p.contentStatus === 'approved').map(p => {
                                ...
                                return (
                                  <label key={p.id} className={`flex items-start gap-2 p-2.5 rounded-xl border cursor-pointer ${checked ? 'border-[#4CCD99]/60 bg-[#4CCD99]/5' : 'border-primary-200 border-white/15'}`}>
                                    <input
                                      type="checkbox"
                                      checked={checked}
                                      onChange={() => {
                                    ...
                                    />
                                    <div className="flex-1 min-w-0">
                                      <p className="text-xs font-medium text-primary-900 text-white">{p.name}</p>
                                      ...
                                      {checked && discountMode === 'price' && (
                                        <input
                                          ...
                                          className="mt-1.5 w-full px-2.5 py-1.5 rounded-lg border border-primary-200 border-white/20 bg-transparent text-xs text-primary-900 text-white"
                                          dir="ltr"
                                        />
                                      )}
                            ...
                            <div className="flex gap-2">
                              ...
                              <button type="button" onClick={() => setSellerPromoModal(null)} className="px-5 py-2.5 rounded-full border border-primary-200 border-white/30 text-sm text-primary-700 text-white">لغو</button>
                            </div>
                          </div>
                        </div>
                      )}
                      ...
                      {sellerPromoModal === 'gift' && (
                        <div className="site-modal-root" role="dialog" aria-modal="true">
                          ...
                          <div className="site-modal-panel bg-white bg-primary-900 border border-primary-200 border-white/15 p-4 sm:p-5">
                            <div className="flex items-center justify-between gap-2 mb-3">
                              <h3 className="text-sm font-bold text-primary-900 text-white">ساخت کد هدیه</h3>
                              <button type="button" onClick={() => setSellerPromoModal(null)} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-primary-100 hover:bg-primary-800" aria-label="بستن"><Icon name="x" size={16} /></button>
                            </div>
                            ...
                            <div className="flex gap-2 mb-3">
                              <input
                                ...
                                readOnly
                                dir="ltr"
                                className="flex-1 px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-primary-50 bg-primary-900 text-sm font-latin text-primary-900 text-white"
                              />
                              <button type="button" onClick={() => setGiftCodeForm(f => ({ ...f, code: generateGiftCode() }))} className="px-3 py-2 rounded-full border border-primary-200 border-white/25 text-xs text-primary-700 text-white whitespace-nowrap">کد جدید</button>
                            </div>
                            ...
                            <input
                              ...
                              className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white mb-3"
                              dir="ltr"
                            />
                            ...
                            <input
                              ...
                              className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white mb-2"
                            />
                            <div className="space-y-1.5 max-h-48 overflow-y-auto mb-3">
                              {((adminBuyers || []).filter(b => {
                                ...
                                return (
                                  <label key={b.id} className={`flex items-center gap-2 p-2 rounded-xl border cursor-pointer ${checked ? 'border-[#4CCD99]/50 bg-[#4CCD99]/5' : 'border-primary-100 border-white/10'}`}>
                                    <input
                                      type="checkbox"
                                      checked={checked}
                                      onChange={() => {
                                    ...
                                    />
                                    <span className="text-xs text-primary-900 text-white">{b.name || 'خریدار'}</span>
                  ...
                  {sellerTab === 'stats' && (
                    <div className="space-y-6">
                      <h2 className="text-base font-bold text-primary-900 text-white">آمار و گزارش</h2>
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        {[
                          ...
                          <div key={s.l} className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900"><p className="text-xs text-primary-500">{s.l}</p><p className="text-lg font-bold text-primary-900 text-white mt-1">{s.v}</p></div>
                        ))}
                      </div>
                      {/* نمودار ساده فروش هفتگی */}
                      <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
                        <h3 className="text-sm font-bold text-primary-900 text-white mb-3">روند فروش ۷ روز اخیر</h3>
                        <div className="flex items-end gap-1.5 h-28">
                          {[0, 0, 0, 0, 0, 0, 0].map((h, i) => (
                            <div key={i} className="flex-1 flex flex-col items-center gap-1">
                              <div className="w-full rounded-t-md bg-gradient-to-t from-apple-blue to-sky-400 from-[#4CCD99] to-orange-300" style={{ height: `${h}%` }} />
                      ...
                      <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
                        <h3 className="text-sm font-bold text-primary-900 text-white mb-3">ترکیب فروش</h3>
                          ...
                          {[
                            ...
                            <div key={r.label}>
                              <div className="flex justify-between text-xs mb-1">
                                <span className="text-primary-600 text-white/70">{r.label}</span>
                                <span className="font-medium text-primary-900 text-white">{toFa(r.pct)}٪</span>
                              </div>
                              <div className="h-2 rounded-full bg-primary-100 bg-primary-900 overflow-visible">
                      ...
                      <div>
                        <h3 className="text-sm font-bold text-primary-900 text-white mb-3">محصولات پرفروش</h3>
                        <div className="space-y-2">
                          {[...(sellerProducts||[])].sort((a,b)=>(b.salesCount||0)-(a.salesCount||0)).slice(0,5).map(p => (
                            <div key={p.id} className="flex items-center gap-3 p-3 rounded-xl border border-primary-200 border-white/15 bg-white bg-primary-900">
                              <img src={p.colors?.[0]?.image} alt="" className="w-10 h-10 rounded-lg object-cover"  loading="lazy" decoding="async" />
                              <p className="flex-1 text-xs font-medium text-primary-900 text-white line-clamp-1">{p.name}</p>
                      ...
                      <div>
                        <h3 className="text-sm font-bold text-primary-900 text-white mb-3">پرفروش‌ترین‌ها در تخفیف</h3>
                        <div className="space-y-2">
                          {[...(sellerProducts||[])].filter(p => p.discount || p.oldPrice).sort((a,b)=>(b.salesCount||0)-(a.salesCount||0)).slice(0,3).map(p => (
                            <div key={p.id} className="flex items-center gap-3 p-3 rounded-xl border border-amber-200/60 border-amber-800/40 bg-amber-50/50 bg-amber-900/10">
                              <img src={p.colors?.[0]?.image} alt="" className="w-10 h-10 rounded-lg object-cover"  loading="lazy" decoding="async" />
                              <div className="flex-1 min-w-0">
                                <p className="text-xs font-medium text-primary-900 text-white line-clamp-1">{p.name}</p>
                  ...
                  {sellerTab === 'tools' && (
                    <div className="space-y-6">
                      <div>
                        <h2 className="text-base font-bold text-primary-900 text-white">ابزارهای کمکی</h2>
                      </div>
                      {/* ورود / خروجی CSV */}
                      <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
                        <h3 className="text-sm font-bold text-primary-900 text-white flex items-center gap-2"><Icon name="upload" size={16} /> ورود و خروجی CSV</h3>
                        <p className="text-xs text-primary-500 text-white/60">فایل استاندارد ووکامرس یا شاپیفای را وارد کنید یا از محصولات فعلی خروجی بگیرید.</p>
                        <div className="flex flex-wrap gap-2">
                          <label className="text-xs px-3 py-1.5 rounded-full border border-emerald-300 text-emerald-700 text-emerald-300 font-medium flex items-center gap-1 cursor-pointer hover:bg-emerald-50 hover:bg-emerald-900/20">
                            <Icon name="upload" size={14} /> ورود CSV
                            <input type="file" accept=".csv,text/csv" className="hidden" onChange={(e) => {
                              const f = e.target.files?.[0];
                              if (!f) return;
                          ...
                          <button type="button" onClick={() => backupSellerProducts('csv')} className="text-xs px-3 py-1.5 rounded-full border border-primary-200 border-white/20 text-primary-700 text-white font-medium flex items-center gap-1 hover:bg-primary-50 hover:bg-primary-800">
                            <Icon name="download" size={14} /> خروجی CSV
                          </button>
                        </div>
                      </div>

                      <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-4 overflow-visible">
                        <h3 className="text-sm font-bold text-primary-900 text-white">ویرایشگر تصویر محصول</h3>
                        ...
                        {mediaToolStep === 'upload' && (
                          <div className="w-full overflow-visible">
                            <label className="flex flex-col items-center justify-center gap-2 w-full min-h-[88px] px-4 py-4 rounded-xl border-2 border-dashed border-primary-200 border-white/20 bg-primary-50/50 bg-primary-900/40 cursor-pointer hover:border-apple-blue transition">
                              <Icon name="plus" size={20} className="text-primary-400" />
                              <span className="text-xs font-medium text-primary-700 text-white">انتخاب تصویر</span>
                        ...
                        {mediaToolStep === 'crop' && mediaToolSrc && (
                          <div className="space-y-3">
                            <div className="relative mx-auto w-[200px] h-[250px] overflow-visible rounded-xl border-2 border-dashed border-apple-blue bg-primary-100 bg-primary-900 touch-none"
                              onMouseDown={(e) => {
                            ...
                            <div className="flex items-center justify-center gap-2">
                              <button type="button" aria-label="جابه‌جایی به چپ" onClick={() => setMediaToolOffset(o => ({ ...o, x: o.x - 12 }))} className="w-9 h-9 rounded-full border border-primary-200 border-white/30 flex items-center justify-center text-primary-700 text-white hover:bg-primary-50 hover:bg-primary-800">
                                <Icon name="chevronLeft" size={16} />
                              </button>
                              <div className="flex flex-col gap-1">
                                <button type="button" aria-label="جابه‌جایی به بالا" onClick={() => setMediaToolOffset(o => ({ ...o, y: o.y - 12 }))} className="w-9 h-9 rounded-full border border-primary-200 border-white/30 flex items-center justify-center text-primary-700 text-white hover:bg-primary-50 hover:bg-primary-800">
                                  <Icon name="chevronUp" size={16} />
                                </button>
                                <button type="button" aria-label="جابه‌جایی به پایین" onClick={() => setMediaToolOffset(o => ({ ...o, y: o.y + 12 }))} className="w-9 h-9 rounded-full border border-primary-200 border-white/30 flex items-center justify-center text-primary-700 text-white hover:bg-primary-50 hover:bg-primary-800">
                                  <Icon name="chevronDown" size={16} />
                                </button>
                              </div>
                              <button type="button" aria-label="جابه‌جایی به راست" onClick={() => setMediaToolOffset(o => ({ ...o, x: o.x + 12 }))} className="w-9 h-9 rounded-full border border-primary-200 border-white/30 flex items-center justify-center text-primary-700 text-white hover:bg-primary-50 hover:bg-primary-800">
                                <Icon name="chevronRight" size={16} />
                              </button>
                              <button type="button" onClick={() => setMediaToolOffset({ x: 0, y: 0 })} className="text-xs px-2 py-1 rounded-full border border-primary-200 border-white/30 text-primary-600 text-white/80">مرکز</button>
                            </div>
                            ...
                            <div className="flex gap-2">
                              ...
                              <button type="button" onClick={() => { setMediaToolStep('upload'); setMediaToolSrc(null); }} className="px-4 py-2.5 rounded-full border border-primary-200 border-white/30 text-sm">انصراف</button>
                            </div>
                          </div>
                        )}
                        {mediaToolStep === 'assign' && mediaToolResult && (
                          <div className="space-y-3">
                            <div className="flex gap-3 items-start">
                              <img src={mediaToolResult.dataUrl} alt="" className="w-24 h-30 rounded-lg object-cover border border-primary-200 border-white/20" style={{ height: 120 }}  loading="lazy" decoding="async" />
                              <div className="text-xs text-primary-500">
                                <p>۸۰۰×۱۰۰۰ WebP</p>
                                <p>حجم ≈ {toFa(Math.round(mediaToolResult.bytes/1024))} کیلوبایت</p>
                              </div>
                            </div>
                            <p className="text-xs font-bold text-primary-900 text-white">انتخاب یک محصول (فقط یکی)</p>
                            <input
                              type="search"
                              value={mediaToolSearch}
                              onChange={(e) => setMediaToolSearch(e.target.value)}
                              placeholder="جستجوی نام محصول…"
                              className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white placeholder:text-primary-400 focus:outline-none focus:border-apple-blue"
                            />
                            <div className="max-h-48 overflow-y-auto space-y-1.5 border border-primary-100 border-white/10 rounded-xl bg-white bg-primary-900 p-2">
                              {(sellerProducts || [])
                                .filter((pr) => {
                                ...
                                .map((pr) => (
                                <label key={pr.id} className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer ${mediaToolAssign.productId === pr.id ? 'bg-apple-blue/10 border border-apple-blue' : 'hover:bg-primary-50 hover:bg-primary-900'}`}>
                                  ...
                                  <span className="text-xs text-primary-900 text-white line-clamp-1">{pr.name}</span>
                              ...
                              return (
                                <div className="grid grid-cols-2 gap-2">
                                  <div>
                                    ...
                                    <select value={mediaToolAssign.colorName} onChange={e => setMediaToolAssign(a => ({...a, colorName: e.target.value}))} className="w-full px-2 py-1.5 rounded-lg border border-primary-200 border-white/20 bg-transparent text-xs">
                                  ...
                                  <div>
                                    ...
                                    <select value={mediaToolAssign.size} onChange={e => setMediaToolAssign(a => ({...a, size: e.target.value}))} className="w-full px-2 py-1.5 rounded-lg border border-primary-200 border-white/20 bg-transparent text-xs">
                      ...
                      <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-2">
                        <h3 className="text-sm font-bold text-primary-900 text-white">استاندارد تصویر</h3>
...
{sellerTab === 'kyc' && (
                    <div className="space-y-6 w-full">
                      <div>
                        <h2 className="text-base font-bold text-primary-900 text-white">احراز هویت و مدارک</h2>
                        <p className="text-xs text-primary-500 mt-1">همه مدارک پس از ارسال منتظر تأیید ادمین می‌مانند.</p>
                      </div>
                      <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-bold text-primary-800 text-white">وضعیت احراز</span>
                        ...
                        <div>
                          ...
                          <input id="kyc-national" defaultValue={sellerUser.nationalId||''} dir="ltr" maxLength={10} className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin text-primary-900 text-white" />
                        </div>
                        <div>
                          <label className="text-xs text-primary-500 mb-1 block">شماره شناسنامه / ثبتی</label>
                          <input id="kyc-idno" defaultValue={sellerUser.idNumber||''} dir="ltr" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin text-primary-900 text-white" />
                        </div>
                        <div>
                          <label className="text-xs text-primary-500 mb-1 block">تصویر کارت ملی (روی)</label>
                          <input type="file" accept="image/*" id="kyc-card-front" className="text-xs text-primary-700 text-white w-full" />
                        </div>
                        <div>
                          <label className="text-xs text-primary-500 mb-1 block">تصویر کارت ملی (پشت)</label>
                          <input type="file" accept="image/*" id="kyc-card-back" className="text-xs text-primary-700 text-white w-full" />
                        </div>
                        <div>
                          <label className="text-xs text-primary-500 mb-1 block">مجوز / جواز کسب <span className="text-red-500">*</span></label>
                          <input type="file" accept="image/*,.pdf" id="kyc-license" className="text-xs text-primary-700 text-white w-full max-w-full" />
                        </div>
                  ...
                  {sellerTab === 'finance' && (
                    <div className="space-y-6 w-full">
                      <div>
                        <h2 className="text-base font-bold text-primary-900 text-white">مالی حساب</h2>
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
                          <p className="text-xs text-primary-500">موجودی قابل برداشت</p>
                          <p className="text-lg font-bold text-primary-900 text-white mt-1">{toFa((sellerUser.balance||0).toLocaleString())} <span className="text-xs font-normal">تومان</span></p>
                        </div>
                        <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
                          <p className="text-xs text-primary-500">در انتظار تأیید مالی</p>
                          <p className="text-lg font-bold text-primary-900 text-white mt-1">{toFa((sellerUser.pendingPayout||0).toLocaleString())} <span className="text-xs font-normal">تومان</span></p>
                        </div>
                      </div>
                      {/* نمودار درآمد ماهانه */}
                      <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
                        <h3 className="text-sm font-bold text-primary-900 text-white mb-1">درآمد ماهانه</h3>
                        ...
                        <div className="flex items-end gap-2 h-32">
                          {[
                            ...
                            <div key={b.m} className="flex-1 flex flex-col items-center gap-1">
                              <span className="text-xs font-medium text-primary-600 text-white/70">{toFa(b.v)}</span>
                              <div className="w-full rounded-t-lg bg-gradient-to-t from-emerald-600 to-emerald-400 from-emerald-500 to-lime-400" style={{ height: `${(b.v / 45) * 100}%` }} />
                      ...
                      <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
                        <h3 className="text-sm font-bold text-primary-900 text-white mb-1">تفکیک مالی این ماه</h3>
                        <p className="text-xs text-primary-400 mb-3">مبنای کارمزد: مبلغ فروش بعد از تخفیف</p>
                        <div className="space-y-2">
                          {[
                            { label: 'فروش ناخالص', v: '۴۵,۲۰۰,۰۰۰', cls: 'text-primary-900 text-white' },
                            { label: 'هزینه خدمات پلتفرم', v: '۳,۶۱۶,۰۰۰', cls: 'text-red-500' },
                            { label: 'سهم شما', v: '۴۱,۵۸۴,۰۰۰', cls: 'text-emerald-600' },
                          ].map(r => (
                            <div key={r.label} className="flex justify-between text-xs py-1.5 border-b border-primary-50 border-white/5 last:border-0">
                      ...
                      <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
                        <h3 className="text-sm font-bold text-primary-900 text-white">شماره شبا</h3>
                        <input id="fin-sheba" defaultValue={sellerUser.sheba||''} dir="ltr" placeholder="IR..." className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin text-primary-900 text-white" />
                        <input id="fin-holder" defaultValue={sellerUser.accountHolder||sellerUser.ownerName||''} placeholder="نام صاحب حساب" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white" />
                        ...
                        }} className="px-5 py-2.5 rounded-full border border-primary-200 border-white/30 text-sm text-primary-800 text-white hover:bg-primary-50 hover:bg-primary-800 transition">ذخیره شبا (نیاز به تأیید)</button></div>
                      </div>
                      <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-2">
                        <h3 className="text-sm font-bold text-primary-900 text-white">درخواست مالی (غیرفعال)</h3>
                        <button
                          type="button"
                      ...
                      {(sellerUser.payoutRequests||[]).length > 0 && (
                        <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-2">
                          <h3 className="text-sm font-bold text-primary-900 text-white">سابقه درخواست‌ها</h3>
                          {(sellerUser.payoutRequests||[]).slice(0,5).map(r => (
                            <div key={r.id} className="flex justify-between text-xs p-2 rounded-lg bg-primary-50 bg-primary-900/50">
...
{sellerTab === 'shop' && (
                    <div className="p-4 sm:p-5 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">

                      <div className="mt-6 p-4 rounded-2xl border border-primary-200 border-white/15 space-y-3" dir="rtl">
                        ...
                        <input type="password" id="seller-new-pw" placeholder="رمز جدید (حداقل ۶)" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white" />
                        <input type="password" id="seller-new-pw2" placeholder="تکرار رمز" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white" />
                        <button
                          type="button"
                      ...
                      <h2 className="text-base font-bold text-primary-900 text-white mb-4">اطلاعات فروشگاه</h2>
                      ...
                      <div className="space-y-4 w-full max-w-none">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div>
                            ...
                            <div className="flex items-center gap-3">
                              <div className="w-20 h-20 rounded-full border border-primary-200 border-white/20 overflow-visible bg-primary-50 bg-primary-800 flex items-center justify-center flex-shrink-0">
                                <img
                                  src={sellerLogoUrl || ""}
                                  alt=""
                              ...
                              <div className="flex flex-col gap-1.5">
                                ...
                                <button type="button" disabled={!!sellerMediaBusy} onClick={() => sellerLogoInputRef.current && sellerLogoInputRef.current.click()} className="text-xs px-3 py-1.5 rounded-full border border-primary-300 border-white/30 bg-white bg-primary-900 disabled:opacity-50">
                          ...
                          <div>
                            ...
                            <div className="space-y-2">
                              <div className="w-full h-24 rounded-xl border border-primary-200 border-white/20 overflow-visible bg-primary-50 bg-primary-800 flex items-center justify-center">
                                {sellerBannerUrl ? (
                              ...
                              <div className="flex flex-wrap gap-2">
                                ...
                                <button type="button" disabled={!!sellerMediaBusy} onClick={() => sellerBannerInputRef.current && sellerBannerInputRef.current.click()} className="text-xs px-3 py-1.5 rounded-full border border-primary-300 border-white/30 bg-white bg-primary-900 disabled:opacity-50">
                        ...
                        <div><label className="text-xs text-primary-500 mb-1 block">نام فروشگاه <span className="text-red-500">*</span></label><input defaultValue={sellerUser.shopName} id="s-shop" required style={{ color: '#fff', WebkitTextFillColor: '#fff', caretColor: '#fff' }} className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-white focus:outline-none focus:border-apple-blue" /></div>
                        <div><label className="text-xs text-primary-500 mb-1 block">نام مسئول <span className="text-red-500">*</span></label><input defaultValue={sellerUser.ownerName} id="s-owner" required className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white focus:outline-none focus:border-apple-blue" /></div>
                        <div><label className="text-xs text-primary-500 mb-1 block">موبایل <span className="text-red-500">*</span></label><input value={sellerUser.phone} disabled dir="ltr" className="w-full px-3 py-2.5 rounded-xl border border-primary-100 bg-primary-50 bg-primary-900 text-sm text-primary-500 text-left" /></div>
                        <div><label className="text-xs text-primary-500 mb-1 block">ایمیل <span className="text-primary-400">(اختیاری)</span></label><input defaultValue={sellerUser.email||''} id="s-email" type="email" dir="ltr" placeholder="email@example.com" inputMode="email" autoComplete="email" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left text-primary-900 text-white focus:outline-none focus:border-apple-blue" /></div>
                        <div className="grid grid-cols-2 gap-3">
                          <div><label className="text-xs text-primary-500 mb-1 block">استان <span className="text-red-500">*</span></label><input defaultValue={sellerUser.province||''} id="s-prov" required className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white focus:outline-none focus:border-apple-blue" /></div>
                          <div><label className="text-xs text-primary-500 mb-1 block">شهر <span className="text-red-500">*</span></label><input defaultValue={sellerUser.city||''} id="s-city" required className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white focus:outline-none focus:border-apple-blue" /></div>
                        </div>
                        ...
                        <div><label className="text-xs text-primary-500 mb-1 block">کد پستی <span className="text-red-500">*</span></label><input defaultValue={sellerUser.postal||''} id="s-postal" dir="ltr" maxLength={10} required placeholder="۱۰ رقم" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left text-primary-900 text-white focus:outline-none focus:border-apple-blue" /></div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <div>
                            <label className="text-xs text-primary-500 mb-1 block">روزهای باز بودن <span className="text-red-500">*</span></label>
                            <input defaultValue={sellerUser.openDays||'شنبه تا پنجشنبه'} id="s-open-days" required placeholder="مثلاً شنبه تا پنجشنبه" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white focus:outline-none focus:border-apple-blue" />
                          </div>
                          <div>
                            <label className="text-xs text-primary-500 mb-1 block">ساعت کاری <span className="text-red-500">*</span></label>
                            <input defaultValue={sellerUser.openHours||'۹ تا ۲۱'} id="s-open-hours" required dir="ltr" placeholder="09:00 - 21:00" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left text-primary-900 text-white focus:outline-none focus:border-apple-blue" />
                          </div>
                        </div>
                        ...
                        <div><label className="text-xs text-primary-500 mb-1 block">اینستاگرام <span className="text-primary-400">(اختیاری)</span></label><input defaultValue={sellerUser.instagram||''} id="s-ig" dir="ltr" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left text-primary-900 text-white focus:outline-none focus:border-apple-blue" /></div>
                        <div><label className="text-xs text-primary-500 mb-1 block">لینک بله <span className="text-primary-400">(اختیاری · پشتیبانی)</span></label><input defaultValue={sellerUser.bale||''} id="s-bale" dir="ltr" placeholder="https://ble.ir/..." className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left text-primary-900 text-white focus:outline-none focus:border-apple-blue" /></div>
                        {/* شبا فقط در بخش مالی */}
                        <div className="p-3 rounded-xl bg-white bg-primary-900 border border-primary-200 border-white/15 space-y-2">
                          <div className="flex items-center justify-between gap-2">
                            <label className="text-xs font-bold text-primary-800 text-white">موقعیت فروشگاه روی نقشه نشان (مبدأ ارسال)</label>
                          ...
                          <div className="relative rounded-xl overflow-visible border border-primary-200 border-white/20 h-40 bg-primary-100 bg-primary-900">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <div>
                              ...
                              <input type="number" step="0.00001" dir="ltr" defaultValue={sellerUser.lat||35.6892} id="s-lat" className="w-full px-2 py-1.5 rounded-lg border border-primary-200 border-white/20 bg-transparent text-xs text-left font-latin text-primary-900 text-white" />
                            </div>
                            <div>
                              <label className="text-xs text-primary-500">طول</label>
                              <input type="number" step="0.00001" dir="ltr" defaultValue={sellerUser.lng||51.389} id="s-lng" className="w-full px-2 py-1.5 rounded-lg border border-primary-200 border-white/20 bg-transparent text-xs text-left font-latin text-primary-900 text-white" />
                            </div>
                          </div>
                          <div className="flex flex-wrap gap-2">
                            <button type="button" onClick={() => {
                            ...
                            }} className="text-xs px-3 py-1.5 rounded-full border plp-filter-chip border-primary-300 border-white/50 !text-primary-900 !text-white bg-white bg-[#2A2C30] font-medium">موقعیت فعلی</button>
                            <button type="button" onClick={() => {
                            ...
                            <a href={`https://neshan.org/maps/@${sellerUser.lat||35.689},${sellerUser.lng||51.389},15.z`} target="_blank" rel="noopener noreferrer" className="text-xs px-3 py-1.5 rounded-full border plp-filter-chip border-primary-300 border-white/50 !text-primary-900 !text-white bg-white bg-[#2A2C30] font-medium">باز کردن در نشان</a>
                          </div>
                        </div>
                        <div>
                            ...
                            {(adminShippingMethods || []).filter((m) => m.enabled !== false).map((m) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`px-3 py-1.5 rounded-full text-xs border ${selected ? 'bg-apple-blue text-white border-apple-blue' : 'plp-filter-chip border-primary-300 border-white/50 !text-primary-900 !text-white bg-white bg-[#2A2C30] font-medium'}`}
                                >
                                  {m.name}
                                </button>
                              );
