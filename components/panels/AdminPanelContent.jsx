 ...
 const [sellerRankRange, setSellerRankRange] = useState('month'); // day|week|month|year
  ...
  const runServerBackupDownload = async () => {
    ...
    try {
      ...
      const needReauth = res.status === 403 && (json?.code === 'ADMIN_REAUTH_REQUIRED' || /کد تأیید|reauth|پیامک/i.test(String(json?.error || '')));
  ...
  <>
      <div className="panel-content-wrap w-full max-w-none px-2 sm:px-4 py-4 sm:py-10 pb-24">
       ...
       <div className="flex flex-col md:flex-row gap-4 md:gap-6">
        ...
        <div className="panel-main flex-1 min-w-0 min-h-0 pb-8 p-4 sm:p-6 rounded-2xl border border-primary-200/80 dark:border-white/10 bg-white/95 dark:bg-primary-900/95 backdrop-blur-sm shadow-[0_8px_30px_rgba(15,23,42,0.06)] w-full max-w-none">
...
{/* Categories */}
           <div>
            ...
            {!filterTaxCategories(adminCategories).length && <p className="text-sm text-primary-400 text-center py-8">{taxFilter === 'archived' ? 'آرشیو خالی است' : 'دسته‌ای ثبت نشده'}</p>}
          </div>
           </div>
...
{/* Tags */}
           <div>
            ...
            {filterTaxTags(adminTags).map((tg) => (
              <div key={tg.id} className={`flex flex-wrap items-center gap-3 p-3 rounded-xl border bg-white dark:bg-primary-900 ${taxSelectedIds.includes(String(tg.id)) ? 'border-apple-blue ring-1 ring-apple-blue/30' : 'border-primary-200 dark:border-white/15'}`}>
            ...
            {!filterTaxTags(adminTags).length && <p className="text-sm text-primary-400 text-center py-8">{taxFilter === 'archived' ? 'آرشیو خالی است' : 'برچسبی ثبت نشده'}</p>}
          </div>
           </div>
           </div>
         )}
