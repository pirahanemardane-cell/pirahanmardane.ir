...
export default function AdminPanelContent() {
 ...
 const AdminProductActions = ({ product }) => {
  ...
  const runTaxBulkCategory = async (mode) => {
    ...
    if (isPurge) msg = 'حذف دائم ' + ids.size + ' دسته از آرشیو؟ برگشت‌پذیر نیست.';
    if (isRestore) msg = 'بازگردانی ' + ids.size + ' دسته؟';
    const ok = typeof siteConfirm === 'function' ? await siteConfirm(msg, isPurge ? 'حذف دائم' : (isRestore ? 'بازگردانی' : 'آرشیو')) : window.confirm(msg);
    if (!ok) return;
    setTaxBusy(true);
    try {
      if (isPurge) {
        ...
        saveAdminCategories((adminCategories || []).map((x) => ids.has(String(x.id)) ? { ...x, status: 'active', active: true } : x));
      } else {
        saveAdminCategories((adminCategories || []).map((x) => ids.has(String(x.id)) ? { ...x, status: 'archived', active: false } : x));
      }
      setTaxSelectedIds([]);
...
{/* Categories */}
           <div>
            ...
            {filterTaxCategories(adminCategories).length > 0 && (
              <label className="flex items-center gap-2 px-1 py-1 text-xs text-primary-600 dark:text-white/70 cursor-pointer select-none">
                <input type="checkbox"
                  checked={filterTaxCategories(adminCategories).length > 0 && filterTaxCategories(adminCategories).every((x) => taxSelectedIds.includes(String(x.id)))}
                  onChange={() => {
