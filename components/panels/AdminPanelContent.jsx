...
import { LumaSpinBlock, LumaSpin } from "@/components/ui/luma-spin";
  ...
  return (
    <div className="space-y-4 p-1">
      <div className="flex flex-wrap gap-2">
        <button type="button" onClick={() => setScope('buyers')} className={`px-3 py-1.5 rounded-full text-xs font-medium border ${scope === 'buyers' ? 'bg-apple-blue text-white border-apple-blue' : 'border-primary-200 border-white/20'}`}>خریداران</button>
        <button type="button" onClick={() => setScope('sellers')} className={`px-3 py-1.5 rounded-full text-xs font-medium border ${scope === 'sellers' ? 'bg-apple-blue text-white border-apple-blue' : 'border-primary-200 border-white/20'}`}>فروشندگان</button>
        <button type="button" onClick={load} className="px-3 py-1.5 rounded-full text-xs border border-primary-200 border-white/20">بازنشانی</button>
      </div>
      <div className="flex flex-wrap gap-2">
        {kinds.map((k) => (
          <button key={k.id} type="button" onClick={() => setKind(k.id)} className={`px-3 py-1.5 rounded-full text-xs font-medium border ${kind === k.id ? 'bg-primary-900 text-white bg-white text-primary-900 border-transparent' : 'border-primary-200 border-white/20'}`}>{k.label}</button>
        ))}
      </div>
      {loading ? (
        <LumaSpinBlock label="در حال بارگذاری…" />
      ) : items.length === 0 ? (
        <p className="text-sm text-primary-500 text-[#13ABC4] py-8 text-center">موردی نیست</p>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {items.map((it) => (
            <div key={it.id} className="rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 overflow-hidden flex flex-col">
              <div className="aspect-square bg-primary-50 bg-primary-950 relative">
              ...
              <div className="p-2 space-y-1 text-right">
                <p className="text-[11px] font-medium text-primary-900 text-white truncate">{it.ownerName}</p>
                <p className="text-[10px] text-primary-400 text-[#13ABC4] truncate" dir="ltr">{it.kind}{it.status ? ` · ${it.status}` : ''}</p>
                {editId === it.id ? (
                  <div className="space-y-1">
                    <input dir="ltr" value={editUrl} onChange={(e) => setEditUrl(e.target.value)} className="w-full text-[11px] px-2 py-1 rounded-lg border border-primary-200 border-white/20 bg-transparent" placeholder="https://..." />
                  ...
                  <div className="flex gap-1">
                    <button type="button" onClick={() => { setEditId(it.id); setEditUrl(it.url || ''); }} className="flex-1 text-[10px] py-1 rounded-lg border border-primary-200 border-white/25">ویرایش</button>
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-base font-bold text-primary-900 text-white">باشگاه مشتریان</h2>
          <p className="text-xs text-primary-500 text-white/60 mt-1">شماره‌های فرم تخفیف صفحه اصلی</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <input value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') load(q); }} placeholder="جستجوی شماره…" className="px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm w-40 sm:w-52" dir="ltr" />
          <button type="button" onClick={() => load(q)} className="text-xs px-3 py-2 rounded-full border border-primary-200 border-white/20">جستجو</button>
      ...
      {loading ? (
        ...
        <p className="text-sm text-primary-500 text-[#13ABC4] py-10 text-center">موردی ثبت نشده</p>
      ) : (
        <div className="space-y-2">
          {items.map((row) => (
            <div key={row.id} className="flex items-center justify-between gap-3 p-3 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <div className="min-w-0 text-right">
                <p className="font-medium text-primary-900 text-white font-latin" dir="ltr">{row.phone || row.phone_normalized}</p>
                <p className="text-[11px] text-primary-400 text-[#13ABC4] mt-0.5">
...
export default function AdminPanelContent() {
 ...
 const [sellerRankRange, setSellerRankRange] = useState('month'); // day|week|month|year
  ...
  const runServerBackupDownload = async () => {
    ...
    try {
      let res = await fetch('/api/admin/backup', { credentials: 'include' });
      let json = await res.json().catch(() => ({}));
      const needReauth = false; // REAUTH_UI_DISABLED — دیگر OTP برای کار ادمین نپرس
      const _needReauthLegacy = res.status === 403 && (json?.code === 'ADMIN_REAUTH_REQUIRED' || /کد تأیید|reauth|پیامک/i.test(String(json?.error || '')));
      const sessionExpired = res.status === 401 && (json?.code === 'ADMIN_SESSION_EXPIRED' || /نشست ادمین منقضی/i.test(String(json?.error || '')));
      if (sessionExpired) {
        showToast({ message: 'نشست ادمین منقضی شده. یک‌بار خارج شوید و دوباره وارد شوید.', variant: 'error', duration: 5000, position: 'top-center' });
        return;
      }
      if (needReauth) {
        const req = await fetch('/api/admin/reauth/request', { method: 'POST', credentials: 'include' });
        const reqJ = await req.json().catch(() => ({}));
        if (!req.ok || !reqJ?.ok) {
          showToast({ message: reqJ?.error || 'ارسال کد تأیید ناموفق', variant: 'error', duration: 4500, position: 'top-center' });
          return;
        }
        showToast({ message: reqJ?.message || 'کد تأیید ارسال شد', variant: 'success', duration: 3000, position: 'top-center' });
        const raw = window.prompt('کد تأیید پیامکی بک‌آپ را وارد کنید (فارسی یا انگلیسی):');
        if (raw == null || !String(raw).trim()) return;
        const code = toEn(raw);
        const ver = await fetch('/api/admin/reauth/verify', {
          method: 'POST',
          credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ code }),
        });
        const verJ = await ver.json().catch(() => ({}));
        if (!ver.ok || !verJ?.ok) {
          showToast({ message: verJ?.error || 'کد نادرست است', variant: 'error', duration: 4500, position: 'top-center' });
          return;
        }
        res = await fetch('/api/admin/backup', { credentials: 'include' });
 ...
 const { BarList, CarouselArrows, EmptyState, EmptyStateBox, ErrorState, Icon, Kpi, LoadingState, OWN_SELLER, PRODUCT_IMG, SeoPixelBars, SimpleEditor, Table, TableBody, TableCell, TableHead, TableHeader, TableRow, Textarea, TipText, activeSellerId, activeTip, add, addBlogComment, addBrandMention, addToCart, addressDeleteConfirm, addressForm, addressFormOpen, addresses, adm, adminAnalyticsRange, adminAnalyticsSub, adminAuthError, adminAuthLoading, adminAuthOpen, adminAuthOtp, adminAuthOtpTimer, adminAuthPhone, adminAuthStep, adminBlogCategories, adminBlogTags, adminBuyerDetailId, adminBuyerSearch, adminBuyers, adminCatalogAttributes, adminCatalogBrands, adminCatalogColors, adminCatalogSizes, adminCategories, adminContentTab, adminCouponForm, adminCouponFormOpen, adminCoupons, hydrateCatalogFromApi, setAdminOrderStatus, hydrateAdminStatsFromApi, requestSellerPayout, hydrateSellerPayouts, hydrateBlogPostsFromApi, hydrateAdminCoupons, persistCampaignOnServer, hydrateCampaignsFromApi, createAdminCouponOnServer, hydrateSellerOrdersFromApi, adminFrontEditForm, adminFrontEditOpen, adminGscDim, adminGscInspectResult, adminGscInspectUrl, adminGscRange, adminGscSub, adminLoading, adminModerationQueue, adminOrderDetailId, adminOrderFilter, adminOrderNote, adminOrderSearch, adminOrders, adminPageContent, adminPageSeoStep, adminProductDetailId, adminProductFilter, adminProductSearch, adminProducts, adminRejectReason, adminSellerDetailId, adminSellerFilter, adminSellerSearch, adminSellers, adminSeoHubKey, adminSettings, adminShippingMethods, adminStatusBadge, adminStatusLabel, adminTab, adminTags, adminTicketDetailId, adminTicketFilter, adminTicketReply, adminTickets, adminUser, aiGenerateSeoMeta, aiOptimizeTextHints, aiSuggestFaq, allLists, analyzeOnPageSeo, apply, applyCoupon, applyFromUrl, applyRealtimePayload, applySellerDescFormat, approveAdminProduct, assertNoUserLinks, attrsKeyPart, attrsMatch, authError, authFailCount, authLastName, authLoading, authLockedUntil, authMode, authName, authOpen, authOtp, authOtpTimer, authPhone, authReturnTo, authStep, authTermsAccepted, backupAdminProducts, backupDestPath, backupSellerProducts, bar, blankShippingMethod, blogCommentName, blogCommentText, blogComments, blogForm, blogPostId, blogPosts, brandDetailId, brandQuery, brandsList, buildArticleSchema, buildBreadcrumbSchema, buildCheckoutOrderDraft, buildFaqSchema, buildGa4Seed, buildGscSeed, buildImageAlt, buildLlmsTxt, buildLocalBusinessSchema, buildNewsSitemapXml, buildProductSchema, buildRobotsTxt, buildSitemapIndexXml, buildSitemapXml, buildVariantMatrix, buildVideoSitemapXml, buyerGifts, buyerTicketBody, buyerTicketDetailId, buyerTicketError, buyerTicketFormOpen, buyerTicketSubject, buyerTickets, campaignForm, campaignNow, campaignsList, canonicalBase, cardQtys, carouselIndex, cart, cartItemKey, cartItemLoading, cartOpen, cartesianAttrCombos, catOpen, catalogForm, catalogProducts, changeCartColor, checkoutContact, checkoutErrors, checkoutNewAddress, checkoutNote, checkoutPaymentMethod, checkoutPlacing, checkoutSelectedAddressId, checkoutShippingMethod, checkoutStep, checkoutUseNewAddress, classifyToastVariant, clearAllSearchFilters, clearCart, clearCartConfirm, clearCompare, clearFavorites, clearPlpFilters, clearRecentSearches, clearSellerListFilters, close, closeAdminAuth, closeAdminPanel, closeAuth, closeCartPage, closeCheckout, closeComparePage, closeMobileMenuOnWide, closePDP, closePLP, closeProfilePage, closeRecentPage, closeSeller, closeSellerPanel, closeSellersList, closeSiteDialog, closeStaticPage, closeWishlistPage, collect, collectFullSiteBackup, compare, compareOnlyDiffs, compareOpen, compareReplaceOpen, compareToast, completeRegister, confirmPaymentFail, confirmPaymentSuccess, consumeSeoAiQuota, contactForm, contactFormError, contentEditorTarget, conversationChannelLabel, cookieConsent, copyShareLink, copyTextToClipboard, countFor, couponApplied, couponInput, couponMsg, dark, dealsMinDiscount, dealsSort, defaultAdminBlogCategories, defaultAdminCategories, defaultAdminTags, defaultOrganizationSchema, defaultSeoConfig, defaultShippingMethods, demoOtpCode, deriveCollar, deriveFabric, deriveSleeve, detectImportSource, didYouMean, discountMode, discountPercent, discountPickIds, discountPrices, downloadBlobFile, downloadFullSiteBackup, downloadSeoFile, editingAddressId, editingCouponId, editingSellerProductId, emptyTaxonomyForm, enqueueModeration, ensureAdminSeed, ensureProductCode, esc, existingSellerOrders, expandQuery, exportRedirectsForServer, faqCat, faqQuery, favToast, favorites, fileToImage, finalizePaidOrder, findOpenChatConversation, findProductVariant, findSlider, finishAuthSuccess, fire, fmt, fmtMoney, formatPrice, ga4Aggregate, ga4FilterEvents, ga4Store, generateGiftCode, generateProductCode, generateTicketCode, getAttrDimensions, getCheckoutShippingCost, getCheckoutTaxRate, getCheckoutTotals, getCurrentPageSeoContext, getFavEntry, getPageCms, getPageShareUrl, getProductPublicPath, getProductPublicUrl, getResolvedPageSeo, getSellerEnabledShippingIds, getSellerMaxDiscount, getSellerMinPrice, getSeoAiQuota, getShippingOptions, getShopSeoBody, getUsedPromoCodes, getVariantPrice, getVariantStock, giftCodeForm, gscAggregate, gscInspect, gscStore, has, hasMounted, headerRevealedAfterHero, htmlToPlain, imgZoom, importExternalProductsCsv, installBuyerPwa, isBlogLiked, isDealActive, isFavorite, isLive, isNumericField, isSlider, isVerticalScrollable, ix, lastAutoBackupAt, likedBlogs, list, liveToasts, loadForm, loadGa4Store, loadGscStore, logSeo404, logout, logoutAdmin, hydrateAdminProducts, hydrateAdminSellers, hydrateAdminOrders, adminPatchProductStatus, adminDeleteProduct, adminPurgeProduct, adminBulkArchiveProducts, adminBulkPurgeProducts, adminBulkArchiveSellers, adminBulkPurgeSellers, adminPatchSellerStatus, adminDeleteSeller, adminPurgeSeller, adminPatchOrderStatus, adminListLoading, adminListError, logoutAllDevices, logoutSeller, mapExternalRowToProduct, markAllNotifsRead, markNotifRead, markPromoCodeUsed, matchCatalogBrand, matchCatalogColor, matchCatalogSize, matchCategory, measureSeoPx, mediaToolAssign, mediaToolOffset, mediaToolProcessing, mediaToolResult, mediaToolScale, mediaToolSearch, mediaToolSrc, mediaToolStep, megaOpen, mirrorConversationToAdmin, mirrorSellerOrderToBuyer, mobileMenuOpen, move, nativeShare, newestTab, newsletterPhone, normKey, normalize, normalizeAttrMap, normalizeCategoryKey, normalizeSearch, notifPanelOpen, notifPulling, notifications, oldPriceOpen, onBeforeInput, onBip, onClickCapture, onDragStart, onFocusIn, onInput, onInstalled, onKey, onKeyDown, onMouseDown, onMouseMove, onMouseUp, onPointer, onPointerDown, onScroll, onStorage, onWheel, onlyDigits, openAdminAuth, openAdminFrontEdit, openAdminPanel, openAuth, openCartPage, openCategory, openCheckout, openComparePage, openNewShippingMethod, openPDP, openPLP, openProfilePage, openQuickAdd, openRecentPage, openSeller, openSellerAuth, openSellerPanel, openSellersList, openStaticPage, openTagPage, openTaxonomyHub, openTaxonomyWizard, openWishlistPage, orderDetailId, orderFailed, orderRateDraft, orderReturnOpen, orderStatusColor, orderSuccess, orders, ordersFilter, pageSeoMap, parseCsvText, parseResponseHours, pct, pdpAttrs, pdpColorIdx, pdpExpress, pdpGalleryIdx, pdpGiftWrap, pdpHeight, pdpNotifyOpen, pdpProduct, pdpQText, pdpQaFilter, pdpQty, pdpReviewFilter, pdpSize, pdpSizeRec, pdpSizeRecOpen, pdpSticky, pdpTab, pdpTouchX, pdpWeight, pdpZoom, pendingPayOrder, persistBlogComments, persistCompare, persistFavorites, persistGa4, persistGsc, persistLikedBlogs, persistSession, pickField, pingIndexNow, placeOrder, plpCats, plpCities, plpCityInput, plpCityOpen, plpColors, plpDiscountOnly, plpFabrics, plpFastShipOnly, plpFilterOpen, plpFilterTab, plpInStockOnly, plpMinDiscount, plpPriceMax, plpPriceMin, plpQuery, plpSellers, plpSidebarOpen, plpSizes, plpSkeleton, plpSort, plpSortOpen, plpTagFilter, plpView, plpVisible, poll, portalMounted, pos, prev, printOrderInvoice, processToProductWebP, processProductImageFile, productBackupPayload, productImportReport, productSlugFromNameAndShop, products, productsToCsv, profileTab, publishRealtime, pullNotifications, pushImg, pushLiveToast, pushNotification, pushRecentSearch, pushSellerNotification, pwaInstallEvent, pwaInstalled, quickAdd, quickColorIdx, quickDescOpen, quickGalleryIdx, quickQty, quickSize, readFile, readSessionUser, recentOpen, recentSearches, recentlyViewed, releaseDrag, removeCoupon, removeFavoritesBulk, removeFromCart, removeRecentSearch, renderContentSeoBox, renderList, renderProductCard, renderShareBar, replaceCompareAt, restoreAdminProductsFromFile, restoreFullSiteBackup, restoreSellerProductsFromFile, row, run, runSeoHealthCheck, same, saveAddresses, saveAdminBlogCategories, saveAdminBlogTags, saveAdminBuyers, saveAdminCatalogAttributes, saveAdminCatalogBrands, saveAdminCatalogColors, saveAdminCatalogSizes, saveAdminCategories, saveAdminCoupons, saveAdminFrontEdit, saveAdminOrders, saveAdminPageContentMap, saveAdminProducts, saveAdminSellers, saveAdminSettings, saveAdminShippingMethods, saveAdminTags, saveAdminTickets, saveBlogPosts, saveBuyerGifts, saveBuyerOrders, saveBuyerTickets, saveCampaigns, saveModerationQueue, saveNotifications, savePageSeoMap, saveSellerGifts, saveSellerOrders, saveSellerProducts, saveSellerTickets, saveSellerUser, saveSeo404Log, saveSeoPatch, saveSeoRedirects, saveShippingMethodForm, saveSiteFaqs, saveTaxonomy, saveUser, scoreProduct, scrollCarousel, scrolled, searchActiveIdx, searchCategories, searchColors, searchOpen, searchPhIdx, searchQuery, searchSizes, searchSuggestOpen, seedAddresses, seedAdminData, seedNotifications, seedOrders, seedSellerOrders, seedSellerProducts, seedSellerTickets, selectColor, selectedColors, selectedSizes, sellerBannerIdx, sellerCanSell, sellerCancelForm, sellerCancelOrder, sellerCat, sellerCatMenuOpen, sellerCityInput, sellerCityOpen, sellerConfirmOrder, sellerDescDraft, sellerDescEditorOpen, sellerDescError, sellerDiscountOnly, sellerFaqOpen, sellerFilterSheetOpen, sellerFollowed, sellerGifts, sellerListCities, sellerListMaxResponse, sellerListMinProducts, sellerListMinRating, sellerListQuery, sellerListSort, sellerMarkPackingDone, sellerMediaToolOpen, sellerNewTicket, sellerNewTicketOpen, sellerOpenOrderTicket, sellerOrderDetailId, sellerOrderStatusColor, sellerOrders, sellerOrdersFilter, sellerPriceMap, sellerProductDeleteId, sellerProductFilter, sellerProductForm, sellerProductFormOpen, sellerProductSearch, sellerProductStep, sellerProducts, sellerPromoModal, sellerReportOpen, sellerReportSent, sellerSearchOpen, sellerShareToast, sellerShipOrder, sellerShopOpen, sellerSort, sellerSortMenuOpen, sellerStickyBar, sellerTab, sellerTaxonomyPicker, sellerTaxonomySearch, sellerTicketDetailId, sellerTicketReply, sellerTickets, sellerTrackForm, sellerUser, sendAdminOtp, sendOtp, seo404Log, seoAiDaily, seoCfg, seoCharHint, seoOnChange, seoPixelReport, seoRedirectForm, seoRedirects, setActiveSellerId, setActiveTip, setAddressDeleteConfirm, setAddressForm, setAddressFormOpen, setAddresses, setAdminAnalyticsRange, setAdminAnalyticsSub, setAdminAuthError, setAdminAuthLoading, setAdminAuthOpen, setAdminAuthOtp, setAdminAuthOtpTimer, setAdminAuthPhone, setAdminAuthStep, setAdminBlogCategories, setAdminBlogTags, setAdminBuyerDetailId, setAdminBuyerSearch, setAdminBuyers, setAdminCatalogAttributes, setAdminCatalogBrands, setAdminCatalogColors, setAdminCatalogSizes, setAdminCategories, setAdminContentTab, setAdminCouponForm, setAdminCouponFormOpen, setAdminCoupons, setAdminFrontEditForm, setAdminFrontEditOpen, setAdminGscDim, setAdminGscInspectResult, setAdminGscInspectUrl, setAdminGscRange, setAdminGscSub, setAdminLoading, setAdminModerationQueue, setAdminOrderDetailId, setAdminOrderFilter, setAdminOrderNote, setAdminOrderSearch, setAdminOrders, setAdminPageContent, setAdminPageSeoStep, setAdminProductDetailId, setAdminProductFilter, setAdminProductSearch, setAdminProducts, setAdminRejectReason, setAdminSellerDetailId, setAdminSellerFilter, setAdminSellerSearch, setAdminSellers, setAdminSeoHubKey, setAdminSettings, setAdminShippingMethods, setAdminTab, setAdminTags, setAdminTicketDetailId, setAdminTicketFilter, setAdminTicketReply, setAdminTickets, setAdminUser, setAuthError, setAuthFailCount, setAuthLastName, setAuthLoading, setAuthLockedUntil, setAuthMode, setAuthName, setAuthOpen, setAuthOtp, setAuthOtpTimer, setAuthPhone, setAuthReturnTo, setAuthStep, setAuthTermsAccepted, setBackupDestPath, setBlogCommentName, setBlogCommentText, setBlogComments, setBlogForm, setBlogPostId, setBlogPosts, setBrandDetailId, setBrandQuery, setBrandsList, setBuyerGifts, setBuyerTicketBody, setBuyerTicketDetailId, setBuyerTicketError, setBuyerTicketFormOpen, setBuyerTicketSubject, setBuyerTickets, setCampaignForm, setCampaignNow, setCampaignsList, setCanonicalLink, setCardQtys, setCarouselIndex, setCart, setCartItemLoading, setCartOpen, setCatOpen, setCatalogForm, setCheckoutContact, setCheckoutErrors, setCheckoutNewAddress, setCheckoutNote, setCheckoutPaymentMethod, setCheckoutPlacing, setCheckoutSelectedAddressId, setCheckoutShippingMethod, setCheckoutStep, setCheckoutUseNewAddress, setClearCartConfirm, setCompare, setCompareOnlyDiffs, setCompareOpen, setCompareReplaceOpen, setCompareToast, setContactForm, setContactFormError, setContentEditorTarget, setCookieConsent, setCouponApplied, setCouponInput, setCouponMsg, setDark, setDealsMinDiscount, setDealsSort, setDemoOtpCode, setDiscountMode, setDiscountPercent, setDiscountPickIds, setDiscountPrices, setEditingAddressId, setEditingCouponId, setEditingSellerProductId, setFaqCat, setFaqQuery, setFavToast, setFavorites, setGa4Store, setGiftCodeForm, setGscStore, setHasMounted, setHeaderRevealedAfterHero, setImgZoom, setLastAutoBackupAt, setLikedBlogs, setLiveToasts, setMediaToolAssign, setMediaToolOffset, setMediaToolProcessing, setMediaToolResult, setMediaToolScale, setMediaToolSearch, setMediaToolSrc, setMediaToolStep, setMegaOpen, setMeta, setMobileMenuOpen, setNewestTab, setNewsletterPhone, setNotifPanelOpen, setNotifPulling, setNotifications, setOldPriceOpen, setOrCreateMeta, setOrderDetailId, setOrderFailed, setOrderRateDraft, setOrderReturnOpen, setOrderSuccess, setOrders, setOrdersFilter, setPageSeoMap, setPdpAttrs, setPdpColorIdx, setPdpExpress, setPdpGalleryIdx, setPdpGiftWrap, setPdpHeight, setPdpNotifyOpen, setPdpProduct, setPdpQText, setPdpQaFilter, setPdpQty, setPdpReviewFilter, setPdpSize, setPdpSizeRec, setPdpSizeRecOpen, setPdpSticky, setPdpTab, setPdpTouchX, setPdpWeight, setPdpZoom, setPendingPayOrder, setPlpCats, setPlpCities, setPlpCityInput, setPlpCityOpen, setPlpColors, setPlpDiscountOnly, setPlpFabrics, setPlpFastShipOnly, setPlpFilterOpen, setPlpFilterTab, setPlpInStockOnly, setPlpMinDiscount, setPlpPriceMax, setPlpPriceMin, setPlpQuery, setPlpSellers, setPlpSidebarOpen, setPlpSizes, setPlpSkeleton, setPlpSort, setPlpSortOpen, setPlpTagFilter, setPlpView, setPlpViewPersist, setPlpVisible, setPortalMounted, setProductImportReport, setProfileTab, setPwaInstallEvent, setPwaInstalled, setQuickAdd, setQuickColorIdx, setQuickDescOpen, setQuickGalleryIdx, setQuickQty, setQuickSize, setRecentOpen, setRecentSearches, setRecentlyViewed, setScrolled, setSearchActiveIdx, setSearchCategories, setSearchColors, setSearchOpen, setSearchPhIdx, setSearchQuery, setSearchSizes, setSearchSuggestOpen, setSelectedColors, setSelectedSizes, setSellerBannerIdx, setSellerCancelForm, setSellerCat, setSellerCatMenuOpen, setSellerCityInput, setSellerCityOpen, setSellerDescDraft, setSellerDescEditorOpen, setSellerDescError, setSellerDiscountOnly, setSellerFaqOpen, setSellerFilterSheetOpen, setSellerFollowed, setSellerGifts, setSellerListCities, setSellerListMaxResponse, setSellerListMinProducts, setSellerListMinRating, setSellerListQuery, setSellerListSort, setSellerMediaToolOpen, setSellerNewTicket, setSellerNewTicketOpen, setSellerOrderDetailId, setSellerOrders, setSellerOrdersFilter, setSellerProductDeleteId, setSellerProductFilter, setSellerProductForm, setSellerProductFormOpen, setSellerProductOutOfStock, setSellerProductReorderPoint, setSellerProductSearch, setSellerProductStep, setSellerProducts, setSellerPromoModal, setSellerReportOpen, setSellerReportSent, setSellerSearchOpen, setSellerShareToast, setSellerShopOpen, setSellerSort, setSellerSortMenuOpen, setSellerStickyBar, setSellerTab, setSellerTaxonomyPicker, setSellerTaxonomySearch, setSellerTicketDetailId, setSellerTicketReply, setSellerTickets, setSellerTrackForm, setSellerUser, setSeo404Log, setSeoAiDaily, setSeoRedirectForm, setSeoRedirects, setShippingMethodForm, setShippingMethodFormOpen, setShowAdminPanel, setShowCartPage, setShowCheckout, setShowComparePage, setShowPLP, setShowProfilePage, setShowRecentPage, setShowSellerPanel, setShowSellersList, setShowTaxonomyHub, setShowTop, setShowTracking, setShowWishlistPage, setSiteDialog, setSiteFaqs, setStaticPage, setStep, setStockNotifyIds, setTaxonomyForm, setTaxonomyFormOpen, setTopSellersTab, setUser, setWishlistClearConfirm, setWishlistFilter, setWishlistOpen, setWishlistSelected, setWishlistSort, setWishlistView, shareSeller, shippingMethodForm, shippingMethodFormOpen, shopCodePrefix, shouldNormalize, show, showAdminPanel, showBrowserPush, showCartPage, showCheckout, showComparePage, showCompareToast, showPLP, showProfilePage, showRecentPage, showSellerPanel, showSellersList, showTaxonomyHub, showToast, showTop, showTracking, showWishlistPage, simulateBrandScan, siteConfirm, siteDialog, siteFaqs, sitePrompt, sitePromptFields, slugifyTaxonomy, smartScore, splitList, staticPage, statusBadge, statusLabel, statusOf, stockNotifyIds, stripHtmlSeo, stripLinksForDisplay, submitSearch, suggestInternalLinks, suggestSizeFromBody, sync, syncFormVariants, takeSnap, taxonomyForm, taxonomyFormOpen, taxonomyTypeLabel, textContainsForbiddenLink, tick, ticketMessagesToChatUI, toEnDigits, toFa, toggleBlogLike, toggleCompare, toggleFavorite, toggleSearchCategory, toggleSearchColor, toggleSearchSize, toggleSellerFollow, toggleSellerListCity, top, topSellersRanked, topSellersTab, trackGa4Event, up, updatePageCms, updateQty, updateSellerOrderStatus, updateSellerProductStock, upsertJsonLd, upsertRankKeyword, user, validateCheckout, validateProductBackup, variantKey, verifyAdminOtp, verifyOtp, warnFaKeyboard, wishlistClearConfirm, wishlistFilter, wishlistOpen, wishlistProducts, wishlistSelected, wishlistSort, wishlistView, wrap, yes } = api;
  ...
  const runTaxBulkCategory = async (mode) => {
    if (typeof saveAdminCategories !== 'function') {
      try { showToast({ message: 'ذخیره دسته در دسترس نیست', variant: 'error', duration: 3000, position: 'top-center' }); } catch (_) {}
      return;
    }
  ...
  const filterTaxTags = (list) => {
    const arr = Array.isArray(list) ? list : [];
    if (taxFilter === 'archived') return arr.filter(isTaxArchived);
    if (taxFilter === 'active') return arr.filter((x) => !isTaxArchived(x));
    return arr;
  };
  const runTaxBulkTag = async (mode) => {
    if (typeof saveAdminTags !== 'function') {
      try { showToast({ message: 'ذخیره برچسب در دسترس نیست', variant: 'error', duration: 3000, position: 'top-center' }); } catch (_) {}
      return;
    }
    const ids = new Set(taxSelectedIds.map(String));
    if (!ids.size) {
 ...
 return (
  <>
      <div className="panel-content-wrap w-full max-w-none px-2 sm:px-4 py-4 sm:py-10 pb-24">
       <div className="panel-admin-header flex flex-wrap items-center justify-between gap-4 mb-6 p-4 sm:p-5 rounded-2xl border border-primary-200 border-white/20 bg-white bg-primary-900">
        <div className="flex items-center gap-3">
         <div className="w-14 h-14 rounded-full bg-primary-800 bg-[#4CCD99] text-white flex items-center justify-center text-xl font-bold">{(adminUser.name || 'ا')[0]}</div>
         <div>
          <h1 className="text-xl sm:text-2xl font-bold text-primary-900 text-white">{adminUser.name}</h1>
          <p className="text-xs text-primary-500 text-[#13ABC4] !text-white">{adminUser.role || 'Super Admin'} · <span className="text-emerald-600" dir="ltr">{adminUser.phone}</span></p>
         </div>
        </div>
       ...
       <div className="flex flex-col md:flex-row gap-4 md:gap-6">
        <aside className="panel-sidebar w-full md:w-64 flex-shrink-0 sticky top-[var(--site-header-h,3.5rem)] z-40 md:top-0 md:h-[calc(100vh-0px)] md:self-start">
         <div className="admin-tabs-strip panel-nav flex md:flex-col gap-0.5 overflow-x-auto p-3 rounded-xl border border-black/5 border-white/5 bg-white/50 bg-white/[0.03] md:min-h-[calc(100vh-6rem)]">
          {[
           ...
           <button key={t.id} type="button" onClick={() => { setAdminTab(t.id); if (t.id === 'campaigns' && typeof hydrateCampaignsFromApi === 'function') { try { hydrateCampaignsFromApi(true); } catch(_){} } if (t.id === 'dashboard' && typeof hydrateAdminStatsFromApi === 'function') { try { hydrateAdminStatsFromApi(); } catch(_){} } if (t.id === 'coupons' && typeof hydrateAdminCoupons === 'function') { try { hydrateAdminCoupons(); } catch(_){} } if (t.id === 'blog-new') { const defCat = ((adminBlogCategories || []).find(c => c.active !== false) || {}).name || 'راهنمای خرید'; setBlogForm({ id: '', title: '', cat: defCat, excerpt: '', body: '', status: 'published', author: 'تحریریه', read: '۵ دقیقه', publishAtDate: '', publishAtTime: '10:00', publishAtMs: null, publishAtFa: '', seoTitle: '', seoDescription: '', seoFocusKeywords: '', seoCanonical: '', seoOgImage: '', imageAlt: '', image: '', seoNoindex: false, seoFaq: [] }); } else if (t.id === 'blog') { setBlogForm(null); if (typeof hydrateBlogPostsFromApi === 'function') try { hydrateBlogPostsFromApi(); } catch(_){} } setAdminSellerDetailId(null); setAdminProductDetailId(null); setAdminOrderDetailId(null); setAdminTicketDetailId(null); setAdminBuyerDetailId(null); setAdminLoading(false); requestAnimationFrame(() => scrollAdminPanelToTop()); }}
            ...
            {t.label}
            {t.id === 'tickets' && adminUnreadTickets > 0 && <span className="mr-auto text-xs bg-transparent hover:bg-primary-50 hover:bg-white/10 border border-primary-200 border-white/25 text-primary-900 text-white rounded-full px-1.5 min-w-[18px] text-center">{toFa(adminUnreadTickets)}</span>}
           </button>
          ))}
         </div>
        </aside>
        <div className="panel-main flex-1 min-w-0 min-h-0 pb-8 p-4 sm:p-6 rounded-2xl border border-primary-200/80 border-white/10 bg-white/95 bg-primary-900/95 backdrop-blur-sm shadow-[0_8px_30px_rgba(15,23,42,0.06)] w-full max-w-none">
         {/* لودر سراسری بین صفحات کافی است — اسپینر اضافه پنل حذف شد */}

         {/* Dashboard */}
         {!adminLoading && adminTab === 'dashboard' && (
          <div className="space-y-6">
           <h2 className="text-base font-bold text-primary-900 text-white">داشبورد ادمین</h2>
           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
            <div>
             <p className="text-sm font-bold text-primary-900 text-white">پشتیبان دادهٔ سرور</p>
             <p className="text-xs text-primary-500 text-white/60 mt-1 leading-6">
            ...
            <div className="flex flex-wrap gap-2">
             <button
              type="button"
              onClick={runServerBackupDownload}
              className="text-xs px-3 py-2 rounded-full bg-apple-blue text-white font-medium"
             >
              دانلود پشتیبان JSON
             </button>
             <label className="text-xs px-3 py-2 rounded-full border border-primary-200 border-white/25 text-primary-800 text-white cursor-pointer font-medium">
              بازیابی از فایل
              <input
               type="file"
           ...
           <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {(() => {
             ...
             <div key={i} className="p-3 sm:p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <p className="text-xs text-primary-500 text-[#13ABC4] !text-white mb-1">{c.label}</p>
              <p className="text-lg sm:text-xl font-bold text-primary-900 text-white">{c.value}</p>
             </div>
            ))}
           </div>
           <div className="grid sm:grid-cols-2 gap-3">
            <div className="p-4 rounded-2xl border border-amber-200 border-amber-800/40 bg-amber-50/50 bg-amber-950/20">
             <p className="text-xs font-bold text-amber-700 text-amber-300 mb-2">هشدارها</p>
             <ul className="space-y-1.5 text-xs text-primary-700 text-white/80">
            ...
            <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
             <p className="text-xs font-bold text-primary-700 text-white mb-2">میان‌بر سریع</p>
             <div className="flex flex-wrap gap-2">
              {[{t:'sellers',l:'فروشندگان'},{t:'products',l:'محصولات'},{t:'orders',l:'سفارش‌ها'},{t:'tickets',l:'تیکت‌ها'}].map(x=>(
               <button key={x.t} type="button" onClick={()=>setAdminTab(x.t)} className="px-3 py-1.5 rounded-full text-xs border border-primary-200 border-white/30 hover:bg-primary-50 hover:bg-primary-900">{x.l}</button>
              ))}
             </div>
            </div>
           </div>
           <div>
            <h3 className="text-sm font-bold text-primary-900 text-white mb-3">آخرین سفارش‌ها</h3>
            <div className="space-y-2">
             {(adminOrders||[]).slice(0,8).map(o=>(
              <button key={o.id} type="button" onClick={()=>{setAdminTab('orders');setAdminOrderDetailId(o.id);}} className="w-full flex items-center justify-between gap-3 p-3 rounded-xl border border-primary-100 border-white/10 hover:bg-primary-50 hover:bg-primary-900 text-right">
               <div className="min-w-0">
                <p className="text-sm font-medium text-primary-900 text-white">{o.id}</p>
                <p className="text-xs text-primary-500">{o.buyerName} · {o.date}</p>
                <p className="text-xs text-primary-600 text-white/70 mt-0.5 truncate">فروشنده: {o.sellerName || o.seller?.name || o.items?.[0]?.sellerName || '—'}</p>
               </div>
             ...
             {!(adminOrders||[]).length && <p className="text-sm text-primary-400 text-[#13ABC4] text-center py-8">سفارشی وجود ندارد</p>}
            </div>
           </div>
           {/* نمودارها و گزارش فروشگاه‌ها */}
           <div className="grid sm:grid-cols-2 gap-4">
            <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
             <h3 className="text-sm font-bold text-primary-900 text-white mb-3">وضعیت سفارش‌ها</h3>
             {(() => {
                ...
                {statuses.map(s => {
                 ...
                 return (
                  <div key={s.k}>
                   ...
                   <div className="h-2 rounded-full bg-primary-100 bg-primary-800 overflow-visible"><div className={`h-full ${s.c}`} style={{ width: pct + '%' }} /></div>
                  </div>
                 );
                })}
               </div>
            ...
            <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
             <h3 className="text-sm font-bold text-primary-900 text-white mb-3">سود / کارمزد (خلاصه)</h3>
             {(() => {
              const list = adminOrders || [];
              ...
              return (
               <div className="space-y-3 text-sm">
                ...
                <div className="h-3 rounded-full overflow-visible flex bg-primary-100 bg-primary-800">
           ...
           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
             <h3 className="text-sm font-bold text-primary-900 text-white">رتبه‌بندی فروشگاه‌ها</h3>
            ...
            {(() => {
             ...
             const row = (list, label) => {
              const darkBg =
               label === 'بیشترین فروش' ? 'bg-emerald-950/70 border-emerald-800/50' :
               label === 'کمترین فروش' ? 'bg-amber-950/60 border-amber-800/45' :
               label === 'بیشترین امتیاز' ? 'bg-sky-950/70 border-sky-800/50' :
               'bg-orange-950/50 border-orange-800/40';
              return (
               <div key={label}>
                <p className="text-xs font-bold text-primary-600 text-white/70 mb-1">{label}</p>
                <div className="space-y-1">
                 {list.length ? list.map((s, i) => (
                  <div key={s.id || i} className={`flex justify-between text-xs p-2 rounded-lg bg-primary-50/80 border border-transparent ${darkBg} text-primary-900 text-white`}>
         ...
         {!adminLoading && adminTab === 'sellers' && !adminSellerDetailId && (
          <div>
           <div className="admin-products-head flex flex-wrap items-center justify-between gap-3 mb-2">
            <h2 className="text-base font-bold text-primary-900 text-white">مدیریت فروشندگان</h2>
            <input value={adminSellerSearch} onChange={e=>setAdminSellerSearch(e.target.value)} placeholder="جستجو نام یا موبایل…" className="px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm w-full sm:w-56 focus:outline-none focus:border-apple-blue" />
           </div>
           <div className="admin-products-filters flex gap-1 overflow-x-auto mb-2">
            {[{id:'all',l:'همه'},{id:'pending',l:'در انتظار'},{id:'approved',l:'تأیید‌شده'},{id:'rejected',l:'رد‌شده'},{id:'suspended',l:'محدود شده'},{id:'archived',l:'آرشیو شده‌ها'}].map(f=>(
             <button key={f.id} type="button" onClick={()=>setAdminSellerFilter(f.id)} className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium border transition ${adminSellerFilter===f.id?'panel-filter-chip--on':'plp-filter-chip border-primary-300 border-white/50 !text-primary-900 !text-white bg-white bg-[#2A2C30] font-medium'}`}>{f.l}</button>
            ))}
           </div>
           ...
           <div className="space-y-2">
            ...
            <div className="mb-3 flex flex-wrap items-center gap-2 p-2.5 rounded-xl border border-red-200 bg-red-50/80 bg-red-950/30 border-red-500/30">
             <span className="text-xs font-medium text-red-800 text-red-200">{adminSelectedSellerIds.length} فروشنده انتخاب شده</span>
             <button type="button" disabled={adminBulkBusy} onClick={() => runAdminBulkSellers(adminSellerFilter === 'archived' ? 'purge' : 'archive')} className="text-xs px-3 py-1.5 rounded-full bg-transparent hover:bg-primary-50 hover:bg-white/10 border border-primary-200 border-white/25 text-primary-900 text-white font-medium disabled:opacity-50">
              {adminBulkBusy ? 'در حال اجرا…' : (adminSellerFilter === 'archived' ? 'حذف دائم گروهی' : 'آرشیو گروهی')}
             </button>
             <button type="button" disabled={adminBulkBusy} onClick={() => setAdminSelectedSellerIds([])} className="text-xs px-3 py-1.5 rounded-full border border-primary-300 border-white/30">لغو انتخاب</button>
            </div>
           )}
           {(() => {
            ...
            {visibleSellers.length > 0 && (
             <label className="flex items-center gap-2 px-1 py-1 text-xs text-primary-600 text-white/70 cursor-pointer select-none">
            ...
            {visibleSellers.map(s=>{
              ...
              return (
             <div key={s.id} className={`p-3 sm:p-4 rounded-2xl border bg-white bg-primary-900 flex flex-col sm:flex-row sm:items-center gap-3 ${adminSelectedSellerIds.includes(String(s.id)) ? 'border-apple-blue ring-1 ring-apple-blue/30' : 'border-primary-200 border-white/15'}`}>
             <input type="checkbox" checked={adminSelectedSellerIds.includes(String(s.id))} onChange={() => toggleAdminSellerSelect(s.id)} className="rounded border-primary-300 flex-shrink-0" aria-label="انتخاب فروشنده" />
              <button type="button" onClick={()=>setAdminSellerDetailId(s.id)} className="flex-1 text-right min-w-0">
               <div className="flex items-start gap-3">
                {s.logo ? (
                 <img src={s.logo} alt="" className="w-12 h-12 rounded-xl object-cover border border-primary-100 border-white/10 flex-shrink-0" onError={(e)=>{e.currentTarget.style.display='none'}} />
                ) : (
                 <div className="w-12 h-12 rounded-xl bg-primary-100 bg-primary-800 flex items-center justify-center text-sm font-bold text-primary-500 text-[#13ABC4] flex-shrink-0">{(s.shopName||'ف').slice(0,1)}</div>
                )}
                <div className="min-w-0 flex-1">
                 <p className="font-medium text-primary-900 text-white truncate">{s.shopName || 'بدون نام فروشگاه'}</p>
                 <p className="text-xs text-primary-500 text-[#13ABC4] !text-white mt-0.5">
                  {s.ownerName ? s.ownerName + ' · ' : ''}<span dir="ltr">{s.phone || '—'}</span>{s.city ? ' · ' + s.city : ''}
                 </p>
                 <p className="text-xs text-primary-400 text-[#13ABC4] mt-0.5">
              ...
              <div className="flex items-center gap-2 flex-wrap">
               ...
               {s.status==='pending'&&(<>
                ...
                <button type="button" onClick={()=>{sitePrompt('دلیل رد (اختیاری):').then((r)=>{patchSellerStatus(s.id, 'rejected');});}} className="text-xs px-2.5 py-1 rounded-full bg-transparent hover:bg-primary-50 hover:bg-white/10 border border-primary-200 border-white/25 text-primary-900 text-white">رد</button>
               </>)}
               ...
               {String(s.status||'').toLowerCase()==='archived' ? (
                <>
                 ...
                 <button type="button" onClick={async ()=>{
                  const ok = typeof siteConfirm === 'function'
                   ? await siteConfirm('این فروشنده برای همیشه از پایگاه داده و سایت حذف شود؟ محصولاتش آرشیو می‌مانند ولی فروشگاه پاک می‌شود. این عمل برگشت‌پذیر نیست.', 'حذف دائم')
                   : (typeof window !== 'undefined' && window.confirm('حذف دائم؟ برگشت‌پذیر نیست.'));
                  if (!ok) return;
            ...
            {!(adminListLoading && adminListLoading.sellers) && !(adminSellers||[]).length && (
             <p className="text-center text-sm text-primary-400 text-[#13ABC4] py-10">
              {(adminListError && adminListError.sellers) ? 'بارگذاری ناموفق بود' : 'فروشنده‌ای ثبت نشده'}
             </p>
            )}
            {!visibleSellers.length && !!(adminSellers||[]).length && (
             <p className="text-center text-sm text-primary-400 text-[#13ABC4] py-10">موردی با این فیلتر نیست</p>
            )}
              </>
             );
            })()}
         ...
         {!adminLoading && adminTab === 'sellers' && adminSellerDetailId && (() => {
          ...
          return (
           <div className="space-y-4">
            ...
            <h2 className="text-base font-bold text-primary-900 text-white">{String(s.shopName || s.name || 'فروشگاه')}</h2>

            <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-4">
             <h3 className="text-sm font-bold text-primary-900 text-white">تصاویر فروشگاه (تأیید انتشار)</h3>
             <div className="grid sm:grid-cols-2 gap-4">
              <div className="space-y-2">
            ...
            <div className="grid sm:grid-cols-2 gap-4 text-sm">
             <div className="p-4 rounded-2xl bg-white bg-primary-900 border border-primary-200 border-white/15 space-y-1.5">
             ...
             <div className="p-4 rounded-2xl bg-white bg-primary-900 border border-primary-200 border-white/15 space-y-3">
            ...
            <div className="space-y-2">
             {sellerProds.map((p) => (
              <div key={String(p.id)} className="flex flex-col sm:flex-row sm:items-center gap-3 p-3 rounded-xl bg-white bg-primary-900 border border-primary-100 border-white/10">
               <img
               ...
               <div className="flex-1 min-w-0 text-right">
                <p className="text-sm font-medium text-primary-900 text-white truncate">{String(p.name || p.title || 'محصول')}</p>
                <p className="text-xs text-primary-500 text-[#13ABC4] mt-0.5">{fa(Number(p.price || 0).toLocaleString())} ت · موجودی {fa(p.stock || 0)}</p>
               </div>
               <div className="flex flex-col items-end gap-1 flex-shrink-0">
                <span className={`text-xs px-2 py-0.5 rounded-full ${badge(p.status)}`}>{label(p.status)}</span>
                <button type="button" onClick={() => setAdminProductDetailId(p.id)} className="text-xs px-2 py-0.5 rounded-full border border-primary-200 border-white/30">جزئیات</button>
               </div>
              </div>
             ))}
         ...
         {!adminLoading && adminTab === 'products' && !adminProductDetailId && (
          <div>
           <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
            <h2 className="text-base font-bold text-primary-900 text-white">مدیریت محصولات</h2>
            <div className="admin-products-actions flex flex-wrap items-center gap-1.5 w-full sm:w-auto">
             <button type="button" onClick={() => { try { window.dispatchEvent(new CustomEvent('admin-products-refetch')); window.dispatchEvent(new CustomEvent('admin-sellers-refetch')); } catch(_){} }} className="text-xs px-2 py-1 rounded-full border border-primary-200 border-white/20 text-primary-700 text-white">بازخوانی از سرور</button>
             <button
              type="button"
              className="text-xs px-3 py-1.5 rounded-full border border-primary-200 border-white/20 text-primary-700 text-[#13ABC4] footer-link text-white font-medium flex items-center gap-1 hover:bg-primary-50 hover:bg-primary-800"
             ...
             <label
              className="text-xs px-3 py-1.5 rounded-full border border-primary-200 border-white/20 text-primary-700 text-white font-medium flex items-center gap-1 cursor-pointer hover:bg-primary-50 hover:bg-primary-800"
             ...
             <input value={adminProductSearch} onChange={e=>setAdminProductSearch(e.target.value)} placeholder="جستجو نام محصول…" className="admin-products-search px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm w-full sm:w-56 focus:outline-none focus:border-apple-blue" />
            </div>
           </div>
           <div className="admin-tabs-strip flex gap-1 overflow-x-auto mb-4">
            {[{id:'all',l:'همه'},{id:'active',l:'فعال'},{id:'pending',l:'در انتظار'},{id:'rejected',l:'رد‌شده'},{id:'inactive',l:'غیرفعال'},{id:'archived',l:'آرشیو شده‌ها'},{id:'purge_requested',l:'درخواست حذف'}].map(f=>(
             <button key={f.id} type="button" onClick={()=>setAdminProductFilter(f.id)} className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium border ${adminProductFilter===f.id?'panel-filter-chip--on':'plp-filter-chip border-primary-300 border-white/50 !text-primary-900 !text-white bg-white bg-[#2A2C30] font-medium'}`}>{f.l}</button>
            ))}
           </div>
           {adminSelectedProductIds.length > 0 && (
            <div className="mb-3 flex flex-wrap items-center gap-2 p-2.5 rounded-xl border border-red-200 bg-red-50/80 bg-red-950/30 border-red-500/30">
             <span className="text-xs font-medium text-red-800 text-red-200">{adminSelectedProductIds.length} محصول انتخاب شده</span>
             ...
             <button type="button" disabled={adminBulkBusy} onClick={() => setAdminSelectedProductIds([])} className="text-xs px-3 py-1.5 rounded-full border border-primary-300 border-white/30">لغو انتخاب</button>
            </div>
           )}
           <div className="space-y-2">
            {(() => {
            ...
            {visibleProducts.map(p=>(
             <div key={p.id} className={`flex flex-col sm:flex-row sm:items-stretch gap-3 p-3 rounded-2xl border bg-white bg-primary-900 ${adminSelectedProductIds.includes(String(p.id)) ? 'border-apple-blue ring-1 ring-apple-blue/30' : 'border-primary-200 border-white/15'}`}>
              ...
              <img
               ...
               alt=""
               className="w-full sm:w-20 h-40 sm:h-24 object-cover rounded-xl flex-shrink-0 bg-primary-50 bg-primary-800"
               onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = '/logo.webp'; }}
              />
              </div>
              <div className="flex-1 min-w-0 text-right space-y-1">
               <p className="text-sm font-bold text-primary-900 text-white truncate">{p.name || p.title || 'بدون نام'}</p>
               <p className="text-xs text-primary-600 text-white/80">
               ...
               <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[11px] text-primary-500 text-white/60 mt-0.5">
                <span>موجودی <span className="font-medium text-primary-800 text-white/90">{toFa(p.stock || 0)}</span></span>
                {p.category ? (
                 <>
                  <span className="text-primary-300 text-white/20">·</span>
                  <button type="button" onClick={(e)=>{e.preventDefault();e.stopPropagation();try{if(typeof openCategory==='function')openCategory(p.category);else if(typeof openPLP==='function')openPLP({cat:p.category});}catch(_){}}} className="hover:text-apple-blue text-[#13ABC4] underline-offset-2 hover:underline">
                   دسته <span className="font-medium text-primary-800 text-white/90">{p.category}</span>
                  </button>
                 </>
                ) : null}
                {p.brand ? (
                 <>
                  <span className="text-primary-300 text-white/20">·</span>
                  <button type="button" onClick={(e)=>{e.preventDefault();e.stopPropagation();try{if(typeof openPLP==='function')openPLP({query:p.brand});}catch(_){}}} className="hover:text-apple-blue text-[#13ABC4] underline-offset-2 hover:underline">
                   برند <span className="font-medium text-primary-800 text-white/90">{p.brand}</span>
                  </button>
                 </>
                ) : null}
               </div>
               ...
               {p.attributes && typeof p.attributes === 'object' && Object.keys(p.attributes).length > 0 && (
                <p className="text-xs text-primary-400 text-[#13ABC4] truncate">
               ...
               <button
                ...
                className="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-full bg-transparent border border-primary-200 border-white/25 text-primary-900 text-white hover:bg-transparent border border-primary-200 border-white/25 transition"
               >
               ...
               <div className="flex flex-wrap gap-1 justify-end">
                {p.status === 'pending' && (
                 <>
                  ...
                  <button type="button" onClick={() => { sitePrompt('دلیل رد:').then(() => { patchAdminProductStatus(p.id, 'rejected'); }); }} className="text-xs px-2 py-0.5 rounded-full bg-transparent hover:bg-primary-50 hover:bg-white/10 border border-primary-200 border-white/25 text-primary-900 text-white">رد</button>
                 </>
                )}
                {p.status === 'active' && (
                ...
                {p.status === 'archived' && (
                 <button
                  type="button"
                  onClick={() => {
                   if (typeof adminPatchProductStatus === 'function') adminPatchProductStatus(p.id, 'active');
                   else if (typeof patchAdminProductStatus === 'function') patchAdminProductStatus(p.id, 'active');
                  }}
                  className="text-xs px-2 py-0.5 rounded-full border border-emerald-300 text-emerald-700 font-medium"
                 >
                  بازگردانی از آرشیو
                 </button>
                )}
                <button type="button" onClick={() => setAdminProductDetailId(p.id)} className="text-xs px-2 py-0.5 rounded-full border border-primary-200 border-white/30">جزئیات / ویرایش</button>
                {p.status === 'purge_requested' ? (
                 <>
                  <button
                   ...
                   className="text-xs px-2 py-0.5 rounded-full border border-red-500 bg-transparent hover:bg-primary-50 hover:bg-white/10 text-primary-900 text-white font-medium"
                  >
                   تأیید حذف دائم
                  </button>
                  <button
                 ...
                 <button
                  type="button"
                  onClick={() => {
                   siteConfirm('این محصول به آرشیو منتقل شود؟ از فروشگاه مخفی می‌شود و بعداً می‌توانید بازگردانید یا حذف دائم کنید.').then((ok) => {
                    if (!ok) return;
                    if (typeof adminDeleteProduct === 'function') adminDeleteProduct(p.id);
                    else if (typeof adminPatchProductStatus === 'function') adminPatchProductStatus(p.id, 'archived');
                    else if (typeof patchAdminProductStatus === 'function') patchAdminProductStatus(p.id, 'archived');
                   });
                  }}
                  className="text-xs px-2 py-0.5 rounded-full border border-amber-300 text-amber-700 font-medium"
                 >
                  آرشیو
                 </button>
                )}
            ...
            {!(adminProducts||[]).length && <p className="text-center text-sm text-primary-400 text-[#13ABC4] py-10">محصولی نیست</p>}
            {!visibleProducts.length && !!(adminProducts||[]).length && <p className="text-center text-sm text-primary-400 text-[#13ABC4] py-10">موردی با این فیلتر نیست</p>}
              </>
             );
            })()}
           </div>
         ...
         {!adminLoading && adminTab === 'products' && adminProductDetailId && (() => {
          ...
          if (!p) {
           return (
            <div className="p-6 text-center">
             <p className="text-sm text-primary-500 text-[#13ABC4] mb-3">محصول یافت نشد</p>
          ...
          return (
           <div className="space-y-4" key={p.id}>
            ...
            <div className="p-3 sm:p-4 rounded-2xl border border-primary-200 border-white/15 bg-primary-50/80 bg-primary-950/50 space-y-3">
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <p className="text-xs font-bold text-primary-900 text-white">بج‌های کارت محصول</p>
                <p className="text-[10px] text-primary-500 text-white/50">با زدن هر گزینه بلافاصله ذخیره می‌شود</p>
              </div>
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  { key: 'featuredTop', label: 'برترین پیراهن', on: !!(p.featuredTop || p.featured_top), cls: 'bg-primary-900 text-white bg-white text-primary-900' },
                ].map((b) => (
                  <button
                    key={b.key}
                    type="button"
                    ...
                    className={`text-xs font-semibold px-3 py-1.5 rounded-full border transition ${
                      b.on
                        ...
                        : 'bg-white bg-primary-900 text-primary-700 text-white/80 border-primary-200 border-white/20'
                    }`}
                  >
                    {b.on ? '✓ ' : ''}{b.label}
                  </button>
              ...
              <div className="flex flex-wrap items-end gap-2">
                <label className="text-xs text-primary-600 text-white/70 space-y-1">
                  ...
                  <input
                    ...
                    className="w-24 px-2 py-1.5 rounded-xl border border-primary-200 border-white/20 bg-white bg-primary-900 text-sm"
                  />
                </label>
                <button
                  type="button"
            ...
            <div className="flex flex-wrap items-start gap-4 p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
             ...
             <div className="flex-1 min-w-0 space-y-1 text-right">
              <h2 className="text-base font-bold text-primary-900 text-white">{p.name || p.title}</h2>
            ...
            <form
             className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3 text-right"
             onSubmit={async (e) => {
              e.preventDefault();
            ...
            >
             <h3 className="text-sm font-bold text-primary-900 text-white">ویرایش محصول</h3>
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <label className="text-xs text-primary-500 text-[#13ABC4] space-y-1"><span>نام محصول</span><input name="name" defaultValue={p.name || p.title || ''} className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm" /></label>
              <label className="text-xs text-primary-500 text-[#13ABC4] space-y-1"><span>قیمت (تومان)</span><input name="price" defaultValue={p.price || 0} inputMode="numeric" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm" dir="ltr" /></label>
              <label className="text-xs text-primary-500 text-[#13ABC4] space-y-1"><span>موجودی</span><input name="stock" defaultValue={p.stock || 0} inputMode="numeric" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm" dir="ltr" /></label>
                            <div className="sm:col-span-2 space-y-2">
                <p className="text-xs text-primary-500 text-[#13ABC4] font-medium">دسته‌بندی محصول <span className="text-primary-400 text-[#13ABC4] font-normal">(از فهرست ادمین — کلیک برای باز شدن زبانه)</span></p>
                <button
                  type="button"
                  onClick={() => { setAdminProductTaxPicker((v) => v === 'cats' ? null : 'cats'); setAdminProductTaxSearch(''); }}
                  className="w-full text-right px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-white bg-primary-900 text-sm text-primary-900 text-white flex items-center justify-between gap-2"
                >
                  <span className="truncate">{adminEditCats.length ? adminEditCats.join('، ') : 'انتخاب دسته…'}</span>
                  <span className="text-[11px] text-primary-400 text-[#13ABC4] shrink-0">{adminEditCats.length}/۳</span>
                </button>
              ...
              <div className="sm:col-span-2 space-y-2">
                <p className="text-xs text-primary-500 text-[#13ABC4] font-medium">برند محصول <span className="text-primary-400 text-[#13ABC4] font-normal">(فقط از فهرست ادمین — کلیک برای باز شدن زبانه)</span></p>
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="text-[11px] text-primary-400 text-[#13ABC4] shrink-0">تکی</span>
                </button>
              ...
              <div className="sm:col-span-2 space-y-2">
                <p className="text-xs text-primary-500 text-[#13ABC4] font-medium">برچسب محصول <span className="text-primary-400 text-[#13ABC4] font-normal">(چندتایی از فهرست ادمین — کلیک برای باز شدن زبانه)</span></p>
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="text-[11px] text-primary-400 text-[#13ABC4] shrink-0">{adminEditTags.length}/۳</span>
                </button>
              ...
              {adminProductTaxPicker && (
                <div className="sm:col-span-2 rounded-2xl border border-primary-200 border-white/15 bg-primary-50/80 bg-primary-950/80 overflow-hidden">
                  <div className="flex items-center justify-between gap-2 px-3 py-2.5 border-b border-primary-100 border-white/10">
                    <p className="text-xs font-bold text-primary-900 text-white">
                      {adminProductTaxPicker === 'cats' ? 'انتخاب دسته‌بندی'
                        : adminProductTaxPicker === 'tags' ? 'انتخاب برچسب'
                        : 'انتخاب برند'}
                    </p>
                    <button type="button" onClick={() => setAdminProductTaxPicker(null)} className="text-[11px] px-2 py-1 rounded-full border border-primary-200 border-white/20">بستن</button>
                  </div>
                  <div className="px-3 py-2 border-b border-primary-100 border-white/10">
                    <input
                      type="search"
                      value={adminProductTaxSearch}
                      onChange={(e) => setAdminProductTaxSearch(e.target.value)}
                      placeholder="جستجو…"
                      className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-white bg-primary-900 text-sm text-primary-900 text-white focus:outline-none focus:border-apple-blue"
                    />
                  </div>
                  <div className="max-h-48 overflow-y-auto p-2 flex flex-wrap gap-1.5">
                    {(() => {
                      ...
                      if (!items.length) {
                        return <p className="text-xs text-primary-400 text-[#13ABC4] w-full text-center py-4">موردی یافت نشد</p>;
                      }
                      return items.map((item) => {
                        ...
                        return (
                          <label
                            key={item.id || name}
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-full border text-xs cursor-pointer transition ${selected ? 'bg-apple-blue text-white border-apple-blue' : 'border-primary-200 border-white/20 bg-white bg-primary-900 text-primary-800 text-white'}`}
                          >
                            <input
              ...
              <label className="text-xs text-primary-500 text-[#13ABC4] space-y-1"><span>آدرس تصویر شاخص</span><input name="image" defaultValue={p.image || ''} className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm" dir="ltr" /></label>
              <label className="text-xs text-primary-500 text-[#13ABC4] space-y-1 sm:col-span-2"><span>رنگ‌ها (با ویرگول)</span><input name="colors" defaultValue={(Array.isArray(p.colors) ? p.colors.map((c) => (typeof c === 'string' ? c : c?.name || '')).filter(Boolean) : []).join('، ')} className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm" /></label>
              <label className="text-xs text-primary-500 text-[#13ABC4] space-y-1 sm:col-span-2"><span>سایزها (با ویرگول)</span><input name="sizes" defaultValue={(Array.isArray(p.sizes) ? p.sizes : []).join('، ')} className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm" /></label>
              <label className="text-xs text-primary-500 text-[#13ABC4] space-y-1 sm:col-span-2"><span>توضیحات</span><textarea name="description" defaultValue={p.description || ''} rows={4} className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm" /></label>
             </div>
             <div className="flex flex-wrap gap-2 pt-2">
              ...
              {p.status === 'purge_requested' ? (
               <>
                <button type="button" onClick={() => { siteConfirm('درخواست فروشنده تأیید شود و محصول برای همیشه از پایگاه داده حذف شود؟ برگشت‌پذیر نیست.').then((ok) => { if (!ok) return; if (typeof adminPurgeProduct === 'function') adminPurgeProduct(p.id); setAdminProductDetailId(null); }); }} className="text-xs px-3 py-2 rounded-full border border-red-500 bg-transparent hover:bg-primary-50 hover:bg-white/10 text-primary-900 text-white font-medium">تأیید حذف دائم</button>
         ...
         {!adminLoading && adminTab === 'orders' && !adminOrderDetailId && (
          <div>
           <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
            <h2 className="text-base font-bold text-primary-900 text-white">مدیریت سفارش‌ها</h2>
            <input value={adminOrderSearch} onChange={e=>setAdminOrderSearch(e.target.value)} placeholder="شماره سفارش یا موبایل…" className="px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm w-full sm:w-56 focus:outline-none focus:border-apple-blue" />
           </div>
           <div className="admin-tabs-strip flex gap-1 overflow-x-auto mb-4">
            {[{id:'all',l:'همه'},{id:'pending',l:'در انتظار پرداخت'},{id:'preparing',l:'آماده‌سازی'},{id:'shipped',l:'ارسال‌شده'},{id:'delivered',l:'تحویل‌شده'},{id:'cancelled',l:'لغو'},{id:'returned',l:'مرجوع'}].map(f=>(
             <button key={f.id} type="button" onClick={()=>setAdminOrderFilter(f.id)} className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium border ${adminOrderFilter===f.id?'panel-filter-chip--on':'plp-filter-chip border-primary-300 border-white/50 !text-primary-900 !text-white bg-white bg-[#2A2C30] font-medium'}`}>{f.l}</button>
            ))}
           </div>
           <div className="space-y-2">
            {(adminOrders||[]).filter(o=>{
             ...
             <button key={o.id} type="button" onClick={()=>{setAdminOrderDetailId(o.id);setAdminOrderNote(o.adminNote||'');}} className="w-full p-3 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 text-right flex items-center justify-between gap-3">
              <div className="min-w-0">
               <p className="text-sm font-medium text-primary-900 text-white">{o.id}</p>
               <p className="text-xs text-primary-500">{o.buyerName} · {o.date}</p>
               <p className="text-xs text-primary-600 text-white/70 mt-0.5 truncate">فروشنده: {o.sellerName || o.seller?.name || o.items?.[0]?.sellerName || o.items?.[0]?.seller || '—'}</p>
              </div>
            ...
            {!(adminOrders||[]).length && <p className="text-center text-sm text-primary-400 text-[#13ABC4] py-10">سفارشی نیست</p>}
           </div>
          </div>
         )}
         {!adminLoading && adminTab === 'orders' && adminOrderDetailId && (()=>{
          ...
          return (
           <div>
            ...
            <div className="space-y-4 text-sm">
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 space-y-2 bg-white bg-primary-900">
              ...
              <div className="mt-2 pt-2 border-t border-primary-100 border-white/10 space-y-1">
               <p className="text-xs font-bold text-primary-800 text-white">فروشنده / فروشگاه</p>
             ...
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <p className="text-xs font-bold mb-2 text-primary-900 text-white">کالاها</p>
              {(o.items||[]).map((it,i)=>(
               <div key={i} className="flex justify-between py-2 border-b border-primary-100 border-white/10 text-xs last:border-0">
             ...
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <p className="text-xs font-bold mb-2 text-primary-900 text-white">تایم‌لاین</p>
              <ul className="space-y-1 text-xs text-primary-600 text-white/70">
             ...
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-2">
              <label className="text-xs text-primary-500 text-[#13ABC4] block mb-1">تغییر وضعیت</label>
              ...
              }} className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm">
               {['pending','preparing','shipped','delivered','cancelled','returned'].map(s=><option key={s} value={s}>{adminStatusLabel(s)}</option>)}
              </select>
             </div>
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <label className="text-xs text-primary-500 text-[#13ABC4] block mb-1">یادداشت ادمین</label>
              ...
              <div className="mt-3 flex flex-wrap gap-1">
               {['paid','processing','shipped','delivered','cancelled'].map((st) => (
                <button
                 key={st}
                 type="button"
                 className="text-[10px] px-2 py-1 rounded-full border border-primary-200 border-white/20"
                 onClick={async () => {
         ...
         {!adminLoading && adminTab === 'coupons' && (
          <div className="space-y-4">
           <div className="flex items-center justify-between gap-2 flex-wrap">
            <h2 className="text-base font-bold text-primary-900 text-white">کدهای تخفیف</h2>
            <button
             type="button"
           ...
           <div className="space-y-2">
            ...
            {(adminCoupons || []).map((c) => (
             <div key={c.id || c.code} className="flex items-center justify-between gap-2 p-3 rounded-xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <div className="text-right">
               <p className="text-sm font-bold font-latin text-primary-900 text-white">{c.code}</p>
               <p className="text-xs text-primary-500">
         ...
         {!adminLoading && adminTab === 'audit' && (
          <div className="space-y-3" key="audit-panel">
           <div className="flex items-center justify-between">
            <h2 className="text-base font-bold text-primary-900 text-white">لاگ اقدامات ادمین</h2>
            <button
             type="button"
              ...
              try {
               ...
               if (listEl) {
                listEl.innerHTML = items.map((row) => {
                 ...
                 return '<div class="p-3 rounded-xl border border-primary-200 border-white/15 bg-white bg-primary-900 text-right space-y-1">'
                  + '<p class="text-sm font-bold text-primary-900 text-white">' + (row.action || '') + '</p>'
         ...
         {!adminLoading && adminTab === 'tickets' && !adminTicketDetailId && (
          <div>
           <h2 className="text-base font-bold text-primary-900 text-white mb-4">تیکت‌های پشتیبانی</h2>
           <div className="admin-tabs-strip flex gap-1 overflow-x-auto mb-4">
            {[{id:'all',l:'همه'},{id:'open',l:'باز'},{id:'pending',l:'در انتظار'},{id:'closed',l:'بسته'}].map(f=>(
             <button key={f.id} type="button" onClick={()=>setAdminTicketFilter(f.id)} className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium border ${adminTicketFilter===f.id?'panel-filter-chip--on':'plp-filter-chip border-primary-300 border-white/50 !text-primary-900 !text-white bg-white bg-[#2A2C30] font-medium'}`}>{f.l}</button>
            ))}
           </div>
           <div className="space-y-2">
            {(adminTickets||[]).filter(t=>adminTicketFilter==='all'||t.status===adminTicketFilter).map(t=>(
             ...
             }} className="w-full p-3 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 text-right flex items-center justify-between gap-3">
              <div className="min-w-0">
               <p className="text-sm font-medium text-primary-900 text-white flex items-center gap-2 flex-wrap">{t.unread&&<span className="w-2 h-2 rounded-full bg-transparent hover:bg-primary-50 hover:bg-white/10 border border-primary-200 border-white/25 flex-shrink-0"/>}{t.subject}
                <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${(t.type==='seller'||t.from==='seller')?'bg-violet-100 text-violet-700 bg-violet-900/40 text-violet-300':'bg-sky-100 text-sky-700 bg-sky-900/40 text-sky-300'}`}>{(t.type==='seller'||t.from==='seller')?'فروشنده':'خریدار'}</span>
                <span className="text-xs px-1.5 py-0.5 rounded-full font-bold bg-primary-100 text-primary-700 bg-primary-800 text-white/80">{conversationChannelLabel(t)}</span>
               </p>
            ...
            {!(adminTickets||[]).length && <p className="text-center text-sm text-primary-400 text-[#13ABC4] py-10">تیکتی نیست</p>}
           </div>
          </div>
         )}
         {!adminLoading && adminTab === 'tickets' && adminTicketDetailId && (()=>{
          ...
          return (
           <div>
            ...
            <p className="text-xs text-primary-500 text-[#13ABC4] mb-1 font-latin" dir="ltr">کد تیکت: {t.code || t.id}</p>
            <p className="text-xs text-primary-500 text-[#13ABC4] mb-4">{t.fromName} · {t.date} · <span className={`px-2 py-0.5 rounded-full ${adminStatusBadge(t.status)}`}>{adminStatusLabel(t.status)}</span></p>
            <div className="space-y-3 mb-4">
             {(t.messages||[]).map((m,i)=>(
              <div key={i} className={`p-3 rounded-xl text-sm ${m.from==='admin'?'bg-apple-blue/10 border border-apple-blue/20':'bg-primary-50 bg-primary-900 border border-primary-100 border-white/10'}`}>
               <p className="text-xs font-medium mb-1">{m.from==='admin'?'ادمین':m.from==='seller'?'فروشنده':m.from==='buyer'?'خریدار':'سیستم'} · {m.date}</p>
               <p className="text-primary-800 text-white/90">{stripLinksForDisplay(m.text)}</p>
              </div>
             ))}
            </div>
            {t.status!=='closed' && (
         ...
         {!adminLoading && adminTab === 'buyers' && !adminBuyerDetailId && (
          <div>
           <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
            <h2 className="text-base font-bold text-primary-900 text-white">مدیریت خریداران</h2>
            <input value={adminBuyerSearch} onChange={e=>setAdminBuyerSearch(e.target.value)} placeholder="جستجو نام یا موبایل…" className="px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm w-full sm:w-56" />
           </div>
           <div className="space-y-2">
            {(adminBuyers||[]).filter(b=>{
             ...
             <div key={b.id} className="p-3 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-3 min-w-0 overflow-hidden">
              <button type="button" onClick={()=>setAdminBuyerDetailId(b.id)} className="text-right flex-1 min-w-0 overflow-hidden">
               <p className="text-sm font-medium text-primary-900 text-white truncate">{b.name}</p>
               <p className="text-xs text-primary-500 text-[#13ABC4] truncate" dir="ltr">{b.phone} · {toFa(b.ordersCount||0)} سفارش · {b.joinDate}</p>
              </button>
            ...
            {!(adminBuyers||[]).length && <p className="text-center text-sm text-primary-400 text-[#13ABC4] py-10">خریداری نیست</p>}
           </div>
          </div>
         )}
         {!adminLoading && adminTab === 'buyers' && adminBuyerDetailId && (()=>{
          ...
          return (
           <div>
            ...
            <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 mb-4 space-y-2">
             <h2 className="text-base font-bold text-primary-900 text-white">{b.name}</h2>
             <p className="text-sm text-primary-500" dir="ltr">{b.phone}</p>
             <p className="text-xs text-primary-600 text-white/70">عضویت: {b.joinDate} · سفارش‌ها: {toFa(b.ordersCount||0)} · وضعیت: {b.status==='blocked'||b.status==='suspended'?'مسدود':'فعال'}</p>
            ...
            <h3 className="text-sm font-bold mb-2 text-primary-900 text-white">سفارش‌های مرتبط</h3>
            <div className="space-y-2">
             {(buyerOrders.length?buyerOrders:(adminOrders||[]).slice(0,3)).map(o=>(
              <div key={o.id} className="p-3 rounded-xl bg-white bg-primary-900 border border-primary-200 border-white/15 text-xs flex justify-between shadow-sm">
...
{/* Categories */}
           <div>
            <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
             <div>
              <h2 className="text-base font-bold text-primary-900 text-white">دسته‌بندی محصولات</h2>
              <p className="text-xs text-primary-500 text-[#13ABC4] !text-white mt-0.5">ایندکس‌پذیر · URL قابل ویرایش · تصویر و توضیح · در فرم فروشنده (حداکثر ۳)</p>
             </div>
             <div className="flex gap-2">
              <button type="button" onClick={() => openTaxonomyHub('categories')} className="text-xs px-3 py-1.5 rounded-full border plp-filter-chip border-primary-300 border-white/50 !text-primary-900 text-[#13ABC4] !text-white bg-white bg-[#2A2C30] font-medium">مشاهده همه</button>
              <button
               type="button"
            ...
            <div className="flex flex-wrap gap-1.5 mb-1">
              {[{ id: 'all', l: 'همه' }, { id: 'active', l: 'فعال' }, { id: 'archived', l: 'آرشیو شده‌ها' }].map((f) => (
                <button key={f.id} type="button" onClick={() => setTaxFilter(f.id)}
                  className={`text-xs px-3 py-1.5 rounded-full border font-medium ${taxFilter === f.id ? 'bg-primary-900 text-white border-primary-900 bg-white text-primary-900' : 'border-primary-200 border-white/20 bg-white bg-primary-900'}`}>{f.l}</button>
              ))}
            </div>
            {filterTaxCategories(adminCategories).length > 0 && (
              <label className="flex items-center gap-2 px-1 py-1 text-xs text-primary-600 text-white/70 cursor-pointer select-none">
                <input type="checkbox"
                  checked={filterTaxCategories(adminCategories).every((x) => taxSelectedIds.includes(String(x.id)))}
                  onChange={() => {
                    const ids = filterTaxCategories(adminCategories).map((x) => String(x.id));
                    const all = ids.every((id) => taxSelectedIds.includes(id));
                    setTaxSelectedIds(all ? [] : ids);
                  }}
                  className="rounded border-primary-300"
                />
                انتخاب همه در این فهرست ({filterTaxCategories(adminCategories).length})
              </label>
            )}
            {taxSelectedIds.length > 0 && (
              <div className="flex flex-wrap items-center gap-2 p-2.5 rounded-xl bg-red-50 bg-red-950/30 border border-red-200 border-red-800/40">
                <span className="text-xs font-medium text-red-800 text-red-200">{taxSelectedIds.length} مورد انتخاب شده</span>
                ...
                {taxFilter === 'archived' ? (
                  <>
                    ...
                    <button type="button" disabled={taxBusy} onClick={() => runTaxBulkCategory('purge')} className="text-xs px-3 py-1.5 rounded-full bg-red-600 text-white font-medium disabled:opacity-50">حذف دائم گروهی</button>
                  </>
                ) : (
                  <button type="button" disabled={taxBusy} onClick={() => runTaxBulkCategory('archive')} className="text-xs px-3 py-1.5 rounded-full bg-red-600 text-white font-medium disabled:opacity-50">آرشیو گروهی</button>
                )}
                <button type="button" onClick={() => setTaxSelectedIds([])} className="text-xs px-3 py-1.5 rounded-full border border-primary-300 border-white/30">لغو انتخاب</button>
              </div>
            )}
            {filterTaxCategories(adminCategories).map((c) => (
              <div key={c.id} className={`flex flex-wrap items-center gap-3 p-3 rounded-xl border bg-white bg-primary-900 ${taxSelectedIds.includes(String(c.id)) ? 'border-apple-blue ring-1 ring-apple-blue/30' : 'border-primary-200 border-white/15'}`}>
                ...
                {c.image ? <img src={c.image} alt="" className="w-14 h-14 rounded-lg object-cover flex-shrink-0"  loading="lazy" decoding="async" /> : <div className="w-14 h-14 rounded-lg bg-primary-100 bg-primary-800 flex-shrink-0" />}
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-primary-900 text-white">{c.name}</p>
                  <p className="text-xs text-primary-500 text-[#13ABC4] !text-white mt-0.5 font-latin" dir="ltr">{c.url || `/${c.slug || c.name}`}</p>
                  {c.description && <p className="text-xs text-primary-400 text-[#13ABC4] !text-white line-clamp-1 mt-0.5">{c.description}</p>}
                </div>
                ...
                }} className={`text-xs px-2 py-0.5 rounded-full border font-medium cursor-pointer ${isTaxArchived(c) ? 'bg-red-100 text-red-700 border-red-200 bg-red-900/40 text-red-300' : 'bg-emerald-100 text-emerald-700 border-emerald-200 bg-emerald-900/40 text-emerald-300'}`}>{isTaxArchived(c) ? 'غیرفعال' : 'فعال'}</button>
                <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 bg-emerald-900/40 text-emerald-300">index</span>
                <button type="button" onClick={() => openCategory(c.name)} className="text-xs px-2 py-1 rounded-full border border-primary-200 border-white/20 text-primary-600 text-white">صفحه</button>
                <button type="button" onClick={() => openTaxonomyWizard('category', c)} className="p-1.5 rounded-full hover:bg-primary-50 hover:bg-primary-800 text-primary-500"><Icon name="pencil" size={14} /></button>
                {isTaxArchived(c) ? (
                  <>
                    <button type="button" onClick={() => {
                      siteConfirm('این دسته بازگردانی شود؟').then((ok) => {
                        if (!ok || typeof saveAdminCategories !== 'function') return;
                        saveAdminCategories((adminCategories || []).map((x) => x.id === c.id ? { ...x, status: 'active', active: true } : x));
                      });
                    }} className="text-xs px-2 py-1 rounded-full border border-emerald-300 text-emerald-700">بازگردانی</button>
                    <button type="button" onClick={() => {
                      siteConfirm('حذف دائم این دسته؟ برگشت‌پذیر نیست.').then((ok) => {
                        if (!ok || typeof saveAdminCategories !== 'function') return;
                        saveAdminCategories((adminCategories || []).filter((x) => x.id !== c.id));
                      });
                    }} className="p-1.5 rounded-full hover:bg-red-50 text-red-500"><Icon name="trash" size={14} /></button>
                  </>
                ) : (
                  <button type="button" onClick={() => {
                    siteConfirm('این دسته به آرشیو منتقل شود؟').then((ok) => {
                      if (!ok || typeof saveAdminCategories !== 'function') return;
                      saveAdminCategories((adminCategories || []).map((x) => x.id === c.id ? { ...x, status: 'archived', active: false } : x));
                    });
                  }} className="text-xs px-2 py-1 rounded-full border border-red-300 text-red-600">آرشیو</button>
                )}
              </div>
            ))}
            {!filterTaxCategories(adminCategories).length && <p className="text-sm text-primary-400 text-[#13ABC4] text-center py-8">{taxFilter === 'archived' ? 'آرشیو خالی است' : 'دسته‌ای ثبت نشده'}</p>}
          </div>
           </div>
...
{/* Tags */}
           <div>
            <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
             <div>
              <h2 className="text-base font-bold text-primary-900 text-white">برچسب محصولات</h2>
              <p className="text-xs text-primary-500 text-[#13ABC4] !text-white mt-0.5">صفحه مستقل · ممنوع ایندکس · تصویر و توضیح · در فرم فروشنده (حداکثر ۳)</p>
             </div>
             <div className="flex gap-2">
              <button type="button" onClick={() => openTaxonomyHub('tags')} className="text-xs px-3 py-1.5 rounded-full border plp-filter-chip border-primary-300 border-white/50 !text-primary-900 text-[#13ABC4] !text-white bg-white bg-[#2A2C30] font-medium">مشاهده همه</button>
              <button
               type="button"
            ...
            {filterTaxTags(adminTags).length > 0 && (
              <label className="flex items-center gap-2 px-1 py-1 text-xs text-primary-600 text-white/70 cursor-pointer select-none">
                <input type="checkbox"
                  checked={filterTaxTags(adminTags).every((x) => taxSelectedIds.includes(String(x.id)))}
                  onChange={() => {
                    const ids = filterTaxTags(adminTags).map((x) => String(x.id));
                    const all = ids.every((id) => taxSelectedIds.includes(id));
                    setTaxSelectedIds(all ? [] : ids);
                  }}
                  className="rounded border-primary-300"
                />
                انتخاب همه در این فهرست ({filterTaxTags(adminTags).length})
              </label>
            )}
            {taxSelectedIds.length > 0 && (
              <div className="flex flex-wrap items-center gap-2 p-2.5 rounded-xl bg-red-50 bg-red-950/30 border border-red-200 border-red-800/40">
                ...
                {taxFilter === 'archived' ? (
                  <>
                    ...
                    <button type="button" disabled={taxBusy} onClick={() => runTaxBulkTag('purge')} className="text-xs px-3 py-1.5 rounded-full bg-red-600 text-white font-medium disabled:opacity-50">حذف دائم گروهی</button>
                  </>
                ) : (
                  <button type="button" disabled={taxBusy} onClick={() => runTaxBulkTag('archive')} className="text-xs px-3 py-1.5 rounded-full bg-red-600 text-white font-medium disabled:opacity-50">آرشیو گروهی</button>
                )}
                <button type="button" onClick={() => setTaxSelectedIds([])} className="text-xs px-3 py-1.5 rounded-full border border-primary-300 border-white/30">لغو انتخاب</button>
              </div>
            )}
            {filterTaxTags(adminTags).map((tg) => (
              <div key={tg.id} className={`flex flex-wrap items-center gap-3 p-3 rounded-xl border bg-white bg-primary-900 ${taxSelectedIds.includes(String(tg.id)) ? 'border-apple-blue ring-1 ring-apple-blue/30' : 'border-primary-200 border-white/15'}`}>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-primary-900 text-white">{tg.name}</p>
                  {(tg.slug || tg.url) && <p className="text-xs text-primary-500 text-[#13ABC4] !text-white mt-0.5 font-latin" dir="ltr">{tg.url || tg.slug}</p>}
                </div>
                ...
                }} className={`text-xs px-2 py-0.5 rounded-full border font-medium cursor-pointer ${isTaxArchived(tg) ? 'bg-red-100 text-red-700 border-red-200 bg-red-900/40 text-red-300' : 'bg-emerald-100 text-emerald-700 border-emerald-200 bg-emerald-900/40 text-emerald-300'}`}>{isTaxArchived(tg) ? 'غیرفعال' : 'فعال'}</button>
                <button type="button" onClick={() => openTaxonomyWizard('tag', tg)} className="p-1.5 rounded-full hover:bg-primary-50 hover:bg-primary-800 text-primary-500"><Icon name="pencil" size={14} /></button>
                {isTaxArchived(tg) ? (
                  <>
                    <button type="button" onClick={() => {
                      siteConfirm('این برچسب بازگردانی شود؟').then((ok) => {
                        if (!ok || typeof saveAdminTags !== 'function') return;
                        saveAdminTags((adminTags || []).map((x) => x.id === tg.id ? { ...x, status: 'active', active: true } : x));
                      });
                    }} className="text-xs px-2 py-1 rounded-full border border-emerald-300 text-emerald-700">بازگردانی</button>
                    <button type="button" onClick={() => {
                      siteConfirm('حذف دائم این برچسب؟ برگشت‌پذیر نیست.').then((ok) => {
                        if (!ok || typeof saveAdminTags !== 'function') return;
                        saveAdminTags((adminTags || []).filter((x) => x.id !== tg.id));
                      });
                    }} className="p-1.5 rounded-full hover:bg-red-50 text-red-500"><Icon name="trash" size={14} /></button>
                  </>
                ) : (
                  <button type="button" onClick={() => {
                    siteConfirm('این برچسب به آرشیو منتقل شود؟').then((ok) => {
                      if (!ok || typeof saveAdminTags !== 'function') return;
                      saveAdminTags((adminTags || []).map((x) => x.id === tg.id ? { ...x, status: 'archived', active: false } : x));
                    });
                  }} className="text-xs px-2 py-1 rounded-full border border-red-300 text-red-600">آرشیو</button>
                )}
              </div>
            ))}
            {!filterTaxTags(adminTags).length && <p className="text-sm text-primary-400 text-[#13ABC4] text-center py-8">{taxFilter === 'archived' ? 'آرشیو خالی است' : 'برچسبی ثبت نشده'}</p>}
          </div>
           </div>
           </div>
         )}
...
{/* Brands catalog */}
           <div>
            <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
             <div>
              <h2 className="text-base font-bold text-primary-900 text-white">برندها</h2>
             ...
             <button type="button" onClick={() => { closeAdminPanel(); openStaticPage('brands'); }} className="text-xs px-3 py-1.5 rounded-full border plp-filter-chip border-primary-300 border-white/50 !text-primary-900 text-[#13ABC4] !text-white bg-white bg-[#2A2C30] font-medium font-medium">مشاهده همه</button>
             </div>
            </div>
            <div className="space-y-2">
            {(<>
            ...
            {filterTaxBrands(adminCatalogBrands).map((b) => (
              <div key={b.id} className={`flex flex-wrap items-center gap-2 p-3 rounded-xl border bg-white bg-primary-900 ${taxSelectedIds.includes(String(b.id)) ? 'border-apple-blue ring-1 ring-apple-blue/30' : 'border-primary-200 border-white/15'}`}>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-primary-900 text-white">{b.name}</p>
                  {b.seoTitle && <p className="text-xs text-primary-500 text-white/70 truncate">{b.seoTitle}</p>}
                </div>
                ...
                }} className={`text-xs px-2 py-0.5 rounded-full border font-medium cursor-pointer ${isTaxArchived(b) ? 'bg-red-100 text-red-700 border-red-200 bg-red-900/40 text-red-300' : 'bg-emerald-100 text-emerald-700 border-emerald-200 bg-emerald-900/40 text-emerald-300'}`}>{isTaxArchived(b) ? 'غیرفعال' : 'فعال'}</button>
                <button type="button" onClick={() => openTaxonomyWizard('brand', b)} className="p-1.5 rounded-full hover:bg-primary-50 hover:bg-primary-800 text-primary-500"><Icon name="pencil" size={14} /></button>
                {isTaxArchived(b) ? (
                  <>
                    <button type="button" onClick={() => {
            ...
            {!filterTaxBrands(adminCatalogBrands).length && <p className="text-sm text-primary-400 text-[#13ABC4] text-center py-8">{taxFilter === 'archived' ? 'آرشیو خالی است' : 'برندی ثبت نشده'}</p>}
          </>)}
          </div>
           </div>
         ...
         {!adminLoading && adminTab === 'pages' && (
          <div className="space-y-4 max-w-none w-full">
           <div>
            <h2 className="text-base font-bold text-primary-900 text-white">برگه‌ها</h2>
            <p className="text-xs text-primary-500 text-[#13ABC4] mt-1">محتوا با ویرایشگر متن · سوالات متداول · سئو هر برگه</p>
           </div>
           {(() => {
            ...
            return (
             <div className="space-y-3">
              <div className="rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 divide-y divide-primary-50 divide-white/5 overflow-visible">
               {sitePages.map(item => {
                const saved = pageSeoMap[item.key] || getPageCms(item.cmsKey);
                const isOn = adminSeoHubKey === item.key;
                return (
                 <div key={item.key} className={`flex items-center gap-2 px-3 py-2.5 ${isOn ? 'bg-primary-50 bg-primary-800' : ''}`}>
                  <button type="button" onClick={() => { setAdminSeoHubKey(isOn ? null : item.key); if (!isOn) setAdminPageSeoStep(1); }} className="flex-1 text-right flex items-center justify-between gap-2 min-w-0">
                   <span className="text-sm font-medium text-primary-900 text-white truncate min-w-0 flex-1">{item.label}</span>
                   <span className="text-[10px] text-primary-400 text-[#13ABC4] flex-shrink-0">{saved ? 'دارای محتوا/سئو' : 'پیش‌فرض'}</span>
                  </button>
                  <button type="button" onClick={item.open} className="text-[10px] px-2 py-1 rounded-full border border-primary-200 border-white/20 text-primary-600 text-[#13ABC4] footer-link text-white/80 whitespace-nowrap">مشاهده</button>
                 </div>
                );
               })}
              </div>
              {active && (
               <div className="space-y-4">
                <div className="flex items-center justify-between gap-2">
                 <h3 className="text-sm font-bold text-primary-900 text-white">{active.label}</h3>
                ...
                {(active.hasBody || active.cmsKey === 'shop') && (
                 <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
                  <p className="text-xs font-bold text-primary-900 text-white">{active.bodyLabel || 'محتوای صفحه (ویرایشگر متن)'}</p>
                  <SimpleEditor
                  ...
                  <div className="grid sm:grid-cols-2 gap-2">
                   <div className="space-y-2">
                    <label className="block text-xs text-primary-500 text-white/60">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <input
                        ...
                        className="flex-1 min-w-[12rem] px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left"
                      />
                      <label className="inline-flex items-center gap-1.5 text-xs px-3 py-2 rounded-xl border border-primary-200 border-white/25 bg-primary-50 bg-primary-800 text-primary-800 text-white cursor-pointer">
                        آپلود تصویر
                        <input
                          type="file"
                    ...
                    {cms.image ? (
                      <div className="w-full max-w-xs h-28 rounded-2xl overflow-hidden border border-primary-200 border-white/15 relative">
                   ...
                   <input dir="ltr" value={cms.video || ''} onChange={(e) => updatePageCms(active.cmsKey, { video: e.target.value })} placeholder="آدرس ویدیو / آپارات (اختیاری)" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left" />
                  </div>
                ...
                {active.hasSiteFaqs && (
                 <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
                  <div className="flex items-center justify-between gap-2">
                   <p className="text-xs font-bold text-primary-900 text-white">سوالات متداول صفحه</p>
                   <button type="button" className="text-xs text-apple-blue" onClick={() => saveSiteFaqs([...(siteFaqs || []), { cat: 'عمومی', q: '', a: '' }])}>+ افزودن سوال</button>
                  </div>
                  {(siteFaqs || []).map((f, i) => (
                   <div key={i} className="p-3 rounded-xl border border-primary-100 border-white/10 space-y-2">
                    <div className="grid sm:grid-cols-2 gap-2">
                     <input value={f.cat || ''} onChange={(e) => { const next = [...siteFaqs]; next[i] = { ...next[i], cat: e.target.value }; saveSiteFaqs(next); }} placeholder="دسته (مثلاً ارسال)" className="w-full px-2 py-1.5 rounded-lg border border-primary-200 border-white/15 bg-transparent text-xs" />
                    ...
                    <input value={f.q || ''} onChange={(e) => { const next = [...siteFaqs]; next[i] = { ...next[i], q: e.target.value }; saveSiteFaqs(next); }} placeholder="سؤال" className="w-full px-2 py-1.5 rounded-lg border border-primary-200 border-white/15 bg-transparent text-xs font-medium" />
                    <SimpleEditor
                     key={`site-faq-${i}`}
                     mode="admin"
                     value={f.a || ''}
                ...
                {active.hasPageFaqs && (
                 <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
                  <div className="flex items-center justify-between gap-2">
                   <p className="text-xs font-bold text-primary-900 text-white">سوالات متداول این صفحه</p>
                  ...
                  {pageFaqs.map((f, i) => (
                   <div key={i} className="p-3 rounded-xl border border-primary-100 border-white/10 space-y-2">
                    <div className="flex gap-2 items-center">
                     <input value={f.cat || ''} onChange={(e) => { const next = [...pageFaqs]; next[i] = { ...next[i], cat: e.target.value }; updatePageCms(active.cmsKey, { faqs: next }); }} placeholder="دسته" className="flex-1 px-2 py-1.5 rounded-lg border border-primary-200 border-white/15 bg-transparent text-xs" />
                    ...
                    <input value={f.q || ''} onChange={(e) => { const next = [...pageFaqs]; next[i] = { ...next[i], q: e.target.value }; updatePageCms(active.cmsKey, { faqs: next }); }} placeholder="سؤال" className="w-full px-2 py-1.5 rounded-lg border border-primary-200 border-white/15 bg-transparent text-xs font-medium" />
                    <SimpleEditor
                ...
                {(() => {
                 ...
                 return (
                  <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-4">
                   <div className="flex items-center justify-between gap-2 flex-wrap">
                    <h3 className="text-sm font-bold text-primary-900 text-white">سئوی «{active.label}»</h3>
                   ...
                   <div className="flex flex-wrap gap-1">
                    {PAGE_SEO_STEPS.map((s) => (
                     <button
                      ...
                      className={`panel-step text-[10px] px-2.5 py-1 rounded-full border transition ${step === s.n ? 'panel-step--active bg-apple-blue text-white border-apple-blue shadow-sm' : 'border-primary-200 border-white/20 text-primary-600 text-white/70 hover:border-primary-400'}`}
                     >{toFa(s.n)}</button>
                    ))}
                   </div>
                   <p className="text-xs font-semibold text-primary-700 text-white/90">{current.label}</p>
                   {renderContentSeoBox({
                    title: f.title,
                   ...
                   <div className="flex items-center justify-between gap-2 pt-1" dir="rtl">
                    <button
                     ...
                     className={`text-xs px-4 py-2 rounded-full border ${step <= 1 ? 'opacity-40 border-primary-100 text-primary-300' : 'border-primary-200 border-white/25 text-primary-800 text-white'}`}
                    >مرحله قبل</button>
                    {step < 8 ? (
                     <button
                      type="button"
...
{/* Colors catalog */}
           <div>
            <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
             <div>
              <h2 className="text-base font-bold text-primary-900 text-white">رنگ‌ها</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
             {(adminCatalogColors || []).map(col => (
              <div key={col.id} className="flex items-center gap-2 p-3 rounded-xl border border-primary-200 border-white/15 bg-white bg-primary-900">
               <span className="color-swatch w-8 h-8 rounded-full border border-primary-200 border-white/70 flex-shrink-0" style={{ ["--swatch-color"]: col.hex || "#888", backgroundColor: col.hex || "#888" }} />
               <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-primary-900 text-white">{col.name}</p>
           ...
           <div>
            <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
             <div>
              <h2 className="text-base font-bold text-primary-900 text-white">سایزها</h2>
            ...
            <div className="flex flex-wrap gap-2">
             {(adminCatalogSizes || []).map(sz => (
              <div key={sz.id} className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-xl border ${sz.active === false ? 'opacity-50 border-primary-200' : 'border-primary-200 border-white/15'} bg-white bg-primary-900`}>
               <span className="latin-label text-sm font-bold text-primary-900 text-white" dir="ltr" lang="en">{sz.name}</span>
               <button type="button" onClick={() => saveAdminCatalogSizes((adminCatalogSizes || []).map(x => x.id === sz.id ? { ...x, active: x.active === false } : x))} className="text-xs text-primary-500 text-[#13ABC4] hover:text-apple-blue">{sz.active === false ? 'فعال‌سازی' : 'غیرفعال'}</button>
           ...
           <div>
            <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
             <div>
              <h2 className="text-base font-bold text-primary-900 text-white">ویژگی‌های محصول</h2>
            ...
            <div className="space-y-2">
             {(adminCatalogAttributes || []).map(attr => (
              <div key={attr.id} className="p-3 rounded-xl border border-primary-200 border-white/15 bg-white bg-primary-900">
               <div className="flex flex-wrap items-center gap-2 mb-2">
                <p className="flex-1 text-sm font-bold text-primary-900 text-white">{attr.name}</p>
               ...
               <div className="flex flex-wrap gap-1">
                {(attr.options || []).map(opt => (
                 <span key={opt} className="text-xs px-2 py-0.5 rounded-full bg-primary-50 bg-primary-900 text-primary-700 text-white/80 border border-primary-100 border-white/10">{opt}</span>
                ))}
               </div>
              </div>
             ))}
         ...
         {!adminLoading && adminTab === 'blog-categories' && (
          ...
          <div className="space-y-4">
           <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
             <h2 className="text-base font-bold text-primary-900 text-white">دسته‌بندی مجله</h2>
             <p className="text-xs text-primary-500 text-[#13ABC4] !text-white mt-0.5">دسته‌های مجله — URL دستی یا خودکار از نام</p>
            </div>
            <button
             type="button"
           ...
           <div className="space-y-2">
            ...
            {filterTaxBlogCategories(adminBlogCategories).map((c) => (
              <div key={c.id} className={`flex flex-wrap items-center gap-2 p-3 rounded-xl border bg-white bg-primary-900 ${taxSelectedIds.includes(String(c.id)) ? 'border-apple-blue ring-1 ring-apple-blue/30' : 'border-primary-200 border-white/15'}`}>
                <input type="checkbox" checked={taxSelectedIds.includes(String(c.id))} onChange={() => toggleTaxSelect(c.id)} className="rounded border-primary-300 flex-shrink-0" aria-label="انتخاب" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-primary-900 text-white">{c.name}</p>
                  {c.slug ? <p className="text-[10px] text-primary-400 text-[#13ABC4] font-latin mt-0.5" dir="ltr">/مجله?cat={c.slug}</p> : null}
                </div>
                <button type="button" title={isTaxArchived(c) ? 'فعال‌سازی' : 'غیرفعال‌سازی'} onClick={() => {
                  if (typeof saveAdminBlogCategories !== 'function') return;
                  const nextOn = isTaxArchived(c);
...
<div className="space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-3">
             <div>
              <h2 className="text-base font-bold text-primary-900 text-white">برچسب مقالات مجله</h2>
              <p className="text-xs text-primary-500 text-[#13ABC4] mt-0.5">صفحه هر برچسب هرگز ایندکس نمی‌شود (noindex دائمی)</p>
             </div>
            ...
            {filterTaxBlogTags(adminBlogTags).map((tg) => (
              <div key={tg.id} className={`flex flex-wrap items-center gap-2 p-3 rounded-xl border bg-white bg-primary-900 ${taxSelectedIds.includes(String(tg.id)) ? 'border-apple-blue ring-1 ring-apple-blue/30' : 'border-primary-200 border-white/15'}`}>
                <input type="checkbox" checked={taxSelectedIds.includes(String(tg.id))} onChange={() => toggleTaxSelect(tg.id)} className="rounded border-primary-300 flex-shrink-0" aria-label="انتخاب" />
                <p className="flex-1 text-sm font-medium text-primary-900 text-white">{tg.name}</p>
                <button type="button" title={isTaxArchived(tg) ? 'فعال‌سازی' : 'غیرفعال‌سازی'} onClick={() => {
         ...
         {!adminLoading && adminTab === 'shipping' && (
          <div className="space-y-6">
           <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
             <h2 className="text-base font-bold text-primary-900 text-white">روش‌های ارسال</h2>
             <p className="text-xs text-primary-500 text-[#13ABC4] mt-0.5">روش‌های ارسال فعال برای فروشنده و خریدار</p>
            </div>
            <button
             type="button"
           ...
           <div className="space-y-3">
            {(adminShippingMethods || []).length === 0 && (
             <p className="text-sm text-primary-500 text-[#13ABC4] p-4 rounded-2xl bg-white bg-primary-900 border border-dashed border-primary-200 border-white/15 text-center">
              هنوز روش ارسالی تعریف نشده. با دکمه «افزودن روش ارسال» شروع کنید.
             </p>
            )}
            {(adminShippingMethods || []).map((m) => (
             <div key={m.id} className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
               <div>
                <p className="text-sm font-bold text-primary-900 text-white">{m.name}</p>
               ...
               <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={() => { setShippingMethodForm({ ...m, isNew: false }); setShippingMethodFormOpen(true); }} className="p-1.5 rounded-full hover:bg-primary-50 hover:bg-primary-800 text-primary-500" aria-label="ویرایش"><Icon name="pencil" size={14} /></button>
                <button type="button" onClick={() => {
                ...
                }} className="p-1.5 rounded-full hover:bg-red-50 hover:bg-red-900/20 text-red-500" aria-label="حذف"><Icon name="trash" size={14} /></button>
               </div>
              </div>
              <div className="grid sm:grid-cols-2 gap-2 text-xs">
               <div className="p-2 rounded-lg bg-primary-50 bg-primary-900/50">
                <span className="text-primary-400">API:</span> {m.apiEnabled ? 'متصل' : 'خاموش'} · {m.apiProvider || m.id}
               </div>
               <div className="p-2 rounded-lg bg-primary-50 bg-primary-900/50 font-latin text-left" dir="ltr">
                {m.apiEndpoint || 'endpoint خالی'}
               </div>
              </div>
              {m.note && <p className="text-xs text-primary-500 text-[#13ABC4] !text-white leading-relaxed">{m.note}</p>}
             </div>
            ))}
           </div>
           {shippingMethodFormOpen && shippingMethodForm && typeof document !== 'undefined' && createPortal(
            <div className="site-modal-root" role="dialog" aria-modal="true" aria-label="فرم روش ارسال">
             ...
             <div className="site-modal-panel bg-white bg-primary-900 p-5 border border-primary-200 border-white/15">
              <h3 className="text-base font-bold text-primary-900 text-white mb-4">{shippingMethodForm.isNew || !(adminShippingMethods || []).some((x) => x.id === shippingMethodForm.id) ? 'روش ارسال جدید' : (`ویرایش ${shippingMethodForm.name || ''}`)}</h3>
              <div className="space-y-3">
               <div>
                <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">نام نمایشی</label>
                <input value={shippingMethodForm.name||''} onChange={e => setShippingMethodForm(f => ({...f, name: e.target.value}))} className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white" />
               </div>
               <div>
                <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">نوع قیمت</label>
                <select value={shippingMethodForm.priceMode||'fixed'} onChange={e => setShippingMethodForm(f => ({...f, priceMode: e.target.value}))} className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white">
                 <option value="fixed">ثابت (در ثبت سفارش)</option>
                 <option value="dynamic_cod">داینامیک / پرداخت در مقصد</option>
                </select>
               </div>
               <div>
                <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">مبلغ پایه (تومان)</label>
                <input type="number" value={shippingMethodForm.baseCost||0} onChange={e => setShippingMethodForm(f => ({...f, baseCost: Number(e.target.value)||0}))} dir="ltr" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left text-primary-900 text-white" />
               </div>
               <div>
                <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">زمان تقریبی</label>
                <input value={shippingMethodForm.eta||''} onChange={e => setShippingMethodForm(f => ({...f, eta: e.target.value}))} className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white" />
               </div>
               <div>
                <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">کلید API</label>
                <input value={shippingMethodForm.apiKey||''} onChange={e => setShippingMethodForm(f => ({...f, apiKey: e.target.value}))} dir="ltr" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin text-primary-900 text-white" placeholder="API Key" />
               </div>
               <div>
                <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">Endpoint API</label>
                <input value={shippingMethodForm.apiEndpoint||''} onChange={e => setShippingMethodForm(f => ({...f, apiEndpoint: e.target.value}))} dir="ltr" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin text-primary-900 text-white" placeholder="https://api.example.com/..." />
               </div>
               <label className="flex items-center gap-2 text-xs text-primary-700 text-white cursor-pointer">
               ...
               <div>
                <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">توضیح برای خریدار</label>
              ...
              <div className="flex gap-2 mt-5">
               ...
               <button type="button" onClick={() => setShippingMethodFormOpen(false)} className="px-5 py-2.5 rounded-full border border-primary-200 border-white/30 text-sm">لغو</button>
              </div>
             </div>
            </div>,
            document.body
               ...
               return (
                <>
                 <div className="flex items-start justify-between gap-2 mb-3">
                  <div>
                   <h3 className="text-base font-bold text-primary-900 text-white">
                    {taxonomyForm.id ? 'ویرایش' : 'افزودن'} {label}
                   </h3>
                   <p className="text-[10px] text-primary-400 text-[#13ABC4] mt-0.5">مرحله {toFa(step)} از {toFa(12)}</p>
                  </div>
                  <button type="button" onClick={() => setTaxonomyFormOpen(false)} className="text-xs text-primary-500">بستن</button>
                 </div>
                 <div className="flex flex-wrap gap-1 mb-3">
                  {stepsMeta.map(s => (
                   <button key={s.n} type="button" onClick={() => setStep(s.n)} className={`text-[10px] w-6 h-6 rounded-full border ${step === s.n ? 'bg-apple-blue text-white border-apple-blue' : 'border-primary-200 border-white/25 text-primary-500'}`}>{toFa(s.n)}</button>
                  ))}
                 </div>
                 <p className="text-xs font-semibold text-primary-800 text-white mb-3">{stepsMeta[step - 1].title}</p>
                 ...
                 {step === 1 && (
                  <div>
                   <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">نام</label>
                   <input
                    ...
                    className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white focus:outline-none focus:border-apple-blue"
                 ...
                 {step === 2 && (
                  <div>
                   <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">نامک (Slug) — قابل ویرایش</label>
                   <input
                    ...
                    dir="ltr"
                    className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin text-primary-900 text-white focus:outline-none focus:border-apple-blue"
                   />
                   <p className="text-[10px] text-primary-400 text-[#13ABC4] mt-1 font-latin" dir="ltr">{taxonomyForm.url || `${prefix}${taxonomyForm.slug}`}</p>
                   <p className="text-[10px] text-primary-500 text-[#13ABC4] mt-1">اختیاری: خالی بماند از روی نام ساخته می‌شود. می‌توانید دستی تغییر دهید.</p>
                  </div>
                 )}
                 ...
                 {step === 3 && (
                  <div className="space-y-3">
                   <div className="flex flex-wrap gap-2">
                    {images.map((src, i) => (
                     <div key={i} className={`relative w-24 rounded-xl border p-1 ${taxonomyForm.featuredImageIndex === i ? 'border-apple-blue ring-1 ring-apple-blue/40' : 'border-primary-200 border-white/20'}`}>
                      ...
                      <input
                       ...
                       placeholder="تگ آلت"
                       className="mt-1 w-full px-1.5 py-1 rounded-md border border-primary-100 border-white/10 bg-transparent text-[10px] text-primary-900 text-white"
                      />
                      <div className="flex gap-1 mt-1">
                       <button type="button" className="flex-1 text-[9px] py-0.5 rounded bg-primary-100 bg-primary-800 text-primary-700 text-[#13ABC4] footer-link text-white" onClick={() => setTaxonomyForm(f => ({ ...f, featuredImageIndex: i, image: images[i] }))}>شاخص</button>
                   ...
                   <label className="inline-flex items-center gap-2 text-xs px-3 py-2 rounded-full border border-dashed border-primary-300 border-white/30 cursor-pointer text-primary-700 text-white">
                   ...
                   <input
                    ...
                    className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-xs text-left"
                   />
                  </div>
                 )}
                 ...
                 <div className="flex flex-wrap items-center justify-between gap-2 mt-5 pt-3 border-t border-primary-100 border-white/10" dir="rtl">
                  <button type="button" disabled={step <= 1} onClick={() => setStep(step - 1)} className={`text-xs px-4 py-2 rounded-full border ${step <= 1 ? 'opacity-40 border-primary-100' : 'border-primary-200 border-white/25'} text-primary-800 text-white`}>مرحله قبل</button>
                  <div className="flex gap-2">
                   {canPublish && (
                 ...
                 {step >= 4 && step < 12 && (
                  <p className="text-[10px] text-primary-400 text-[#13ABC4] mt-2">از مرحله ۴ می‌توانید ذخیره کنید · مراحل ۵–۱۲ برای سئو اختیاری‌اند</p>
                 )}
                </>
               );
              })()}
...
{false && !adminLoading && adminTab === 'content' && (
          <div className="space-y-6">
           <div className="flex gap-2 mb-2">
            <button type="button" onClick={() => setAdminContentTab('moderation')} className={`text-xs px-3 py-1.5 rounded-full border bg-white bg-primary-900 ${adminContentTab==='moderation'?'!bg-apple-blue text-white border-apple-blue':'plp-filter-chip border-primary-300 border-white/50 !text-primary-900 !text-white bg-white bg-[#2A2C30] font-medium'}`}>صف تأیید ({toFa((adminModerationQueue||[]).filter(x=>x.status==='pending').length)})</button>
            <button type="button" onClick={() => setAdminContentTab('pages')} className={`text-xs px-3 py-1.5 rounded-full border bg-white bg-primary-900 ${adminContentTab==='pages'?'!bg-apple-blue text-white border-apple-blue':'plp-filter-chip border-primary-300 border-white/50 !text-primary-900 !text-white bg-white bg-[#2A2C30] font-medium'}`}>ویرایش محتوای صفحات</button>
           </div>

           {adminContentTab === 'moderation' && (
            <div className="space-y-3">
             <h2 className="text-base font-bold text-primary-900 text-white">تأیید محتوای فروشنده</h2>
             ...
             {(adminModerationQueue||[]).filter(x=>x.status==='pending').length === 0 && (
              <p className="text-sm text-primary-400 text-[#13ABC4] text-center py-10 border border-dashed border-primary-200 border-white/20 rounded-2xl bg-white bg-primary-900">مورد معلقی نیست</p>
             )}
             {(adminModerationQueue||[]).filter(x=>x.status==='pending').map(item => (
              <div key={item.id} className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 flex flex-wrap gap-3 items-start">
               {item.preview && <img src={item.preview} alt="" className="w-16 h-20 object-cover rounded-lg"  loading="lazy" decoding="async" />}
               <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-primary-900 text-white">{item.productName || item.type}</p>
                <p className="text-xs text-primary-500 text-[#13ABC4] mt-0.5">{item.type === 'product_image' ? 'تصویر محصول' : item.type === 'product' ? 'محصول / متن' : item.type === 'seller_location' ? 'لوکیشن فروشنده' : item.type === 'seller_kyc' ? 'احراز هویت فروشنده' : item.type === 'seller_sheba' ? 'شبا فروشنده' : item.type === 'seller_payout' ? 'درخواست مالی' : item.type === 'seller_discount' ? 'تخفیف فروشنده' : item.type}</p>
                {item.text && <p className="text-xs text-primary-600 text-white/70 mt-1 line-clamp-3">{item.text}</p>}
                {item.type === 'seller_discount' && Array.isArray(item.items) && (
                 <ul className="mt-2 space-y-1">
                  {item.items.map((it, i) => (
                   <li key={i} className="text-xs text-primary-700 text-white/80">• {it.name}: {toFa(Number(it.oldPrice).toLocaleString())} → {toFa(Number(it.salePrice).toLocaleString())} ({toFa(it.percent)}٪)</li>
                  ))}
                 </ul>
                )}
                {item.aparatEmbed && (
                 <div className="mt-2 space-y-1">
                  ...
                  <p className="text-xs font-latin text-left text-primary-400 text-[#13ABC4] break-all" dir="ltr">{item.aparatEmbed}</p>
                  <div className="aspect-video max-w-sm rounded-xl overflow-visible border border-primary-200 border-white/15 bg-black">
               ...
               <div className="flex gap-2">
                ...
                }} className="px-3 py-1.5 rounded-full bg-transparent hover:bg-primary-50 hover:bg-white/10 border border-primary-200 border-white/25 text-primary-900 text-white text-xs">رد</button>
               </div>
              </div>
             ))}
            </div>
           ...
           {adminContentTab === 'pages' && (
            <div className="space-y-4">
             <h2 className="text-base font-bold text-primary-900 text-white">ویرایش محتوای سایت</h2>
             <div className="grid sm:grid-cols-2 gap-2">
              {[
               ...
               <button key={x.type} type="button" onClick={() => setContentEditorTarget({ type: x.type })} className={`text-right p-3 rounded-xl border text-sm bg-white bg-primary-900 ${contentEditorTarget?.type===x.type?'border-apple-blue ring-1 ring-apple-blue/30':'border-primary-200 border-white/15'} text-primary-900 text-white`}>{x.label}</button>
              ))}
             </div>
             ...
             {contentEditorTarget?.type === 'categories' && (
              <div className="space-y-2">
               ...
               {(adminCategories||[]).map(c => (
                <div key={c.id} className="flex items-center gap-2 p-3 rounded-xl bg-white bg-primary-900 border border-primary-200 border-white/15">
                 {c.image && <img src={c.image} alt="" className="w-12 h-12 rounded object-cover"  loading="lazy" decoding="async" />}
                 <div className="flex-1 min-w-0"><p className="text-sm font-bold text-primary-900 text-white">{c.name}</p><p className="text-xs text-primary-400 text-[#13ABC4] line-clamp-1">{c.description}</p></div>
                 <button type="button" onClick={() => { setAdminTab('product-categories'); openTaxonomyWizard('category', c); }} className="text-xs px-2 py-1 rounded-full border border-primary-200 border-white/30">ویرایش</button>
                 <button type="button" onClick={() => openCategory(c.name)} className="text-xs px-2 py-1 rounded-full bg-primary-100 bg-primary-800">مشاهده صفحه</button>
                </div>
               ))}
              </div>
             )}
             {contentEditorTarget?.type === 'tags' && (
              <div className="space-y-2">
               {(adminTags||[]).map(tg => (
                <div key={tg.id} className="flex items-center gap-2 p-3 rounded-xl bg-white bg-primary-900 border border-primary-200 border-white/15">
                 {tg.image && <img src={tg.image} alt="" className="w-12 h-12 rounded object-cover"  loading="lazy" decoding="async" />}
                 <div className="flex-1 min-w-0"><p className="text-sm font-bold text-primary-900 text-white">{tg.name}</p><p className="text-xs text-primary-400 text-[#13ABC4] line-clamp-1">{tg.description}</p></div>
                 <button type="button" onClick={() => { setAdminTab('product-tags'); openTaxonomyWizard('tag', tg); }} className="text-xs px-2 py-1 rounded-full border border-primary-200 border-white/30">ویرایش</button>
                 <button type="button" onClick={() => openTagPage(tg.name)} className="text-xs px-2 py-1 rounded-full bg-primary-100 bg-primary-800">مشاهده صفحه</button>
                </div>
               ))}
              </div>
             )}
             {contentEditorTarget?.type === 'blogs' && (
              <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 text-sm text-primary-600 text-white/70 space-y-2">
             ...
             {['home','about','contact','faq','terms','privacy','returns','size-guide','become-seller'].includes(contentEditorTarget?.type) && (
              <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
               <h3 className="text-sm font-bold text-primary-900 text-white">محتوای صفحه: {contentEditorTarget.type}</h3>
               <SimpleEditor
               ...
               />
               <input dir="ltr" value={(getPageCms(contentEditorTarget.type) || {}).image || ''} onChange={(e) => updatePageCms(contentEditorTarget.type, { image: e.target.value })} placeholder="آدرس تصویر (اختیاری)" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left" />
               <input dir="ltr" value={(getPageCms(contentEditorTarget.type) || {}).video || ''} onChange={(e) => updatePageCms(contentEditorTarget.type, { video: e.target.value })} placeholder="آدرس ویدیو / آپارات (اختیاری)" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left" />
             ...
             {(contentEditorTarget?.type === 'blog_cats' || contentEditorTarget?.type === 'blog_tags') && (
              <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 text-sm text-primary-600 text-white/70">
...
{!adminLoading && adminTab === 'blog-new' && (() => {
          ...
          <div className="space-y-4">
           <div className="flex flex-wrap items-center justify-between gap-2">
            <div>
             <h2 className="text-base font-bold text-primary-900 text-white">افزودن مطلب مجله</h2>
             <p className="text-xs text-primary-500 text-[#13ABC4] !text-white mt-0.5">عنوان، دسته، برچسب، خلاصه و متن — ظاهر حرفه‌ای، منطق بدون تغییر</p>
            </div>
            <button type="button" onClick={() => { setBlogForm(null); setAdminTab('blog'); }} className="text-xs px-3 py-1.5 rounded-full border border-primary-200 border-white/30 text-primary-700 text-white">مشاهده مطالب</button>
           </div>
           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-4 p-5 sm:p-6 shadow-sm">
             <div>
              <label className="text-[11px] font-medium text-primary-600 text-white/70 mb-1.5 block">عنوان مطلب <span className="text-red-500">*</span></label>
             <input value={bf.title} onChange={e => setBlogForm(prev => ({ ...(prev && !prev.id ? prev : emptyBlog()), title: e.target.value, id: '' }))} placeholder="عنوان مطلب را بنویسید…" className="w-full px-3.5 py-2.5 rounded-xl border border-primary-200/80 border-white/15 bg-white bg-primary-950 text-sm text-primary-900 text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-400/30 focus:border-primary-400 transition" />
             </div>
             <div>
              <label className="text-[11px] font-medium text-primary-600 text-white/70 mb-1.5 block">آدرس URL</label>
              <input
               value={bf.slug || ''}
               ...
               className="w-full px-3.5 py-2.5 rounded-xl border border-primary-200/80 border-white/15 bg-white bg-primary-950 text-sm text-primary-900 text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-400/30 focus:border-primary-400 transition font-latin"
               dir="ltr"
              />
              <p className="text-[10px] text-primary-400 text-[#13ABC4] mt-1.5 font-latin" dir="ltr">/مجله/{(bf.slug || 'از-عنوان').toString()}</p>
             </div>
             <div className="grid sm:grid-cols-2 gap-4">
              <div>
               <label className="text-[11px] font-medium text-primary-600 text-white/70 mb-1.5 block">دسته‌بندی <span className="text-red-500">*</span></label>
               <div className="relative">
                <select
                 value={bf.cat || ''}
                 onChange={e => setBlogForm(prev => ({ ...(prev && !prev.id ? prev : emptyBlog()), cat: e.target.value, id: '' }))}
                 className="w-full appearance-none px-3.5 py-2.5 pe-10 rounded-xl border border-primary-200/80 border-white/15 bg-primary-50/40 bg-primary-950 text-sm text-primary-900 text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-400/30 focus:border-primary-400 transition cursor-pointer"
                >
                ...
                <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-primary-400 text-white/50">
              ...
              <div>
               <label className="text-[11px] font-medium text-primary-600 text-white/70 mb-1.5 block">وضعیت انتشار</label>
               <div className="relative">
                <select
                 value={bf.status}
             ...
             <div>
              <div className="flex items-center justify-between gap-2 mb-1.5">
               <label className="text-[11px] font-medium text-primary-600 text-white/70">برچسب‌ها</label>
               <span className="text-[10px] text-primary-400 text-white/40">ایندکس نمی‌شوند</span>
              </div>
              <div className="flex flex-wrap gap-2 p-3 min-h-[3rem] rounded-xl border border-primary-200/80 border-white/15 bg-primary-50/30 bg-primary-950/80">
               {(adminBlogTags || []).filter(t => t.active !== false).map(tg => {
                ...
                return (
                 <button
                  ...
                  className={`text-[11px] px-3 py-1.5 rounded-full border transition ${selected ? 'panel-filter-chip--on bg-primary-200 text-primary-900 border-primary-200 shadow-sm' : 'border-primary-200 border-white/20 text-primary-700 text-white/80 hover:border-primary-400 bg-white bg-primary-900'}`}
                 >{tg.name}</button>
                );
               })}
               {!(adminBlogTags || []).some(t => t.active !== false) && (
                <span className="text-[11px] text-primary-400 text-[#13ABC4] self-center">ابتدا از منوی «برچسب مقالات مجله» برچسب بسازید</span>
               )}
              </div>
             </div>
             ...
             {bf.status === 'scheduled' && (
              <div className="grid sm:grid-cols-2 gap-3 p-3 rounded-xl border border-primary-200 border-white/15 bg-primary-50/50 bg-primary-900/30">
               <div>
                <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">تاریخ انتشار</label>
                ...
                }} className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-white bg-primary-900 text-sm text-primary-900 text-white" />
               </div>
               <div>
                <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">ساعت انتشار</label>
             ...
             <div>
              <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">برچسب‌ها (ایندکس نمی‌شوند)</label>
              <div className="flex flex-wrap gap-2 p-2 rounded-xl border border-primary-200 border-white/20">
               {(adminBlogTags || []).filter(t => t.active !== false).map(tg => {
                const selected = (bf.tags || []).includes(tg.name);
                return (
                 <button
                  ...
                  className={`text-[11px] px-2.5 py-1 rounded-full border transition ${selected ? 'panel-filter-chip--on bg-primary-200 text-primary-900' : 'border-primary-200 border-white/20 text-primary-700 text-white'}`}
                 >{tg.name}</button>
                );
               })}
               {!(adminBlogTags || []).some(t => t.active !== false) && (
             ...
             <input value={bf.excerpt} onChange={e => setBlogForm(prev => ({ ...(prev && !prev.id ? prev : emptyBlog()), excerpt: e.target.value, id: '' }))} placeholder="یک خلاصه کوتاه برای نمایش در کارت‌ها…" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white" />
         ...
         {!adminLoading && adminTab === 'blog' && (
          <div className="space-y-4">
           <div className="flex flex-wrap items-center justify-between gap-2">
            <div>
             <h2 className="text-base font-bold text-primary-900 text-white">مطالب مجله</h2>
             <p className="text-xs text-primary-500 text-[#13ABC4] !text-white mt-0.5">لیست مطالب — انتخاب برای ویرایش</p>
            </div>
            <button type="button" onClick={() => {
             const defCat = ((adminBlogCategories || []).find(c => c.active !== false) || {}).name || 'راهنمای خرید';
           ...
           {blogForm && blogForm.id && (
            <div className="p-4 rounded-2xl border border-apple-blue/40 border-[#4CCD99]/40 bg-white bg-primary-900 space-y-2">
             <div className="flex items-center justify-between gap-2 mb-1">
              <p className="text-sm font-bold text-primary-900 text-white">ویرایش مطلب</p>
              <button type="button" onClick={() => setBlogForm(null)} className="text-xs px-2.5 py-1 rounded-full border border-primary-200 border-white/25">بستن</button>
             </div>
             <div>
              <label className="text-[11px] font-medium text-primary-600 text-white/70 mb-1.5 block">عنوان مطلب <span className="text-red-500">*</span></label>
              <input value={blogForm.title} onChange={e => setBlogForm(f => ({ ...f, title: e.target.value }))} placeholder="عنوان مطلب را بنویسید…" className="w-full px-3.5 py-2.5 rounded-xl border border-primary-200/80 border-white/15 bg-white bg-primary-950 text-sm text-primary-900 text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-400/30 focus:border-primary-400 transition" />
             </div>
             <div>
              <input
               value={blogForm.slug || ''}
               ...
               dir="ltr"
              />
              <p className="text-[10px] text-primary-400 text-[#13ABC4] mt-1.5 font-latin" dir="ltr">/مجله/{(blogForm.slug || 'از-عنوان').toString()}</p>
             </div>
             <div className="grid sm:grid-cols-2 gap-4">
              <div>
               <label className="text-[11px] font-medium text-primary-600 text-white/70 mb-1.5 block">دسته‌بندی <span className="text-red-500">*</span></label>
             ...
             <input value={blogForm.excerpt} onChange={e => setBlogForm(f => ({ ...f, excerpt: e.target.value }))} placeholder="خلاصه" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white" />
           ...
           <div className="space-y-2">
            {!(blogPosts || []).length && (
             <p className="text-sm text-primary-500 text-[#13ABC4] text-center py-10">هنوز مطلبی ثبت نشده · از «افزودن بلاگ» مطلب جدید بنویسید</p>
            )}
            {(<>
            <div className="flex flex-wrap gap-1.5 mb-1">
              {[{ id: 'all', l: 'همه' }, { id: 'active', l: 'فعال' }, { id: 'archived', l: 'آرشیو شده‌ها' }].map((f) => (
            ...
            {filterTaxBlogPosts(blogPosts).map((p) => (
              <div
                key={p.id}
                className={`flex flex-wrap items-center gap-2 p-3 rounded-xl border bg-white bg-primary-900 transition ${taxSelectedIds.includes(String(p.id)) ? 'border-apple-blue ring-1 ring-apple-blue/30' : (blogForm && blogForm.id === p.id ? 'border-apple-blue border-[#4CCD99] ring-1 ring-apple-blue/30' : 'border-primary-200 border-white/15')}`}
              >
                ...
                <button type="button" onClick={() => setBlogForm({ ...p })} className="flex-1 min-w-0 text-right">
                  <p className="text-sm font-bold text-primary-900 text-white truncate">{p.title || 'بدون عنوان'}</p>
                  <p className="text-xs text-primary-500">
                ...
                <button type="button" onClick={() => setBlogForm({ ...p })} className="text-xs px-2.5 py-1 rounded-full border border-primary-200 border-white/20 text-primary-800 text-white">ویرایش</button>
                {isTaxArchived(p) ? (
                  <>
            ...
            {!filterTaxBlogPosts(blogPosts).length && (
              <p className="text-sm text-primary-500 text-[#13ABC4] text-center py-10">{taxFilter === 'archived' ? 'آرشیو خالی است' : 'هنوز مطلبی ثبت نشده · از «افزودن بلاگ» مطلب جدید بنویسید'}</p>
            )}
          </>)}
           </div>
          </div>
         ...
         {!adminLoading && adminTab === 'campaigns' && (
          <div className="space-y-4">
           <div className="flex flex-wrap items-center justify-between gap-2">
            <h2 className="text-base font-bold text-primary-900 text-white">کمپین‌ها</h2>
           ...
           {campaignForm && (
            <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-2">
             <input value={campaignForm.title} onChange={e => setCampaignForm(f => ({ ...f, title: e.target.value }))} placeholder="عنوان کمپین *" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white" />
             <input value={campaignForm.desc} onChange={e => setCampaignForm(f => ({ ...f, desc: e.target.value }))} placeholder="توضیح کوتاه" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white" />
             <input value={campaignForm.rules || ''} onChange={e => setCampaignForm(f => ({ ...f, rules: e.target.value }))} placeholder="قوانین کمپین" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white" />
             <div className="flex flex-wrap gap-3 items-center text-sm">
              <label className="flex items-center gap-1.5 text-xs text-primary-700 text-white"><input type="checkbox" checked={!!campaignForm.active} onChange={e => setCampaignForm(f => ({ ...f, active: e.target.checked }))} /> فعال</label>
              <label className="text-xs text-primary-600 text-white/70">مدت (روز): <input type="number" min={1} value={campaignForm.days || 7} onChange={e => setCampaignForm(f => ({ ...f, days: Number(e.target.value) || 7 }))} className="w-16 mx-1 px-2 py-1 rounded border border-primary-200 border-white/20 bg-transparent" /></label>
             </div>
             <div className="flex gap-2">
              <button type="button" onClick={() => {
           ...
           <div className="space-y-2">
            {campaignsList.map(c => (
             <div key={c.id} className="flex flex-wrap items-center gap-2 p-3 rounded-xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <div className="flex-1 min-w-0">
               <p className="text-sm font-bold text-primary-900 text-white">{c.title} {c.status === 'pending' ? <span className="text-xs text-amber-600">در انتظار تأیید</span> : c.active ? <span className="text-xs text-emerald-600">فعال</span> : <span className="text-xs text-primary-400">غیرفعال</span>}{c.sellerName ? <span className="text-xs text-primary-400"> · {c.sellerName}</span> : null}</p>
               <p className="text-xs text-primary-500 text-[#13ABC4] truncate">{c.desc}</p>
              </div>
              {c.status === 'pending' && (
              ...
              <button type="button" onClick={() => saveCampaigns(campaignsList.map(x => x.id === c.id ? { ...x, active: !x.active } : x))} className="text-xs px-2.5 py-1 rounded-full border border-primary-200 border-white/20">{c.active ? 'غیرفعال' : 'فعال'} کردن</button>
         ...
         {/* SEO & Indexing */}
          ...
          <div className="space-y-4">
           <h2 className="text-lg font-bold text-primary-900 text-white">نظرات مشتریان واقعی</h2>
          ...
          <div className="space-y-6 max-w-none w-full">
           <div>
            <h2 className="text-base font-bold text-primary-900 text-white">سئو و ابزارهای فنی</h2>
            <p className="text-xs text-primary-500 text-[#13ABC4] mt-1">سئوی هر صفحه جداگانه · اسکیما، سایت‌مپ، Search Console، GA و GTM</p>
           </div>

           {/* Page SEO hub — جایگزین ایندکس سراسری و تگ‌های عمومی */}
           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-4 overflow-hidden min-w-0">
            <div className="min-w-0">
             <h3 className="text-sm font-bold text-primary-900 text-white">سئوی صفحات سایت</h3>
             <p className="text-xs text-primary-500 text-[#13ABC4] mt-1 leading-6 break-words max-w-full">برای هر صفحه: تحلیل، کلمات کلیدی، Title، Meta، FAQ، پیش‌نمایش گوگل/شبکه، سپس ایندکس و کنونیکال. سئوی محصول در پنل فروشنده است.</p>
            </div>
            {(() => {
             const base = (seoCfg().canonicalBase || 'https://pirahanemardane.ir').replace(/\/$/, '');
             const staticPages = [
             ...
             return (
              <div className="space-y-3">
               <div className="seo-page-list max-h-72 overflow-y-auto rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 divide-y divide-primary-100 divide-white/10">
                {all.map(item => {
               ...
               {active && (() => {
                ...
                return (
                 <div className="space-y-3 pt-1">
                  <p className="text-xs text-primary-500">در حال ویرایش: <span className="font-bold text-primary-900 text-white">{active.label}</span></p>
                  {renderContentSeoBox({
               ...
               <p className="text-xs text-primary-400 text-[#13ABC4] leading-6 break-words max-w-full">سئوی محصول از پنل فروشنده · سئوی دسته/برند هنگام ساخت همان آیتم نیز در دسترس است · ایندکس سراسری حذف شد؛ ایندکس هر صفحه اینجاست.</p>
              </div>
             );
            })()}
           </div>
           ...
           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
            <h3 className="text-sm font-bold text-primary-900 text-white">آدرس پایه سایت (Canonical Base)</h3>
            <p className="text-xs text-primary-500">برای ساخت URL کامل صفحات و سایت‌مپ</p>
            <input defaultValue={seoCfg().canonicalBase} id="seo-canon" dir="ltr" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left text-primary-900 text-white" />
            <button type="button" onClick={() => {
           ...
           {/* Schema */}
           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
            <h3 className="text-sm font-bold text-primary-900 text-white">اتصال به موتورهای جستجو</h3>
            ...
            {[
             ...
             <div key={key}>
              <label className="text-xs text-primary-500 text-[#13ABC4] block mb-1">{label}</label>
              <input autoComplete="one-time-code" inputMode="numeric" id={`seo-v-${key}`} defaultValue={seoCfg()[key] || ''} dir="ltr" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left text-primary-900 text-white" placeholder="کد verification" />
             </div>
            ))}
            <div>
             <label className="text-xs text-primary-500 text-[#13ABC4] block mb-1">متاهای سفارشی (هر خط: name=content)</label>
            ...
            <div className="flex flex-wrap gap-2 pt-2">
             <a href="https://search.google.com/search-console" target="_blank" rel="noreferrer" className="text-xs px-3 py-1.5 rounded-full border border-primary-200 text-primary-700 text-[#13ABC4] footer-link text-white">Google Search Console</a>
             <a href="https://www.bing.com/webmasters" target="_blank" rel="noreferrer" className="text-xs px-3 py-1.5 rounded-full border border-primary-200 text-primary-700 text-[#13ABC4] footer-link text-white">Bing Webmaster</a>
             <a href="https://webmaster.yandex.com" target="_blank" rel="noreferrer" className="text-xs px-3 py-1.5 rounded-full border border-primary-200 text-primary-700 text-[#13ABC4] footer-link text-white">Yandex</a>
            </div>
           </div>

           {/* Sitemap & robots */}
           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
            <h3 className="text-sm font-bold text-primary-900 text-white">سایت‌مپ و robots.txt</h3>
            ...
            <div className="flex flex-wrap gap-2">
             ...
             <button type="button" onClick={() => downloadSeoFile('sitemap-index.xml', buildSitemapIndexXml(), 'application/xml;charset=utf-8')} className="text-xs px-3 py-2 rounded-full border border-primary-200 border-white/20 font-medium">دانلود sitemap-index.xml</button>
             <button type="button" onClick={exportRedirectsForServer} className="text-xs px-3 py-2 rounded-full border border-primary-200 border-white/20 font-medium">خروجی ریدایرکت سروری (JSON)</button>
             <button type="button" onClick={() => { const h = runSeoHealthCheck(); if (h.ok) showToast({ message: 'بررسی سلامت سئو: موردی یافت نشد', variant: 'success', duration: 4000, position: 'top-center' }); else showToast({ message: h.issues[0], variant: 'error', duration: 5000, position: 'top-center' }); }} className="text-xs px-3 py-2 rounded-full border border-primary-200 border-white/20 font-medium">بررسی سلامت سئو (P0)</button>
             <button type="button" onClick={() => downloadSeoFile('robots.txt', buildRobotsTxt(), 'text/plain;charset=utf-8')} className="text-xs px-3 py-2 rounded-full border border-primary-200 border-white/20 text-primary-700 text-white font-medium flex items-center gap-1"><Icon name="download" size={14} /> دانلود robots.txt</button>
             <button type="button" onClick={() => downloadSeoFile('llms.txt', buildLlmsTxt(), 'text/plain;charset=utf-8')} className="text-xs px-3 py-2 rounded-full border border-primary-200 border-white/20 text-primary-700 text-white font-medium flex items-center gap-1"><Icon name="download" size={14} /> دانلود llms.txt</button>
            </div>
            <div>
             <label className="text-xs text-primary-500 text-[#13ABC4] block mb-1">خطوط اضافه robots.txt</label>
             ...
             <label className="text-xs text-primary-500 text-[#13ABC4] block mt-3 mb-1">متن اضافی llms.txt</label>
           ...
           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-2">
            <h3 className="text-sm font-bold text-primary-900 text-white">ریدایرکت دستی</h3>
           ...
           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
            <div className="flex items-center justify-between gap-2">
             <h3 className="text-sm font-bold text-primary-900 text-white">مانیتور ۴۰۴</h3>
             <button type="button" onClick={() => saveSeo404Log([])} className="text-xs text-primary-500 text-[#13ABC4] hover:text-red-500">پاک کردن لاگ</button>
            </div>
            ...
            <div className="space-y-1 max-h-48 overflow-y-auto">
             ...
             {(seo404Log || []).map(e => (
              <div key={e.id} className="text-xs py-1.5 border-b border-primary-50 border-white/5 flex flex-wrap gap-x-3 gap-y-0.5">
               <span className="font-latin text-primary-900 text-white" dir="ltr">{e.path}</span>
               <span className="text-primary-400">{e.atFa}</span>
               {e.referrer ? <span className="text-primary-400 text-[#13ABC4] truncate max-w-[12rem]" dir="ltr">از: {e.referrer}</span> : null}
           ...
           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
            <h3 className="text-sm font-bold text-primary-900 text-white">Image SEO</h3>
            <label className="flex items-center gap-2 text-xs text-primary-800 text-white cursor-pointer">
             <input type="checkbox" defaultChecked={seoCfg().imageSeoAutoAlt !== false} id="seo-img-auto-alt" />
             تولید خودکار ALT از قالب
            </label>
            <div>
             <label className="text-xs text-primary-500 text-[#13ABC4] block mb-1">قالب ALT (متغیرها: {'{name}'} {'{brand}'} {'{category}'} {'{keyword}'})</label>
             <input id="seo-img-alt-tpl" defaultValue={seoCfg().imageSeoAltTemplate || '{name} | {brand} | پیراهن مردانه'} className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white" />
            </div>
            <button type="button" onClick={() => {
             saveSeoPatch({
           ...
           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
            <h3 className="text-sm font-bold text-primary-900 text-white">IndexNow</h3>
            ...
            <input id="seo-indexnow-key" defaultValue={seoCfg().indexNowKey || ''} dir="ltr" placeholder="API Key" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin" />
            <div className="flex flex-wrap gap-2">
             <button type="button" onClick={() => {
              saveSeoPatch({
             ...
             <button type="button" onClick={() => pingIndexNow(['/'])} className="text-xs px-3 py-2 rounded-full border border-primary-200 border-white/20">پینگ صفحه اصلی</button>
             <button type="button" onClick={() => {
              const urls = (products || []).slice(0, 20).map(pr => `/product/${pr.slug || pr.id}`);
              pingIndexNow(urls);
             }} className="text-xs px-3 py-2 rounded-full border border-primary-200 border-white/20">پینگ ۲۰ محصول</button>
            </div>
           </div>
           ...
           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
            <h3 className="text-sm font-bold text-primary-900 text-white">Google Search Console (API)</h3>
            ...
            <div className="flex flex-wrap gap-2">
             ...
             }} className="text-xs px-3 py-2 rounded-full border border-primary-200 border-white/20">وضعیت اتصال</button>
             <button type="button" onClick={async () => {
              try {
             ...
             <a href="https://search.google.com/search-console" target="_blank" rel="noreferrer" className="text-xs px-3 py-2 rounded-full border border-primary-200 border-white/20">باز کردن GSC</a>
            </div>
           </div>

<div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
            <div className="flex items-center justify-between gap-2">
             <h3 className="text-sm font-bold text-primary-900 text-white">Custom Schema Builder</h3>
            ...
            {(Array.isArray(seoCfg().customSchemas) ? seoCfg().customSchemas : []).map((cs, i) => (
             <div key={cs.id || i} className="p-3 rounded-xl bg-white bg-primary-900 border border-primary-100 border-white/10 space-y-2">
              <div className="flex flex-wrap gap-2 items-center">
               <input defaultValue={cs.name || ''} onBlur={e => {
                const list = [...(seoCfg().customSchemas || [])];
                list[i] = { ...list[i], name: e.target.value };
                saveSeoPatch({ customSchemas: list });
               }} className="flex-1 min-w-[8rem] px-2 py-1.5 rounded-lg border border-primary-200 border-white/15 bg-transparent text-xs" placeholder="نام" />
               <select defaultValue={cs.when || 'all'} onChange={e => {
                const list = [...(seoCfg().customSchemas || [])];
                list[i] = { ...list[i], when: e.target.value };
                saveSeoPatch({ customSchemas: list });
               }} className="px-2 py-1.5 rounded-lg border border-primary-200 border-white/15 bg-white bg-primary-900 text-xs">
              ...
              <textarea defaultValue={typeof cs.json === 'string' ? cs.json : JSON.stringify(cs.json || {}, null, 2)} onBlur={e => {
               ...
               saveSeoPatch({ customSchemas: list });
              }} rows={5} dir="ltr" className="w-full px-2 py-1.5 rounded-lg border border-primary-200 border-white/15 bg-transparent text-xs font-latin text-left" />
             </div>
            ))}
           ...
           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
            <div className="flex items-center justify-between gap-2 flex-wrap">
             <h3 className="text-sm font-bold text-primary-900 text-white">Local SEO</h3>
            ...
            <div className="grid sm:grid-cols-2 gap-2">
             <input defaultValue={seoCfg().localBusinessName || ''} onBlur={e => saveSeoPatch({ localBusinessName: e.target.value })} placeholder="نام کسب‌وکار" className="px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm" />
             <input defaultValue={seoCfg().localPhone || ''} onBlur={e => saveSeoPatch({ localPhone: e.target.value })} dir="ltr" placeholder="تلفن" className="px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin" />
             <input defaultValue={seoCfg().localEmail || ''} onBlur={e => saveSeoPatch({ localEmail: e.target.value })} dir="ltr" placeholder="ایمیل" className="px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin" />
             <input defaultValue={seoCfg().localPriceRange || ''} onBlur={e => saveSeoPatch({ localPriceRange: e.target.value })} placeholder="بازه قیمت (مثلاً $$)" className="px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm" />
            </div>
            <div className="flex items-center justify-between">
             <p className="text-xs font-bold text-primary-700 text-white">لوکیشن‌ها</p>
             <button type="button" className="text-xs text-apple-blue" onClick={() => {
            ...
            {(seoCfg().localLocations || []).map((L, i) => (
             <div key={L.id || i} className="p-3 rounded-xl bg-white bg-primary-900 border border-primary-100 border-white/10 space-y-2">
              <div className="grid sm:grid-cols-2 gap-2">
               {[
                ['name', 'نام شعبه'],
                ['address', 'آدرس'],
                ...
                <input key={k} defaultValue={L[k] || ''} placeholder={lab} dir={k === 'lat' || k === 'lng' || k === 'phone' ? 'ltr' : undefined}
                 ...
                 className="px-2 py-1.5 rounded-lg border border-primary-200 border-white/15 bg-transparent text-xs"
                />
               ))}
              </div>
           ...
           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
            <h3 className="text-sm font-bold text-primary-900 text-white">News و Video Sitemap</h3>
            ...
            <div className="flex flex-wrap gap-2">
             <button type="button" disabled={!seoCfg().newsSitemapEnabled} onClick={() => downloadSeoFile('news-sitemap.xml', buildNewsSitemapXml(), 'application/xml;charset=utf-8')} className="text-xs px-3 py-2 rounded-full border border-primary-200 border-white/20 disabled:opacity-40">دانلود news-sitemap.xml</button>
             <button type="button" disabled={!seoCfg().videoSitemapEnabled} onClick={() => downloadSeoFile('video-sitemap.xml', buildVideoSitemapXml(), 'application/xml;charset=utf-8')} className="text-xs px-3 py-2 rounded-full border border-primary-200 border-white/20 disabled:opacity-40">دانلود video-sitemap.xml</button>
            </div>
           ...
           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
            <div className="flex items-center justify-between gap-2 flex-wrap">
             <h3 className="text-sm font-bold text-primary-900 text-white">رصد برند در AI (Brand Visibility)</h3>
            ...
            <div className="space-y-1 max-h-40 overflow-y-auto">
             ...
             {(seoCfg().brandMentions || []).slice(0, 20).map(m => (
              <div key={m.id} className="text-xs py-1.5 border-b border-primary-50 border-white/5 flex flex-wrap gap-x-2">
               ...
               <span className="text-primary-800 text-white">{m.note}</span>
              </div>
             ))}
            </div>
            <button type="button" className="text-xs text-primary-500 text-[#13ABC4] footer-link" onClick={() => saveSeoPatch({ brandMentions: [] })}>پاک کردن لاگ برند</button>
           </div>

           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
            <h3 className="text-sm font-bold text-primary-900 text-white">Keyword Rank Tracker</h3>
            <p className="text-xs text-primary-500">ثبت دستی رتبه کلمات (از GSC یا ابزارهای دیگر). تاریخچه تا ۳۰ نقطه.</p>
            <div className="grid sm:grid-cols-3 gap-2">
             <input id="rank-kw" placeholder="کلمه کلیدی" className="px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm" />
             <input id="rank-pos" type="number" min="1" max="100" placeholder="رتبه (۱–۱۰۰)" dir="ltr" className="px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin" />
             <input id="rank-url" placeholder="URL مرتبط (اختیاری)" dir="ltr" className="px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin" />
            </div>
            <button type="button" onClick={() => {
            ...
            <div className="space-y-1 max-h-48 overflow-y-auto">
             ...
             {(seoCfg().rankKeywords || []).map(r => {
              ...
              return (
               <div key={r.id} className="flex items-center gap-2 text-xs py-1.5 border-b border-primary-50 border-white/5">
                <span className="font-bold text-primary-900 text-white min-w-[4rem]">#{toFa(r.position)}</span>
                <span className="flex-1 text-primary-800 text-white truncate">{r.keyword}</span>
                {delta != null && delta !== 0 && (
           ...
           {/* GTM & GA */}
           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
            <h3 className="text-sm font-bold text-primary-900 text-white">Google Tag Manager و Analytics</h3>
            <div>
             <label className="text-xs text-primary-500 text-[#13ABC4] block mb-1">GTM Container ID</label>
             <input id="seo-gtm" defaultValue={seoCfg().gtmId} dir="ltr" placeholder="GTM-XXXXXXX" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left text-primary-900 text-white" />
            </div>
            <div>
             <label className="text-xs text-primary-500 text-[#13ABC4] block mb-1">Google Analytics 4 Measurement ID</label>
             <input id="seo-ga" defaultValue={seoCfg().gaId} dir="ltr" placeholder="G-XXXXXXXX" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left text-primary-900 text-white" />
            </div>
            <button type="button" onClick={() => {
             saveSeoPatch({
         ...
         {!adminLoading && adminTab === 'redirects' && (
          <div className="space-y-5 max-w-none w-full">
           <div>
            <h2 className="text-base font-bold text-primary-900 text-white">ریدایرکت دستی</h2>
            <p className="text-xs text-primary-500 text-[#13ABC4] mt-1">مسیر قدیمی را به مسیر جدید بفرستید · ۳۰۱ دائم · ۳۰۲ موقت · ۴۱۰ حذف‌شده · فقط ادمین</p>
           </div>

           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
            <h3 className="text-sm font-bold text-primary-900 text-white">افزودن ریدایرکت</h3>
            <div className="grid sm:grid-cols-2 gap-3">
             <div>
              <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">از مسیر (From)</label>
              <input
               value={seoRedirectForm.from}
               onChange={e => setSeoRedirectForm(f => ({ ...f, from: e.target.value }))}
               dir="ltr"
               placeholder="/old-path یا /shop?cat=old"
               className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin text-primary-900 text-white"
              />
             </div>
             <div>
              <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">به مسیر (To) · برای ۴۱۰ خالی بگذارید</label>
              <input
             ...
             <div>
              <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">نوع</label>
              <select
               value={seoRedirectForm.type}
               onChange={e => setSeoRedirectForm(f => ({ ...f, type: e.target.value }))}
               className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-white bg-primary-900 text-sm text-primary-900 text-white"
              >
             ...
             <div>
              <label className="text-xs text-primary-500 text-[#13ABC4] mb-1 block">یادداشت (اختیاری)</label>
              <input
               value={seoRedirectForm.note || ''}
               onChange={e => setSeoRedirectForm(f => ({ ...f, note: e.target.value }))}
               placeholder="مثلاً تغییر نامک دسته"
               className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white"
              />
             </div>
            </div>
            <div className="flex flex-wrap gap-2">
             ...
             {seoRedirectForm.editId && (
              <button
               ...
               className="text-xs px-4 py-2.5 rounded-full border border-primary-200 border-white/25 text-primary-800 text-white"
              >
               انصراف
              </button>
             )}
             <button
              ...
              className="text-xs px-4 py-2.5 rounded-full border border-primary-200 border-white/25 text-primary-800 text-[#13ABC4] footer-link text-white"
             >
              خروجی JSON سرور
             </button>
            </div>
           ...
           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
            <div className="flex items-center justify-between gap-2 flex-wrap">
             <h3 className="text-sm font-bold text-primary-900 text-white">لیست ریدایرکت‌ها</h3>
             <span className="text-xs text-primary-400">{toFa((seoRedirects || []).length)} مورد</span>
            </div>
            {(seoRedirects || []).length === 0 ? (
             <p className="text-xs text-primary-400 text-[#13ABC4] py-4 text-center">هنوز ریدایرکتی ثبت نشده</p>
            ) : (
             <div className="overflow-x-auto max-h-80 overflow-y-auto rounded-xl border border-primary-100 border-white/10">
              <table className="w-full text-xs min-w-[520px]">
               <thead className="sticky top-0 bg-primary-50 bg-primary-800">
                <tr className="text-right text-primary-700 text-white">
               ...
               <tbody>
                {(seoRedirects || []).map(r => (
                 <tr key={r.id} className="border-t border-primary-100 border-white/10">
                  <td className="p-2">
                   <span className={`px-1.5 py-0.5 rounded font-latin ${r.type === '301' ? 'bg-emerald-100 text-emerald-800 bg-emerald-900/40 text-emerald-200' : r.type === '410' ? 'bg-red-100 text-red-700 bg-red-900/40 text-red-200' : 'bg-amber-100 text-amber-800 bg-amber-900/40 text-amber-200'}`}>{r.type}</span>
                  </td>
                  <td className="p-2 font-latin text-left text-primary-900 text-white" dir="ltr">{r.from}</td>
                  <td className="p-2 font-latin text-left text-primary-800 text-white/90" dir="ltr">{r.type === '410' ? 'Gone' : (r.to || '—')}</td>
                  <td className="p-2 text-primary-500 text-[#13ABC4] max-w-[8rem] truncate">{r.note || '—'}</td>
                  <td className="p-2 whitespace-nowrap">
                   <button
                    type="button"
                    className="text-apple-blue text-[#4CCD99] me-2"
            ...
            <p className="text-[10px] text-primary-400 text-[#13ABC4] leading-relaxed">ریدایرکت‌های سئو در صورت اتصال API از سرور خوانده می‌شوند؛ در غیر این صورت از ذخیره‌سازی مرورگر به‌عنوان پشتیبان استفاده می‌شود.</p>
           </div>
          </div>
         )}
         ...
         {!adminLoading && adminTab === 'search-console' && (() => {
          ...
          const Kpi = ({ label, value }) => (
           <div className="p-3 rounded-xl border border-primary-100 border-white/10 bg-primary-50/50 bg-primary-800/40">
            <p className="text-[10px] text-primary-500 text-[#13ABC4] mb-1">{label}</p>
            <p className="text-lg font-bold text-primary-900 text-white tabular-nums" dir="ltr">{value}</p>
           </div>
          );
          ...
          <div className="space-y-4 max-w-none w-full">
           <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
             <h2 className="text-base font-bold text-primary-900 text-white">سرچ کنسول (Search Console)</h2>
             <p className="text-xs text-primary-500 text-[#13ABC4] mt-1 font-latin" dir="ltr">{gscStore?.property || 'https://pirahanemardane.ir/'}</p>
            </div>
            <div className="flex flex-wrap gap-1.5">
             {['7d', '28d', '3m', '6m', '16m'].map(r => (
              <button key={r} type="button" onClick={() => setAdminGscRange(r)} className={`text-xs px-2.5 py-1.5 rounded-full border ${adminGscRange === r ? 'bg-apple-blue text-white border-apple-blue' : 'border-primary-200 border-white/20 text-primary-800 text-white'}`}>{r}</button>
             ))}
            </div>
           </div>
           <div className="admin-tabs-strip flex gap-1 overflow-x-auto pb-1" style={{ WebkitOverflowScrolling: 'touch' }}>
            {subs.map(s => (
             <button key={s.id} type="button" onClick={() => setAdminGscSub(s.id)} className={`flex-shrink-0 text-xs px-3 py-2 rounded-full border ${sub === s.id ? 'bg-primary-900 text-white bg-white text-primary-900 border-transparent' : 'border-primary-200 border-white/20 text-primary-700 text-white/80'}`}>{s.label}</button>
            ))}
           </div>
           ...
           {sub === 'performance' && (
            <div className="space-y-4">
             ...
             <div className="admin-tabs-strip flex gap-1 overflow-x-auto">
              {dims.map(d => (
               <button key={d.id} type="button" onClick={() => setAdminGscDim(d.id)} className={`flex-shrink-0 text-xs px-2.5 py-1.5 rounded-full border ${adminGscDim === d.id ? 'bg-emerald-600 text-white border-emerald-600' : 'border-primary-200 border-white/20'}`}>{d.label}</button>
              ))}
             </div>
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 overflow-x-auto">
              <table className="w-full text-xs min-w-[520px]">
               <thead>
                <tr className="text-primary-500 text-[#13ABC4] text-right">
               ...
               <tbody>
                {agg.rows.slice(0, 40).map(r => (
                 <tr key={r.key} className="border-t border-primary-50 border-white/5">
                  <td className="p-2 font-latin text-left text-primary-900 text-white" dir="ltr">{r.key}</td>
           ...
           {sub === 'url' && (
            <div className="space-y-4">
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
              <h3 className="text-sm font-bold text-primary-900 text-white">URL Inspection</h3>
              <div className="flex flex-col sm:flex-row gap-2">
               <input value={adminGscInspectUrl} onChange={e => setAdminGscInspectUrl(e.target.value)} dir="ltr" placeholder="https://pirahanemardane.ir/shop یا /shop" className="flex-1 px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin text-primary-900 text-white" />
              ...
              {adminGscInspectResult && (
               <div className="space-y-2 text-xs border-t border-primary-100 border-white/10 pt-3">
                {[
                 ...
                 <div key={k} className="flex justify-between gap-2 py-1 border-b border-primary-50 border-white/5">
                  <span className="text-primary-500">{k}</span>
                  <span className="text-primary-900 text-white font-latin text-left" dir="ltr">{String(v)}</span>
                 </div>
                ))}
           ...
           {sub === 'indexing' && (
            <div className="space-y-4">
             ...
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 overflow-x-auto">
              <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">Page indexing</h3>
              <table className="w-full text-xs min-w-[400px]">
               <thead><tr className="text-primary-500 text-[#13ABC4] text-right"><th className="p-2">Reason</th><th className="p-2">Type</th><th className="p-2">Count</th></tr></thead>
               <tbody>
                {cov.map((c, i) => (
                 <tr key={i} className="border-t border-primary-50 border-white/5">
                  <td className="p-2 text-primary-900 text-white font-latin text-left" dir="ltr">{c.reason}</td>
           ...
           {sub === 'sitemaps' && (
            <div className="space-y-4">
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
              <h3 className="text-sm font-bold text-primary-900 text-white">Sitemaps</h3>
              <div className="flex flex-col sm:flex-row gap-2">
               <input id="gsc-sitemap-add" dir="ltr" placeholder="https://pirahanemardane.ir/sitemap.xml" className="flex-1 px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin" />
               <button type="button" onClick={() => {
              ...
              <div className="overflow-x-auto">
               <table className="w-full text-xs min-w-[520px]">
                <thead><tr className="text-primary-500 text-[#13ABC4] text-right"><th className="p-2">Sitemap</th><th className="p-2">Status</th><th className="p-2">Discovered</th><th className="p-2">Last read</th><th className="p-2"></th></tr></thead>
                <tbody>
                 {(gscStore?.sitemaps || []).map((s, i) => (
                  <tr key={i} className="border-t border-primary-50 border-white/5">
                   <td className="p-2 font-latin text-left text-primary-900 text-white" dir="ltr">{s.path}</td>
              ...
              <div className="flex flex-wrap gap-2">
               <button type="button" onClick={() => downloadSeoFile('sitemap.xml', buildSitemapXml(), 'application/xml;charset=utf-8')} className="text-xs px-3 py-2 rounded-full border border-primary-200 border-white/20">دانلود sitemap.xml</button>
               <button type="button" onClick={() => downloadSeoFile('sitemap-index.xml', buildSitemapIndexXml(), 'application/xml;charset=utf-8')} className="text-xs px-3 py-2 rounded-full border border-primary-200 border-white/20">دانلود index</button>
              </div>
             </div>
            </div>
           )}
           ...
           {sub === 'experience' && (
            <div className="space-y-4">
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-4">
              <h3 className="text-sm font-bold text-primary-900 text-white">Core Web Vitals</h3>
              {['lcp', 'inp', 'cls'].map(metric => {
               ...
               return (
                <div key={metric}>
                 <p className="text-xs font-bold text-primary-800 text-white mb-1 uppercase font-latin">{metric}</p>
                 <div className="flex h-3 rounded-full overflow-visible">
                  ...
                  <div className="bg-transparent hover:bg-primary-50 hover:bg-white/10 border border-primary-200 border-white/25" style={{ width: `${(m.poor / total) * 100}%` }} />
                 </div>
                 <p className="text-[10px] text-primary-400 text-[#13ABC4] mt-1">Good {m.good}% · Needs improvement {m.needs}% · Poor {m.poor}%</p>
                </div>
               );
              })}
             </div>
           ...
           {sub === 'links' && (
            <div className="grid sm:grid-cols-2 gap-3">
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">Internal links</h3>
             ...
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">External linking sites</h3>
              <div className="space-y-2">
           ...
           {sub === 'removals' && (
            <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
             <h3 className="text-sm font-bold text-primary-900 text-white">Removals</h3>
             <div className="flex flex-col sm:flex-row gap-2">
              <input id="gsc-removal-url" dir="ltr" placeholder="/path-to-remove" className="flex-1 px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin" />
              <button type="button" onClick={() => {
             ...
             <table className="w-full text-xs">
              <thead><tr className="text-primary-500 text-[#13ABC4] text-right"><th className="p-2">URL</th><th className="p-2">Type</th><th className="p-2">Status</th></tr></thead>
              <tbody>
           ...
           {sub === 'security' && (
            <div className="space-y-3">
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold text-primary-900 text-white mb-2">Manual actions</h3>
             ...
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold text-primary-900 text-white mb-2">Security issues</h3>
           ...
           {sub === 'settings' && (
            <div className="space-y-4">
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-2">
              <h3 className="text-sm font-bold text-primary-900 text-white">Property</h3>
              <input defaultValue={gscStore?.property || ''} id="gsc-property" dir="ltr" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin" />
              <button type="button" onClick={() => {
             ...
             <div className="flex flex-wrap gap-2">
              <button type="button" className="text-xs px-3 py-2 rounded-full border border-primary-200 border-white/20" onClick={() => { const s = buildGscSeed(); persistGsc(s); showToast({ message: 'داده نمونه GSC بازنشانی شد', variant: 'success', duration: 3000, position: 'top-center' }); }}>بازنشانی داده نمونه</button>
              <button type="button" className="text-xs px-3 py-2 rounded-full border border-primary-200 border-white/20" onClick={() => {
             ...
             <p className="text-[10px] text-primary-400 text-[#13ABC4] leading-relaxed">شبیه‌سازی کامل گزارش‌های Search Console روی داده محلی/نمونه. برای اتصال API واقعی از تب سئو (کد تأیید و OAuth) استفاده کنید.</p>
            </div>
           )}
          </div>
          );
         ...
         {!adminLoading && adminTab === 'analytics' && (() => {
            ...
            {(rows || []).map((row, i) => {
             ...
             return (
              <div key={i} className="flex items-center gap-2 text-xs">
               <span className="w-28 sm:w-40 truncate text-primary-800 text-white/90" title={String(label)}>{label}</span>
               <div className="flex-1 h-2 rounded-full bg-primary-100 bg-white/10 overflow-visible">
                <div className="h-full rounded-full bg-apple-blue" style={{ width: `${Math.min(100, (val / max) * 100)}%` }} />
               </div>
               <span className="w-14 text-left tabular-nums text-primary-600 text-white/70" dir="ltr">{fmt(val)}</span>
              </div>
             );
            })}
           </div>
          ...
          const Kpi = ({ label, value, sub }) => (
           <div className="p-3 rounded-xl border border-primary-100 border-white/10 bg-primary-50/50 bg-primary-800/40">
            <p className="text-[10px] text-primary-500 text-[#13ABC4] mb-1">{label}</p>
            <p className="text-lg font-bold text-primary-900 text-white tabular-nums" dir="ltr">{value}</p>
            {sub ? <p className="text-[10px] text-primary-400 text-[#13ABC4] mt-0.5">{sub}</p> : null}
           </div>
          );
          ...
          <div className="space-y-4 max-w-none w-full">
           <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
             <h2 className="text-base font-bold text-primary-900 text-white">آنالیتیکس (GA4-like)</h2>
             <p className="text-xs text-primary-500 text-[#13ABC4] mt-1">رویدادمحور · جذب · تعامل · درآمد · جمعیت · فناوری · کاوش · پیکربندی</p>
            </div>
            <div className="flex flex-wrap gap-1.5">
             {['24h', '7d', '28d', '90d'].map(r => (
              <button key={r} type="button" onClick={() => setAdminAnalyticsRange(r)} className={`text-xs px-2.5 py-1.5 rounded-full border ${adminAnalyticsRange === r ? 'bg-apple-blue text-white border-apple-blue' : 'border-primary-200 border-white/20 text-primary-800 text-white'}`}>{r}</button>
             ))}
            </div>
           </div>
           <div className="admin-tabs-strip flex gap-1 overflow-x-auto pb-1" style={{ WebkitOverflowScrolling: 'touch' }}>
            {subs.map(s => (
             <button key={s.id} type="button" onClick={() => setAdminAnalyticsSub(s.id)} className={`flex-shrink-0 text-xs px-3 py-2 rounded-full border ${sub === s.id ? 'bg-primary-900 text-white bg-white text-primary-900 border-transparent' : 'border-primary-200 border-white/20 text-primary-700 text-white/80'}`}>{s.label}</button>
            ))}
           </div>
           ...
           {sub === 'overview' && (
            <div className="space-y-4">
             ...
             <div className="grid sm:grid-cols-2 gap-3">
              <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
               <h3 className="text-sm font-bold text-primary-900 text-white mb-3">کانال‌های جذب</h3>
               <BarList rows={agg.sourceRows.slice(0, 8).map(r => [r.key, r.users])} />
              </div>
              <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
               <h3 className="text-sm font-bold text-primary-900 text-white mb-3">صفحات برتر</h3>
               <BarList rows={agg.topPages.slice(0, 8)} />
              </div>
             </div>
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold text-primary-900 text-white mb-3">فعالیت ساعتی</h3>
              ...
              <div className="flex justify-between text-[9px] text-primary-400 text-[#13ABC4] mt-1" dir="ltr"><span>0</span><span>6</span><span>12</span><span>18</span><span>23</span></div>
             </div>
            </div>
           )}
           ...
           {sub === 'realtime' && (
            <div className="space-y-4">
             ...
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-2 text-primary-900 text-white">رویدادهای اخیر</h3>
              <div className="max-h-72 overflow-y-auto space-y-1">
               {liveEvents.slice(0, 50).map((e, i) => (
                <div key={i} className="flex gap-2 text-[11px] border-b border-primary-50 border-white/5 py-1.5">
                 <span className="text-primary-400 text-[#13ABC4] w-14 flex-shrink-0" dir="ltr">{new Date(e.ts).toLocaleTimeString('fa-IR')}</span>
                 <span className="font-medium text-apple-blue text-[#4CCD99] w-28 flex-shrink-0">{e.name}</span>
                 <span className="truncate text-primary-600 text-white/70" dir="ltr">{e.page_path}</span>
                 <span className="text-primary-400 text-[#13ABC4] flex-shrink-0">{e.device}</span>
                </div>
               ))}
           ...
           {sub === 'acquisition' && (
            <div className="space-y-4">
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 overflow-x-auto">
              <h3 className="text-sm font-bold text-primary-900 text-white mb-3">Traffic acquisition</h3>
              <table className="w-full text-xs min-w-[560px]">
               <thead><tr className="text-primary-500 text-[#13ABC4] text-right"><th className="p-2">Session source / medium</th><th className="p-2">Users</th><th className="p-2">Sessions</th><th className="p-2">Events</th><th className="p-2">Revenue</th></tr></thead>
               <tbody>
             ...
             <div className="grid sm:grid-cols-2 gap-3">
              <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
               <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">Campaigns</h3>
               <BarList rows={agg.topCampaigns} />
              </div>
              <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
               <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">Landing pages</h3>
           ...
           {sub === 'engagement' && (
            <div className="space-y-4">
             <div className="grid sm:grid-cols-2 gap-3">
              <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
               <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">Events</h3>
               <BarList rows={agg.topEvents} />
              </div>
              <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
               <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">Pages and screens</h3>
               <BarList rows={agg.topPages} />
              </div>
             </div>
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">Site search</h3>
           ...
           {sub === 'monetization' && (
            <div className="space-y-4">
             ...
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">Ecommerce purchases funnel</h3>
              <div className="flex flex-col sm:flex-row gap-2 items-stretch">
               {[
                ...
                <div key={name} className="flex-1 p-3 rounded-xl bg-primary-50 bg-primary-800/50 text-center">
                 <p className="text-[10px] text-primary-500 text-[#13ABC4] font-latin">{name}</p>
                 <p className="text-base font-bold text-primary-900 text-white" dir="ltr">{fmt(val)}</p>
             ...
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 overflow-x-auto">
              <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">Item performance</h3>
              <table className="w-full text-xs min-w-[400px]">
               <thead><tr className="text-primary-500 text-[#13ABC4] text-right"><th className="p-2">Item</th><th className="p-2">Qty</th><th className="p-2">Revenue</th></tr></thead>
               <tbody>
                {agg.topItems.map(it => (
                 <tr key={it.id} className="border-t border-primary-50 border-white/5">
                  <td className="p-2 text-primary-900 text-white">{it.name || it.id}</td>
           ...
           {sub === 'retention' && (
            <div className="space-y-4">
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-2 text-primary-900 text-white">Retention overview</h3>
              <p className="text-xs text-primary-500 text-[#13ABC4] mb-3">کاربران تکراری بر اساس user_id در بازه انتخابی</p>
              {(() => {
               const counts = {};
             ...
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">Activity by day</h3>
           ...
           {sub === 'demographics' && (
            <div className="grid sm:grid-cols-2 gap-3">
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">Cities</h3>
              <BarList rows={agg.topCities} />
             </div>
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">Countries</h3>
           ...
           {sub === 'tech' && (
            <div className="grid sm:grid-cols-3 gap-3">
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">Device category</h3>
              <BarList rows={agg.topDevices} />
             </div>
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">Browser</h3>
              <BarList rows={agg.topBrowsers} />
             </div>
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">Operating system</h3>
           ...
           {sub === 'explore' && (
            <div className="space-y-4">
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">Funnel exploration — Purchase</h3>
              <div className="space-y-2">
               {[
                ['view_item', agg.funnel.view_item],
                ['add_to_cart', agg.funnel.add_to_cart],
                ...
                return (
                 <div key={name} className="flex items-center gap-2 text-xs">
                  ...
                  <div className="flex-1 h-3 rounded-full bg-primary-100 bg-white/10 overflow-visible">
             ...
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">Path — top pages sequence (approx)</h3>
              <BarList rows={agg.topPages.slice(0, 12)} />
             </div>
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-3 text-primary-900 text-white">Free-form — event counts</h3>
              <BarList rows={agg.topEvents} />
             </div>
            </div>
           )}
           ...
           {sub === 'ads' && (
            <div className="space-y-4">
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-2 text-primary-900 text-white">Advertising snapshot</h3>
              <p className="text-xs text-primary-500 text-[#13ABC4] mb-3">کمپین‌های CPC و social از دادهٔ رویدادها</p>
           ...
           {sub === 'config' && (
            <div className="space-y-4">
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-2">
              <h3 className="text-sm font-bold text-primary-900 text-white">Data stream</h3>
              <p className="text-xs text-primary-600 text-white/80 font-latin" dir="ltr">{ga4Store?.stream?.measurement_id || 'G-LOCALDEMO'} — {ga4Store?.stream?.name || 'Web'}</p>
              <p className="text-xs text-primary-400">{ga4Store?.stream?.default_uri}</p>
             </div>
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-2 text-primary-900 text-white">Conversions</h3>
              <div className="flex flex-wrap gap-1.5">
               {(ga4Store?.conversions || []).map(c => (
                <span key={c} className="text-xs px-2 py-1 rounded-full bg-emerald-100 text-emerald-800 bg-emerald-900/40 text-emerald-200 font-latin">{c}</span>
               ))}
              </div>
             </div>
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-2 text-primary-900 text-white">Custom dimensions</h3>
              <ul className="text-xs space-y-1 text-primary-700 text-white/80">
             ...
             <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
              <h3 className="text-sm font-bold mb-2 text-primary-900 text-white">Audiences</h3>
              <ul className="text-xs space-y-2">
               {(ga4Store?.audiences || []).map(a => (
                <li key={a.id} className="flex justify-between gap-2 border-b border-primary-50 border-white/5 pb-1">
                 <span className="font-medium text-primary-900 text-white">{a.name}</span>
                 <span className="text-primary-400 text-[#13ABC4] font-latin" dir="ltr">{a.rule}</span>
                </li>
               ))}
              </ul>
             </div>
             ...
             <p className="text-[10px] text-primary-400 text-[#13ABC4] leading-relaxed">این ماژول رویدادمحور محلی شبیه GA4 است و با gtag/dataLayer هم هم‌تراز می‌شود. برای Measurement ID واقعی از تنظیمات سئو / GTM استفاده کنید.</p>
            </div>
           )}
          </div>
          );
         ...
         {!adminLoading && adminTab === 'backup' && (
          <div className="space-y-4 p-4 sm:p-5 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900">
           <h2 className="text-base font-bold text-primary-900 text-white">بک‌آپ و بازیابی کامل سایت</h2>
            <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-2 mb-4">
              <p className="text-sm font-bold text-primary-900 text-white">خطاهای سیستم</p>
              <p className="text-xs text-primary-500">آخرین لاگ‌های critical از سرور (حداکثر ۵۰).</p>
              <button
                type="button"
                className="btn-cta text-xs px-3 py-1.5 rounded-full border border-primary-200 border-white/30 text-primary-800 text-[#13ABC4] footer-link text-white font-bold"
              ...
              {adminErrorLogsOpen && (
                <div className="max-h-64 overflow-y-auto space-y-2 mt-2">
                  ...
                  {adminErrorLogs.map((row, i) => (
                    <div key={row.id || i} className="text-[11px] p-2 rounded-xl border border-primary-100 border-white/10">
                      ...
                      <p className="text-primary-800 text-white/90 mt-1 break-words">{String(row.message || '')}</p>
                    </div>
                  ))}
                </div>
              )}
           ...
           <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-2 mb-4">
            <p className="text-sm font-bold text-primary-900 text-white">بکاپ سرور (JSON)</p>
            ...
            <label className="text-xs px-3 py-1.5 rounded-full border border-primary-200 border-white/30 text-primary-800 text-white font-medium flex items-center gap-1 cursor-pointer inline-flex">
           ...
           <p className="text-xs text-primary-500 text-[#13ABC4] !text-white">شامل محصولات، دسته‌ها، برندها، فروشندگان، خریداران، سفارش‌ها، تیکت‌ها، تنظیمات، بلاگ، کمپین و سایر داده‌های local. </p>
           <div className="flex flex-wrap gap-2">
            ...
            <label className="px-4 py-2 rounded-full border border-primary-200 border-white/30 text-xs font-medium flex items-center gap-1 cursor-pointer">
           ...
           <div className="p-3 rounded-xl bg-white bg-primary-900 border border-primary-100 border-white/10 space-y-2">
            <p className="text-xs font-bold text-primary-800 text-white">مسیر ذخیره / ارسال بک‌آپ</p>
            <p className="text-xs text-primary-500">پیش‌زمینه اتصال به Google Drive — مسیر مقصد را مشخص کنید؛ </p>
            <input value={backupDestPath} onChange={e => { setBackupDestPath(e.target.value); try { localStorage.setItem('backupDestPath', e.target.value); } catch(_){} }} dir="ltr" className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-left font-latin text-primary-900 text-white" placeholder="google-drive://folder-id/ یا مسیر دلخواه" />
        ...
        />
         ...
         {!adminLoading && adminTab === 'profile' && (
          <div className="w-full max-w-none space-y-4">
           <h2 className="text-base font-bold text-primary-900 text-white">پروفایل ادمین</h2>
           <div className="p-4 rounded-2xl bg-white bg-primary-900 border border-primary-200 border-white/15 space-y-2 text-sm">
           ...
           <div className="rounded-2xl border border-primary-100 border-white/10 bg-white bg-primary-900 p-4 space-y-3 mb-4">
            <h3 className="text-sm font-bold text-primary-900 text-white">امنیت و رمز عبور</h3>
            <p className="text-xs text-primary-500 text-white/60">ساخت یا تغییر رمز برای ورود بدون پیامک (مثل خریدار و فروشنده). فراموشی: ورود با OTP سپس رمز جدید.</p>
            <input type="password" id="admin-new-pw" placeholder="رمز جدید (حداقل ۶ کاراکتر)" autoComplete="new-password" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white" />
            <input type="password" id="admin-new-pw2" placeholder="تکرار رمز" autoComplete="new-password" className="w-full px-3 py-2.5 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm text-primary-900 text-white" />
            <button
             type="button"
