...
export default function AdminPanelContent() {
 ...
 useEffect(() => {
  ...
  const runTaxBulkCategory = async (mode) => {
    ...
    const isPurge = mode === "purge";
    const msg = isPurge
      ? `حذف دائم ${ids.size} دسته از آرشیو؟ برگشت‌پذیر نیست.`
      : `آرشیو ${ids.size} دسته انتخاب‌شده؟`;
    const ok = typeof siteConfirm === "function"
      ? await siteConfirm(msg, isPurge ? "حذف دائم گروهی" : "آرشیو گروهی")
      : window.confirm(msg);
    if (!ok) return;
    ...
    try {
      if (isPurge) {
        saveAdminCategories((adminCategories || []).filter((x) => !ids.has(String(x.id))));
      } else {
        saveAdminCategories((adminCategories || []).map((x) => ids.has(String(x.id)) ? { ...x, status: "archived" } : x));
      }
      setTaxSelectedIds([]);
  ...
  const taxToolbarCategory = (visibleList) => {
      ...
      <div className="space-y-2 mb-3">
        ...
        {taxSelectedIds.length > 0 && (
          <div className="flex flex-wrap items-center gap-2 p-2.5 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800/40">
            ...
            <button type="button" disabled={taxBusy} onClick={() => runTaxBulkCategory(taxFilter === "archived" ? "purge" : "archive")}
            ...
            {taxFilter === "archived" && (
              <button type="button" disabled={taxBusy} onClick={async () => {
                const ids = new Set(taxSelectedIds.map(String));
                const ok = typeof siteConfirm === "function" ? await siteConfirm("بازگردانی دسته‌های انتخاب‌شده؟", "بازگردانی") : window.confirm("بازگردانی؟");
                if (!ok) return;
                setTaxBusy(true);
                try {
                  saveAdminCategories((adminCategories || []).map((x) => ids.has(String(x.id)) ? { ...x, status: "active" } : x));
                  setTaxSelectedIds([]);
  ...
  const runTaxBulkTag = async (mode) => {
    ...
    const isPurge = mode === "purge";
    const msg = isPurge
      ? `حذف دائم ${ids.size} برچسب از آرشیو؟ برگشت‌پذیر نیست.`
      : `آرشیو ${ids.size} برچسب انتخاب‌شده؟`;
    const ok = typeof siteConfirm === "function"
      ? await siteConfirm(msg, isPurge ? "حذف دائم گروهی" : "آرشیو گروهی")
      : window.confirm(msg);
    if (!ok) return;
    ...
    try {
      if (isPurge) {
        saveAdminTags((adminTags || []).filter((x) => !ids.has(String(x.id))));
      } else {
        saveAdminTags((adminTags || []).map((x) => ids.has(String(x.id)) ? { ...x, status: "archived" } : x));
      }
      setTaxSelectedIds([]);
      try { showToast({ message: isPurge ? "حذف دائم انجام شد" : "به آرشیو منتقل شد", variant: "success", duration: 3000, position: "top-center" }); } catch (_) {}
    } finally {
  ...
  const taxToolbarTag = (visibleList) => {
    const allIds = (visibleList || []).map((x) => String(x.id));
    const allSelected = allIds.length > 0 && allIds.every((id) => taxSelectedIds.includes(id));
    return (
      <div className="space-y-2 mb-3">
        ...
        {taxSelectedIds.length > 0 && (
          <div className="flex flex-wrap items-center gap-2 p-2.5 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800/40">
            <span className="text-xs font-medium text-red-800 dark:text-red-200">{taxSelectedIds.length} مورد انتخاب شده</span>
            <button type="button" disabled={taxBusy} onClick={() => runTaxBulkTag(taxFilter === "archived" ? "purge" : "archive")}
              className="text-xs px-3 py-1.5 rounded-full bg-red-600 text-white font-medium disabled:opacity-50">
              {taxBusy ? "در حال اجرا…" : (taxFilter === "archived" ? "حذف دائم گروهی" : "آرشیو گروهی")}
            </button>
            {taxFilter === "archived" && (
              <button type="button" disabled={taxBusy} onClick={async () => {
                const ids = new Set(taxSelectedIds.map(String));
                const ok = typeof siteConfirm === "function" ? await siteConfirm("بازگردانی برچسب‌های انتخاب‌شده؟", "بازگردانی") : window.confirm("بازگردانی؟");
                if (!ok) return;
                setTaxBusy(true);
                try {
                  saveAdminTags((adminTags || []).map((x) => ids.has(String(x.id)) ? { ...x, status: "active" } : x));
                  setTaxSelectedIds([]);
                  try { showToast({ message: "بازگردانی شد", variant: "success", duration: 2500, position: "top-center" }); } catch (_) {}
                } finally { setTaxBusy(false); }
              }} className="text-xs px-3 py-1.5 rounded-full border border-emerald-300 text-emerald-700 bg-emerald-50">بازگردانی گروهی</button>
  ...
  const runTaxBulkBlogCategory = async (mode) => {
    ...
    const isPurge = mode === "purge";
    const msg = isPurge
      ? `حذف دائم ${ids.size} دسته مقاله؟ برگشت‌پذیر نیست.`
      : `آرشیو ${ids.size} دسته مقاله؟`;
    const ok = typeof siteConfirm === "function"
      ? await siteConfirm(msg, isPurge ? "حذف دائم گروهی" : "آرشیو گروهی")
      : window.confirm(msg);
    if (!ok) return;
    setTaxBusy(true);
    try {
      if (isPurge) saveAdminBlogCategories((adminBlogCategories || []).filter((x) => !ids.has(String(x.id))));
      else saveAdminBlogCategories((adminBlogCategories || []).map((x) => ids.has(String(x.id)) ? { ...x, status: "archived" } : x));
      setTaxSelectedIds([]);
      try { showToast({ message: isPurge ? "حذف دائم انجام شد" : "به آرشیو منتقل شد", variant: "success", duration: 3000, position: "top-center" }); } catch (_) {}
    } finally { setTaxBusy(false); }
  };
  const runTaxBulkBlogTag = async (mode) => {
    ...
    const isPurge = mode === "purge";
    const msg = isPurge
      ? `حذف دائم ${ids.size} برچسب مقاله؟ برگشت‌پذیر نیست.`
      : `آرشیو ${ids.size} برچسب مقاله؟`;
    const ok = typeof siteConfirm === "function"
      ? await siteConfirm(msg, isPurge ? "حذف دائم گروهی" : "آرشیو گروهی")
      : window.confirm(msg);
    if (!ok) return;
    setTaxBusy(true);
    try {
      if (isPurge) saveAdminBlogTags((adminBlogTags || []).filter((x) => !ids.has(String(x.id))));
      else saveAdminBlogTags((adminBlogTags || []).map((x) => ids.has(String(x.id)) ? { ...x, status: "archived" } : x));
      setTaxSelectedIds([]);
      try { showToast({ message: isPurge ? "حذف دائم انجام شد" : "به آرشیو منتقل شد", variant: "success", duration: 3000, position: "top-center" }); } catch (_) {}
    } finally { setTaxBusy(false); }
  };
  const taxToolbarBlogCategory = (visibleList) => {
    const allIds = (visibleList || []).map((x) => String(x.id));
    const allSelected = allIds.length > 0 && allIds.every((id) => taxSelectedIds.includes(id));
    return (
      <div className="space-y-2 mb-3">
        ...
        {taxSelectedIds.length > 0 && (
          <div className="flex flex-wrap items-center gap-2 p-2.5 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800/40">
            <span className="text-xs font-medium text-red-800 dark:text-red-200">{taxSelectedIds.length} مورد انتخاب شده</span>
            <button type="button" disabled={taxBusy} onClick={() => runTaxBulkBlogCategory(taxFilter === "archived" ? "purge" : "archive")}
              className="text-xs px-3 py-1.5 rounded-full bg-red-600 text-white font-medium disabled:opacity-50">
              {taxBusy ? "در حال اجرا…" : (taxFilter === "archived" ? "حذف دائم گروهی" : "آرشیو گروهی")}
            </button>
            {taxFilter === "archived" && (
              <button type="button" disabled={taxBusy} onClick={async () => {
                const ids = new Set(taxSelectedIds.map(String));
                const ok = typeof siteConfirm === "function" ? await siteConfirm("بازگردانی؟", "بازگردانی") : window.confirm("بازگردانی؟");
                if (!ok) return;
                setTaxBusy(true);
                try {
                  saveAdminBlogCategories((adminBlogCategories || []).map((x) => ids.has(String(x.id)) ? { ...x, status: "active" } : x));
                  setTaxSelectedIds([]);
                  try { showToast({ message: "بازگردانی شد", variant: "success", duration: 2500, position: "top-center" }); } catch (_) {}
                } finally { setTaxBusy(false); }
              }} className="text-xs px-3 py-1.5 rounded-full border border-emerald-300 text-emerald-700 bg-emerald-50">بازگردانی گروهی</button>
        ...
        {allIds.length > 0 && (
          <label className="flex items-center gap-2 text-xs text-primary-600 dark:text-white/70 cursor-pointer">
            <input type="checkbox" checked={allSelected} onChange={() => { if (allSelected) setTaxSelectedIds([]); else setTaxSelectedIds(allIds); }} className="rounded border-primary-300" />
            انتخاب همه ({allIds.length})
          </label>
        )}
      </div>
  ...
  const taxToolbarBlogTag = (visibleList) => {
    const allIds = (visibleList || []).map((x) => String(x.id));
    const allSelected = allIds.length > 0 && allIds.every((id) => taxSelectedIds.includes(id));
    return (
      <div className="space-y-2 mb-3">
        ...
        {taxSelectedIds.length > 0 && (
          <div className="flex flex-wrap items-center gap-2 p-2.5 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800/40">
            <span className="text-xs font-medium text-red-800 dark:text-red-200">{taxSelectedIds.length} مورد انتخاب شده</span>
            <button type="button" disabled={taxBusy} onClick={() => runTaxBulkBlogTag(taxFilter === "archived" ? "purge" : "archive")}
              className="text-xs px-3 py-1.5 rounded-full bg-red-600 text-white font-medium disabled:opacity-50">
              {taxBusy ? "در حال اجرا…" : (taxFilter === "archived" ? "حذف دائم گروهی" : "آرشیو گروهی")}
            </button>
            {taxFilter === "archived" && (
              <button type="button" disabled={taxBusy} onClick={async () => {
                ...
                setTaxBusy(true);
                try {
                  saveAdminBlogTags((adminBlogTags || []).map((x) => ids.has(String(x.id)) ? { ...x, status: "active" } : x));
                  setTaxSelectedIds([]);
                  try { showToast({ message: "بازگردانی شد", variant: "success", duration: 2500, position: "top-center" }); } catch (_) {}
                } finally { setTaxBusy(false); }
              }} className="text-xs px-3 py-1.5 rounded-full border border-emerald-300 text-emerald-700 bg-emerald-50">بازگردانی گروهی</button>
...
{/* Categories */}
           <div>
            ...
            <div className="space-y-2">
             {taxToolbarCategory(filterTaxList(adminCategories))}
...
{/* Tags */}
           <div>
            ...
            <div className="space-y-2">
             {taxToolbarTag(filterTaxList(adminTags))}
...
{/* Colors catalog */}
         ...
         {!adminLoading && adminTab === 'blog-categories' && (
          <div className="space-y-4">
           ...
           <div className="space-y-2">
            {taxToolbarBlogCategory(filterTaxList(adminBlogCategories))}
           ...
           <div className="pt-6 border-t border-primary-100 dark:border-white/10 space-y-3">
            ...
            <div className="space-y-2">
             {taxToolbarBlogTag(filterTaxList(adminBlogTags))}
