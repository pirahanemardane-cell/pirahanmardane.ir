...
export default function PermissionsPanelContent({ adminSettings, setAdminSettings, saveAdminSettings, showToast }) {
  ...
  return (
    <div className="space-y-6 w-full max-w-none">
      <div>
        <h2 className="text-lg font-bold text-primary-900 text-white mb-1">مجوزهای سایت</h2>
        <p className="text-xs text-primary-500 text-white/60">
      ...
      <div className="p-4 rounded-2xl border border-primary-200 border-white/15 bg-white bg-primary-900 space-y-3">
        <h3 className="text-sm font-bold text-primary-900 text-white">اینماد</h3>
        <label className="flex items-center gap-2 text-sm text-primary-800 text-white/90">
          <input type="checkbox" checked={enamadOn} onChange={(e) => setEnamadOn(e.target.checked)} />
          نمایش لوگو در فوتر
        </label>
        <div>
          <label className="block text-xs mb-1 text-primary-600 text-white/70">شناسه اینماد (id)</label>
          <input value={enamadId} onChange={(e) => setEnamadId(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm" placeholder="id از پنل اینماد" dir="ltr" />
        </div>
        <div>
          <label className="block text-xs mb-1 text-primary-600 text-white/70">کد اینماد (Code)</label>
          <input value={enamadCode} onChange={(e) => setEnamadCode(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-primary-200 border-white/20 bg-transparent text-sm" placeholder="Code از پنل اینماد" dir="ltr" />
        </div>
      </div>
