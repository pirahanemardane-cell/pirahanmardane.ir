...
function normSlug(s, fallback) {
  const slug = String(s || '')
    ...
    .replace(/-+/g, '-')
    .replace(/^-|-$/g, '')
  return slug || fallback || `tag-${Date.now().toString(36)}`
}
...
export async function GET() {
  try {
    ...
    try { await logCritical('app/api/blog/tags/route.js', e) } catch (_) {}
...
export async function PATCH(request) {
  try {
    ...
    if (body.slug != null) patch.slug = normSlug(body.slug, 'tag')
    if (body.active != null) patch.active = !!body.active
    if (body.sort_order != null) patch.sort_order = parseInt(body.sort_order, 10) || 0
    const { data, error } = await gate.admin.from('blog_tags').update(patch).eq('id', id).select('*').single()
    if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 400 })
    return NextResponse.json({ ok: true, item: data })
  } catch (e) {
    return NextResponse.json({ ok: false, error: String(e?.message || e) }, { status: 500 })
...
export async function DELETE(request) {
  try {
    ...
    if (!id) return NextResponse.json({ ok: false, error: 'id الزامی است' }, { status: 400 })
    const { error } = await gate.admin.from('blog_tags').delete().eq('id', id)
    if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 400 })
