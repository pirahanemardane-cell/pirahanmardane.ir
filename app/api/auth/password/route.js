...
export async function POST(req) {
  try {
    ...
    // عملیات حساس: OTP یا سشن تأییدشدهٔ اخیر
    const code = String(body.code || body.otp || '').replace(/\D/g, '')
    if (code) {
      if (!isValidIranMobile(phone)) {
        return NextResponse.json({ ok: false, error: 'شماره موبایل برای تأیید یافت نشد' }, { status: 400 })
      }
      ...
      const ok = phone ? await isPhoneVerified(phone) : false
      if (!ok) {
        return NextResponse.json(
          {
            ok: false,
            error: 'برای تغییر رمز، ابتدا کد پیامک را تأیید کنید',
            needs_otp: true,
            phone: phone || null,
          },
          { status: 401 }
    ...
    try {
      ...
      if (adminErr) {
        ...
        if (error) {
          return NextResponse.json(
            { ok: false, error: adminErr.message || error.message || 'خطا در ذخیره رمز' },
            { status: 400 }
          )
        }
      }
    ...
    try {
      if (phone) await clearPhoneVerified(phone)
    } catch (_) {}
    ...
    return NextResponse.json({
      ok: true,
      message: 'رمز ذخیره شد. از این به بعد می‌توانید با همان شماره و رمز وارد شوید.',
    })
  } catch (e) {
    try {
