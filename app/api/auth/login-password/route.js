...
function normalizePhone(p) {
  let d = toEnglishDigits(p).replace(/\D/g, '')
...
export async function POST(req) {
  try {
    ...
    try {
      ...
      profileFound = !!(profs && profs.length)
      ...
      return NextResponse.json(
        {
          ...
          error: profileFound
            ...
            : 'حسابی با این شماره ثبت نشده — ثبت‌نام را تکمیل کنید',
          code: profileFound ? 'INVALID_PASSWORD' : 'USER_NOT_FOUND',
        },
        { status: 401 }
      )
    }
    ...
    if (profile?.id && isAdminPhone(phone)) {
      ...
      // MFA_DISABLED_BY_REQUEST: ورود ادمین فقط با رمز — بدون پیامک
      const ADMIN_MFA_ENABLED = false
