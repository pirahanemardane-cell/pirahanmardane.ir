  ...
  const taxToolbar = (kind, visibleList) => {
    ...
    return (
      <div className="space-y-2 mb-3">
        <div className="flex flex-wrap gap-1.5">
          {[{ id: 'active', l: 'فعال‌ها' }, { id: 'archived', l: 'آرشیو شده‌ها' }].map((f) => (
            <button key={f.id} type="button" onClick={() => setTaxFilter(f.id)}
              className={`text-xs px-3 py-1.5 rounded-full border font-medium ${taxFilter === f.id ? 'bg-primary-900 text-white border-primary-900 bg-white text-primary-900' : 'border-primary-200 border-white/20'}`}>{f.l}</button>
          ))}
        </div>
        ...
        {allIds.length > 0 && (
          <label className="flex items-center gap-2 text-xs text-primary-600 text-white/70 cursor-pointer">
...
# categories
text = text.replace(
    "{(adminCategories || []).map((c) => (\n              <div key={c.id} className=\"flex flex-wrap items-center gap-3 p-3 rounded-xl border border-primary-200 border-white/15 bg-white bg-primary-900\">",
...
# tags
text = text.replace(
    "{(adminTags || []).map((t) => (\n              <div key={t.id} className=\"flex flex-wrap items-center gap-3 p-3 rounded-xl border border-primary-200 border-white/15 bg-white bg-primary-900\">",
    "{taxToolbar('tag', filterTaxList(adminTags))}\n"
    "            {filterTaxList(adminTags).map((t) => (\n"
    "              <div key={t.id} className={`flex flex-wrap items-center gap-3 p-3 rounded-xl border bg-white bg-primary-900 ${taxSelectedIds.includes(String(t.id)) ? 'border-apple-blue ring-1 ring-apple-blue/30' : 'border-primary-200 border-white/15'}`}>\n"
